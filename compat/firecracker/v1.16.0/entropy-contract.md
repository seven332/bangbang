...
| `corpus:entropy` | audit required | All applicable live API/device behavior, host randomness, limiting, retry, metrics, exact owner traversal, and detached state are implemented. **Wave 6** owns optional-device state encoding, artifact integration, restore construction, migration/clone behavior, portability policy, and signed restored-guest outcomes. |
| `semantic.device:entropy-queues-limits-metrics-and-state` | audit required | Live queue processing, the 64-KiB request cap, host entropy failures, dual-bucket limiting, retry wakeups, metrics, MMIO/PCI ownership, capture-ready state, failure ordering, redaction, and cleanup are implemented. **Wave 6** owns serialized/restored entropy state and aggregate artifact/portability certification. |
