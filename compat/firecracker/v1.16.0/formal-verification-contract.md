...
| `token-bucket-refill-accounting` | native-width private refill path used by the runtime bucket | Explicit small capacity/refill bounds prove that the native path is available and establish partial/full/unchanged accounting; timers, sub-millisecond progress and persistence remain tested, not modeled. |
