...
| `--enable-pci` | Flag; enable Firecracker PCIe support for every configured virtio device. | Implemented on macOS arm64 with the HVF/GIC-MSI symbols required by the product path. Exact flag syntax selects one immutable all-virtio transport: balloon, block, network, pmem, vsock, entropy, and virtio-mem use deterministic modern PCI functions, while platform devices including serial remain MMIO. Default startup remains all-virtio-MMIO. Unsupported hosts fail before API/no-API readiness; complete slot/BAR/vector capacity fails before `Running`. Current native-v2 2.12 create/load supports required platform-MMIO serial plus independently optional profile-3 block/pmem storage, entropy, balloon, virtio-mem, network/MMDS, and vsock state in all 64 optional-component products under the same 31-slot budget; exact 2.3–2.11 and frozen native-v1 retain their original profiles. Kind 13 persists CID/selector/cursor/queues/reset and canonical PCI placement while excluding live socket/session authority, then reconstructs fresh owners from the captured or overridden selector with empty work. Live Running/Paused non-root block, pmem, and network transactions share the same owner and capacity boundary; runtime vsock hotplug remains unsupported. Signed direct MMIO/PCI vsock matrices prove source reset/loss, event acknowledgement/RX gating/live TX, preserved listeners, deterministic bidirectional multistream/half-close, Paused gating, cursor and immutable-clone isolation, recapture, and fresh metrics. Signed normal-production/App Sandbox MMIO/PCI coverage adds exact grants, replacement, malformed/no-device/missing-authority failures, cancellation, both death orders, retry, redaction, containment, and cleanup without new entitlements or helpers. | process PCI / I+V (#1419, #1420, #1421, #1422, #1423, #1444, #1461, #1589, #1616, #1617, #1634, #1651, #1652, #1665, #1666, #1680, #1681, #1697, #1698, #1715, #1716, #1735, #1736) | Parser and pre-readiness probe in [`Args::parse` and `run`](../../../crates/bangbang/src/main.rs), all-virtio assembly and retained runtime manager in [`startup.rs`](../../../crates/hvf/src/startup.rs), profile-3 capture/reconstruction in [`snapshot_v2_storage_platform.rs`](../../../crates/hvf/src/snapshot_v2_storage_platform.rs), network reconstruction in [`snapshot_v2_network_platform.rs`](../../../crates/hvf/src/snapshot_v2_network_platform.rs), vsock reconstruction in [`snapshot_v2_vsock_platform.rs`](../../../crates/hvf/src/snapshot_v2_vsock_platform.rs), signed direct evidence in [`signed_executable_certifies_native_v2_vsock_snapshot_over_product_pci`](../../../crates/bangbang/tests/executable_hvf_e2e.rs), and signed contained evidence in [`normal_bundle_certifies_native_v2_vsock_restored_guest_lifecycle_and_containment`](../../../crates/launcher/tests/production_bundle_e2e.rs) |
...
| `--snapshot-version` | Flag; early command that prints Firecracker's supported snapshot data-format version. | Prints the current bangbang-native public writer version `v2.12.0` and exits before fd-table, socket, signal, or HVF setup. Exact network/MMDS `2.11.0`, virtio-mem `2.10.0`, balloon `2.9.0`, entropy `2.8.0`, serial-required `2.7.0`, storage-only `2.6.0`, block-only profile-2 `2.5.0`, singleton-root `2.4.0`, and device-free `2.3.0` remain describable and loadable legacy profiles. Current 2.12 adds optional vsock kind 13 without changing exact 2.3–2.11. The product intentionally names its native format and does not claim Firecracker v10 artifact compatibility. | snapshot process / I+V (#1578, #1589, #1616, #1617, #1634, #1651, #1652, #1665, #1666, #1680, #1681, #1697, #1698, #1715, #1716, #1735, #1736) | Native implementation in [`run`](../../../crates/bangbang/src/main.rs), exact direct process evidence in [`executable_reports_native_snapshot_versions_before_socket_publication`](../../../crates/bangbang/tests/process_e2e.rs), and signed App Sandbox evidence in [`sandboxed_bundle_reports_current_native_v2_snapshot_version`](../../../crates/bangbang/tests/app_sandbox_process_e2e.rs) |
...
2.11 may add network/MMDS kind 12, and current 2.12 may add vsock kind 13
across all 64 coherent MMIO/PCI products. Restored virtio-mem uses
