...
| `tool-operation:firecracker/run` | The executable entrypoint has 19 implemented arguments and two terminal platform-excluded seccomp arguments; PCI is terminal, but the aggregate still includes native-only snapshot-version leaves. | audit; remains nonterminal until those independent snapshot leaves and aggregate ordering reach terminal outcomes. |
