...
| `--enable-pci` | Flag; enable Firecracker PCIe support for every configured virtio device. | Implemented on macOS arm64 with the HVF/GIC-MSI symbols required by the product path. Exact flag syntax selects one immutable all-virtio transport: balloon, block, network, pmem, vsock, entropy, and virtio-mem use deterministic modern PCI functions, while platform devices remain MMIO. Default startup remains all-virtio-MMIO. Unsupported hosts fail before API/no-API readiness; complete slot/BAR/vector capacity fails before `Running`. Current native-v2 2.4 create/load supports exactly one read-only File/Sync root over PCI; legacy device-free 2.3 load and frozen native-v1 load retain their original transport profiles. #1420, #1421, #1422, and #1423 certify the live immutable profile for transactional Running/Paused non-root block, pmem, and network PUT/DELETE with manual guest coordination, type-scoped IDs, one shared 31-slot budget, concurrent owner serialization, and truthful mixed configuration. #1461 adds direct and contained regular/block-special block replacement, capacity/config refresh, runtime attach/delete/same-slot reuse, and exact cleanup without automatic guest notification. #1589 adds exact current-root graph persistence/reconstruction, including BAR/capability/common configuration, queues, MSI-X, registry placement, and destination backing authorization. Pmem includes direct file-backed mapping leases, targeted exact-prefix flush/unmap, and exact range reuse; network includes per-entry MMDS/vmnet packet-I/O ownership, actual contained-vmnet accounting, generation-safe metrics, and exact slot reuse. Optional/multi-device PCI snapshots and automatic guest notification remain deferred. | process PCI / I+V (#1419, #1420, #1421, #1422, #1423, #1444, #1461, #1589) | Parser and pre-readiness probe in [`Args::parse` and `run`](../../../crates/bangbang/src/main.rs), all-virtio assembly and retained runtime manager in [`startup.rs`](../../../crates/hvf/src/startup.rs), dynamic mappings in [`memory.rs`](../../../crates/hvf/src/memory.rs), exact/attached/duplicate and process-preflight coverage in [`main.rs`](../../../crates/bangbang/src/main.rs) and [`process_e2e.rs`](../../../crates/bangbang/tests/process_e2e.rs), focused mixed identity/configuration/concurrency and 31-slot resource tests in [`vmm.rs`](../../../crates/bangbang/src/vmm.rs) and [`startup.rs`](../../../crates/hvf/src/startup.rs), signed all-class discovery/MSI-X/I/O in [`signed_executable_runs_all_startup_virtio_devices_over_product_pci`](../../../crates/bangbang/tests/executable_hvf_e2e.rs), signed public root snapshot evidence in [`signed_executable_restores_native_v2_root_io_over_mmio_and_pci`](../../../crates/bangbang/tests/executable_hvf_e2e.rs), and direct/contained regular, block-special, pmem, all-MMDS network, and root snapshot gates in [`executable_hvf_e2e.rs`](../../../crates/bangbang/tests/executable_hvf_e2e.rs) and [`production_bundle_e2e.rs`](../../../crates/launcher/tests/production_bundle_e2e.rs) |
...
| `--snapshot-version` | Flag; early command that prints Firecracker's supported snapshot data-format version. | Prints the current bangbang-native public writer version `v2.4.0` and exits before fd-table, socket, signal, or HVF setup. Exact `2.3.0` remains a describable and loadable legacy device-free profile. The product intentionally names its native format and does not claim Firecracker v10 artifact compatibility. | snapshot process / I+V (#1578, #1589) | Native implementation in [`run`](../../../crates/bangbang/src/main.rs), exact direct process evidence in [`executable_reports_native_snapshot_versions_before_socket_publication`](../../../crates/bangbang/tests/process_e2e.rs), and signed App Sandbox evidence in [`sandboxed_bundle_reports_current_native_v2_snapshot_version`](../../../crates/bangbang/tests/app_sandbox_process_e2e.rs) |
