...
| `semantic.process:signals-exits-fd-and-cleanup` | SIGINT/SIGTERM request clean shutdown; SIGPIPE is nonfatal and counted; every Firecracker fatal signal retains its stable exit class. Proven external Darwin SIGHUP/SIGXCPU/SIGXFSZ delivery atomically records a Store and wakes one ordinary best-effort terminal/cleanup attempt. Self/resource delivery, SIGSYS, and indistinguishable SIGILL/SIGBUS/SIGSEGV faults take minimal immediate `_exit`; the latter three and macOS seccomp remain checked platform-zero. Best-effort fd-table preallocation never clobbers an inherited target descriptor. | I+V; production logic, Darwin classifier/state/fault subprocesses in [`main.rs`](../../../crates/bangbang/src/main.rs), all-signal process cases in [`process_e2e.rs`](../../../crates/bangbang/tests/process_e2e.rs), signed live-VM Store/Incremental coverage in [`executable_hvf_e2e.rs`](../../../crates/bangbang/tests/executable_hvf_e2e.rs), App Sandbox classes in [`app_sandbox_process_e2e.rs`](../../../crates/bangbang/tests/app_sandbox_process_e2e.rs), and actual contained-worker/supervisor evidence in [`production_bundle_e2e.rs`](../../../crates/launcher/tests/production_bundle_e2e.rs). |
