...
| `--enable-pci` | Flag; enable Firecracker PCIe support for every configured virtio device. | Implemented on macOS arm64 with the HVF/GIC-MSI symbols required by the product path. Exact flag syntax selects one immutable all-virtio transport: balloon, block, network, pmem, vsock, entropy, and virtio-mem use deterministic modern PCI functions, while platform devices including serial remain MMIO. Default startup remains all-virtio-MMIO. Unsupported hosts fail before API/no-API readiness; complete slot/BAR/vector capacity fails before `Running`. Current native-v2 2.11 create/load supports required platform-MMIO serial plus independently optional profile-3 block/pmem storage, entropy, balloon, virtio-mem, and network/MMDS state in all 32 optional-component products under the same 31-slot budget; exact 2.3–2.10 and frozen native-v1 retain their original profiles. Profile 3 persists exact block and pmem state; kind 9 persists entropy queue/limiter/retry state; kind 10 persists balloon state; kind 11 binds virtio-mem to matching memory extents; and kind 12 persists portable network/MMDS queue, limiter, placement, and protocol configuration while excluding source host/session authority. Live Running/Paused non-root block, pmem, and network transactions share the same owner and capacity boundary. Signed direct and normal-production matrices certify every persisted family over MMIO and PCI. The exact 2.11 V1/V2 MMDS matrix additionally proves complete clone-local overrides, fresh network/MMDS owners, empty/reseeded data, source TCP/token loss, fresh-session success, immutable artifacts, failures, retry, death-order cleanup, and all-MMDS containment without vmnet authority. Vsock snapshot persistence remains deferred. | process PCI / I+V (#1419, #1420, #1421, #1422, #1423, #1444, #1461, #1589, #1616, #1617, #1634, #1651, #1652, #1665, #1666, #1680, #1681, #1697, #1698, #1715, #1716) | Parser and pre-readiness probe in [`Args::parse` and `run`](../../../crates/bangbang/src/main.rs), all-virtio assembly and retained runtime manager in [`startup.rs`](../../../crates/hvf/src/startup.rs), profile-3 capture/reconstruction in [`snapshot_v2_storage_platform.rs`](../../../crates/hvf/src/snapshot_v2_storage_platform.rs), balloon reconstruction in [`snapshot_v2_balloon_platform.rs`](../../../crates/hvf/src/snapshot_v2_balloon_platform.rs), virtio-mem reconstruction in [`snapshot_v2_memory_hotplug_platform.rs`](../../../crates/hvf/src/snapshot_v2_memory_hotplug_platform.rs), network reconstruction in [`snapshot_v2_network_platform.rs`](../../../crates/hvf/src/snapshot_v2_network_platform.rs), signed direct evidence including [`signed_executable_certifies_native_v2_network_mmds_snapshot_continuation`](../../../crates/bangbang/tests/executable_hvf_e2e.rs), and signed contained evidence including [`normal_bundle_certifies_native_v2_network_mmds_snapshot_continuation_and_containment`](../../../crates/launcher/tests/production_bundle_e2e.rs) |
...
| `--snapshot-version` | Flag; early command that prints Firecracker's supported snapshot data-format version. | Prints the current bangbang-native public writer version `v2.11.0` and exits before fd-table, socket, signal, or HVF setup. Exact virtio-mem `2.10.0`, balloon `2.9.0`, entropy `2.8.0`, serial-required `2.7.0`, storage-only `2.6.0`, block-only profile-2 `2.5.0`, singleton-root `2.4.0`, and device-free `2.3.0` remain describable and loadable legacy profiles. Current 2.11 adds optional network/MMDS kind 12 without changing exact 2.3–2.10. The product intentionally names its native format and does not claim Firecracker v10 artifact compatibility. | snapshot process / I+V (#1578, #1589, #1616, #1617, #1634, #1651, #1652, #1665, #1666, #1680, #1681, #1697, #1698, #1715, #1716) | Native implementation in [`run`](../../../crates/bangbang/src/main.rs), exact direct process evidence in [`executable_reports_native_snapshot_versions_before_socket_publication`](../../../crates/bangbang/tests/process_e2e.rs), and signed App Sandbox evidence in [`sandboxed_bundle_reports_current_native_v2_snapshot_version`](../../../crates/bangbang/tests/app_sandbox_process_e2e.rs) |
...
may add entropy, exact 2.9 may add balloon, exact 2.10 may add virtio-mem, and
current 2.11 may add network/MMDS kind 12 across all 32 coherent MMIO/PCI
products. Restored virtio-mem uses
...
source connections, tokens, and MMDS data do not cross the artifact. Exact
2.3–2.10 remain
unchanged readers.
