...
| `corpus:memory-hotplug` | audit required | All applicable live API/device behavior, metrics, exact owner mapping, and detached state are implemented. **Wave 6** owns optional-device state encoding, artifact integration, restore construction, migration/clone behavior, portability policy, and signed restored-guest outcomes. |
| `semantic.memory-device:virtio-mem-lifecycle-accounting-and-state` | audit required | Live STATE/PLUG/UNPLUG/UNPLUG_ALL, failure-atomic HVF mutation, exact shared ownership, dirty tracking, metrics, MMIO/PCI traversal, capture-ready state, and teardown are implemented. **Wave 6** owns serialized/restored virtio-mem state and aggregate artifact/portability certification. |
