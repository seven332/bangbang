...
use bangbang_firecracker_capability_audit::{
    AuditMode, CAPABILITY_INVENTORY_PATH, CPU_TEMPLATE_AGGREGATE_CAPABILITY_IDS,
    ...
    CPU_TEMPLATE_STRIP_COMPATIBILITY_CAPABILITY_IDS, Disposition,
    LOGGER_COMPATIBILITY_CAPABILITY_IDS, LOGGER_PRODUCER_AUDIT_PATH, LOGGER_PRODUCER_MANIFEST_PATH,
    LoggerClassDisposition, LoggerCompiledEvent, LoggerDeliveryPolicy, LoggerNonApplicableReason,
    ...
    TRACING_CALL_SITE_IDS, TRACING_COMPATIBILITY_CAPABILITY_IDS, WAVE7_AGGREGATE_CAPABILITY_IDS,
    ...
    read_tracing_audit, source_manifest_json, tracing_audit_json, validate,
...
fn jailer_chroot_platform_limit_is_terminal_and_fail_closed() {
    ...
    for id in LATER_AGGREGATE_IDS {
        ...
        assert!(!aggregate.implementation.is_empty());
        assert!(!aggregate.validation.is_empty());
...
fn wave_7_ownership_and_core_api_policy_is_stable() {
    ...
    let cpu_template_strip_implemented = CPU_TEMPLATE_STRIP_COMPATIBILITY_CAPABILITY_IDS
        .into_iter()
        .collect::<BTreeSet<_>>();
    let cpu_template_fingerprint_dump_implemented =
        CPU_TEMPLATE_FINGERPRINT_DUMP_COMPATIBILITY_CAPABILITY_IDS
            .into_iter()
            .collect::<BTreeSet<_>>();
    let cpu_template_fingerprint_compare_implemented =
        CPU_TEMPLATE_FINGERPRINT_COMPARE_COMPATIBILITY_CAPABILITY_IDS
            .into_iter()
            .collect::<BTreeSet<_>>();
    ...
    let specification_benchmark_implemented = SPECIFICATION_BENCHMARK_IMPLEMENTED
        .into_iter()
        .collect::<BTreeSet<_>>();
    let wave7_aggregate_implemented = WAVE7_AGGREGATE_CAPABILITY_IDS
        .into_iter()
        .collect::<BTreeSet<_>>();
    ...
    assert_eq!(wave7_aggregate_implemented.len(), 5);
    ...
    assert!(cpu_template_fingerprint_dump_implemented.is_disjoint(&implemented));
    assert!(cpu_template_fingerprint_dump_implemented.is_disjoint(&logger_implemented));
    assert!(cpu_template_fingerprint_dump_implemented.is_disjoint(&metrics_implemented));
    assert!(cpu_template_fingerprint_dump_implemented.is_disjoint(&metrics_aggregate_implemented));
    assert!(cpu_template_fingerprint_dump_implemented.is_disjoint(&tracing_implemented));
    assert!(
        cpu_template_fingerprint_dump_implemented.is_disjoint(&cpu_template_helper_implemented)
    );
    assert!(cpu_template_fingerprint_dump_implemented.is_disjoint(&cpu_template_strip_implemented));
    assert!(cpu_template_fingerprint_dump_implemented.is_disjoint(&impossible));
    assert!(cpu_template_fingerprint_compare_implemented.is_disjoint(&implemented));
    assert!(cpu_template_fingerprint_compare_implemented.is_disjoint(&logger_implemented));
    assert!(cpu_template_fingerprint_compare_implemented.is_disjoint(&metrics_implemented));
    assert!(
        cpu_template_fingerprint_compare_implemented.is_disjoint(&metrics_aggregate_implemented)
    );
    assert!(cpu_template_fingerprint_compare_implemented.is_disjoint(&tracing_implemented));
    assert!(
        cpu_template_fingerprint_compare_implemented.is_disjoint(&cpu_template_helper_implemented)
    );
    assert!(
        cpu_template_fingerprint_compare_implemented.is_disjoint(&cpu_template_strip_implemented)
    );
    assert!(
        cpu_template_fingerprint_compare_implemented
            .is_disjoint(&cpu_template_fingerprint_dump_implemented)
    );
    assert!(cpu_template_fingerprint_compare_implemented.is_disjoint(&impossible));
    assert!(cpu_template_aggregate_implemented.is_disjoint(&implemented));
    assert!(cpu_template_aggregate_implemented.is_disjoint(&logger_implemented));
    assert!(cpu_template_aggregate_implemented.is_disjoint(&metrics_implemented));
    assert!(cpu_template_aggregate_implemented.is_disjoint(&metrics_aggregate_implemented));
    assert!(cpu_template_aggregate_implemented.is_disjoint(&tracing_implemented));
    assert!(cpu_template_aggregate_implemented.is_disjoint(&cpu_template_helper_implemented));
    assert!(cpu_template_aggregate_implemented.is_disjoint(&cpu_template_strip_implemented));
    assert!(
        cpu_template_aggregate_implemented.is_disjoint(&cpu_template_fingerprint_dump_implemented)
    );
    assert!(
        cpu_template_aggregate_implemented
            .is_disjoint(&cpu_template_fingerprint_compare_implemented)
    );
    assert!(cpu_template_aggregate_implemented.is_disjoint(&impossible));
    ...
    assert!(
        cpu_template_fingerprint_dump_implemented
            .iter()
            .all(|id| owned.contains(id))
    );
    assert!(
    ...
    for id in &cpu_template_fingerprint_dump_implemented {
        let capability = by_id
            .get(id)
            .expect("CPU-template fingerprint dump identity must exist");
        assert_eq!(
            capability.disposition,
            Disposition::ImplementedAndVerified,
            "CPU-template fingerprint dump identity must be terminal: {id}"
        );
        assert!(!capability.implementation.is_empty());
        assert!(!capability.validation.is_empty());
        assert!(capability.exclusion.is_none());
        assert!(
            contract.contains(&format!("| `{id}` | #1794 | `implemented-and-verified` |")),
            "contract must record implemented CPU-template fingerprint dump result: {id}"
        );
    }

    for id in &cpu_template_fingerprint_compare_implemented {
        let capability = by_id
            .get(id)
            .expect("CPU-template fingerprint compare identity must exist");
        assert_eq!(
            capability.disposition,
            Disposition::ImplementedAndVerified,
            "CPU-template fingerprint compare identity must be terminal: {id}"
        );
        assert!(!capability.implementation.is_empty());
        assert!(!capability.validation.is_empty());
        assert!(capability.exclusion.is_none());
        assert!(
            contract.contains(&format!("| `{id}` | #1794 | `implemented-and-verified` |")),
            "contract must record implemented CPU-template fingerprint compare result: {id}"
        );
    }

    for id in &cpu_template_aggregate_implemented {
        let capability = by_id
            .get(id)
            .expect("aggregate CPU-template identity must exist");
        assert_eq!(
            capability.disposition,
            Disposition::ImplementedAndVerified,
            "aggregate CPU-template identity must be terminal: {id}"
        );
        assert!(!capability.implementation.is_empty());
        assert!(!capability.validation.is_empty());
        assert!(capability.exclusion.is_none());
        assert!(
            contract.contains(&format!("| `{id}` | #1795 | `implemented-and-verified` |")),
            "contract must record implemented aggregate CPU-template result: {id}"
        );
    }
    ...
    for id in &wave7_aggregate_implemented {
        let capability = by_id.get(id).expect("Wave 7 aggregate identity must exist");
        assert_eq!(
            capability.disposition,
            Disposition::ImplementedAndVerified,
            "Wave 7 aggregate identity must be terminal: {id}"
        );
        assert!(!capability.implementation.is_empty());
        assert!(!capability.validation.is_empty());
        assert!(capability.exclusion.is_none());
        assert!(
            contract.contains(&format!("| `{id}` | #1799 | `implemented-and-verified` |")),
            "contract must record implemented Wave 7 aggregate result: {id}"
        );
    }
    ...
    let selected_without_guest_workflow_or_formal_verification = implemented
        ...
        .chain(wave7_aggregate_implemented.iter())
    ...
    assert!(wave7_aggregate_implemented.is_disjoint(&guest_workflow_implemented));
    assert!(wave7_aggregate_implemented.is_disjoint(&formal_verification_implemented));
    assert!(wave7_aggregate_implemented.is_disjoint(&specification_benchmark_implemented));
    ...
    assert!(normalized_contract.contains("376 implemented, nine audit-required"));
    assert!(normalized_contract.contains("80 implemented and 13 proven-platform-impossible"));
}
...
fn checked_cpu_template_helper_compatibility_is_terminal_and_fail_closed() {
    ...
    for id in CPU_TEMPLATE_AGGREGATE_CAPABILITY_IDS {
        let capability = by_id
            .get(id)
            .unwrap_or_else(|| panic!("terminal aggregate capability must exist: {id}"));
        assert_eq!(
            capability.disposition,
            Disposition::ImplementedAndVerified,
            "{id}"
    ...
    let error = validate_cpu_template_helper_transition(&hybrid)
        ...
        .to_string();
    assert!(error.contains("exact ordered"));
    ...
    for capability in &mut dump_verify_terminal.capabilities {
        if CPU_TEMPLATE_STRIP_COMPATIBILITY_CAPABILITY_IDS.contains(&capability.id.as_str())
            ...
            || CPU_TEMPLATE_FINGERPRINT_COMPARE_COMPATIBILITY_CAPABILITY_IDS
                .contains(&capability.id.as_str())
            || CPU_TEMPLATE_AGGREGATE_CAPABILITY_IDS.contains(&capability.id.as_str())
        {
            capability.disposition = Disposition::AuditRequired;
            capability.implementation.clear();
            capability.validation.clear();
    ...
    let mut aggregate_hybrid = inventory.clone();
    let capability = aggregate_hybrid
        .capabilities
        .iter_mut()
        .find(|capability| capability.id == CPU_TEMPLATE_AGGREGATE_CAPABILITY_IDS[0])
        .expect("aggregate capability must exist");
    capability.disposition = Disposition::AuditRequired;
    capability.implementation.clear();
    capability.validation.clear();
    let error = validate_cpu_template_helper_transition(&aggregate_hybrid)
        .expect_err("a partial #1795 aggregate transition must fail")
        .to_string();
    assert!(error.contains("exact ordered"));
}
...
fn checked_cpu_template_strip_compatibility_is_terminal_and_fail_closed() {
    ...
    assert!(strip_ids.is_disjoint(&CPU_TEMPLATE_AGGREGATE_CAPABILITY_IDS.into_iter().collect()));
...
fn checked_cpu_template_fingerprint_dump_compatibility_is_terminal_and_fail_closed() {
    ...
    let by_id = inventory
        ...
        .map(|capability| (capability.id.as_str(), capability))
        .collect::<BTreeMap<_, _>>();
    let contract_path = "compat/firecracker/v1.16.0/cpu-template-fingerprint-contract.md";
    let contract = std::fs::read_to_string(repository_root.join(contract_path))
        .expect("CPU-template fingerprint contract must be readable");

    validate_cpu_template_fingerprint_dump_compatibility(&manifest, &inventory, &repository_root)
        .expect("checked CPU-template fingerprint dump scope must be terminal");

    let fingerprint_ids = CPU_TEMPLATE_FINGERPRINT_DUMP_COMPATIBILITY_CAPABILITY_IDS
        .into_iter()
        .collect::<BTreeSet<_>>();
    assert_eq!(
        fingerprint_ids.len(),
        CPU_TEMPLATE_FINGERPRINT_DUMP_COMPATIBILITY_CAPABILITY_IDS.len(),
        "CPU-template fingerprint dump capability set must remain exact and duplicate-free"
    );
    assert!(
        fingerprint_ids.is_disjoint(
            &CPU_TEMPLATE_HELPER_COMPATIBILITY_CAPABILITY_IDS
                .into_iter()
                .collect()
        )
    ...
    assert!(
        fingerprint_ids.is_disjoint(&CPU_TEMPLATE_AGGREGATE_CAPABILITY_IDS.into_iter().collect())
    );
    for id in CPU_TEMPLATE_FINGERPRINT_DUMP_COMPATIBILITY_CAPABILITY_IDS {
        let capability = by_id
            .get(id)
            .unwrap_or_else(|| panic!("CPU-template fingerprint capability must exist: {id}"));
        assert_eq!(
            capability.disposition,
            Disposition::ImplementedAndVerified,
            "fingerprint dump capability must remain terminal: {id}"
        );
        assert_eq!(
            contract
                .matches(&format!("| `{id}` | `implemented-and-verified` |"))
                .count(),
            1,
            "fingerprint contract must contain one exact terminal row: {id}"
        );
    }
    for id in CPU_TEMPLATE_FINGERPRINT_COMPARE_COMPATIBILITY_CAPABILITY_IDS {
        assert_eq!(
            by_id
                .get(id)
                .expect("terminal CPU-template fingerprint compare record must exist")
                .disposition,
            Disposition::ImplementedAndVerified,
            "CPU-template fingerprint compare record must remain terminal: {id}"
        );
    }
    for id in CPU_TEMPLATE_AGGREGATE_CAPABILITY_IDS {
        let capability = by_id
    ...
    let capability = hybrid
        ...
        .iter_mut()
        .find(|capability| {
            capability.id == CPU_TEMPLATE_FINGERPRINT_DUMP_COMPATIBILITY_CAPABILITY_IDS[0]
        })
        .expect("fingerprint capability must exist");
    capability.disposition = Disposition::AuditRequired;
    capability.implementation.clear();
    capability.validation.clear();
    let error = validate_cpu_template_helper_transition(&hybrid)
        .expect_err("partial #1866 transition must fail")
        .to_string();
    assert!(error.contains("exact ordered"));
    ...
    let error = validate_cpu_template_helper_transition(&dependency_regression)
        .expect_err("fingerprint dump cannot be terminal before strip")
        .to_string();
    assert!(error.contains("exact ordered"));

    let mut strip_terminal = inventory.clone();
    for capability in &mut strip_terminal.capabilities {
        if CPU_TEMPLATE_FINGERPRINT_DUMP_COMPATIBILITY_CAPABILITY_IDS
            .contains(&capability.id.as_str())
            || CPU_TEMPLATE_FINGERPRINT_COMPARE_COMPATIBILITY_CAPABILITY_IDS
                .contains(&capability.id.as_str())
            || CPU_TEMPLATE_AGGREGATE_CAPABILITY_IDS.contains(&capability.id.as_str())
    ...
    validate_cpu_template_helper_transition(&strip_terminal)
        .expect("exact #1793 terminal handoff must remain accepted");
    validate_cpu_template_strip_compatibility(&manifest, &strip_terminal, &repository_root)
        .expect("strip certification must not require fingerprint completion");
    let error = validate_cpu_template_fingerprint_dump_compatibility(
        &manifest,
        &strip_terminal,
        &repository_root,
    )
    .expect_err("fingerprint final gate must reject the historical handoff")
    .to_string();
    assert!(error.contains("requires implemented-and-verified capability"));
}
...
fn checked_cpu_template_fingerprint_compare_compatibility_is_terminal_and_fail_closed() {
    ...
    let by_id = inventory
        ...
        .map(|capability| (capability.id.as_str(), capability))
        .collect::<BTreeMap<_, _>>();
    let contract_path = "compat/firecracker/v1.16.0/cpu-template-fingerprint-compare-contract.md";
    let contract = std::fs::read_to_string(repository_root.join(contract_path))
        .expect("CPU-template fingerprint compare contract must be readable");
    ...
    .expect("checked CPU-template fingerprint compare scope must be terminal");

    let compare_ids = CPU_TEMPLATE_FINGERPRINT_COMPARE_COMPATIBILITY_CAPABILITY_IDS
        .into_iter()
        .collect::<BTreeSet<_>>();
    assert_eq!(
        compare_ids.len(),
        CPU_TEMPLATE_FINGERPRINT_COMPARE_COMPATIBILITY_CAPABILITY_IDS.len(),
        "CPU-template fingerprint compare capability set must remain exact and duplicate-free"
    );
    for dependency in [
        CPU_TEMPLATE_HELPER_COMPATIBILITY_CAPABILITY_IDS
            .into_iter()
        ...
        CPU_TEMPLATE_FINGERPRINT_DUMP_COMPATIBILITY_CAPABILITY_IDS
            .into_iter()
            .collect(),
        CPU_TEMPLATE_AGGREGATE_CAPABILITY_IDS.into_iter().collect(),
    ] {
        assert!(compare_ids.is_disjoint(&dependency));
    }
    for id in CPU_TEMPLATE_FINGERPRINT_COMPARE_COMPATIBILITY_CAPABILITY_IDS {
        let capability = by_id.get(id).unwrap_or_else(|| {
            panic!("CPU-template fingerprint compare capability must exist: {id}")
        });
        assert_eq!(
            capability.disposition,
            Disposition::ImplementedAndVerified,
            "fingerprint compare capability must remain terminal: {id}"
        );
        assert!(!capability.implementation.is_empty(), "{id}");
        assert!(!capability.validation.is_empty(), "{id}");
        assert_eq!(
            contract
                ...
                .count(),
            1,
            "fingerprint compare contract must contain one exact terminal row: {id}"
        );
    }
    for id in CPU_TEMPLATE_AGGREGATE_CAPABILITY_IDS {
        let capability = by_id
    ...
    let capability = hybrid
        ...
        .iter_mut()
        .find(|capability| {
            capability.id == CPU_TEMPLATE_FINGERPRINT_COMPARE_COMPATIBILITY_CAPABILITY_IDS[0]
        })
        .expect("compare capability must exist");
    capability.disposition = Disposition::AuditRequired;
    capability.implementation.clear();
    capability.validation.clear();
    let error = validate_cpu_template_helper_transition(&hybrid)
        .expect_err("partial #1867 transition must fail")
        .to_string();
    assert!(error.contains("exact ordered"));
    ...
    let error = validate_cpu_template_helper_transition(&dependency_regression)
        .expect_err("fingerprint compare cannot be terminal before dump")
        .to_string();
    assert!(error.contains("exact ordered"));

    let mut dump_terminal = inventory.clone();
    for capability in &mut dump_terminal.capabilities {
        if CPU_TEMPLATE_FINGERPRINT_COMPARE_COMPATIBILITY_CAPABILITY_IDS
            .contains(&capability.id.as_str())
            || CPU_TEMPLATE_AGGREGATE_CAPABILITY_IDS.contains(&capability.id.as_str())
        {
            capability.disposition = Disposition::AuditRequired;
    ...
    validate_cpu_template_helper_transition(&dump_terminal)
        .expect("exact #1866 terminal handoff must remain accepted");
    validate_cpu_template_fingerprint_dump_compatibility(
        &manifest,
        &dump_terminal,
        &repository_root,
    )
    .expect("dump certification must not require compare completion");
    let error = validate_cpu_template_fingerprint_compare_compatibility(
        &manifest,
        &dump_terminal,
        &repository_root,
    )
    .expect_err("compare final gate must reject the #1866 handoff")
    .to_string();
    assert!(error.contains("requires implemented-and-verified capability"));
}
...
fn checked_cpu_template_aggregate_compatibility_is_terminal_and_fail_closed() {
    ...
    let manifest = read_source_manifest(&repository_root.join(SOURCE_MANIFEST_PATH))
        .expect("checked source manifest must parse");
    let audit =
        read_cpu_template_helper_audit(&repository_root.join(CPU_TEMPLATE_HELPER_AUDIT_PATH))
            .expect("checked CPU-template helper audit must parse");

    validate_cpu_template_compatibility(&manifest, &inventory, &audit, &repository_root)
        .expect("checked aggregate CPU-template compatibility scope must be terminal");
    for id in CPU_TEMPLATE_AGGREGATE_CAPABILITY_IDS {
        let capability = inventory
            .capabilities
            .iter()
            .find(|capability| capability.id == id)
            .unwrap_or_else(|| panic!("aggregate CPU-template capability must exist: {id}"));
        assert_eq!(
            capability.disposition,
            Disposition::ImplementedAndVerified,
            "aggregate capability must remain terminal: {id}"
        );
        assert!(!capability.implementation.is_empty(), "{id}");
        assert!(!capability.validation.is_empty(), "{id}");
    }
    ...
    let mut historical = inventory.clone();
    for capability in &mut historical.capabilities {
        if CPU_TEMPLATE_AGGREGATE_CAPABILITY_IDS.contains(&capability.id.as_str()) {
            capability.disposition = Disposition::AuditRequired;
            capability.implementation.clear();
            capability.validation.clear();
        }
    ...
    validate_cpu_template_helper_transition(&historical)
        .expect("the exact #1867 terminal handoff must remain accepted");
    validate_cpu_template_fingerprint_compare_compatibility(
        &manifest,
        &historical,
        &repository_root,
    )
    .expect("the compare certification must not require aggregate completion");
    let error =
        validate_cpu_template_compatibility(&manifest, &historical, &audit, &repository_root)
            .expect_err("aggregate certification must reject the historical handoff")
            .to_string();
    assert!(error.contains("requires exact terminal capability evidence"));

    let certification_error = |inventory, audit| {
        validate_cpu_template_compatibility(&manifest, inventory, audit, &repository_root)
            .expect_err("mutated aggregate CPU-template scope must fail certification")
            .to_string()
    };
    ...
    let capability = hybrid
        ...
        .iter_mut()
        .find(|capability| capability.id == CPU_TEMPLATE_AGGREGATE_CAPABILITY_IDS[0])
        .expect("aggregate CPU-template capability must exist");
    capability.disposition = Disposition::AuditRequired;
    capability.implementation.clear();
    capability.validation.clear();
    let error = certification_error(&hybrid, &audit);
    assert!(error.contains("requires one exact ordered"));
    assert!(error.contains("requires exact terminal capability evidence"));
    ...
    evidence_drift
        .capabilities
        .iter_mut()
        .find(|capability| capability.id == CPU_TEMPLATE_AGGREGATE_CAPABILITY_IDS[1])
        .expect("aggregate CPU-template capability must exist")
        .validation
        .swap(0, 1);
    assert!(
        certification_error(&evidence_drift, &audit)
            .contains("requires exact terminal capability evidence")
    );
    ...
    missing_foundation
        ...
        .pop();
    assert!(
        certification_error(&inventory, &missing_foundation)
            .contains("exact ordered implemented foundation set")
    );

    let mut stale_evidence = audit.clone();
    stale_evidence.scenarios[0].validation[0] = Reference::Local {
        path: "tools/cpu-template-helper/tests/hvf_e2e.rs".to_string(),
        anchor: Some("fn missing_signed_composition_test".to_string()),
    };
    assert!(certification_error(&inventory, &stale_evidence).contains("anchor does not resolve"));
}
...
fn snapshot_paging_terminal_policy_is_stable() {
    ...
    assert_eq!(count(Disposition::ImplementedAndVerified), 379);
...
fn delivery_closure_policy_is_stable() {
    ...
    for id in CPU_TEMPLATE_FINGERPRINT_DUMP_COMPATIBILITY_CAPABILITY_IDS {
        assert_eq!(
            by_id
                .get(id)
                .expect("terminal CPU-template fingerprint dump record must exist")
                .disposition,
            Disposition::ImplementedAndVerified,
            "CPU-template fingerprint dump record must remain terminal: {id}"
        );
    }
    for id in CPU_TEMPLATE_FINGERPRINT_COMPARE_COMPATIBILITY_CAPABILITY_IDS {
        assert_eq!(
    ...
    for id in CPU_TEMPLATE_AGGREGATE_CAPABILITY_IDS {
        let capability = by_id
            .get(id)
            .expect("terminal aggregate CPU-template record must exist");
        assert_eq!(
            capability.disposition,
            Disposition::ImplementedAndVerified,
            "aggregate CPU-template record must remain terminal: {id}"
        );
        assert!(!capability.implementation.is_empty(), "{id}");
        assert!(!capability.validation.is_empty(), "{id}");
    }
    ...
    let original = IMPLEMENTED_ORIGINAL
        ...
        .chain(CPU_TEMPLATE_AGGREGATE_CAPABILITY_IDS)
