...
mod tests {
    ...
    fn exact_contract_scope_is_fail_closed() {
        let rows = SPECIFICATION_BENCHMARK_CAPABILITY_IDS
            ...
            .join("\n");
        let exact = format!(
            "SPECIFICATION.md docs/network-performance.md scripts/specification-benchmark.py whole-process RSS logger.missed_metrics_count #1378 371 implemented-and-verified 377/6/3/32 377/5/3/33 379/3/3/33 380/3/2/33 381/3/1/33\n{rows}"
        );
        let mut errors = Vec::new();
