...
general registers, raw core, EL1 translation, and thread-context system-
register subsets, baseline SIMD/FP state, raw virtual-timer state, CPU-level
IRQ/FIQ pending injection levels, opaque GIC device state, and raw EL1 GIC ICC
CPU-interface registers. These are internal snapshot-feasibility primitives
only: public snapshot create/load, restore, a persistent schema, remaining vCPU
state, EL2 GIC CPU-interface state, and complete emulated-device state remain
unsupported.
