...
this request, `host_dev_name` is stored as configuration only; bangbang does not
open host networking resources, deliver packets to a host backend, or attach a
virtio-net device yet. Internal helpers can prepare inactive virtio-net device
resources from stored configs, register them in a fresh internal MMIO
dispatcher, and derive FDT-ready virtio-mmio metadata without opening host
networking resources. Startup still does not write network nodes into the guest
FDT.
