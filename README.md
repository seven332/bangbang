...
`bangbang` binds the configured socket path, serves `GET /`, `GET /version`, `GET /vm/config`, `GET /machine-config`, pre-boot `PUT /machine-config` configuration storage, pre-boot `PUT /boot-source` configuration storage, pre-boot `PUT /drives/{drive_id}` configuration storage, and process-owned `PUT /actions` startup preparation, and stays running until `SIGINT` or `SIGTERM` requests shutdown. Unsupported Firecracker process options such as `--config-file`, `--no-api`, seccomp, logging, metrics, snapshot, MMDS, and PCI flags are rejected instead of ignored.
...
preparation and returns `204 No Content` only after the session is retained and
the instance state becomes `Running`. Preparation failures return `400 Bad
Request` with a `fault_message` body and leave the instance `Not started`.
Continuous vCPU execution, boot smoke output, pause/resume, and public runner
loop scheduling remain deferred.
