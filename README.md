...
- complete interrupt delivery, including timer EOI/deactivation-driven unmasking, public configured guest execution beyond startup preparation, unbounded or public continuous vCPU run loops, HVF runner-loop notification scheduling, public serial output streaming, serial/backend interrupt wiring beyond the internal boot block notification and retained serial capture paths, device-backed feature negotiation, indirect descriptors, device-backed MMIO loops, complete device emulation, multi-vCPU setup, or PSCI behavior
...
then returns `204 No Content` only after the session is retained and the
instance state becomes `Running`. Preparation failures return `400 Bad Request`
with a `fault_message` body and leave the instance `Not started`. Continuous
vCPU execution, boot smoke output, pause/resume, public serial streaming, and
public runner loop scheduling remain deferred.
