...
`bangbang` binds the configured socket path, serves `GET /`, `GET /version`, `GET /machine-config`, pre-boot `PUT /machine-config` configuration storage, and pre-boot `PUT /drives/{drive_id}` configuration storage, recognizes parser-level `PUT /boot-source` with an unsupported fault, and stays running until `SIGINT` or `SIGTERM` requests shutdown. Unsupported Firecracker process options such as `--config-file`, `--no-api`, seccomp, logging, metrics, snapshot, MMDS, and PCI flags are rejected instead of ignored.
