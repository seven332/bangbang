...
`boot-source` is included after it is configured. Metrics output configuration
is omitted because it is process observability state, not guest configuration.
Unsupported sections such as logger, network, vsock, and snapshots are omitted
until their models exist.
...
writing output. Full Firecracker metrics counters, periodic flush, logger
integration, and CLI `--metrics-path` support remain deferred.
