...
- public `/boot-source` behavior beyond recording valid boot-source configuration, successful public `/actions` behavior, or public block-device behavior beyond recording valid `/drives/{drive_id}` configuration
...
`bangbang` binds the configured socket path, serves `GET /`, `GET /version`, `GET /machine-config`, pre-boot `PUT /machine-config` configuration storage, pre-boot `PUT /boot-source` configuration storage, and pre-boot `PUT /drives/{drive_id}` configuration storage, and stays running until `SIGINT` or `SIGTERM` requests shutdown. Unsupported Firecracker process options such as `--config-file`, `--no-api`, seccomp, logging, metrics, snapshot, MMDS, and PCI flags are rejected instead of ignored.
