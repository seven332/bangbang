...
mod macos_arm64 {
    ...
    const DIRECT_ROOTFS_MMDS_SNAPSHOT_SECTORS: u64 = 5;
    ...
    fn signed_executable_streams_default_serial_stdio_across_lifecycle_boundaries() {
        ...
        assert_capture_ready_snapshot_succeeds(
            ...
            ExpectedSnapshotOptionalProducts::NONE,
        );
    ...
    fn run_signed_balloon_snapshot_continuation(enable_pci: bool) {
        ...
        assert_capture_ready_snapshot_succeeds(
            ...
            ExpectedSnapshotOptionalProducts::STORAGE_AND_BALLOON,
        );
    ...
    fn run_signed_memory_hotplug_snapshot_continuation(enable_pci: bool) {
        ...
        assert_capture_ready_snapshot_succeeds(
            &socket_path,
            test_dir.path(),
            &format!("paused {transport} memory-hotplug exact-2.11 publication"),
            ExpectedSnapshotOptionalProducts::STORAGE_AND_MEMORY_HOTPLUG,
        );
        let state_path = test_dir.path().join("capture-ready.state");
        let memory_path = test_dir.path().join("capture-ready.memory");
    ...
    fn run_signed_entropy_capture_lifecycle(enable_pci: bool) {
        ...
        assert_capture_ready_snapshot_succeeds(
            ...
            ExpectedSnapshotOptionalProducts::STORAGE_AND_ENTROPY,
        );
        assert_no_content_response(
            &http_json(&socket_path, "PATCH", "/vm", r#"{"state":"Resumed"}"#),
    ...
    fn assert_pending_entropy_snapshot(
        ...
        assert!(
            ...
            "{context} entropy certification artifact should not add balloon state"
        );
        let expected_transport = if enable_pci {
            SnapshotV2DeviceTransportKind::Pci
        } else {
    ...
    fn run_signed_remaining_device_certification(enable_pci: bool) {
        ...
        assert_capture_ready_snapshot_succeeds(
            ...
            ExpectedSnapshotOptionalProducts::STORAGE_ENTROPY_BALLOON_AND_MEMORY_HOTPLUG,
        );
        assert_no_content_response(
            &http_json(&socket_path, "PATCH", "/vm", r#"{"state":"Resumed"}"#),
    ...
    fn signed_executable_hotplugs_replaces_and_reuses_async_block_over_product_pci() {
        ...
        assert_capture_ready_snapshot_succeeds(
            ...
            ExpectedSnapshotOptionalProducts::STORAGE,
        );
        write_block_marker_at(
            &control_backing_path,
            bangbang_runtime::block::VIRTIO_BLOCK_SECTOR_SIZE,
    ...
    fn signed_executable_hotplugs_mmds_network_and_reuses_product_pci_slot() {
        ...
        assert_capture_ready_snapshot_succeeds(
            ...
            ExpectedSnapshotOptionalProducts::STORAGE_AND_NETWORK,
        );
        write_block_marker_at(
            &control_backing_path,
    ...
    const SNAPSHOT_VSOCK_METRIC_FIELDS: [&str; 20] = [
        "activate_fails",
        "cfg_fails",
        "conn_event_fails",
        "conns_added",
    ...
    fn snapshot_vsock_metric(vsock: &serde_json::Value, field: &str) -> u64 {
        vsock
            .get(field)
            .and_then(serde_json::Value::as_u64)
            .expect("snapshot-vsock metric should be an unsigned integer")
    }

    fn assert_snapshot_vsock_metrics_shape(vsock: &serde_json::Value, context: &str) {
        let object = vsock
            .as_object()
            .expect("snapshot-vsock metrics should be an object");
        assert_eq!(
            object.len(),
            SNAPSHOT_VSOCK_METRIC_FIELDS.len(),
            "{context} snapshot-vsock metrics must contain the exact pinned object: {vsock}"
        );
        for field in SNAPSHOT_VSOCK_METRIC_FIELDS {
            assert!(
                object
                    .get(field)
                    .and_then(serde_json::Value::as_u64)
                    .is_some(),
                "{context} snapshot-vsock metrics must contain numeric field {field}: {vsock}"
            );
        }
    }

    fn latest_snapshot_vsock_metrics(
        metrics_path: &Path,
    ...
    fn flush_direct_snapshot_vsock_metrics(
        ...
        let (generation, vsock) = latest_snapshot_vsock_metrics(metrics_path);
        assert_snapshot_vsock_metrics_shape(&vsock, context);
        assert!(
            snapshot_vsock_metric(&vsock, "conns_added") >= expected_connections,
        ...
        for field in [
            ...
            assert!(
                snapshot_vsock_metric(&vsock, field) > 0,
                "{context} snapshot-vsock metrics must record {field}: {generation}"
            );
        }
        // A restored queue can retain guest-posted RX buffers and deliver host
        // work without a fresh guest notification. Focused tests pin the
        // source-admission semantics of the exact queue-event field.
        assert!(
            snapshot_vsock_metric(&vsock, "conns_removed")
                <= snapshot_vsock_metric(&vsock, "conns_added"),
            "{context} snapshot-vsock removals must not exceed additions: {generation}"
        );
        for field in [
            "activate_fails",
            "cfg_fails",
            ...
            assert_eq!(
                snapshot_vsock_metric(&vsock, field),
                0,
                "{context} successful snapshot-vsock traffic must leave {field} at zero: {generation}"
            );
        }

        assert_no_content_response(
            &http_put_json(api_socket, "/actions", r#"{"action_type":"FlushMetrics"}"#),
            &format!("FlushMetrics {context} zero-event snapshot-vsock interval"),
        );
        let (zero_generation, zero_vsock) = latest_snapshot_vsock_metrics(metrics_path);
        assert_snapshot_vsock_metrics_shape(&zero_vsock, context);
        for field in SNAPSHOT_VSOCK_METRIC_FIELDS {
            assert_eq!(
                snapshot_vsock_metric(&zero_vsock, field),
                0,
    ...
    impl ExpectedSnapshotOptionalProducts {
        ...
        const STORAGE_AND_BALLOON: Self = Self {
            balloon: true,
            ..Self::STORAGE
        };
        const STORAGE_AND_MEMORY_HOTPLUG: Self = Self {
            memory_hotplug: true,
            ..Self::STORAGE
        };
        const STORAGE_AND_NETWORK: Self = Self {
            network: true,
            ..Self::STORAGE
        };
        const STORAGE_ENTROPY_BALLOON_AND_MEMORY_HOTPLUG: Self = Self {
            entropy: true,
            balloon: true,
            memory_hotplug: true,
            ..Self::STORAGE
    ...
    fn assert_capture_ready_snapshot_succeeds(
        ...
        directory: &Path,
        context: &str,
        expected: ExpectedSnapshotOptionalProducts,
    ) {
        ...
        assert_eq!(
            state.entropy().is_some(),
            expected.entropy,
            "{context} entropy profile presence should match"
        );
        assert_eq!(
