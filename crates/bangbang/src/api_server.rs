...
fn handle_api_request(request: ApiRequest, vmm: &mut impl VmmRequestHandler) -> HttpResponse {
    ...
    match request {
        ...
        ApiRequest::GetBalloonHintingStatus => handle_balloon_hinting_status(
            vmm.handle_get_request(GetApiRequest::BalloonHintingStatus),
        ),
...
fn handle_balloon_hinting_status(
    result: Result<VmmData, bangbang_runtime::VmmActionError>,
) -> HttpResponse {
    match result {
        Ok(VmmData::BalloonHintingStatus(status)) => HttpResponse::balloon_hinting_status(
            balloon_hinting_status_response_from_runtime(status),
        ),
        Ok(
            VmmData::Empty
            | VmmData::VmmVersion(_)
...
fn balloon_hinting_status_response_from_runtime(
    status: BalloonHintingStatus,
) -> BalloonHintingStatusResponse {
    BalloonHintingStatusResponse::new(status.host_cmd(), status.guest_cmd())
}
...
mod tests {
    ...
    use bangbang_hvf::HvfArm64BootSnapshotV2CaptureError;
    use bangbang_runtime::balloon::{
    ...
    use bangbang_runtime::snapshot_artifact::{
        ...
        NativeSnapshotPublicationOutcome, NativeV2MemoryHotplugSnapshotCandidateState,
        NativeV2MemoryHotplugSnapshotPreparation, SnapshotArtifactPaths,
        ...
        ) -> Result<ProcessSnapshotV2RootLoadSuccess<Self::Session>, NativeV2SnapshotLoadError>
        {
            ...
            let root =
                DriveConfigInput::new("root", "root", "/private/fake-api-restored-root", true)
                    ...
                    .map_err(|_| {
                        NativeV2SnapshotLoadError::ProcessPreparation(BackendError::InvalidState(
                            "fake snapshot root configuration failed",
                        ))
                    })?;
    ...
    fn fake_native_v2_current_state(
        ...
        let memory_hotplug = memory_hotplug.map(|state| {
            state
                .encode(NATIVE_V2_MEMORY_HOTPLUG_STATE_COMPATIBILITY_VERSION)
                .expect("native-v2 fixture virtio-mem should encode")
        });
    ...
    fn fake_native_v2_memory_hotplug_state_for_request(
        ...
        let fixture = SnapshotV2MemoryHotplugState::decode(
            NATIVE_V2_MEMORY_HOTPLUG_STATE_COMPATIBILITY_VERSION,
            &decode_fixture_hex(hex),
        )
        .expect("native-v2 fixture virtio-mem should decode");
        assert_eq!(
            fixture.config(),
            config,
            "native-v2 fixture must match the requested virtio-mem configuration"
        );
        SnapshotV2MemoryHotplugState::try_new(
            config,
            fixture.config_space().with_requested_size(
                requested_size_mib
                    .checked_mul(MIB)
                    .expect("requested virtio-mem size should fit bytes"),
            ),
        ...
        .expect("request-specific native-v2 virtio-mem state should validate")
    }
    ...
    fn fake_native_v2_publication_memory(
        ...
        if let Some(state) = memory_hotplug {
            for queue in state.virtio().queues() {
                ...
                for (start, size) in [
                    (
                        queue.descriptor_table().raw_value(),
                        u64::from(queue.size()) * 16,
                    ),
                    (
                        queue.driver_ring().raw_value(),
                        u64::from(queue.size()) * 2 + 6,
                    ),
                    (
                        queue.device_ring().raw_value(),
                        u64::from(queue.size()) * 8 + 6,
                    ),
                ] {
                    ...
                    base_end = base_end.max(
                        start
                            ...
                            .expect("native-v2 fixture queue range should fit")
                            & !(BASE_ALIGNMENT - 1),
                    );
                }
            }
        ...
        let mut ranges = vec![
            GuestMemoryRange::new(GuestAddress::new(base_start), base_end - base_start)
                .expect("native-v2 fixture base memory should validate"),
        ];
        if let Some(state) = memory_hotplug {
            ...
            for plugged in state.plugged_ranges() {
                let start = plugged
                    ...
                    .expect("native-v2 fixture plugged-memory start should fit");
                let size = plugged
                    .block_count()
                    .checked_mul(config_space.block_size())
                    .expect("native-v2 fixture plugged-memory size should fit");
                ranges.push(
                    GuestMemoryRange::new(GuestAddress::new(start), size)
                        .expect("native-v2 fixture plugged memory should validate"),
                );
            }
        }
        ranges.sort_by_key(|range| range.start());
        let layout =
            GuestMemoryLayout::new(ranges).expect("native-v2 fixture memory should validate");
    ...
    fn get_mmds_after_preboot_get_returns_null_after_start() {
        let mut vmm = test_controller_with_starter(TestInstanceStarter::success());
        let preboot_get_response =
            handle_request_bytes(b"GET /mmds HTTP/1.1\r\nHost: localhost\r\n\r\n", &mut vmm);
        assert_eq!(
            preboot_get_response.status(),
            bangbang_api::http::StatusCode::Ok
        );
        assert_eq!(preboot_get_response.body(), "null");
        ...
        let start_response = put_action_over_socket(&mut vmm, "mmds-preget-a", "InstanceStart");
        assert!(start_response.starts_with("HTTP/1.1 204 No Content\r\n"));
    ...
    #[test]
    fn put_mmds_after_start_without_data_store_returns_fault() {
        let mut vmm = test_controller_with_starter(TestInstanceStarter::success());
        let boot_request = request_with_body(
            "PUT",
            "/boot-source",
        ...
        let start_response = put_action_over_socket(&mut vmm, "put-mmds-start", "InstanceStart");
        assert!(start_response.starts_with("HTTP/1.1 204 No Content\r\n"));
    ...
    #[test]
    fn put_mmds_after_preboot_get_succeeds_after_start() {
        let mut vmm = test_controller_with_starter(TestInstanceStarter::success());
        let preboot_get_response =
            handle_request_bytes(b"GET /mmds HTTP/1.1\r\nHost: localhost\r\n\r\n", &mut vmm);
        assert_eq!(
        ...
        let start_response = put_action_over_socket(&mut vmm, "mmds-preget-b", "InstanceStart");
        assert!(start_response.starts_with("HTTP/1.1 204 No Content\r\n"));
    ...
    fn returns_stateful_faults_for_runtime_snapshot_endpoints() {
        ...
        vmm.handle_action(VmmAction::PutDrive(
            DriveConfigInput::new("root", "root", "/tmp/snapshot-stateful-root", true)
                .with_is_read_only(true)
                .with_io_engine(DriveIoEngine::Sync),
        ))
        .expect("snapshot root should configure");
    ...
    fn public_snapshot_create_returns_no_content_and_preserves_committed_pair_on_collision() {
        ...
        vmm.handle_action(VmmAction::PutDrive(
            DriveConfigInput::new("root", "root", "/tmp/snapshot-rootfs.ext4", true)
                .with_is_read_only(true)
                .with_io_engine(DriveIoEngine::Sync),
        ))
        .expect("snapshot root should configure");
        ...
        assert!(
            request_over_socket(
                &mut vmm,
                "spm",
                &request_with_body("PATCH", "/hotplug/memory", r#"{"requested_size_mib":128}"#,),
            )
            .starts_with("HTTP/1.1 204 No Content\r\n")
        );
        assert!(
        ...
        memory_hotplug
            ...
            .expect("public exact-2.10 virtio-mem binding should close");
        ...
        let mut restored = test_controller_with_starter(TestInstanceStarter::snapshot_success());
        ...
        .to_string();
        let load_response = request_over_socket(
            &mut restored,
            "spl",
            &request_with_body("PUT", "/snapshot/load", &load_body),
        );
        assert!(load_response.starts_with("HTTP/1.1 204 No Content\r\n"));
        assert_eq!(
            restored.instance_info().state,
            bangbang_runtime::InstanceState::Paused
        );
        let status_response = request_over_socket(
            &mut restored,
            "spg",
            "GET /hotplug/memory HTTP/1.1\r\nHost: localhost\r\n\r\n",
        );
        ...
        assert!(
            status_response.contains(
                r#"{"block_size_mib":2,"plugged_size_mib":0,"requested_size_mib":128,"slot_size_mib":128,"total_size_mib":1024}"#
            ),
            "{status_response}"
        );
    ...
    fn stores_and_returns_balloon_config_over_socket() {
        ...
        let hinting_status_response = request_over_socket(
            &mut vmm,
            "b-hsr",
            "GET /balloon/hinting/status HTTP/1.1\r\nHost: localhost\r\n\r\n",
        );
    ...
    fn configured_metrics_records_snapshot_create_latencies_on_snapshot_fault() {
        ...
        vmm.handle_action(VmmAction::PutDrive(
            DriveConfigInput::new("root", "root", "/tmp/snapshot-metrics-root", true)
                .with_is_read_only(true)
                .with_io_engine(DriveIoEngine::Sync),
        ))
        .expect("snapshot root should configure");
    ...
    fn synchronous_snapshot_serializes_api_mmds_and_periodic_work_but_observes_cancellation() {
        ...
        vmm.handle_action(VmmAction::PutDrive(
            DriveConfigInput::new(
                "root",
                "root",
                "/tmp/snapshot-serialization-rootfs.ext4",
                true,
            )
            .with_is_read_only(true)
            .with_io_engine(DriveIoEngine::Sync),
