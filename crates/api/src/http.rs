...
impl MachineConfigRequest {
    ...
    pub const fn cpu_template_configured(&self) -> bool {
        self.cpu_template_configured
    }
...
fn parse_machine_config_request(body: &[u8]) -> Result<ApiRequest, RequestError> {
    let value = serde_json::from_slice::<serde_json::Value>(body)
        .map_err(|_| RequestError::MalformedRequest)?;
    let cpu_template_configured = json_object_contains_key(&value, "cpu_template");
...
mod tests {
    ...
    fn parses_put_machine_config_with_minimal_body() {
        ...
        assert!(!config.cpu_template_configured());
    ...
    fn parses_put_machine_config_with_firecracker_cpu_templates() {
        for (template, expected) in [
            ...
            assert!(config.cpu_template_configured(), "{template}");
        }
    }
