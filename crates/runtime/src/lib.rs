...
pub mod snapshot_diff_v2_13;
pub mod snapshot_entropy_v2_8;
...
impl VmmController {
    ...
    pub fn prepare_pmem_update(
        &self,
        input: pmem::PmemUpdateInput,
    ) -> Result<(pmem::PmemUpdate, pmem::PmemConfig), VmmActionError> {
        self.pmem_configs
            .prepare_update(input)
    ...
    fn handle_action_inner(&mut self, action: VmmAction) -> Result<VmmData, VmmActionError> {
        ...
        match action {
            ...
            VmmAction::PatchPmem(input) => {
                ...
                let (update, _) = self.prepare_pmem_update(input)?;
                if !update.is_noop() {
                    return Err(VmmActionError::PmemUpdateUnsupported);
                }
...
mod tests {
    ...
    const fn pmem_rate_limiter() -> PmemRateLimiterConfig {
    ...
    #[test]
    fn private_native_v2_balloon_commit_atomically_publishes_and_clears_all_configs() {
        let mut controller = VmmController::new("demo-1", "0.1.0", "bangbang");
        let machine = MachineConfigInput::new(2, 256)
            .validate()
            .expect("restored machine fixture should validate");
        ...
        let entropy = EntropyConfigInput::new()
            .validate()
            .expect("restored entropy fixture should validate");
        let balloon: BalloonConfig = BalloonConfigInput::new(64, false)
            .with_stats_polling_interval_s(30)
            .with_free_page_hinting(true)
            .into();
        let commit = SnapshotV2ControllerCommit::with_serial_storage_entropy_and_balloon_configs(
            machine,
            boot.clone(),
            Some(CaptureReadyStorageConfigs::new(
                vec![drive.clone()],
        ...
        assert!(!diagnostics.contains("/private/restored.img"));
        assert!(!diagnostics.contains("serial-grant"));
        ...
        assert_eq!(controller.serial_config(), &serial);
        assert_eq!(controller.entropy_config(), Some(entropy));
        assert_eq!(controller.balloon_config(), Some(balloon));
    ...
    #[test]
    fn put_pmem_accepts_rate_limiter_and_replaces_existing_config() {
        let mut controller = VmmController::new("demo-1", "0.1.0", "bangbang");
        controller
        ...
        let data = controller
            .handle_action(VmmAction::PutPmem(
                pmem_input("pmem0", "/tmp/pmem-new.img").with_rate_limiter(pmem_rate_limiter()),
            ))
            .expect("configured pmem rate limiter should be stored");
        ...
        assert_eq!(
            controller.pmem_configs()[0].rate_limiter(),
            Some(pmem_rate_limiter())
        );
    }
    ...
    #[test]
    fn patch_pmem_reaches_backend_boundary_without_mutating() {
        let mut controller = VmmController::new("demo-1", "0.1.0", "bangbang");
        controller
            .handle_action(VmmAction::PutPmem(pmem_input("pmem0", "/tmp/pmem.img")))
            .expect("pmem config should be stored");
        ...
        let err = controller
            .handle_action(VmmAction::PatchPmem(
                pmem_update_input("pmem0").with_rate_limiter(pmem_rate_limiter()),
            ))
            .expect_err("standalone controller cannot apply a live pmem update");

        assert_eq!(err, VmmActionError::PmemUpdateUnsupported);
        assert_eq!(controller.instance_info().state, InstanceState::Running);
        assert_eq!(controller.pmem_configs().len(), 1);
        assert_eq!(controller.pmem_configs()[0].id(), "pmem0");
        assert_eq!(controller.pmem_configs()[0].path_on_host(), "/tmp/pmem.img");
