...
full Firecracker metrics counters, immediate startup metrics flushing, full logger integration,
...
`FlushMetrics` yet. Minimal periodic metrics flushing uses the same runtime
metrics sink every 60 seconds while the VM is running. Public drive configuration is
...
recognized `PATCH /drives/{drive_id}` requests already map through a minimal internal VMM
...
| `--start-time-us <MICROS>` | parsed and reported in minimal metrics | Accepts non-negative `u64` microsecond values passed by Firecracker-style launchers. When provided, explicit runtime `FlushMetrics` and periodic runtime metrics output include `start_time_us`. Full `ProcessTimeReporter` parity remains deferred. |
| `--start-time-cpu-us <MICROS>` | parsed and reported in minimal metrics | Accepts non-negative `u64` microsecond values passed by Firecracker-style launchers. When provided, explicit runtime `FlushMetrics` and periodic runtime metrics output include `start_time_cpu_us`. Full `ProcessTimeReporter` parity remains deferred. |
| `--parent-cpu-time-us <MICROS>` | parsed and reported in minimal metrics | Accepts non-negative `u64` microsecond values passed by Firecracker-style launchers. When provided, explicit runtime `FlushMetrics` and periodic runtime metrics output include `parent_cpu_time_us`. Full `ProcessTimeReporter` parity remains deferred. |
...
metrics are configured and the VM is running, explicit `FlushMetrics` and
periodic runtime metrics flushes write provided values under the minimal `vmm`
metrics object; omitted timing arguments remain omitted. Parsed `GET /`,
...
| `PUT` | `/snapshot/create`, `/snapshot/load` | recognized; parser-level validation then rejected | Parses Firecracker-shaped snapshot create and load request bodies, rejects malformed or invalid bodies first, and then returns the snapshot-specific unsupported fault for valid bodies. Real snapshot creation, loading, file formats, dirty-page tracking, host memory backends, and VM/vCPU/device-state serialization remain deferred to future snapshot support. |
| `GET` | `/balloon` | recognized; rejected | Returns a balloon-specific unsupported fault. Real virtio-balloon device support, memory accounting, and runtime update behavior need a dedicated design. |
| `PUT`, `PATCH` | `/balloon` | recognized; parser-level validation then rejected | Parses Firecracker-shaped balloon config and target-size update request bodies, rejects malformed or invalid bodies first, and then returns the balloon-specific unsupported fault for valid bodies. Real virtio-balloon device support, memory accounting, and runtime update behavior need a dedicated design. |
| `GET` | `/balloon/statistics` | recognized; rejected | Returns a balloon-specific unsupported fault. Real statistics polling and guest-reported memory accounting need a dedicated design. |
| `PATCH` | `/balloon/statistics` | recognized; parser-level validation then rejected | Parses Firecracker-shaped statistics interval update request bodies, rejects malformed or invalid bodies first, and then returns the balloon-specific unsupported fault for valid bodies. Real statistics polling and guest-reported memory accounting need a dedicated design. |
| `PATCH` | `/balloon/hinting/start` | recognized; parser-level validation then rejected | Parses Firecracker-shaped free-page hinting start commands, including empty/default commands, rejects malformed or invalid bodies first, and then returns the balloon-specific unsupported fault for valid bodies. Real free-page hinting and reporting need a dedicated design. |
| `PATCH` | `/balloon/hinting/stop` | recognized; rejected | Returns a balloon-specific unsupported fault. Real free-page hinting and reporting need a dedicated design. |
...
| `PUT`, `PATCH` | `/pmem/{id}` | recognized; parser-level validation then rejected | Parses Firecracker-shaped pmem config and update request bodies, rejects malformed, invalid, or mismatched path/body IDs first, and then returns the pmem-specific unsupported fault for valid bodies. Real pmem device configuration, guest attachment, rate limiting, and runtime update behavior need a dedicated device design. |
| `PUT` | `/entropy` | recognized; parser-level validation then rejected | Parses Firecracker-shaped entropy config requests, rejects malformed or invalid bodies first, and then returns the entropy-specific unsupported fault for valid bodies. Real virtio-rng configuration storage, rate limiting, guest randomness wiring, and startup resource attachment need a dedicated device design. |
| `GET`, `PUT`, `PATCH` | `/hotplug/memory` | recognized; parser-level validation then rejected | Parses Firecracker-shaped `PUT` and `PATCH` memory hotplug request bodies, rejects malformed or schema-invalid bodies first, and then returns the memory-hotplug-specific unsupported fault for schema-valid bodies. Real virtio-mem device support, guest memory accounting, semantic config validation, and runtime memory update behavior need a dedicated design. |
...
| `DELETE` | `/drives/{drive_id}`, `/pmem/{id}`, `/network-interfaces/{iface_id}` | recognized; rejected | Firecracker routes bodyless hot-unplug requests in `parsed_request.rs`, but they are not in the `v1.16.0` swagger surface. bangbang returns existing device-update or pmem unsupported faults for bodyless recognized requests. Body-bearing `DELETE` requests fail first as malformed request shape. Real hot-unplug behavior remains deferred. |
...
| `PUT /snapshot/create` | `snapshot_type` | optional; unsupported after parser validation | Accepts Firecracker values `Full` and `Diff`, defaulting to `Full`, before returning the snapshot unsupported fault. Real snapshot type behavior remains deferred. |
| `PUT /snapshot/create` | `snapshot_path` | required; unsupported after parser validation | Parsed as a Firecracker-shaped path string before returning the snapshot unsupported fault. Host file creation remains deferred. |
| `PUT /snapshot/create` | `mem_file_path` | required; unsupported after parser validation | Parsed as a Firecracker-shaped path string before returning the snapshot unsupported fault. Guest memory persistence remains deferred. |
| `PUT /snapshot/create` | unknown fields | rejected | Matches Firecracker's strict request model behavior. |
| `PUT /snapshot/load` | `snapshot_path` | required; unsupported after parser validation | Parsed as a Firecracker-shaped path string before returning the snapshot unsupported fault. Host file reads and restore state remain deferred. |
| `PUT /snapshot/load` | `mem_backend` | required unless deprecated `mem_file_path` is present; unsupported after parser validation | Parsed as a strict object with `backend_path` and `backend_type`. Exactly one of `mem_backend` or `mem_file_path` must be present. Real memory backend loading remains deferred. |
...
| `PUT /snapshot/load` | `mem_file_path` | deprecated-compatible alternative to `mem_backend`; unsupported after parser validation | Accepted syntactically to match Firecracker's current parser. It must not be combined with `mem_backend`. Real deprecated-field handling and restore behavior remain deferred. |
| `PUT /snapshot/load` | `enable_diff_snapshots` | deprecated-compatible optional boolean | Accepted syntactically to match Firecracker's current parser before returning the snapshot unsupported fault. |
| `PUT /snapshot/load` | `track_dirty_pages`, `resume_vm`, `clock_realtime` | optional booleans | Parsed before returning the snapshot unsupported fault. Real dirty tracking, resume behavior, and architecture-specific clock restore remain deferred. |
| `PUT /snapshot/load` | `network_overrides` | optional | Parsed as Firecracker-shaped override entries with `iface_id` and `host_dev_name` before returning the snapshot unsupported fault. Real network restore override behavior remains deferred. |
| `PUT /snapshot/load` | `vsock_override` | optional | Parsed as a Firecracker-shaped object with `uds_path` before returning the snapshot unsupported fault. Real vsock restore override behavior remains deferred. |
...
| `PUT /snapshot/load` | unknown top-level fields or unknown `mem_backend` fields | rejected | Matches Firecracker's strict top-level and memory-backend request model behavior. |
| `PUT /balloon` | `amount_mib` | required; unsupported after parser validation | Parsed as an unsigned 32-bit Firecracker-shaped target balloon size before returning the balloon unsupported fault. Real memory accounting and guest-visible target changes remain deferred. |
| `PUT /balloon` | `deflate_on_oom` | required; unsupported after parser validation | Parsed as a boolean before returning the balloon unsupported fault. Real guest OOM deflation behavior remains deferred. |
| `PUT /balloon` | `stats_polling_interval_s` | optional | Missing values follow Firecracker's parser default shape. Real statistics polling remains deferred. |
| `PUT /balloon` | `free_page_hinting` | optional | Missing values follow Firecracker's parser default shape. Real free-page hinting remains deferred. |
| `PUT /balloon` | `free_page_reporting` | optional | Missing values follow Firecracker's parser default shape. Real free-page reporting remains deferred. |
| `PUT /balloon` | unknown fields | rejected | Matches Firecracker's strict request model behavior. |
| `PATCH /balloon` | `amount_mib` | required; unsupported after parser validation | Parsed as an unsigned 32-bit Firecracker-shaped target balloon size before returning the balloon unsupported fault. Real runtime target-size updates remain deferred. |
...
| `PUT /pmem/{id}` | path `id` | required | The API parser captures the path ID before returning the pmem unsupported fault. Invalid path IDs continue to fail as invalid path/method. |
| `PUT /pmem/{id}` | body `id` | required | The API parser rejects requests where this does not match the path `id`. |
| `PUT /pmem/{id}` | `path_on_host` | required; unsupported after parser validation | Required Firecracker-shaped host backing path. The parser accepts syntactically valid strings before returning the pmem unsupported fault; real host-path access and redacted open errors remain deferred with virtio-pmem support. |
| `PUT /pmem/{id}` | `root_device` | optional | Missing values follow Firecracker's parser default shape. Real root-device conflict validation remains deferred with virtio-pmem support. |
| `PUT /pmem/{id}` | `read_only` | optional | Missing values follow Firecracker's parser default shape. Real read-only mapping behavior remains deferred with virtio-pmem support. |
| `PUT /pmem/{id}` | `rate_limiter` | optional; unsupported after parser validation | Missing, `null`, or empty objects are accepted. Configured Firecracker-shaped rate limiter objects are validated before returning the pmem unsupported fault. Real pmem flush rate limiting remains deferred. |
| `PUT /pmem/{id}` | unknown fields | rejected | Matches Firecracker's strict request model behavior. |
| `PATCH /pmem/{id}` | path `id` | required | The API parser captures the path ID before returning the pmem unsupported fault. Invalid path IDs continue to fail as invalid path/method. |
| `PATCH /pmem/{id}` | body `id` | required | The API parser rejects requests where this does not match the path `id`. |
| `PATCH /pmem/{id}` | `rate_limiter` | optional; unsupported after parser validation | Missing, `null`, or empty objects are accepted. Configured Firecracker-shaped rate limiter objects are validated before returning the pmem unsupported fault. Real pmem runtime rate-limiter updates remain deferred. |
...
| `PATCH /network-interfaces/{iface_id}` | path `iface_id` | required | The API parser captures this value before rejecting the unsupported update operation. |
...
| `PATCH /network-interfaces/{iface_id}` | `rx_rate_limiter`, `tx_rate_limiter` | optional when absent or `null`; unsupported when configured | The parser validates Firecracker-shaped rate limiter objects before returning the existing network-interface-update unsupported fault. Real rate limiting belongs with future network update work. |
...
| `PUT /entropy` | `rate_limiter` | optional; unsupported when validly configured | Missing or `null` values are accepted by the parser before returning the entropy unsupported fault. Configured Firecracker-shaped rate limiter objects are validated before returning the same unsupported fault. Real virtio-rng rate limiting remains deferred. |
| `PUT /entropy` | unknown fields | rejected | Matches Firecracker's strict request model behavior. |
| `PUT /hotplug/memory` | `total_size_mib` | required; unsupported after parser validation | Required Firecracker-shaped hotpluggable-memory total size. The parser accepts syntactically valid unsigned integer values before returning the memory-hotplug unsupported fault. Real virtio-mem semantic validation and storage remain deferred. |
| `PUT /hotplug/memory` | `block_size_mib` | optional; unsupported after parser validation | Missing values use Firecracker's parser default shape. Present values must be unsigned integers. Real block-size semantic validation remains deferred with virtio-mem support. |
| `PUT /hotplug/memory` | `slot_size_mib` | optional; unsupported after parser validation | Missing values use Firecracker's parser default shape. Present values must be unsigned integers. Real slot-size semantic validation remains deferred with virtio-mem support. |
| `PUT /hotplug/memory` | unknown fields | rejected | Matches Firecracker's strict request model behavior. |
| `PATCH /hotplug/memory` | `requested_size_mib` | required; unsupported after parser validation | Required Firecracker-shaped target hotpluggable-memory size. The parser accepts syntactically valid unsigned integer values before returning the memory-hotplug unsupported fault. Real resize behavior remains deferred. |
...
| `PUT /actions` | `action_type=FlushMetrics` | runtime-only; minimal execution implemented | Rejected before startup. After startup, returns `204 No Content`; if metrics output was configured, appends one minimal JSON line with `metrics_flush_count` and the current selected API request counters, and if logger output was configured and allowed by level/module filters, appends one minimal action line. Periodic metrics flushes reuse the same metrics payload every 60 seconds while running, but are not `/actions` requests and do not write logger action lines. Full Firecracker counters, immediate startup metrics flushing, missed periodic-metrics accounting, and full logger integration remain deferred. |
...
`PUT /metrics` configured an output path. API-enabled and no-api runtime loops
also flush the same metrics line every 60 seconds while the VM is running; those
periodic flushes do not log `/actions` events. The line includes initial
...
Firecracker metrics counters, immediate startup metrics flushing, missed
periodic-metrics accounting, and full logger integration remain deferred.
...
`vmnet:bridged:<interface>`. Startup with configured network interfaces
revalidates the 16-interface limit before selecting packet I/O, can use the
MMDS-only adapter when every configured interface is selected by MMDS config,
and otherwise opens vmnet resources through those supported forms and retains
stop-on-drop cleanup. Startup without network interfaces still uses a no-op TX
sink and an empty RX source. These helpers can advertise configured
guest-visible MTU values, but they do not change host vmnet MTU settings,
...
| `DELETE /drives/{drive_id}`, `DELETE /pmem/{id}`, `DELETE /network-interfaces/{iface_id}` | recognized bodyless hot-unplug; `400` `fault_message` | recognized bodyless hot-unplug; `400` `fault_message` | Bodyless requests return the existing device-specific unsupported faults. Requests with a body fail first as malformed request shape before hot-unplug routing. Real hot-unplug remains deferred. |
| `PUT /network-interfaces/{iface_id}` | implemented; `204` empty response on successful config storage | unsupported after start; `400` `fault_message` | Records up to 16 validated pre-boot configs without opening host networking resources. Startup preparation attaches configured interfaces as virtio-mmio devices in the MMIO dispatcher and guest FDT. `InstanceStart` revalidates the count before selecting packet I/O. If every configured interface is listed in MMDS config, startup validates supported `host_dev_name` syntax and uses process-local MMDS-only packet I/O without opening vmnet resources; otherwise it opens vmnet packet I/O for `vmnet:host`, `vmnet:shared`, and `vmnet:bridged:<interface>` host device names. Unsupported names fail before `Running`. Internal network notification dispatch can route each interface through selected packet I/O, complete TX descriptor heads through a packet sink boundary, and write injected RX packets into guest buffers through a packet source boundary. Public packet movement, real PATCH updates, and DELETE remain deferred. |
| `PATCH /network-interfaces/{iface_id}` | recognized post-boot-only operation; `400` `fault_message` | recognized but unsupported; `400` `fault_message` | Parses Firecracker-shaped update requests, rejects malformed or mismatched bodies first, and then returns the network-interface-update unsupported fault for valid bodies without mutating stored configuration. Real post-boot updates and rate limiting remain deferred. |
...
| `PUT /metrics` | implemented; `204` empty response on successful output initialization | unsupported after start; `400` `fault_message` | Metrics output is process observability state, not guest configuration. Duplicate initialization fails. When configured, runtime `FlushMetrics` and periodic runtime flushes every 60 seconds while the VM is running append minimal metrics lines. |
