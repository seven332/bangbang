...
redacted capture-ready value pairs reconstructible external configuration with
complete UART registers, ordered RX bytes, status, and pending intents while
excluding host descriptors, terminal state, pipe buffers, TX bytes, counters,
locks, and wakeup handles. Paused public snapshot traversal obtains that value
under supervisor/quiescence ownership before any artifact publication.
Native-v1 deliberately keeps its existing encoding and rejects any
nonrepresentable RX/status/intent state instead of dropping it. A public path,
limiter configuration or budget, endpoint reconstruction, and complete UART
encoding remain the explicit Wave 6 handoff. This exact local profile is not
Firecracker snapshot-artifact compatibility.
...
`semantic.memory-device:virtio-mem-lifecycle-accounting-and-state` retain the
Wave 6 virtio-mem encoding, artifact, restore, migration/clone, portability,
and signed restored-guest handoff.
...
`semantic.device:entropy-queues-limits-metrics-and-state` retain the Wave 6
entropy encoding, artifact, restore, migration/clone, portability, and signed
restored-guest handoff.
...
API/live rows are terminal; six remain `audit-required` only for #1490
...
#1578 then promotes four public native-v2 process/snapshot records, so the
current global counts are 233/165/3/17.
...
supported subset documented above. Default serial stdout, terminal/FIFO stdin,
bounded RX readiness, and paused capture-ready traversal are implemented;
public streaming and generalized serial artifact restore remain explicit
boundaries alongside process-global panic/fatal durability and production
...
| `--snapshot-version` | implemented native-envelope inspection | Prints the current public writer version `v2.5.0` and exits successfully before fd-table setup, API socket publication, signal setup, or HVF startup. This names bangbang's native data format and does not claim Firecracker v10 artifact compatibility. |
...
accounting. Current public native-v2 create runs this preflight before
optional-profile rejection but does not serialize virtio-mem state. Signed
executable coverage proves
Linux and the public status surface complete a requested/plugged
`0 -> 128 MiB -> 0` lifecycle. No memory-hotplug DELETE route is exposed.
Runtime device deletion, broader public guest-memory accounting, and
optional-device serialization/restore remain deferred; bangbang does not claim
Firecracker's KVM slot mechanism.
...
| `PUT` | `/network-interfaces/{iface_id}` | pre-boot storage, PCI-only Running/Paused insertion, and capture-ready handoff implemented | Stores up to 16 initial virtio-net configurations before boot without opening host networking resources, including Firecracker-shaped RX/TX bandwidth and ops limiters. Startup preparation attaches configured interfaces over the selected virtio-MMIO/FDT or modern PCI transport. In a live PCI session, a new validated ID/MAC prepares one independent packet-I/O entry using the immutable startup/MMDS policy, checks actual contained vmnet authority, publishes metrics and PCI ownership on the owner thread, and commits live configuration last. Existing entries keep their queues and resources. Duplicate ID/MAC, invalid host config, capacity, authority, command, and publication failures preserve prior state; uncertain cleanup is terminal. Default MMIO rejects runtime insertion. Internal notification dispatch and runtime PATCH retain the same limiter/retry behavior and detailed per-interface/aggregate metrics. Paused capture joins configuration-order packet-I/O and metrics generations to deterministic MMIO/PCI owners, exact queues/features/limiters and reconstructible TX retry, and optional MMDS identity while excluding host handles, callbacks, peer packets, live TCP/ARP state, cached responses, absolute clocks, and token secrets. External direct-vmnet connectivity plus network encoding and restore remain deferred. |
| `PUT` | `/vsock` | supported target; implemented supported live MMIO-or-PCI startup/Unix-socket subset | Repeated valid pre-boot requests atomically replace stored configuration; post-start PUT is stably rejected without mutation. Direct mode defers opening the ordinary path until startup. Contained mode recognizes only exact `bangbang-grant:<GrantId>/<SocketChild>` references, claims one exact `VsockSocketDirectory` after complete request validation, and retains its scope/anchor without reopening the tag; rejected replacement preserves prior public and private state. Startup either binds and inode-tracks the direct path or exclusively publishes a supplied owner-only listener through the exact anchor, then attaches one guest-visible endpoint over the selected startup transport with three 256-entry queues and cleans up only its own socket. Host initiation uses that main listener. Guest initiation in contained mode uses a session-bound launcher facet fixed once to the anchor/child and carrying only monotonic `u32` ports plus connected stream descriptors; the launcher receives no guest payload and the worker gains no outgoing-network entitlement. The live handler supports bounded handshakes and four-packet directional backlogs, one 1023-connection active budget shared across both initiation directions, a separate 256-entry incomplete-host-handshake bound, round-robin host-local-port allocation, dynamic 64-KiB credit windows with wrapping counters, partial/full shutdown, two-second request/shutdown cleanup, reset/error handling, `EVENT_IDX`, validated source-side `TRANSPORT_RESET` publication, restored-origin signaling with runtime-only RX acknowledgement gating, and path/payload-redacted diagnostics. Signed Apple Silicon cases verify both initiation directions, ≥1 MiB direct transfers and a 1-MiB granted host-initiated transfer, both peers' write-half-close/EOF, terminal cleanup, two-stream isolation, an outside-container granted API listener, and no steady-state helper or entitlement change. Indirect descriptors are a supported bangbang extension. PATCH, DELETE, runtime hotplug, broader CID routing, broader event types, runtime PCI hotplug, vhost/KVM, and general performance/artifact parity remain excluded. Internal redacted MMIO/PCI capture and supplied-listener/connector reconstruction are implemented. The paused producer validates the exact controller/runtime/HVF MMIO-or-PCI owner, metrics, memory, reset, and capture before detaching source connection work under one lease; current native-v2 create invokes it before its optional-device rejection and creates no artifact. Internal destination restore resolves captured/override selectors before resource access, prepares owner-only stale-safe direct or exact transactional contained resources without ambient fallback, and transfers cleanup through single-use runtime adoption. The checked vsock ledger certifies all eight API/live records. Public native-v2 records still omit vsock encoding/placement and load rejects overrides; the six aggregate encoding, public invocation, restored-guest acknowledgement/reconnect/override, clone/version, and portability outcomes remain #1490 work. |
...
| `PUT` | `/snapshot/create`, `/snapshot/load` | supported current native-v2 2.5 regular-file profile-2 Full/File lifecycle, exact native-v2 2.4/2.3 loaders, plus frozen native-v1 File/Uffd reader | Parses Firecracker-shaped bodies, rejects malformed input first, normalizes deprecated load fields for native-v1 compatibility, and keeps paths/overrides redacted through typed API/runtime values. Create is paused-only and admits `Full` for a 1–32-vCPU source with configured boot metadata and 1–64 ordered regular-file block devices over selected MMIO or the product PCI budget. The graph may be rootless or have one first root and may mix read-only/read-write, Sync/Async, Unsafe/Writeback, partuuid, limiter/retry, queue, and interrupt state; optional devices/MMDS/boot timer, nondefault serial, and noncanonical live UART remain rejected. Direct and exact contained outputs share the anchored no-clobber memory-first/state-last transaction, cancellation seal, cleanup, durability, metrics, and source-Paused behavior; vhost-user rejects before artifact staging and `Diff` rejects before namespace or capture work. Load is pre-boot-only and requires a pristine process except logger/metrics. It opens and decodes state once, routes pinned Firecracker or unknown bytes to explicit incompatibility, routes frozen native-v1 to the unchanged eager File or macOS pager path with its root/grant/deprecation semantics, retains device-free 2.3 and singleton-root 2.4 loading, and routes current 2.5 to retained read-only File/COW memory plus one exact keyed complete-set backing transaction. Direct and contained 2.5 loads validate the complete state/memory/live-FDT identity/UART/HVF/topology/time/identity/device/transport graph before construction. Every graph selector is inert until destination authority validates exact set, order, identity, role, access, regular-file kind, size, and geometry; there is no ambient pathname fallback or per-drive override. Because Linux may reclaim FDT bytes after boot, graph-bearing profiles require a versioned source-product marker and derive the product shell from typed state rather than reparsing those bytes; FDT address, length, and CRC still bind the memory artifact. Loads keep the complete drive batch provisional until Paused session/controller publication, keep guest-memory writes private, and can repeatedly restore from immutable state/memory. Writable external drive files are deliberately shared between destinations, are not COW-isolated, and require operator serialization. All families commit a real session `Paused` before optional ordinary resume; requested-resume failure is terminal. Retryable preparation failures preserve pristine eligibility only after complete reverse cleanup, while committed identity/time, resource completion, publication, incomplete cleanup, or resume failure latches the process terminal. Signed direct and normal production App Sandbox matrices prove rooted/rootless MMIO/PCI all-drive persistence, restored queue/interrupt and limiter progress, recapture, fresh VM/metrics/Async ownership, explicit/automatic resume, retained descriptor identity, immutable state/memory, shared writable bytes, worker/launcher death cleanup, and redaction. No Firecracker state-file interoperability, Linux UFFD wire compatibility, native-v2 `Uffd`, `Diff`, realtime adjustment, overrides/editing, optional-device/general-serial profile, or broad portability is claimed. |
...
| `PUT` | `/entropy` | implemented and verified for supported MMIO-or-PCI startup, limiting, metrics, and capture-ready state; optional serialization excluded | Stores one strict Firecracker-shaped virtio-rng configuration before boot. Missing, `null`, empty, and all-null `rate_limiter` objects remain unconfigured; valid `bandwidth` and `ops` buckets are stored, echoed through `GET /vm/config`, and applied to the selected queue path. Each writable request is capped at 64 KiB and filled by the session-owned host OS source. Throttled descriptors are returned and retained exactly once, with the earliest backend-neutral retry scheduled by the HVF session. Allocation, length, guest-write, and used-ring failures restore exact pre-consumption bucket state; a published zero-length host-source failure remains consumed. Paused capture validates exact features, activation, queue mapping/geometry/cursors, external limiter configuration, redacted bucket state, one pending descriptor, and scheduler agreement through exactly one MMIO or PCI owner. Native-v1 traverses it before optional-profile rejection but emits no entropy encoding. Signed Linux guests prove marker-gated first/repeated `/dev/hwrng` reads, real dual-bucket throttling, pause/capture/resume, metrics, and cleanup over both transports. Post-start PUT remains unsupported. Firecracker's exact Linux timerfd/eventfd identity and optional-device serialization/restore remain deferred. |
| `GET`, `PUT`, `PATCH` | `/hotplug/memory` | implemented and verified for supported MMIO-or-PCI startup, live metrics, and capture-ready state; DELETE and optional serialization excluded | `PUT` validates and stores block/slot/total sizing before boot; `GET` returns stored pre-start status or exact active requested/plugged status; `PATCH` validates and signals requested-size changes after start. Startup attaches one virtio-mem endpoint over the selected startup transport. Its queue validates request/response descriptors and complete block ranges, answers `STATE`, and applies `PLUG`, `UNPLUG`, and `UNPLUG_ALL` over exact block-owned guest/HVF mappings before ACK. Device state commits only after guest-visible completion; split/combined mappings and partial or late failures use reverse rollback and fail closed. Singleton metrics retain Firecracker's exact 18 fields while separate extensions expose interrupt, rollback, owner-cleanup, and teardown outcomes. Paused capture proves exact transport queue/interrupt state and shared-aperture guest/HVF/dirty ownership; current native-v2 preflights it without serializing it. Focused coverage crosses the conceptual slot boundary and adjacent mappings without claiming KVM slot identity. Signed direct-rootfs coverage proves Linux binding and public requested/plugged size `0 -> 128 MiB -> 0`. Runtime device deletion, broader public accounting, and optional-device serialization/restore remain deferred. |
...
| `PUT /snapshot/create` | `snapshot_type` | optional; `Full` supported, `Diff` rejected | Accepts `Full` and `Diff`, defaulting to `Full`. Only `Full` passes the native-v2 gate; `Diff` returns the snapshot-specific unsupported fault before namespace or capture work. |
...
metrics, and cleanup over MMIO and PCI. The checked entropy closure leaves only
optional-device serialization/restore and Firecracker's exact shared Linux
timerfd/eventfd event-source identity outside this supported live subset.
...
network, and entropy retry quiescence. Before the narrow optional-device
profile rejection it traverses
capture-ready storage, balloon, memory-hotplug, entropy, serial, and network
owners; entropy
preflight requires its pending queue state to agree with the quiesced retry
scheduler and emits no random bytes or wall-clock value. Network preflight
...
live TCP/ARP state, token secret, borrow, or absolute clock. Its detached value
is the exact #1490 serializer/restore handoff; this slice adds no bytes to the
artifact. The accepted baseline
...
current 2.5 additionally derives every inert profile-2 drive request from the
decoded graph and resolves the keyed complete vector through one exact
destination authority transaction. Missing, extra, reordered, aliased,
wrong-access, wrong-role/kind, wrong-size, changed-geometry, or consumed
resources fail before construction without ambient pathname fallback. The
complete batch commits only with the restored session/controller. Exact 2.4
uses the same boundary for its singleton root, while frozen native-v1
additionally adopts its persisted-root input. Direct load keeps no-follow
pathname adapters. Malformed bodies are rejected by the parser first, while
execution faults remain typed and path/reference-redacted.
...
supports native-v2 2.5 Full/File for 1–32 vCPUs and a rooted or rootless vector
of 1–64 regular-file block devices over MMIO or the product PCI budget. The
profile may mix access, Sync/Async engine, cache, partuuid, limiter/retry,
queue, and interrupt state, but has no optional devices and requires the
canonical default UART. The process-standard-stream endpoint is
destination-local and is rebound rather than serialized. Exact 2.4 retains its
singleton read-only File/Sync root, 2.3 retains its device-free profile, and
frozen native-v1 retains the one-vCPU/read-only-root File/Uffd reader. All
include public Paused handoff, optional resume, and
...
serialization. Optional resources, overrides, `Diff`
...
host or kernel buffers. bangbang classifies a frozen native-v1 envelope and the
...
Current native-v2 create invokes it before its optional-device rejection and
creates no artifact. Internal destination preparation resolves the captured
selector and optional override before resource access, publishes direct sockets
with owner-only stale-safe atomic replacement and exact cleanup, reserves exact
contained directory/broker authority without ambient fallback, and transfers
cleanup through a single-use runtime adoption. The checked vsock ledger now
certifies all eight API/live records. Public native-v2 records still omit vsock
encoding/placement and load rejects overrides; the exact six aggregate
...
event types remain excluded. Internal destination restore now resolves
...
transfers cleanup through single-use runtime adoption. The checked vsock ledger
certifies all eight API/live rows; durable artifact encoding, public placement
and invocation, restored-guest acknowledgement/reconnect/override proof,
clone/versioning, and portability remain #1490 work. The quiesced #1516 producer validates and normalizes
source ownership before the unchanged public optional-device rejection; public
load still rejects overrides, so these internals do not by themselves make the
live subset publicly snapshot-compatible.
...
| `PUT /network-interfaces/{iface_id}` | implemented; `204` empty response on successful config storage | implemented in Running/Paused public PCI; `204` after live commit; capture-ready handoff implemented | Records up to 16 validated pre-boot configs without opening host networking resources. Startup attaches them over the selected transport and chooses all-MMDS or vmnet packet I/O. After startup, public PCI accepts only a new validated ID/MAC: the owner thread selects an independent entry class from immutable startup/MMDS policy, checks contained mode/bridge/actual-vmnet count, prepares packet I/O, publishes generation-bound metrics and the PCI endpoint, then commits live configuration. Existing interfaces retain their queues and host state. Default MMIO, duplicate ID/MAC, invalid host config, capacity, authority, and injected transaction failures preserve the prior projection; uncertain cleanup is terminal. Guest PCI rescan is manual. Paused capture validates deterministic startup/runtime MMIO/PCI traversal, generation-bound provider and metrics ownership, backend and MMDS identity, exact queue/feature/limiter state, and one exact TX retry. Generation-aware vmnet callback publication is quiesced and resumed without losing a readiness edge; cached RX and live MMDS protocol work are validated then normalized out for fresh lossy networking. Signed direct MMDS v1/v2 guests traverse the process producer after real exchange and limiter updates; signed HVF covers MMIO/PCI equality, runtime insertion/removal/reuse, rollback, resume, and teardown. This is the detached Wave 6 #1490 handoff, not artifact encoding or restore. External vmnet connectivity and automatic guest notification remain excluded. |
...
owns exactly 35 identities. Thirty-one live and capture-ready outcomes are
`implemented-and-verified`; only the MMDS user guide, network setup corpus, and
the broad MMDS and network semantic records remain `audit-required`. Their
ledger rows name the exact #1378, #1490, and #1491 outcomes still missing.
Focused API/runtime evidence is composed with signed MMIO and PCI transport,
process isolation/hotplug, capture traversal, contained production ownership,
and the fail-closed credential preflight. No blocked credential gate is counted
as successful external packet movement, and capture-ready state is not claimed
as snapshot encoding or restored-session evidence.
...
- packet networking beyond the implemented backend-neutral virtio-MMIO/PCI
  ...
  gratuitous ARP, IPv4 fragment reassembly, network snapshot encoding/restore,
  and automatic PCI notification
- virtio-vsock behavior beyond the **implemented supported live
  virtio-MMIO-or-PCI/Unix-socket subset**. The live subset includes repeatable
  ...
  the tier are PATCH, DELETE, runtime hotplug, broader CID routing, broader
  event types, general performance/Firecracker artifact parity, and
  vhost/KVM transports. Internal redacted MMIO/PCI capture and
  supplied-listener/connector reconstruction are implemented. The paused source
  producer now validates one exact MMIO-or-PCI owner and its metrics/memory,
  publishes reset, and detaches connection work under one lease before the
...
- snapshot behavior beyond the implemented narrow native-v2 Full/File profile.
  ...
  current public `2.5.0` writer carries an exact typed 1–32-vCPU
  ...
  reviewed debug/SME state, and one mandatory profile-2 device graph containing
  1–64 ordered regular-file block configurations, block runtime, limiter/retry,
  queue, common virtio, and MMIO-or-PCI transport state. The graph may be
  rootless or select the first record as root and may mix read-only/read-write,
  Sync/Async, Unsafe/Writeback, and partuuid.
  The focused boundary captures a
  ...
  The public normal-process adapter admits only pristine File/COW destinations,
  validates the live-FDT identity plus versioned product/UART/RTC/time/device
  graph before HVF, authorizes and retains the complete keyed backing vector
  without reopening a contained selector or falling back to paths, installs
  fresh destination serial output, and commits a closed session initially
  Paused.
  ...
  while writable external drive files deliberately are shared rather than
  COW-isolated and require operator serialization. Signed direct and normal
  production rooted/rootless MMIO/PCI matrices prove every drive's
  pre/post-capture persistence, restored retry/interrupt progress, recapture,
  fresh destination ownership, exact authority, and death-order cleanup. The
  CLI reports `v2.5.0` and describes exact v1/v2 versions. There
  is still no general optional-device aggregate. Remaining work includes
  optional-device/general-serial state, native-v2 Uffd, Diff artifacts,
  overrides, editing tools, Firecracker artifact compatibility,
...
- public serial streaming plus generalized artifact encoding, destination
  endpoint reconstruction, restore, migration/clone, and portability policy;
  native-v1 captures only its legacy six UART bytes, rejects nonrepresentable
  RX state, and excludes host endpoints, the output buffer, path, limiter
  state, and counters
...
- memory hotplug beyond the implemented block-granular selected MMIO-or-PCI
  ...
  slot mechanism
- complete production snapshot ownership beyond the current minimal native-v2
  multi-vCPU Full/File profile and frozen one-vCPU native-v1 reader, including
  generic optional devices, general serial state, external-buffer policy, and
  broader migration/portability rules
