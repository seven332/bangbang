...
accounting. Guest-reported optional statistics, statistics polling, hinting
queue processing, reporting, and host memory reclaim remain deferred. The `pmem` section is
parsed through
`PUT /pmem/{id}`;
...
| `GET` | `/balloon` | supported target; pre-boot and runtime config read implemented | Returns the stored Firecracker-shaped balloon configuration after successful `PUT /balloon` or runtime `PATCH /balloon`; returns the balloon-specific unsupported fault when no balloon configuration exists. Runtime can derive backend-neutral virtio-balloon config-space, feature-bit, and queue metadata from the stored config, startup can attach the current guest-visible virtio-mmio/FDT shell, and the HVF boot loop can dispatch inflate/deflate queue notifications with interrupt signaling. Completed descriptors update internal inflated-page accounting after PFN ranges are validated against mapped guest memory, while guest-reported optional statistics, statistics polling, hinting queue processing, reporting, and host reclaim need dedicated follow-up designs. |
| `PUT`, `PATCH` | `/balloon` | partial; pre-boot config storage, startup shell attachment, and runtime target update implemented | `PUT /balloon` stores Firecracker-shaped balloon configuration before boot, replaces prior pre-boot balloon config, rejects targets larger than the current guest memory size without mutating prior balloon config, appears in `GET /balloon` and `GET /vm/config`, and can feed startup attachment of a virtio-balloon MMIO/FDT shell with identity, feature, queue, and config-space registers. Pre-boot `PUT /machine-config` and `PATCH /machine-config` also reject memory sizes smaller than an already configured balloon target. `PATCH /balloon` is post-boot-only and, when a balloon device was configured, updates the stored `amount_mib`, updates the active virtio-balloon `num_pages` config-space value, increments config generation, and raises a config interrupt through the boot run-loop command path. Requests made in the wrong lifecycle state return the normal unsupported-state fault, and requests without configured balloon state return the balloon-specific unsupported fault. Inflate and deflate descriptor dispatch is implemented through the backend-neutral runtime handler and HVF boot loop, and completed descriptors update internal inflated-page accounting for mapped guest PFNs; host reclaim, guest-reported optional statistics, statistics polling, hinting, and reporting remain deferred. |
| `GET` | `/balloon/statistics` | partial; required target and actual fields implemented | Routes through the VMM state/action policy. Statistics queries are post-boot-only, require configured active balloon state, and return Firecracker-shaped required fields: `target_pages`, `actual_pages`, `target_mib`, and `actual_mib`. Target values come from the current stored balloon target, including runtime `PATCH /balloon` updates. Actual values come from active inflated-page accounting. Guest-reported optional statistics and statistics polling need a dedicated design. |
...
| `GET` | `/balloon/hinting/status` | partial; host command status implemented | Routes through the VMM state/action policy. Hinting status is post-boot-only, requires a configured balloon with `free_page_hinting: true`, and returns Firecracker-shaped `host_cmd` and `guest_cmd` fields from active device state. Current status reports the latest start/stop host command and no guest command; hinting queue processing and guest acknowledgement need a dedicated design. |
...
| `PATCH /balloon/hinting/start` | `acknowledge_on_stop` | optional | Missing values follow Firecracker's default `true` command shape. The current implementation preserves the value in host-owned device state for later guest acknowledgement work. Real hinting queue processing remains deferred. |
...
reports that host command state and no guest command until real hinting queue
processing exists. The backend-neutral inflate queue dispatcher
...
accounting.
Guest-reported optional statistics, statistics polling, hinting queue
processing, reporting, and host memory reclaim remain deferred.
...
- full balloon statistics polling, hinting queue processing, reporting, and
  host reclaim
