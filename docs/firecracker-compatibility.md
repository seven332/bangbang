...
existing RX queue path. Guest-visible socket lifecycle beyond host-request
acknowledgement and fail-closed reset headers, guest-initiated
`uds_path_<PORT>` connection success, CID routing beyond current host/guest
checks, event queue dispatch, and data movement remain deferred.
...
- virtio-vsock real socket connection lifecycle, successful guest-initiated
  socket connections, CID routing beyond current host/guest checks, event queue
  ...
  local port allocation, retained host connection table modeling with pending host request packet headers, guest `RESPONSE` acknowledgement to retained host streams, bounded guest-visible `RST` queueing for unsupported or orphan host-destined guest packets, and the internal prepared resource/MMIO
