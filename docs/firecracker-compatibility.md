...
block and pmem persistence, preauthorized live block replacement, and real
sandboxed HVF guests ending through PSCI `SYSTEM_OFF`.
...
live block replacement seam. Snapshots, sockets, observability outputs, dynamic
post-Ready delivery, vmnet provisioning and policy, jailer limits/credentials,
...
limiter-only retention, and preauthorized live block replacement. The signed
normal bundle proves config-file and delayed API use, retained opened identity
after pathname replacement, redacted failure-atomic mismatch handling, guest
block I/O, read-only rejection, pmem read/flush, and live block swap. Socket,
observability, and snapshot path consumers, dynamic brokerage, vmnet
provisioning, Linux seccomp outcome classification, and deployment signing
policy remain later #1351 work.
