...
time/identity: 82 are terminal and three aggregate rows remain
...
launcher/sandboxed-worker pairs. All three retained records belong to
...
restored-device contract, exact 2.8 supplies entropy encoding, and current 2.9
supplies balloon encoding, fresh owner reconstruction, immutable clone, and
signed direct/contained continuation; the other capture-ready values remain private
...
resume. The current global counts are 240/158/3/17.
...
certification. #1680 activates current exact native-v2 2.9 optional balloon,
...
The current aggregate selector is 82 terminal and three `audit-required`, and
the current global counts are 240/158/3/17.
...
| `--snapshot-version` | implemented native-envelope inspection | Prints the current public writer version `v2.9.0` and exits successfully before fd-table setup, API socket publication, signal setup, or HVF startup. This names bangbang's native data format and does not claim Firecracker v10 artifact compatibility. |
...
| `PUT` | `/snapshot/create`, `/snapshot/load` | supported current native-v2 2.9 complete-serial Full/File lifecycle with independently optional entropy, balloon, and regular-file block-and-pmem profile-3 storage; exact native-v2 2.3–2.8 loaders plus frozen native-v1 File/Uffd reader | Parses Firecracker-shaped bodies, rejects malformed input first, and keeps paths/overrides redacted. Paused-only `Full` create admits a 1–32-vCPU source with required serial and independently optional profile-3 storage, entropy, and balloon over coherent MMIO or PCI; optional devices outside serial/pmem/entropy/balloon, MMDS, boot timer, and vhost-user reject before artifacts. Required serial kind 8 and optional storage kind 7/entropy kind 9 retain their exact profiles. Optional balloon kind 10 retains configuration/features, two through five active queue cursors, latest optional statistics, one pending statistics descriptor, DONE-normalized hint history, exact compact accounting, common virtio state, and matching transport. It is capped at 262,144 canonical 4-KiB PFN ranges and 4 MiB inside the 16-MiB state cap. Load opens state once, selects frozen native-v1 or exact 2.3–2.9 File/COW, validates every balloon range against adopted RAM, performs one exact external authority transaction, and constructs fresh serial/entropy/balloon owners before Paused publication without fallback or override. Balloon timers/deadlines, metrics, reclaim state, handles, and cleanup authority do not cross the artifact: a retained descriptor waits one full destination-local interval after resume, no interval advances while Paused, and a new hint run may start after DONE. Guest PFNs remain 4 KiB while Darwin reclaim aligns/coalesces to host pages, normally 16 KiB, and remains best effort without synchronous RSS assurance. State/memory and guest File/COW RAM stay immutable/private; writable external block/pmem prefixes remain deliberately shared. Focused public complete-product and signed HVF owner matrices prove all four balloon-bearing products over MMIO/PCI. Signed direct and normal-production/App-Sandbox storage-plus-balloon matrices prove statistics/hint/report/inflate/deflate/API continuation, destination-only metrics, recapture, immutable clone isolation, malformed/cancellation/death cleanup, containment, exact grants, pathname replacement resistance, explicit/automatic resume, and worker/launcher cleanup alongside existing storage/serial/entropy evidence. No Firecracker artifact interoperability, native-v2 `Uffd`, `Diff`, broader optional-device profile, or broad portability is claimed. |
...
| `PUT`, `PATCH` | `/pmem/{id}` | implemented direct MMIO-or-PCI startup, pmem root, non-root PCI runtime, capture-ready state, aggregate live certification, and exact native-v2 2.6 through current 2.9 serialization/restore | `PUT /pmem/{id}` stores strict Firecracker-shaped configuration with one cross-storage root, exact direct or contained regular-file authority, one file/private-tail mapping, and exact-prefix `MS_SYNC`. Runtime PCI insertion remains non-root-only and preflights every owner capacity before authority or mapping work. Profile 3 serializes exact file/mapped geometry plus configuration/runtime/limiter/queue/interrupt/mapping/transport state and restores it in the same typed complete-set transaction as block; exact 2.7 composes required serial, exact 2.8 may add entropy, and current 2.9 may add balloon. Signed focused and aggregate coverage proves live protection/coherence/reuse; signed rooted pmem-only and rootless mixed MMIO/PCI direct/contained matrices prove shared writable prefix epochs, unchanged read-only peers, zero fresh private tails, immutable state/memory, recapture, exact grants after pathname replacement, and cleanup. |
...
current 2.9 derives the serial endpoint intent, every inert optional profile-3
block and pmem request, and optional entropy/balloon state from the decoded graph. It
...
supports current native-v2 2.9 Full/File for 1–32 vCPUs with required complete
serial state, optional entropy and balloon, and either no storage or a rooted/rootless
...
dispatcher/endpoint/cleanup owners. Optional devices other than serial, pmem,
entropy, and balloon remain excluded. Exact 2.8 retains optional entropy, exact 2.7 retains serial
...
| `PUT /pmem/{id}` | implemented; `204` empty response on successful config storage | implemented in Running/Paused public PCI; `204` after live commit | Pre-boot requests record Firecracker-shaped configuration with one cross-storage root and exact direct or contained regular-file authority. Startup maps each file/private-tail range once and retains the selected MMIO/PCI owner; runtime PCI insertion preflights all capacity before authority or mapping work. Exact-prefix `MS_SYNC`, read-only guest protection, rollback, and cleanup preserve the prior projection. Exact native-v2 2.6 through current 2.9 profile 3 serialize and restore exact pmem configuration/runtime/limiter/queue/interrupt/mapping/transport state through the same complete-set transaction as block; exact 2.7 composes required serial, exact 2.8 may add entropy, and current 2.9 may add balloon. Focused and signed direct/contained rooted pmem-only and rootless mixed matrices certify live and restored behavior, immutable-pair reuse, shared writable prefixes, zero private tails, exact grants, and cleanup. |
...
contained certification; current 2.9 preserves that profile-3 graph alongside
required serial state and optional entropy/balloon. Parser recognition, config echo, or a family-level test
...
- snapshot behavior beyond the implemented narrow native-v2 Full/File profile.
  ...
  complete serial plus optional profile-3 storage, and exact `2.8.0` may add
  entropy. The current public `2.9.0` writer
  ...
  canonical 4-KiB PFN ranges and 4 MiB under the 16-MiB state cap. The storage graph
  ...
  without an RSS promise. The CLI reports `v2.9.0` and
  ...
  entropy, and balloon, native-v2 Uffd, Diff artifacts,
