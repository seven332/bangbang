...
| `PUT /balloon` | `amount_mib` | required; stored pre-boot | Stored as an unsigned 32-bit Firecracker-shaped target balloon size and returned by `GET /balloon` and `GET /vm/config`. The internal virtio-balloon foundation converts this value to 4 KiB `num_pages` with checked arithmetic. `InstanceStart` rejects configured balloon state until real guest-visible balloon memory accounting exists. |
...
This is not wired into startup, FDT, MMIO, memory accounting, or queue handling.
Configured balloon state still makes `InstanceStart` fail before `Running`.
