...
guest-initiated connections can retain a bounded four-packet per-connection backlog of host
`VSOCK_OP_RW` payloads and deliver one queued payload at a time into guest RX
buffers; guest `VSOCK_OP_RST`
...
reset/full-shutdown cleanup, full graceful half-close state tracking, buffered
guest-to-host RW retry, full virtio-vsock credit accounting, CID routing beyond
current host/guest checks, and full event payload dispatch remain deferred. Event
...
- virtio-vsock socket lifecycle beyond connection setup and forceful guest
  ...
  buffered guest-to-host RW retry, full virtio-vsock credit accounting, CID
