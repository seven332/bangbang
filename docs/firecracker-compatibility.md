...
| `GET`, `PUT`, `PATCH` | `/mmds` | supported target; control-plane storage, runtime guest-query formatting, internal guest GET response modeling, request parsing, and response-byte serialization implemented | Stores bounded in-memory JSON object contents in the process runtime, returns stored JSON for control-plane `GET`, applies RFC 7396 merge-patch semantics for `PATCH`, rejects uninitialized `GET`/`PATCH`, and keeps previous data on oversized update failure. The runtime can also resolve initialized metadata by JSON-pointer path, format JSON or Firecracker-shaped IMDS text, parse process-local guest HTTP `GET` request bytes into URI/output-format inputs, map internal guest GET requests to process-local status/content-type/body response values, and serialize those values as deterministic process-local HTTP/1.1 response bytes as prerequisites for later guest-visible delivery. Guest packet handling, token handling, TCP routing, and response delivery remain tied to later #16 work. |
...
currently accepts complete HTTP/1.0 or HTTP/1.1 `GET` request buffers, rejects
...
to IMDS text output. Process-local guest response-byte serialization currently
emits fixed HTTP/1.1 response bytes; preserving a guest request's original HTTP
version remains deferred.
...
- snapshots
- MMDS guest packet access, token handling, TCP routing, and response delivery
