...
identity, queue metadata, feature-bit, and 16-byte config-space foundation.
...
| `PUT /pmem/{id}` | implemented; `204` empty response on successful config storage | unsupported after start; `400` `fault_message` | Records Firecracker-shaped pre-boot config and replaces prior config for the same ID without opening the host backing file during the request. Stored configs appear in `GET /vm/config`; startup opens and validates each backing as a non-zero regular file, mmaps it to a 2 MiB-aligned host range, assigns non-overlapping guest physical range/config-space metadata, attaches one virtio-mmio/FDT node per prepared pmem device, copies the prepared mapping into an HVF-compatible anonymous shadow, registers that shadow with HVF after DRAM, writes writable shadows back after clean unmap, skips writeback after failed unmap cleanup, and retains the handles, mappings, and range metadata. Root-device semantics, rate limiting, runtime updates, queue-driven guest flush dispatch, dirty-range tracking, direct file-backed HVF mapping, and hot-unplug remain deferred. |
...
- pmem root-device semantics, runtime updates, queue-driven guest flush
  dispatch, rate limiting, dirty-range tracking, direct file-backed HVF
  mapping, and hot-unplug beyond startup backing-file validation/mapping,
  guest-visible MMIO/FDT attachment, HVF guest-memory registration, and the
  internal guest-range config-space foundation
