...
`InstanceStart` preflight, transactional startup executor, and successful-start state transition helpers, an internal MMDS guest TCP packet classifier and process-local packet-payload HTTP exchange, backend-neutral guest
...
including token PUT and MMDS v2 GET token enforcement. TCP stream tracking,
split request buffering, response packet synthesis, virtio-net RX delivery, and
vmnet detouring remain deferred.
...
- snapshots
- MMDS guest packet access, TCP routing, and response delivery
