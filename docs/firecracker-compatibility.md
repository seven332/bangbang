...
only the runtime handler's backend-neutral inflate and deflate descriptor
dispatch paths exist today. Boot-loop balloon notification dispatch, memory
accounting, statistics, hinting, reporting, and host memory reclaim remain
deferred.
...
acknowledges descriptor heads with zero-length used-ring entries. Boot-loop
balloon notification dispatch, host memory accounting, statistics, hinting,
reporting, host memory reclaim, and runtime target updates remain deferred.
...
| `PUT /balloon` | partial; `204` empty response on successful pre-boot configuration | unsupported after start; `400` `fault_message` | Stores Firecracker-shaped balloon configuration before startup, exposes it through `GET /balloon` and `GET /vm/config`, and can attach the current virtio-mmio/FDT shell during `InstanceStart`. Runtime-handler inflate and deflate descriptor dispatch is implemented; boot-loop balloon notification dispatch, memory accounting, statistics, hinting, reporting, host reclaim, and runtime target updates remain deferred. |
