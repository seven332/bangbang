...
are copied back to the backing file with positional writes and a data sync after
clean HVF unmap; failed unmap cleanup skips writeback because HVF may still
reference the shadow memory. Runtime also has a backend-neutral virtio-pmem
identity, queue metadata, feature-bit, and 16-byte config-space foundation, but
startup does not attach guest-visible virtio-pmem devices yet.
...
| `GET` | `/balloon` | supported target; pre-boot config read implemented | Returns the stored Firecracker-shaped balloon configuration after successful `PUT /balloon`; returns the balloon-specific unsupported fault when no balloon configuration exists. Runtime can derive backend-neutral virtio-balloon config-space, feature-bit, and queue metadata from the stored config, startup can attach the current guest-visible virtio-mmio/FDT shell, and the HVF boot loop can dispatch inflate/deflate queue notifications with interrupt signaling. Completed descriptors update internal inflated-page accounting after PFN ranges are validated against mapped guest memory, while statistics, hinting, reporting, host reclaim, and runtime update behavior need dedicated follow-up designs. |
| `PUT`, `PATCH` | `/balloon` | partial; pre-boot config storage and startup shell attachment implemented | `PUT /balloon` stores Firecracker-shaped balloon configuration before boot, replaces prior pre-boot balloon config, appears in `GET /balloon` and `GET /vm/config`, and can feed startup attachment of a virtio-balloon MMIO/FDT shell with identity, feature, queue, and config-space registers. `PATCH /balloon` is treated as post-boot-only based on Firecracker's current VMM source classification and reaches the balloon-specific unsupported fault after startup. Requests made in the wrong lifecycle state return the normal unsupported-state fault. Inflate and deflate descriptor dispatch is implemented through the backend-neutral runtime handler and HVF boot loop, and completed descriptors update internal inflated-page accounting for mapped guest PFNs; host reclaim, statistics, hinting, reporting, and runtime target-size updates remain deferred. |
...
| `PUT`, `PATCH` | `/pmem/{id}` | partial; pre-boot config storage, startup backing mapping, HVF guest-memory registration, shadow writeback, and internal guest-range config-space model implemented | `PUT /pmem/{id}` stores Firecracker-shaped pmem configuration before boot, replaces prior config for the same ID, and appears in `GET /vm/config` under `pmem`. It rejects empty backing paths without opening host files during the request. `InstanceStart` opens each configured backing, validates it as a non-zero regular file, mmaps it to a 2 MiB-aligned host range, assigns a deterministic non-overlapping 2 MiB-aligned guest physical range after the aarch64 MMIO64 gap while skipping current guest RAM, creates the HVF VM with the framework-reported maximum IPA size, copies the prepared mapping into an HVF-compatible anonymous shadow, registers that shadow with HVF after DRAM using read-only or read/write non-executable permissions, copies writable shadows back to the backing file after clean unmap, skips writeback after failed unmap cleanup, populates the internal virtio-pmem config-space `start`/`size` values, and retains the handle, mapping, and range metadata without attaching a guest-visible virtio-pmem device yet. Runtime defines the backend-neutral virtio-pmem identity, one 256-entry queue, modern virtio feature bit, 2 MiB alignment constant, and 16-byte config-space layout for future guest attachment. Configured rate limiters are rejected without mutating stored pmem configuration. `PATCH /pmem/{id}` remains post-boot-only unsupported, and real runtime updates, rate limiting, root-device semantics, guest attachment, guest queue-driven flush dispatch, dirty-range tracking, direct file-backed HVF mapping, and hot-unplug need dedicated device designs. |
...
| `PUT /balloon` | `amount_mib` | required; stored pre-boot | Stored as an unsigned 32-bit Firecracker-shaped target balloon size and returned by `GET /balloon` and `GET /vm/config`. The internal virtio-balloon foundation converts this value to 4 KiB `num_pages` with checked arithmetic and exposes it through the startup-attached config space. Runtime target-size updates, guest-visible statistics, and host reclaim remain deferred. |
...
| `PATCH /balloon` | `amount_mib` | required; unsupported after parser validation | Parsed as an unsigned 32-bit Firecracker-shaped target balloon size before VMM dispatch. The value is not retained or echoed by the current unsupported update path. Real runtime target-size updates remain deferred. |
...
| `PUT /pmem/{id}` | `path_on_host` | required; startup-mapped, range-assigned, HVF-registered through a shadow, and writeback-capable | Required Firecracker-shaped host backing path. The value is retained and reported in `GET /vm/config` after rejecting empty paths. Startup opens the path, validates it as a non-zero regular file, mmaps it to a 2 MiB-aligned host range, assigns matching internal guest physical range/config-space metadata, copies the prepared mapping into an HVF-compatible anonymous shadow, registers that shadow with HVF, writes writable shadows back after clean unmap, and redacts path details from backing-open, mapping, range-allocation, shadow-allocation, HVF-registration, or writeback errors. Normalization, guest attachment, and queue-driven guest flush dispatch remain deferred with virtio-pmem support. |
...
| `PUT /pmem/{id}` | `read_only` | optional; startup-open, host mapping, HVF permission access, and writeback policy | Missing values default to `false` and are reported in `GET /vm/config`. Startup opens and maps the host backing read-only when this is `true` and read/write when this is `false`; HVF shadow registration mirrors that intent with read-only or read/write non-executable memory permissions. Read-only shadows never write back, while writable shadows write back after clean unmap. Guest-visible virtio-pmem behavior remains deferred. |
...
loop can drain pending balloon inflate/deflate notifications and signal the
allocated balloon interrupt line when the runtime dispatch summary reports
queue-interrupt intent. Completed descriptors update internal inflated-page
accounting on the owning balloon device after PFN ranges are validated against
mapped guest memory, and reset clears that accounting.
Statistics, hinting, reporting, host memory reclaim, and runtime target updates
remain deferred.
...
| `PUT /pmem/{id}` | implemented; `204` empty response on successful config storage | unsupported after start; `400` `fault_message` | Records Firecracker-shaped pre-boot config and replaces prior config for the same ID without opening the host backing file during the request. Stored configs appear in `GET /vm/config`; startup opens and validates each backing as a non-zero regular file, mmaps it to a 2 MiB-aligned host range, assigns non-overlapping guest physical range/config-space metadata, copies the prepared mapping into an HVF-compatible anonymous shadow, registers that shadow with HVF after DRAM, writes writable shadows back after clean unmap, skips writeback after failed unmap cleanup, and retains the handles, mappings, and range metadata. Runtime has the backend-neutral virtio-pmem config-space foundation, but root-device semantics, guest-visible virtio-pmem attachment, rate limiting, runtime updates, queue-driven guest flush dispatch, dirty-range tracking, direct file-backed HVF mapping, and hot-unplug remain deferred. |
...
| `PUT /balloon` | partial; `204` empty response on successful pre-boot configuration | unsupported after start; `400` `fault_message` | Stores Firecracker-shaped balloon configuration before startup, exposes it through `GET /balloon` and `GET /vm/config`, and can attach the current virtio-mmio/FDT shell during `InstanceStart`. Runtime-handler and HVF boot-loop inflate/deflate descriptor dispatch is implemented with queue interrupt signaling and internal inflated-page accounting for mapped guest PFNs; statistics, hinting, reporting, host reclaim, and runtime target updates remain deferred. |
...
- pmem FDT attachment, root-device semantics, runtime updates, and rate
  limiting beyond startup backing-file validation/mapping, HVF guest-memory
  registration, and the internal guest-range config-space foundation
