...
`OK <local_port>\n` to the retained host stream. Guest-visible socket lifecycle
beyond that acknowledgement, `RST` host responses, guest-initiated
`uds_path_<PORT>` connections, CID routing, event queue dispatch, and data
...
by writing `OK <local_port>\n` to the retained host stream. Guest-visible socket
lifecycle beyond that acknowledgement, `RST` host responses, guest-initiated
`uds_path_<PORT>` connections, CID routing, event queue dispatch, and data
movement remain deferred.
...
- virtio-vsock real socket connection lifecycle, guest-initiated
  socket connections, CID routing, event queue dispatch, and data movement
  ...
  local port allocation, retained host connection table modeling with pending host request packet headers, guest `RESPONSE` acknowledgement to retained host streams, and the internal prepared resource/MMIO
