...
non-zero regular host file and retains the handles. Runtime also has a
backend-neutral virtio-pmem identity, queue metadata, feature-bit, and 16-byte
config-space foundation, but startup does not mmap or attach guest-visible
...
| `PUT`, `PATCH` | `/pmem/{id}` | partial; pre-boot config storage, startup backing validation, and internal config-space model implemented | `PUT /pmem/{id}` stores Firecracker-shaped pmem configuration before boot, replaces prior config for the same ID, and appears in `GET /vm/config` under `pmem`. It rejects empty backing paths without opening host files during the request. `InstanceStart` opens each configured backing, validates it as a non-zero regular file, and retains the handle without attaching a guest-visible virtio-pmem device yet. Runtime defines the backend-neutral virtio-pmem identity, one 256-entry queue, modern virtio feature bit, 2 MiB alignment constant, and 16-byte config-space layout for future guest attachment. Configured rate limiters are rejected without mutating stored pmem configuration. `PATCH /pmem/{id}` remains post-boot-only unsupported, and real runtime updates, rate limiting, root-device semantics, host-file mapping, guest attachment, and hot-unplug need dedicated device designs. |
...
| `PUT /pmem/{id}` | `path_on_host` | required; startup-validated | Required Firecracker-shaped host backing path. The value is retained and reported in `GET /vm/config` after rejecting empty paths. Startup opens the path, validates it as a non-zero regular file, and redacts path details from backing-open errors. Mmap, normalization, and guest attachment remain deferred with virtio-pmem support. |
...
| `PUT /pmem/{id}` | `read_only` | optional; startup-open access | Missing values default to `false` and are reported in `GET /vm/config`. Startup opens the host backing read-only when this is `true` and read/write when this is `false`; real guest-visible read-only mapping behavior remains deferred with virtio-pmem support. |
...
| `PUT /pmem/{id}` | implemented; `204` empty response on successful config storage | unsupported after start; `400` `fault_message` | Records Firecracker-shaped pre-boot config and replaces prior config for the same ID without opening the host backing file during the request. Stored configs appear in `GET /vm/config`; startup opens and validates each backing as a non-zero regular file and retains the handles. Runtime has the backend-neutral virtio-pmem config-space foundation, but root-device semantics, backing-file mapping, guest-visible virtio-pmem attachment, rate limiting, runtime updates, and hot-unplug remain deferred. |
...
- pmem mmap/FDT attachment, root-device semantics, runtime updates, and rate
  limiting beyond startup backing-file validation and the internal config-space
  foundation
