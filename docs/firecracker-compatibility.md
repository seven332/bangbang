...
| `GET`, `PUT`, `PATCH` | `/mmds` | supported target; control-plane storage and runtime guest-query formatting implemented | Stores bounded in-memory JSON object contents in the process runtime, returns stored JSON for control-plane `GET`, applies RFC 7396 merge-patch semantics for `PATCH`, rejects uninitialized `GET`/`PATCH`, and keeps previous data on oversized update failure. The runtime can also resolve initialized metadata by JSON-pointer path and format JSON or Firecracker-shaped IMDS text as a prerequisite for later guest-visible delivery. Guest packet handling remains tied to later #16 work. |
...
- snapshots
- MMDS guest packet access, token handling, and guest HTTP response wiring
