...
pre-boot control-plane configuration, but startup fails until guest-visible
virtio-balloon support exists.
...
| `GET` | `/balloon` | supported target; pre-boot config read implemented | Returns the stored Firecracker-shaped balloon configuration after successful `PUT /balloon`; returns the balloon-specific unsupported fault when no balloon configuration exists. Real virtio-balloon device support, memory accounting, and runtime update behavior need a dedicated design. |
| `PUT`, `PATCH` | `/balloon` | partial; pre-boot config storage implemented | `PUT /balloon` stores Firecracker-shaped balloon configuration before boot, replaces prior pre-boot balloon config, appears in `GET /balloon` and `GET /vm/config`, and causes `InstanceStart` to fail before `Running` until a real virtio-balloon device exists. `PATCH /balloon` is treated as post-boot-only based on Firecracker's current VMM source classification and reaches the balloon-specific unsupported fault after startup. Requests made in the wrong lifecycle state return the normal unsupported-state fault. Real virtio-balloon device support, memory accounting, and runtime update behavior need a dedicated design. |
...
| `PUT /balloon` | `amount_mib` | required; stored pre-boot | Stored as an unsigned 32-bit Firecracker-shaped target balloon size and returned by `GET /balloon` and `GET /vm/config`. `InstanceStart` rejects configured balloon state until real guest-visible balloon memory accounting exists. |
| `PUT /balloon` | `deflate_on_oom` | required; stored pre-boot | Stored as a boolean and returned by `GET /balloon` and `GET /vm/config`. Real guest OOM deflation behavior remains deferred. |
| `PUT /balloon` | `stats_polling_interval_s` | optional; stored pre-boot | Missing values follow Firecracker's parser default shape and are stored as `0`. Real statistics polling remains deferred. |
| `PUT /balloon` | `free_page_hinting` | optional; stored pre-boot | Missing values follow Firecracker's parser default shape and are stored as `false`. Real free-page hinting remains deferred. |
| `PUT /balloon` | `free_page_reporting` | optional; stored pre-boot | Missing values follow Firecracker's parser default shape and are stored as `false`. Real free-page reporting remains deferred. |
...
| `PUT /balloon` | partial; `204` empty response on successful pre-boot configuration | unsupported after start; `400` `fault_message` | Stores Firecracker-shaped balloon configuration before startup and exposes it through `GET /balloon` and `GET /vm/config`. `InstanceStart` rejects configured balloon state until a real guest-visible virtio-balloon device exists. |
