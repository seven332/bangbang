...
other stored balloon fields. Balloon statistics, hinting, reporting, and host
memory reclaim remain deferred. The `pmem` section is parsed through
`PUT /pmem/{id}`;
...
| `GET` | `/balloon` | supported target; pre-boot and runtime config read implemented | Returns the stored Firecracker-shaped balloon configuration after successful `PUT /balloon` or runtime `PATCH /balloon`; returns the balloon-specific unsupported fault when no balloon configuration exists. Runtime can derive backend-neutral virtio-balloon config-space, feature-bit, and queue metadata from the stored config, startup can attach the current guest-visible virtio-mmio/FDT shell, and the HVF boot loop can dispatch inflate/deflate queue notifications with interrupt signaling. Completed descriptors update internal inflated-page accounting after PFN ranges are validated against mapped guest memory, while statistics, hinting, reporting, and host reclaim need dedicated follow-up designs. |
| `PUT`, `PATCH` | `/balloon` | partial; pre-boot config storage, startup shell attachment, and runtime target update implemented | `PUT /balloon` stores Firecracker-shaped balloon configuration before boot, replaces prior pre-boot balloon config, appears in `GET /balloon` and `GET /vm/config`, and can feed startup attachment of a virtio-balloon MMIO/FDT shell with identity, feature, queue, and config-space registers. `PATCH /balloon` is post-boot-only and, when a balloon device was configured, updates the stored `amount_mib`, updates the active virtio-balloon `num_pages` config-space value, increments config generation, and raises a config interrupt through the boot run-loop command path. Requests made in the wrong lifecycle state return the normal unsupported-state fault, and requests without configured balloon state return the balloon-specific unsupported fault. Inflate and deflate descriptor dispatch is implemented through the backend-neutral runtime handler and HVF boot loop, and completed descriptors update internal inflated-page accounting for mapped guest PFNs; host reclaim, statistics, hinting, and reporting remain deferred. |
...
increments config generation, and raises a config interrupt. Statistics,
hinting, reporting, and host memory reclaim remain deferred.
...
| `PUT /balloon` | partial; `204` empty response on successful pre-boot configuration | unsupported after start; `400` `fault_message` | Stores Firecracker-shaped balloon configuration before startup, exposes it through `GET /balloon` and `GET /vm/config`, and can attach the current virtio-mmio/FDT shell during `InstanceStart`. Runtime `PATCH /balloon` can update the target size after startup when a balloon was configured. Runtime-handler and HVF boot-loop inflate/deflate descriptor dispatch is implemented with queue interrupt signaling and internal inflated-page accounting for mapped guest PFNs; statistics, hinting, reporting, and host reclaim remain deferred. |
