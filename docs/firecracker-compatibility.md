...
`corpus:snapshot-page-faults`, so the current global counts are 229/169/3/17.
...
accounting. Native-v1 create runs this preflight before artifact publication
but does not serialize virtio-mem state. Signed executable coverage proves
...
| `PUT` | `/snapshot/create`, `/snapshot/load` | supported narrow native-v1 Full/File plus macOS `Uffd` restore subset | Parses Firecracker-shaped bodies, rejects malformed input first, normalizes deprecated load `mem_file_path` to a `File` backend and dirty tracking to the OR of old/new flags, and keeps paths/overrides redacted through typed API/runtime values. Create is paused-only and admits `Full` for one vCPU, exactly one regular read-only root drive, default serial, and no optional devices or MMDS. Direct mode invokes the path adapter; contained mode validates `bangbang-grant:<GrantId>/<SnapshotOutputChild>` references, retains matching repeatable output-directory anchors, and publishes staging/finals relative to them. Both stream aggregate capture into owner-only staging, exclusively commit memory first and state last, return `204`, and leave the source paused. A tracked source re-protects and advances only after either durable or uncertain-visible Full commit; every pre-visible failure keeps its old epoch, while incomplete rollback prevents resume and tears down safely without misreporting artifact visibility. Load is pre-boot-only and requires successful-action history plus current non-logger/metrics configuration to be pristine. `File` direct mode opens the committed kind-2 pair by path; contained mode duplicates state for bounded preinspection, discovers any grant-tagged persisted root, atomically takes tagged `SnapshotStateInput`, `SnapshotMemoryInput`, and read-only `DriveBacking`, and completes from supplied descriptors without tag reopen. On macOS Apple Silicon, `Uffd` instead requires dirty tracking off and a `bangbang-pager-v1` peer. Platform/profile/consumer/grant preflight precedes path, socket, artifact, and backend access. The loader validates the state-bound image identity, checksum, data length, GPA topology, header-relative source offsets, root, and peer negotiation, then transactionally assembles private-anonymous memory, the Mach and HVF fault planes, mappings, vCPU, devices, and runtime. Direct mode makes a bounded Unix connection; contained mode claims the launcher-connected pager stream without a worker memory-file grant. No lazy failure falls back to `File` or COW. Both backends commit a real session as `Paused` and use ordinary resume when requested. Retryable preparation failures preserve pristine eligibility; uncertain cleanup is terminal. Signed direct and App Sandbox certification proves Paused host demand, exact resumed-guest instruction/read/write pages, before/during/after removal and zero refault, failure/death/repeat cleanup, exact entitlement dictionaries, `SYSTEM_OFF`, and orderly pager shutdown. Admitted successes, capability rejections, and execution failures record snapshot latency; parser and invalid-state failures do not. Typed faults and diagnostics redact paths, grants, identities, and guest/HVF values. No Firecracker state-file interoperability, Linux UFFD wire compatibility, `Diff`, realtime adjustment, overrides, optional-device profile, or cross-host portability is claimed. |
...
| `PUT /snapshot/load` | `mem_backend.backend_type` | required when `mem_backend` is present | Accepts `File` and `Uffd`. `File` retains eager image loading and optional destination dirty tracking. On macOS Apple Silicon, `Uffd` accepts the same narrow fixed-memory native-v1 profile only with dirty tracking disabled and requires a `bangbang-pager-v1` peer. Platform/profile/consumer/grant preflight precedes path, socket, artifact, and backend access. State-bound session identity, memory layout, source offsets, negotiated limits, root identity, and the complete lazy owner graph validate before VM publication. Direct mode uses a bounded connection; contained mode claims only the launcher-connected stream and does not require a worker memory-file grant. #1555's signed cross-slice certification makes this exact checked aggregate terminal. This is not Linux UFFD descriptor or wire compatibility. |
...
Native-v1 create invokes it before its unchanged optional-device rejection and
...
certifies all eight API/live records. Public native-v1 records still omit vsock
...
- virtio-vsock behavior beyond the **implemented supported live
  ...
  checked and terminal; public native-v1 encoding, placement and invocation,
