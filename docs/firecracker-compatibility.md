...
existing RX queue path. Supported guest `VSOCK_OP_REQUEST` packets can connect
to Firecracker-shaped `${uds_path}_${PORT}` sockets, retain successful
...
Guest-visible socket lifecycle beyond connection setup, CID routing beyond
current host/guest checks, event queue dispatch, and data movement remain
deferred.
...
- virtio-vsock socket lifecycle beyond connection setup, CID routing beyond
  current host/guest checks, event queue dispatch, and data movement
