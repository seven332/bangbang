...
reporting, and host memory reclaim remain deferred. The `pmem` section is
parsed through
`PUT /pmem/{id}`;
...
| `GET` | `/balloon` | supported target; pre-boot and runtime config read implemented | Returns the stored Firecracker-shaped balloon configuration after successful `PUT /balloon` or runtime `PATCH /balloon`; returns the balloon-specific unsupported fault when no balloon configuration exists. Runtime can derive backend-neutral virtio-balloon config-space, feature-bit, and queue metadata from the stored config, startup can attach the current guest-visible virtio-mmio/FDT shell, and the HVF boot loop can dispatch inflate/deflate queue notifications with interrupt signaling. Completed descriptors update internal inflated-page accounting after PFN ranges are validated against mapped guest memory, while guest-reported optional statistics, statistics polling, hinting, reporting, and host reclaim need dedicated follow-up designs. |
...
| `GET` | `/balloon/hinting/status` | partial; initial read-only status implemented | Routes through the VMM state/action policy. Hinting status is post-boot-only, requires a configured balloon with `free_page_hinting: true`, and returns Firecracker-shaped `host_cmd` and `guest_cmd` fields from active device state. Current status reports the initial stopped host command and no guest command; real hinting start/stop, queue processing, and guest acknowledgement need a dedicated design. |
...
accounting.
Guest-reported optional statistics, statistics polling, hinting, reporting, and
host memory reclaim remain deferred.
...
| `PUT /balloon` | partial; `204` empty response on successful pre-boot configuration | unsupported after start; `400` `fault_message` | Stores Firecracker-shaped balloon configuration before startup, rejects targets larger than configured guest memory without mutating previous balloon config, exposes it through `GET /balloon` and `GET /vm/config`, and can attach the current virtio-mmio/FDT shell during `InstanceStart`. Runtime `PATCH /balloon` can update the target size after startup when a balloon was configured, and `GET /balloon/statistics` can return required target and actual fields from stored target state plus internal inflated-page accounting. Runtime-handler and HVF boot-loop inflate/deflate descriptor dispatch is implemented with queue interrupt signaling and internal inflated-page accounting for mapped guest PFNs; guest-reported optional statistics, statistics polling, hinting, reporting, and host reclaim remain deferred. |
| `GET /balloon/hinting/status` | post-boot-only unsupported-state fault; `400` `fault_message` | partial; `200` JSON with `free_page_hinting: true`, otherwise `400` `fault_message` | Requires a configured balloon with free-page hinting enabled and returns the initial read-only host and guest command state. Real hinting start/stop, queue processing, guest acknowledgement, and host reclaim remain deferred. |
...
- full balloon statistics polling, hinting, reporting, and host reclaim
