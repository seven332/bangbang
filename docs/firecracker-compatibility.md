...
deterministic SYN-ACK frames, and for non-empty candidate TCP payloads, it can
...
packets, and non-empty MMDS candidate TX payloads only on configured MMDS
...
timeout/retry policy, full TCP stream tracking, out-of-order reassembly,
retransmission policy, FIN/RST/session timeout policy, and stricter
