...
| `--snapshot-version` | implemented native-envelope inspection | Prints the current public writer version `v2.12.0` and exits successfully before fd-table setup, API socket publication, signal setup, or HVF startup. This names bangbang's native data format and does not claim Firecracker v10 artifact compatibility. |
...
| `PUT` | `/snapshot/create`, `/snapshot/load` | supported current native-v2 2.12 complete-serial Full/File lifecycle with independently optional entropy, balloon, virtio-mem, network/MMDS, vsock, and regular-file block-and-pmem profile-3 storage; exact native-v2 2.3–2.11 loaders plus frozen native-v1 File/Uffd reader | Parses Firecracker-shaped bodies, rejects malformed input first, and keeps paths/overrides redacted. Paused-only `Full` create admits a 1–32-vCPU source with required serial and independently optional kinds 7 and 9–13 over coherent MMIO or PCI, yielding all 64 products; boot timer and vhost-user reject before artifacts. Kind 13 has a 640-byte calculated worst case under a defensive 64-KiB cap and retains only portable CID/selector/cursor/virtio/reset/placement state. Load opens state once, selects frozen native-v1 or exact 2.3–2.12 File/COW, validates the complete graph and external authority transaction, requires exact `network_overrides` for kind 12, and accepts at most one validated `vsock_override` for kind 13. It maps memory and constructs fresh serial/entropy/balloon/virtio-mem/network/MMDS/vsock owners before Paused publication. State/memory and guest File/COW RAM stay immutable/private; writable external block/pmem prefixes remain deliberately shared. Focused public matrices prove all 64 products. Signed direct MMIO/PCI vsock matrices prove source reset/loss, event acknowledgement/RX gating/live TX, preserved listeners, fresh bidirectional multistream/half-close traffic, Paused gating, cursor continuation, independent original/override clones, recapture, and fresh metrics. Signed normal-production/App-Sandbox MMIO/PCI coverage adds exact grants, replacement safety, malformed/no-device/missing-authority failures, cancellation/death cleanup, retry, redaction, and complete containment. No Firecracker artifact interoperability, native-v2 `Uffd`, `Diff`, live-peer migration, source dirty/owner/packet/session identity, synchronous RSS, or broad portability is claimed. |
...
current 2.12 derives the serial endpoint intent, every inert optional profile-3
...
supports current native-v2 2.12 Full/File for 1–32 vCPUs with required complete
...
serialization. `Diff`, native-v2 Uffd, per-drive overrides, editing tools,
...
| `PUT /pmem/{id}` | implemented; `204` empty response on successful config storage | implemented in Running/Paused public PCI; `204` after live commit | Pre-boot requests record Firecracker-shaped configuration with one cross-storage root and exact direct or contained regular-file authority. Startup maps each file/private-tail range once and retains the selected MMIO/PCI owner; runtime PCI insertion preflights all capacity before authority or mapping work. Exact-prefix `MS_SYNC`, read-only guest protection, rollback, and cleanup preserve the prior projection. Exact native-v2 2.6 through current 2.12 profile 3 serialize and restore exact pmem configuration/runtime/limiter/queue/interrupt/mapping/transport state through the same complete-set transaction as block; exact 2.7 composes required serial, exact 2.8 may add entropy, exact 2.9 may add balloon, exact 2.10 may add virtio-mem, exact 2.11 may add network/MMDS, and current 2.12 may add vsock. Focused and signed direct/contained rooted pmem-only and rootless mixed matrices certify live and restored behavior, immutable-pair reuse, shared writable prefixes, zero private tails, exact grants, and cleanup. |
...
| `PUT /vsock` | implemented; repeated valid requests replace stored config and return `204` | unsupported after start; `400` `fault_message` without mutation | Implements the live MMIO-or-PCI startup/Unix-socket and exact native-v2 2.12 snapshot subset. Direct requests leave the ordinary path unopened until startup. Contained requests claim one exact directory/listener authority, never reopen the tag, and use a session-bound launcher port connector without guest payload brokerage, `network.client`, or a steady helper. The live endpoint has three 256-entry queues, one 1023 active budget, bounded incomplete requests/backlogs, round-robin host-local ports, wrapping credit, both initiation directions, shutdown/reset, `EVENT_IDX`, exact metrics, and cleanup. Kind 13 captures CID/selector/cursor/queue/reset/placement but no live sockets, connections, packets, deadlines, metrics, grants, sessions, or handles. Capture closes source streams and queues reset; restore starts empty, gates RX until event acknowledgement, keeps TX live, and preserves guest listeners. Load validates the captured or overridden selector before exact direct/contained authority access and rejects a no-device override. Signed direct MMIO/PCI tests prove simultaneous immutable original/override clones, Paused gating, 4 guest-to-host plus 16 host-to-guest deterministic 4-KiB streams, half-close/EOF, exact cursor continuation, recapture, fresh metrics, and cleanup. Signed production/App Sandbox MMIO/PCI tests add replacement, malformed state/memory, no-device/missing-authority faults, cancellation, both death orders, retry, redaction, containment, unchanged entitlements, and session/socket/helper cleanup. All 14 checked vsock identities are terminal. PATCH/DELETE/runtime hotplug, broader CID/event behavior, Firecracker bytes, live-peer/socket/grant migration, Diff/Uffd, vhost/KVM, broad performance parity, and unconstrained cross-host portability remain excluded. |
...
contained certification; current 2.12 preserves that profile-3 graph alongside
...
- virtio-vsock behavior beyond the **implemented live and exact native-v2 2.12
  ...
  Diff/Uffd, unconstrained cross-host portability, and vhost/KVM transports
- snapshot behavior beyond the implemented narrow native-v2 Full/File profile.
  ...
  virtio-mem, and exact `2.11.0` may add network/MMDS. The current public
  `2.12.0` writer
  ...
  are excluded. The storage graph
  ...
  handling, retry, redaction, containment, and cleanup. The CLI reports
  `v2.12.0` and describes exact v1/v2 versions. Remaining work includes
  native-v2 Uffd, Diff artifacts, per-drive overrides, editing tools,
