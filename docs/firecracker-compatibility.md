...
time/identity: 80 are terminal and five aggregate rows remain
...
launcher/sandboxed-worker pairs. All five retained records belong to
...
restored-device contract, while current 2.8 supplies entropy encoding, fresh
owner reconstruction, immutable clone, and signed direct/contained
continuation; the other capture-ready values remain private
...
semantic after signed direct/contained certification. #1665 activates current
exact native-v2 2.8 optional entropy, and #1666 promotes exactly the two
entropy aggregates after signed direct/contained continuation and cleanup
certification. The current aggregate selector is 80 terminal and five
`audit-required`, and the current global counts are 238/160/3/17.
...
| `--snapshot-version` | implemented native-envelope inspection | Prints the current public writer version `v2.8.0` and exits successfully before fd-table setup, API socket publication, signal setup, or HVF startup. This names bangbang's native data format and does not claim Firecracker v10 artifact compatibility. |
...
| `PUT` | `/snapshot/create`, `/snapshot/load` | supported current native-v2 2.8 complete-serial Full/File lifecycle with optional entropy and optional regular-file block-and-pmem profile-3 storage; exact native-v2 2.7/2.6/2.5/2.4/2.3 loaders plus frozen native-v1 File/Uffd reader | Parses Firecracker-shaped bodies, rejects malformed input first, and keeps paths/overrides redacted. Paused-only `Full` create admits a 1–32-vCPU source with complete bounded serial state, optional entropy, and either no storage or 1–64 ordered regular-file block/pmem records over selected MMIO or the product PCI budget, with no root or one first cross-storage root. Serial kind 8 persists endpoint intent, limiter configuration, guest-visible UART registers, bounded RX, status, and pending work; optional profile-3 kind 7 retains exact storage configuration/runtime/limiter/queue/interrupt/mapping/transport state; optional entropy kind 9 retains features, activation, one-queue geometry/cursors, dual-bucket state, one pending descriptor, retry intent, common virtio state, and matching MMIO/PCI placement. Optional devices other than serial, pmem, and entropy, MMDS, boot timer, and vhost-user reject before artifacts. Load opens state once and selects frozen native-v1, exact 2.3, 2.4, block-only 2.5, storage profile-3 2.6, serial-required 2.7, or current 2.8 File/COW. Current load derives one exact serial-plus-optional-storage authority transaction and constructs fresh entropy source/metrics/scheduler/notifier/route/endpoint owners when kind 9 is present. Missing, extra, reordered/swapped, aliased, wrong-role/kind/access/length/geometry, consumed, or canceled authority rejects before construction with no ambient fallback or per-device override. Fresh default stdio or a fresh configured file/FIFO or contained write-only sink replaces source serial endpoints; source descriptors, terminal/FIFO buffers, TX bytes, counters, limiter budget, and metrics do not cross the artifact. Entropy random output, source identity, metrics, scheduler/notifier handles, routes/endpoints, and absolute host time also do not cross it. The batch remains provisional until Paused publication. State/memory and guest File/COW writes stay immutable/private; writable external block and pmem prefixes are deliberately shared and require operator serialization, while each fresh pmem private tail starts zero. Signed direct and normal-production/App-Sandbox MMIO/PCI matrices prove serial and entropy continuation, retained entropy retry without another guest kick, fresh owners and metrics, pathname replacement resistance, storage persistence, limiter/interrupt progress, recapture, exact grants, explicit/automatic resume, immutable same/fresh-process clones, shared prefix epochs, read-only protection, fresh tails, malformed/cancellation/death cleanup, redaction, and worker/launcher cleanup. No Firecracker artifact interoperability, native-v2 `Uffd`, `Diff`, optional-device profile beyond serial, pmem, and entropy, or broad portability is claimed. |
...
| `PUT`, `PATCH` | `/pmem/{id}` | implemented direct MMIO-or-PCI startup, pmem root, non-root PCI runtime, capture-ready state, aggregate live certification, and exact native-v2 2.6/current 2.8 serialization/restore | `PUT /pmem/{id}` stores strict Firecracker-shaped configuration with one cross-storage root, exact direct or contained regular-file authority, one file/private-tail mapping, and exact-prefix `MS_SYNC`. Runtime PCI insertion remains non-root-only and preflights every owner capacity before authority or mapping work. Profile 3 serializes exact file/mapped geometry plus configuration/runtime/limiter/queue/interrupt/mapping/transport state and restores it in the same typed complete-set transaction as block; exact 2.7 composes that unchanged storage graph with required serial state, and current 2.8 may additionally compose entropy. Signed focused and aggregate coverage proves live protection/coherence/reuse; signed rooted pmem-only and rootless mixed MMIO/PCI direct/contained matrices prove shared writable prefix epochs, unchanged read-only peers, zero fresh private tails, immutable state/memory, recapture, exact grants after pathname replacement, and cleanup. |
...
live TCP/ARP state, token secret, borrow, or absolute clock. Network's detached
value remains the exact #1490 serializer/restore handoff and adds no bytes to
the artifact. The accepted baseline
...
current 2.8 derives the serial endpoint intent, every inert optional profile-3
block and pmem request, and optional entropy state from the decoded graph. It
resolves the configured serial sink plus keyed storage vector through one exact
typed destination authority transaction, then constructs fresh entropy owners.
...
supports current native-v2 2.8 Full/File for 1–32 vCPUs with required complete
serial state, optional entropy, and either no storage or a rooted/rootless
vector of 1–64 regular-file block and pmem devices over MMIO or the product PCI
budget. Block
...
metrics, scheduler, notifier, route, and endpoint owners. Optional devices
other than serial, pmem, and entropy remain excluded. Exact 2.7 retains serial
with optional profile-3 storage, exact 2.6 retains the storage-only profile-3
...
| `PUT /pmem/{id}` | implemented; `204` empty response on successful config storage | implemented in Running/Paused public PCI; `204` after live commit | Pre-boot requests record Firecracker-shaped configuration with one cross-storage root and exact direct or contained regular-file authority. Startup maps each file/private-tail range once and retains the selected MMIO/PCI owner; runtime PCI insertion preflights all capacity before authority or mapping work. Exact-prefix `MS_SYNC`, read-only guest protection, rollback, and cleanup preserve the prior projection. Exact native-v2 2.6 through current 2.8 profile 3 serialize and restore exact pmem configuration/runtime/limiter/queue/interrupt/mapping/transport state through the same complete-set transaction as block; exact 2.7 composes it with required serial state and current 2.8 may also compose entropy. Focused and signed direct/contained rooted pmem-only and rootless mixed matrices certify live and restored behavior, immutable-pair reuse, shared writable prefixes, zero private tails, exact grants, and cleanup. |
...
contained certification; current 2.8 preserves that profile-3 graph alongside
required serial state and optional entropy. Parser recognition, config echo, or a family-level test
...
- snapshot behavior beyond the implemented narrow native-v2 Full/File profile.
  ...
  complete serial plus optional profile-3 storage. The current public `2.8.0` writer
  ...
  matching MMIO-or-PCI placement. The storage graph
  ...
  route/endpoint set when kind 9 is present, and publishes only Paused. Signed three-vCPU proof covers repeat immutable
  ...
  fresh destination serial endpoints and optional entropy owners, and commits
  ...
  pathname replacement resistance, and redaction. The CLI reports `v2.8.0` and
  describes exact v1/v2 versions. There is still no general optional-device
  aggregate. Remaining work includes optional devices other than serial, pmem,
  and entropy, native-v2 Uffd, Diff artifacts,
