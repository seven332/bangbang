...
host-initiated multi-payload exchanges through Firecracker-style vsock paths; it
does not cover throughput-oriented streaming, multiple simultaneous streams,
...
- virtio-vsock socket lifecycle beyond connection setup, forceful guest
  reset/full-shutdown cleanup, and the current narrow signed guest-initiated and
  host-initiated multi-payload exchanges; full graceful half-close state
  tracking, full virtio-vsock credit accounting, CID routing beyond current
  host/guest checks, multiple simultaneous streams, and full event payload
  dispatch.
