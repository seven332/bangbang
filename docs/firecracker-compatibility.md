...
| `PUT` | `/snapshot/create`, `/snapshot/load` | recognized; rejected | The API boundary recognizes Firecracker snapshot endpoints and returns a snapshot-specific fault. Real snapshot creation, loading, file formats, dirty-page tracking, and VM/vCPU/device-state serialization remain deferred to future snapshot support. |
