...
exposure, and can retry RX delivery when pending host requests exist without
requiring a fresh guest RX notification. Guest-visible socket lifecycle,
...
accepted-stream handshakes without path exposure, and retry RX delivery when
pending host requests exist without requiring a fresh guest RX notification.
Guest-visible socket lifecycle, `OK`/`RST` host
responses, guest-initiated `uds_path_<PORT>` connections, CID routing, event
queue dispatch, and data movement remain deferred.
