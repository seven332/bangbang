...
| `PUT /drives/{drive_id}` | implemented; `204` empty response for valid file-backed or direct/contained socket-backed pre-boot config | PCI-only non-root file-backed and eligible direct/contained vhost runtime attach implemented; `204` on success | File-backed requests retain optional limiters, deferred direct opening, exact contained grants, selected-transport startup, and transactional PCI runtime attachment. Socket-backed requests use the strict vhost field matrix; direct mode opens an operator path, while contained mode accepts only an exact grant/child reference and obtains a stream through the dedicated authenticated launcher broker. Startup performs bounded discovery before VM construction, selects shared RAM, exposes exact backend-derived config/features, and activates one queue over MMIO or PCI. In Running or Paused all-PCI state, a new non-root vhost ID may attach only when the live memory profile is already shared: owner-side profile/capacity preflight runs before direct access or contained child reservation/broker I/O, bounded discovery follows, owner-thread materialization uses exact live regions, and public config commits after endpoint publication. Default MMIO, root, duplicate, anonymous RAM, dynamic-memory coexistence, invalid backing/grant/socket/negotiation, unavailable session, and capacity exhaustion preserve state; duplicate/preflight failures accept zero candidate connections or broker requests. Same-ID live PUT remains duplicate rejection. Failed contained connection is retryable, and DELETE releases the child lease while retaining directory authority for reinsertion. Manual guest PCI rescan is required. |
| `PUT /pmem/{id}` | implemented; `204` empty response on successful config storage | implemented in Running/Paused public PCI; `204` after live commit | Pre-boot requests record Firecracker-shaped config and replace prior config for the same ID failure-atomically; empty or all-null limiter objects are unconfigured and valid bandwidth/ops buckets are stored and reported. One pmem or ordinary block root is accepted; pmem order supplies `/dev/pmem<i>` and `ro`/`rw`, and cross-family or second-root failure precedes grant/config mutation. Direct pre-boot paths remain unopened until startup; contained grant tags claim and retain exact-ID, exact-access nonzero regular-file descriptors. Startup moves provided descriptors or opens missing direct paths, maps each file/private-tail range once, registers a cloned lease directly with HVF, attaches the selected transport, and retains every owner. After startup, public PCI accepts only a new non-root ID: direct mode opens before owner submission, contained mode reserves an exact unused grant with rollback, and the owner maps the file, allocates a non-overlapping reusable guest range, registers the exact direct mapping, and publishes the endpoint before configuration and grant-consumption commit. Exact-prefix `MS_SYNC`, read-only guest protection, recoverable rollback, and failed-unmap lease retention preserve the prior projection and authority; incomplete cleanup is terminal. Default MMIO rejects runtime insertion before path use. Capture-ready traversal retains exact pmem backing/mapping/device/transport state; dirty-range tracking plus optional-device serialization and restore remain deferred. |
...
- pmem dirty-range tracking, broad sharing/performance certification, and
  optional-device snapshot serialization/restore beyond the capture-ready live
  state, direct file-backed HVF mapping, deterministic
