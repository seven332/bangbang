...
deterministic SYN-ACK frames, identify pure empty-payload ACK-only, FIN close,
guest RST, and unsupported control packets, synthesize ACK plus FIN-ACK frames
for empty FIN close packets, synthesize minimal RST frames for unsupported empty
controls, consume guest RST packets without response, and for non-empty
candidate TCP payloads, it can
...
packets, pure empty-payload MMDS ACK-only packets, pure empty-payload MMDS FIN
close packets, guest RST packets, unsupported empty control packets, and
non-empty MMDS candidate TX payloads only on configured MMDS interfaces, keeps
...
timeout/retry policy, ACK-number validation, full TCP stream tracking,
out-of-order reassembly, retransmission policy, stateful RST policy, session timeout
policy, and stricter
