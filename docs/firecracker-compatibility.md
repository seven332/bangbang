...
`InstanceStart` preflight, transactional startup executor, and successful-start state transition helpers, an internal MMDS guest TCP packet classifier, process-local packet-payload HTTP exchange, process vmnet TX detour, internal MMDS response-frame synthesis, bounded ordered split-request buffering, and queued virtio-net RX delivery with bounded post-TX retry, backend-neutral guest
...
| `GET`, `PUT`, `PATCH` | `/mmds` | supported target; control-plane storage, runtime guest-query formatting, internal guest GET response modeling, request parsing, process-local exchange handling, response-byte serialization, process-local token authority, process-local guest token `PUT` modeling, process-local MMDS v2 GET token enforcement, internal guest TCP packet classification, process-local packet-payload HTTP exchange, process vmnet TX detouring, bounded ordered split-request buffering, and internal response-frame synthesis implemented | Stores bounded in-memory JSON object contents in the process runtime, returns stored JSON for control-plane `GET`, applies RFC 7396 merge-patch semantics for `PATCH`, rejects uninitialized `GET`/`PATCH`, and keeps previous data on oversized update failure. The runtime can also resolve initialized metadata by JSON-pointer path, format JSON or Firecracker-shaped IMDS text, parse process-local guest HTTP `GET` request bytes into URI/output-format/token inputs, map internal guest GET requests to process-local status/content-type/body response values, turn complete process-local guest HTTP request buffers into deterministic HTTP/1.1 response bytes, generate/validate bounded process-local opaque MMDS tokens, and model process-local guest `PUT /latest/api/token` exchanges as prerequisites for guest-visible delivery. When configured for MMDS v2, process-local guest GET requests require exactly one valid `X-metadata-token` or `X-aws-ec2-metadata-token` value generated by token PUT; missing, duplicate, unknown, or expired tokens return `401 Unauthorized`. The runtime can classify raw Ethernet/IPv4/TCP guest packet bytes addressed to the configured MMDS IPv4 address and TCP port 80 while rejecting malformed, truncated, fragmented, non-TCP, or non-MMDS packets, and it can turn non-empty candidate TCP payloads into the same process-local HTTP response bytes as the guest HTTP helper. Process vmnet packet I/O now detours non-empty MMDS candidate TX payloads only on MMDS-configured interfaces, keeps shared process-local MMDS state visible to control-plane and packet paths, buffers ordered split request headers in bounded per-interface process state, synthesizes Ethernet/IPv4/TCP response frames carrying generated HTTP response bytes, retains those frames in bounded per-interface queues, exposes queued frames through the matching virtio-net RX source before vmnet reads, and schedules one bounded post-TX RX retry when that source reports a queued response. Full TCP stream tracking, out-of-order reassembly, retransmission policy, FIN/RST/session timeout policy, and stricter per-interface TCP session ownership remain tied to later #16 work. |
...
queued frames through the matching virtio-net RX source before vmnet reads, and
schedules one bounded post-TX RX retry when that source reports a queued
response. Full TCP stream tracking, out-of-order reassembly, retransmission
policy, FIN/RST/session timeout policy, and stricter per-interface TCP session
