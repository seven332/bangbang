...
`InstanceStart` preflight, transactional startup executor, and successful-start state transition helpers, an internal MMDS guest TCP packet classifier, process-local packet-payload HTTP exchange, process vmnet TX detour, and internal MMDS response-frame synthesis plus queued virtio-net RX delivery with bounded post-TX retry, backend-neutral guest
...
response. TCP stream tracking, split request buffering, retransmission policy,
and stricter per-interface TCP session ownership remain deferred.
