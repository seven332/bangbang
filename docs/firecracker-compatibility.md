...
| --- | --- |
| [#5786 PCI hotplug/hot-unplug](https://github.com/firecracker-microvm/firecracker/pull/5786) for block, pmem, and net | Implemented for non-root block devices in the public PCI profile, including transactional Running/Paused PUT/DELETE and manual guest rescan/removal. Default virtio-MMIO and root mutation retain nonmutating rejection; runtime pmem and network mutation remain deferred. |
...
semantic identities. After the #1420 block-hotplug promotion, the current 418
records contain 73 `implemented-and-verified`, 325 `audit-required`, three
...
| `DELETE` | `/pmem/{id}`, `/network-interfaces/{iface_id}` | recognized; VMM-routed unsupported | The two remaining pinned bodyless hot-unplug routes retain device-specific unsupported behavior after startup and normal unsupported-state behavior before startup. Body-bearing requests fail first with `Empty Delete request.`. Their real runtime removal remains deferred. |
...
GICv3 MBI/ITS, runtime pmem/network attach/delete, PCI snapshot persistence, or
delivery rollback guarantee. Runtime block PUT/DELETE instead uses the retained
bounded PCI/GICv2m resource manager and requires manual guest coordination.
...
| `DELETE /drives/{drive_id}`, `DELETE /pmem/{id}`, `DELETE /network-interfaces/{iface_id}` | recognized bodyless hot-unplug; `400` `fault_message` | PCI-only non-root drive DELETE implemented; pmem/network return stable `400` faults | Bodyless requests route through one `HotUnplugDevice` VMM action. Pre-boot requests return the normal unsupported-state fault. In a Running/Paused public PCI session, drive DELETE removes an existing non-root endpoint only after the guest has been removed manually, commits configuration only after teardown, releases every exact lease for reuse, restores the endpoint after recoverable preparation failure, and makes post-commit corruption terminal. Default MMIO, root, and missing-drive requests are nonmutating. Pmem and network runtime DELETE remain stable device-specific faults. Requests with a body fail first as malformed request shape before hot-unplug routing. |
