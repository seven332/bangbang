...
guest-visible virtio-balloon MMIO/FDT shell from that stored configuration, but
only the runtime handler's backend-neutral deflate descriptor acknowledgement
path exists today. Boot-loop balloon notification dispatch, inflate processing,
memory accounting, statistics, hinting, reporting, and host memory reclaim
remain deferred.
...
update only the local device register state. The backend-neutral deflate queue
dispatcher drains available descriptor heads and publishes zero-length used-ring
entries when runtime-handler notification dispatch is invoked. Boot-loop balloon
notification dispatch, inflate execution, memory accounting, statistics,
hinting, reporting, host memory reclaim, and runtime target updates remain
deferred.
...
| `PUT /balloon` | partial; `204` empty response on successful pre-boot configuration | unsupported after start; `400` `fault_message` | Stores Firecracker-shaped balloon configuration before startup, exposes it through `GET /balloon` and `GET /vm/config`, and can attach the current virtio-mmio/FDT shell during `InstanceStart`. Runtime-handler deflate descriptor acknowledgement is implemented; boot-loop balloon notification dispatch, inflate processing, memory accounting, statistics, hinting, reporting, host reclaim, and runtime target updates remain deferred. |
