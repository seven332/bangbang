...
| `--start-time-us <MICROS>` | parsed and reported in minimal metrics | Accepts non-negative `u64` microsecond values passed by Firecracker-style launchers. When provided, explicit runtime `FlushMetrics` output includes `start_time_us`. Full `ProcessTimeReporter` parity remains deferred. |
| `--start-time-cpu-us <MICROS>` | parsed and reported in minimal metrics | Accepts non-negative `u64` microsecond values passed by Firecracker-style launchers. When provided, explicit runtime `FlushMetrics` output includes `start_time_cpu_us`. Full `ProcessTimeReporter` parity remains deferred. |
| `--parent-cpu-time-us <MICROS>` | parsed and reported in minimal metrics | Accepts non-negative `u64` microsecond values passed by Firecracker-style launchers. When provided, explicit runtime `FlushMetrics` output includes `parent_cpu_time_us`. Full `ProcessTimeReporter` parity remains deferred. |
