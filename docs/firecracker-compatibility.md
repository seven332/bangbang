...
and writable pmem uses read/write non-executable permissions. Runtime also has
a backend-neutral virtio-pmem identity, queue metadata, feature-bit, and 16-byte
config-space foundation, but startup does not attach guest-visible virtio-pmem
devices yet.
...
| `PUT`, `PATCH` | `/pmem/{id}` | partial; pre-boot config storage, startup backing mapping, HVF guest-memory registration, and internal guest-range config-space model implemented | `PUT /pmem/{id}` stores Firecracker-shaped pmem configuration before boot, replaces prior config for the same ID, and appears in `GET /vm/config` under `pmem`. It rejects empty backing paths without opening host files during the request. `InstanceStart` opens each configured backing, validates it as a non-zero regular file, mmaps it to a 2 MiB-aligned host range, assigns a deterministic non-overlapping 2 MiB-aligned guest physical range after the aarch64 MMIO64 gap while skipping current guest RAM, creates the HVF VM with the framework-reported maximum IPA size, copies the prepared mapping into an HVF-compatible anonymous shadow, registers that shadow with HVF after DRAM using read-only or read/write non-executable permissions, populates the internal virtio-pmem config-space `start`/`size` values, and retains the handle, mapping, and range metadata without attaching a guest-visible virtio-pmem device yet. Runtime defines the backend-neutral virtio-pmem identity, one 256-entry queue, modern virtio feature bit, 2 MiB alignment constant, and 16-byte config-space layout for future guest attachment. Configured rate limiters are rejected without mutating stored pmem configuration. `PATCH /pmem/{id}` remains post-boot-only unsupported, and real runtime updates, rate limiting, root-device semantics, guest attachment, shadow writeback/flush behavior, and hot-unplug need dedicated device designs. |
...
| `PUT /pmem/{id}` | `path_on_host` | required; startup-mapped, range-assigned, and HVF-registered through a shadow | Required Firecracker-shaped host backing path. The value is retained and reported in `GET /vm/config` after rejecting empty paths. Startup opens the path, validates it as a non-zero regular file, mmaps it to a 2 MiB-aligned host range, assigns matching internal guest physical range/config-space metadata, copies the prepared mapping into an HVF-compatible anonymous shadow, registers that shadow with HVF, and redacts path details from backing-open, mapping, range-allocation, shadow-allocation, or HVF-registration errors. Normalization and guest attachment remain deferred with virtio-pmem support. |
...
| `PUT /pmem/{id}` | `read_only` | optional; startup-open, host mapping, and HVF permission access | Missing values default to `false` and are reported in `GET /vm/config`. Startup opens and maps the host backing read-only when this is `true` and read/write when this is `false`; HVF shadow registration mirrors that intent with read-only or read/write non-executable memory permissions. Guest-visible virtio-pmem behavior remains deferred. |
...
| `PUT /pmem/{id}` | implemented; `204` empty response on successful config storage | unsupported after start; `400` `fault_message` | Records Firecracker-shaped pre-boot config and replaces prior config for the same ID without opening the host backing file during the request. Stored configs appear in `GET /vm/config`; startup opens and validates each backing as a non-zero regular file, mmaps it to a 2 MiB-aligned host range, assigns non-overlapping guest physical range/config-space metadata, copies the prepared mapping into an HVF-compatible anonymous shadow, registers that shadow with HVF after DRAM, and retains the handles, mappings, and range metadata. Runtime has the backend-neutral virtio-pmem config-space foundation, but root-device semantics, guest-visible virtio-pmem attachment, rate limiting, runtime updates, shadow writeback/flush behavior, and hot-unplug remain deferred. |
