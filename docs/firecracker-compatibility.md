...
`balloon` reports implemented virtio-balloon queue activity and failures. Block
rate-limiter metrics, hotplug/removal counters, vhost-user-block metrics, and
full Firecracker block metric coverage remain deferred. The startup timing
fields from Firecracker's
...
metrics remain deferred until their producers exist. Current virtio-net runtime
