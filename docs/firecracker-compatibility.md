...
implemented virtio-balloon queue activity and failures. HVF block limiter retry
wakeups are wired for active block queues. Hotplug/removal counters,
vhost-user-block metrics, entropy rate-limiter metrics beyond the implemented
subset, and full Firecracker block metric coverage remain deferred. The startup timing fields
from Firecracker's
...
limiter parity, and non-block limiter wakeups remain deferred. It does not provide a public
...
- full Firecracker active timerfd/eventfd rate-limiter wakeup parity beyond the
  current HVF block retry scheduler, including entropy and shared event-source
  behavior
