...
| Unknown endpoint or invalid method/path | `400` `fault_message` | `400` `fault_message` | Matches Firecracker's parser-level invalid path or method handling. Bodyless `PUT` and non-balloon `PATCH` requests on unsupported paths or methods use Firecracker's method-level empty request faults without accepting the route. |
