...
but `InstanceStart` rejects configurations that include it until virtio-mem
startup support exists. The failure happens before the API socket is published
or before no-api readiness is reported.
...
| `GET /hotplug/memory`, `PUT /hotplug/memory`, `PATCH /hotplug/memory` | `PUT` implemented for config storage; `GET` returns initial status after config; backend-neutral virtio-mem handler foundation modeled; `PATCH` unsupported-state before start | `GET` returns the stored initial status if configured; `PUT` unsupported after start; `PATCH` memory-hotplug unsupported after start | `PUT` stores a validated Firecracker-shaped pre-boot config and replaces the prior config without opening host resources. `GET` returns `total_size_mib`, `block_size_mib`, `slot_size_mib`, `plugged_size_mib: 0`, and `requested_size_mib: 0` when configured; without a stored config it returns the memory-hotplug-specific unsupported fault. The runtime has the Firecracker-shaped virtio-mem device ID, one-queue MMIO handler, feature bits, read-only 56-byte config-space layout, and activation-time queue metadata checks ready for later MMIO attachment. `InstanceStart` rejects stored memory-hotplug configs until virtio-mem startup support exists. Runtime resize, request queue parsing, plugged-size tracking, hot-unplug, and guest memory-map changes remain deferred. |
