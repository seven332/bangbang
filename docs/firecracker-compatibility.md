...
only the backend-neutral deflate descriptor acknowledgement path exists today.
Inflate processing, memory accounting, statistics, hinting, reporting, and host
...
entries when notification dispatch is invoked. Inflate execution, memory
accounting, statistics, hinting, reporting, host memory reclaim, and runtime
target updates remain deferred.
...
| `PUT /balloon` | partial; `204` empty response on successful pre-boot configuration | unsupported after start; `400` `fault_message` | Stores Firecracker-shaped balloon configuration before startup, exposes it through `GET /balloon` and `GET /vm/config`, and can attach the current virtio-mmio/FDT shell during `InstanceStart`. Backend-neutral deflate descriptor acknowledgement is implemented; inflate processing, memory accounting, statistics, hinting, reporting, host reclaim, and runtime target updates remain deferred. |
