...
| `GET`, `PUT`, `PATCH` | `/mmds` | parser/action boundary implemented; storage deferred | Parses Firecracker-shaped MMDS content requests, enforces the existing payload limit, rejects malformed or non-object JSON bodies for update requests, and routes to explicit runtime actions that currently return `400` `fault_message` because MMDS data storage, patch semantics, and guest-visible delivery remain tied to #16. |
| `PUT` | `/mmds/config` | parser/action boundary implemented; application deferred | Parses Firecracker-shaped MMDS config with required non-empty `network_interfaces`, optional `version`, optional RFC 3927 usable link-local `ipv4_address`, and optional `imds_compat`. The runtime action currently returns `400` `fault_message`, with post-start requests using the normal unsupported-state policy; validating referenced interface existence, storing config, and exposing MMDS to the guest remain tied to #16. |
...
- snapshots
- MMDS data storage, patch semantics, configuration application, and guest access
