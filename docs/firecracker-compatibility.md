...
| `GET` | `/balloon/hinting/status` | partial; host and guest command status implemented | Routes through the VMM state/action policy. Hinting status is post-boot-only, requires a configured balloon with `free_page_hinting: true`, and returns Firecracker-shaped `host_cmd` and `guest_cmd` fields from active device state. Current status reports the latest start/stop host command and the latest 4-byte guest command observed on the hinting queue; `guest_cmd` remains `null` until the guest sends a command descriptor. Hinting range processing and host reclaim remain deferred. |
...
| `PATCH /balloon/hinting/start` | `acknowledge_on_stop` | optional | Missing values follow Firecracker's default `true` command shape. The current implementation preserves the value in host-owned device state for later automatic done acknowledgement after range processing completes. Hinting queue command acknowledgements are implemented; range processing and host reclaim remain deferred. |
...
accounting.
Guest-reported optional statistics, statistics polling, hinting range
...
- full balloon statistics polling, hinting range processing, reporting, and
  host reclaim
