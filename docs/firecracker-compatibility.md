...
`logger.missed_metrics_count` counter, and logger API request, action, or
boot-timer write failures increment `logger.missed_log_count`, for later successful metrics output. Public drive configuration is
...
| `--module <MODULE>` | filters minimal logger events | Matches the stored `PUT /logger` field and filters current minimal logger events with Firecracker-style module-path prefix matching. API request method/path lines use `bangbang_runtime::api_server`, action logs use `bangbang_runtime::vmm_action`, and boot-timer logs use `bangbang_runtime::boot_timer`; full internal log routing remains deferred. |
...
| `--show-log-origin` | enables origin field for minimal logger events | Writes `origin=<file>:<line>` before minimal API request, action, and boot-timer log messages. Full Firecracker logger integration remains deferred. |
...
| `--describe-snapshot <PATH>` | implemented native-envelope inspection | Opens a regular file with a 16 MiB payload / 16 MiB + 40 byte total cap, validates the complete bangbang-native header, exact length, CRC-64/Jones trailer, exact `1.0.0` version, arm64 identity, 4096-byte guest granule, and zero flags, then prints `v1.0.0`. Missing, non-regular, oversized, malformed, corrupt, future-version, or incompatible files fail before startup with the bad-configuration exit status and path/payload-redacted diagnostics. Firecracker state files are intentionally incompatible. |
...
and failures; `signals.sigpipe` reports handled non-terminating `SIGPIPE`
signals. HVF block and entropy limiter retry wakeups are wired for active
queues. Hotplug/removal counters, vhost-user-block metrics, entropy
rate-limiter metrics beyond the implemented subset, and full Firecracker block
metric coverage remain deferred. The startup timing fields from Firecracker's
`ProcessTimeReporter` are implemented; the broader full Firecracker metrics set
remains deferred.
...
configuration actions. The `drives` section is parsed through
...
runtime rejects `free_page_reporting: true` before storing the config or
starting the VM until real reporting queue behavior exists. The runtime handler
and HVF boot loop can dispatch inflate and deflate queue
...
the configured polling interval while the VM is running, but host memory
reclaim and reporting remain deferred. The `pmem` section is
parsed through
`PUT /pmem/{id}`;
...
configuration because pmem root boot semantics are not implemented yet. Startup
opens and validates each configured pmem backing as a
...
Runtime `PATCH /pmem/{id}` accepts missing, `null`, empty, and all-null
Firecracker-shaped rate limiter objects as no-op updates for existing pmem
devices after startup, while configured pmem rate limiters remain rejected
without echoing limiter values. `PUT /pmem/{id}` rejects `root_device: true`
until pmem root boot semantics are implemented.
...
runtime plugged size when the handler can be queried. Accepted guest
`PLUG`/`UNPLUG` requests use the HVF dynamic memory mapping path, while broader
public guest-memory accounting and public hot-unplug semantics remain deferred.
...
| `PUT` | `/drives/{drive_id}` | supported target; implemented | Stores initial virtio-block device configuration before boot, including optional bandwidth/ops rate limiters; backing files are opened during startup preparation. |
| `PUT` | `/metrics` | supported target; minimal subset implemented | Stores process metrics output before boot, opens the configured file/FIFO path with nonblocking output semantics, and omits metrics from `GET /vm/config` because it is not guest configuration. Duplicate initialization returns a fault. Explicit runtime `FlushMetrics` and periodic runtime metrics flushes every 60 seconds while the VM is running can append minimal output with Firecracker-style `api_server` startup timing fields plus initial `GET /`, `GET /version`, `GET /machine-config`, `GET /mmds`, core configuration PUT, MMDS PUT, selected PATCH, observability PUT, `/actions` API request counters, `logger.missed_metrics_count` after a previous metrics write failure, `logger.missed_log_count` after a previous logger API request, action, or boot-timer write failure, Firecracker-shaped `signals.sigpipe` counts, Firecracker-shaped `uart` metrics for implemented serial TX writes, output errors, missed writes, and rate-limiter drops, top-level aggregate `block`, `net`, `vsock`, `entropy`, `rtc`, and `balloon` objects when active devices report implemented queue, byte, update, or failure activity, plus non-empty per-drive `block_{drive_id}` and per-interface `net_{iface_id}` objects. |
| `PUT` | `/actions` | supported target; internal startup execution and minimal metrics flush implemented | Parses `InstanceStart` and `FlushMetrics` request bodies and routes them through the process VMM owner. Successfully parsed requests can write one minimal API request method/path line before dispatch when logger output is configured and the level/module filters allow it. `InstanceStart` validates stored boot-source and state preflight, prepares an owned HVF boot session with the configured serial output path or the default internal serial MMIO console on success, starts a process-owned internal boot run-loop worker across bounded step windows, writes one minimal action log line when logger output is configured and the level/module filters allow it, and commits `Running` only after the worker handle is retained and any emitted action log write succeeds. `FlushMetrics` is rejected before startup and returns `204 No Content` after startup; configured metrics output receives one minimal JSON line with flush count, selected `get_api_requests`, `put_api_requests`, and `patch_api_requests` counters, `logger.missed_metrics_count` after a previous metrics write failure, `logger.missed_log_count` after a previous logger API request, action, or boot-timer write failure, optional Firecracker-style `api_server` startup timing fields, optional boot-run-loop status, optional `signals.sigpipe`, optional aggregate and per-drive block device metrics, optional aggregate and per-interface network metrics, optional vsock, entropy, RTC, and balloon device metrics, configured logger output receives one minimal action log line when the level/module filters allow it, and unconfigured metrics/logger output is a no-op success. The aarch64 `SendCtrlAltDel` unsupported parser path still contributes to `put_api_requests.actions_count` but not `actions_fails`, matching Firecracker's parser-entry metric placement. Periodic metrics flushes are not `/actions` requests and do not write logger action lines. |
...
| `PUT` | `/logger` | supported target; minimal subset implemented | Stores process logger configuration before boot, opens `log_path` with nonblocking output semantics when provided, accepts optional Firecracker-shaped level/show/module fields, and omits logger state from `GET /vm/config` because it is not guest configuration. Successfully parsed API requests append minimal method/path lines without request bodies when the configured level allows `Info` and the optional module prefix matches `bangbang_runtime::api_server`; successful `InstanceStart` and `FlushMetrics` append deterministic `action=...` lines when the module prefix matches `bangbang_runtime::vmm_action`. Full internal log routing remains deferred. |
| `PUT` | `/serial` | supported target; output path and output rate limiter implemented | Stores Firecracker-shaped pre-boot serial output configuration with optional `serial_out_path` and optional token-bucket `rate_limiter`; `{}` and `"serial_out_path": null` clear the configured public output path. `InstanceStart` opens the configured file/FIFO path with nonblocking output semantics and routes guest TX serial MMIO bytes to it. When a valid limiter is configured, exhausted output bytes are dropped without blocking or sleeping; runtime metrics can report a Firecracker-shaped `uart` object with implemented TX write, output error, missed write, and dropped-byte counters. Serial input/stdin, read counters, flush counters, and public streaming remain deferred. |
...
| `PUT` | `/snapshot/create`, `/snapshot/load` | recognized; typed VMM-routed unsupported | Parses Firecracker-shaped snapshot create and load bodies, rejects malformed input first, normalizes deprecated load `mem_file_path` to a `File` backend and dirty tracking to the OR of old/new flags, and carries every field through debug-redacted API/runtime values. Paths plus network/vsock override contents are retained but never logged, opened, canonicalized, or statted by public dispatch. A centralized native-v1 gate currently admits only Full create, File load, no dirty tracking/clock adjustment/overrides, and a create profile with one vCPU, exactly one read-only root drive, default serial, and no optional devices or MMDS. State-invalid requests keep their existing faults. Rejected paused create profiles return the same snapshot-specific unsupported fault before the supervisor barrier; an admitted profile crosses the scoped-admission/block/entropy-quiescence barrier and still returns unsupported. Load is pre-boot-only and requires both successful-action history and current non-logger/metrics configuration to be pristine; logger/metrics remain allowed, explicit-default/no-op VM configuration is detected by history, and residual state such as failed-patch MMDS presence is detected live. This is deliberately stricter and more state-based than Firecracker `v1.16.0`'s uneven pre-validation `boot_path` flag. Public faults and snapshot latency/deprecation metrics remain compatible. Internally, kind-1 `BANGCMT\0` remains byte-compatible and kind 2 binds memory to an exact five-component `BANGHVF\0` payload. A private create transaction preflights both final namespaces and private staging resources, then streams the complete paused supervisor capture directly into the publisher-owned memory inode, requires writer-close proof and exact record/output matching, and commits with macOS `RENAME_EXCL` memory-first/state-last ordering. A separate private load transaction validates and installs baseline devices without boot writes, constructs and restores a fresh HVF VM, replaces and signals VMGenID, and hands an unrun session to an initially paused supervisor while returning resume intent. Producer/capture failures clean only private staging; a published memory final after a later failure remains a typed orphan, and state-directory sync uncertainty is committed success. The endpoint invokes only the original barrier and does not call either transaction, open request paths, or publish/load files; no Firecracker state-file compatibility is claimed. Public artifact I/O and endpoint success, dirty tracking, optional devices, post-load resume, and fresh-process API continuity remain deferred. |
| `GET` | `/balloon` | supported target; pre-boot and runtime config read implemented | Returns the stored Firecracker-shaped balloon configuration after successful `PUT /balloon`, runtime `PATCH /balloon`, or valid runtime `PATCH /balloon/statistics`; returns the balloon-specific unsupported fault when no balloon configuration exists. Runtime derives backend-neutral virtio-balloon identity, features, queues, and config space from stored config. Startup attaches the current virtio-mmio/FDT shell, and the HVF boot loop can dispatch inflate, deflate, statistics, and free-page hinting notifications with interrupt signaling. Inflate/deflate descriptors update internal inflated-page accounting, hinting command descriptors update `guest_cmd`, active-run hinting ranges are recorded, statistics queue reports are parsed and stored for `GET /balloon/statistics`, process-level periodic scheduling can complete a pending statistics descriptor with queue-interrupt intent while the VM is running, and minimal device metrics can report implemented queue activity and failures. Free-page reporting, host reclaim, and broader host-side hinting behavior remain deferred. |
| `PUT`, `PATCH` | `/balloon` | partial; pre-boot config storage, startup shell attachment, runtime target update, and runtime stats interval update implemented | `PUT /balloon` stores Firecracker-shaped balloon configuration before boot, rejects targets larger than current guest memory without mutating prior config, rejects `free_page_reporting: true` without mutating prior config, appears in `GET /balloon` and `GET /vm/config`, and feeds startup attachment of a virtio-balloon MMIO/FDT shell. Pre-boot machine-config updates also reject memory sizes smaller than an already configured balloon target. Runtime `PATCH /balloon` updates the stored `amount_mib`, active `num_pages` config space, config generation, and config interrupt. Runtime `PATCH /balloon/statistics` can update a nonzero statistics polling interval to another nonzero value while preserving Firecracker's rejection of runtime statistics enable/disable transitions. Runtime queue dispatch covers inflate, deflate, configured statistics reports, pending statistics descriptor completion from process-level periodic scheduling, and active-run hinting ranges; minimal device metrics can report implemented queue activity and failures. Host reclaim and free-page reporting remain deferred. |
...
| `GET`, `PUT`, `PATCH` | `/hotplug/memory` | partial; pre-boot config storage, initial status, startup-visible virtio-mem shell, request-queue completion, plugged-block tracking, failure-aware mutation boundary, active status, and runtime requested-size update implemented | Parses Firecracker-shaped `PUT` and `PATCH` memory hotplug request bodies, rejects malformed or schema-invalid bodies first, and then routes valid requests through the VMM state/action policy. `PUT` is pre-boot-only, applies Firecracker-shaped block/slot/total size semantic validation, stores valid configs, replaces prior configs, and exposes the stored config through `GET /vm/config`. `GET /hotplug/memory` returns the stored config plus `plugged_size_mib: 0` before startup; after startup it queries the active virtio-mem handler and reports the current active `plugged_size_mib` plus the stored `requested_size_mib`. Without a stored config it returns the memory-hotplug-specific unsupported fault, and active query failures return a clear status fault instead of stale data. `PATCH` is post-boot-only, validates the requested size against configured total size and block alignment, updates the active virtio-mem requested-size config field, grows usable config-space size to a slot boundary when needed, increments config generation, and raises a config interrupt before committing the stored requested size. Startup can attach a guest-visible virtio-mem MMIO/FDT shell whose config space exposes the configured block size, a deterministic hotplug aperture, the configured total region size, and zero usable, plugged, and requested bytes. The backend-neutral queue path parses request and response descriptors, publishes used-ring entries, wires queue-interrupt intent through boot runtime/HVF, tracks plugged block ranges, returns plugged/unplugged/mixed `STATE` responses for valid usable ranges, accepts valid `PLUG`/`UNPLUG`/`UNPLUG_ALL` requests through a failure-aware mutation executor boundary, updates config-space `plugged_size`, and keeps invalid ranges, duplicate mutations, unsupported request types, descriptor parse failures, mutation executor failures, response write failures, used-ring failures, and rollback failures fail-closed. A signed executable e2e scenario covers a direct-rootfs guest binding `virtio_mem` and observing a runtime requested-size update. Broader public guest-memory accounting and public memory hot-unplug semantics need dedicated designs. |
...
| `PUT /snapshot/create` | `snapshot_type` | optional; retained and VMM-routed unsupported | Accepts `Full` and `Diff`, defaulting to `Full`, and retains the value through the process boundary. Only `Full` passes the native-v1 gate; `Diff` returns the same snapshot-specific fault before the supervisor barrier. |
| `PUT /snapshot/create` | `snapshot_path` | required; retained/redacted, unopened publicly | Retained through API/runtime values with redacted `Debug`; public dispatch does not open, canonicalize, stat, log, or echo it. The private create transaction uses it only after paused/profile preflight. |
| `PUT /snapshot/create` | `mem_file_path` | required; retained/redacted, unopened publicly | Retained through API/runtime values with the same public redaction/no-file policy as `snapshot_path`. The private create transaction can stream guest memory directly into its destination-anchored staging inode. |
| `PUT /snapshot/create` | unknown fields | rejected | Matches Firecracker's strict request model behavior. |
| `PUT /snapshot/load` | `snapshot_path` | required; retained/redacted, unopened publicly | Public dispatch retains the value without file access or diagnostic exposure. The private native-v1 load executor uses it only after pristine preflight. |
| `PUT /snapshot/load` | `mem_backend` | required unless deprecated `mem_file_path` is present; retained/redacted | Parsed as a strict `backend_path`/`backend_type` object. Exactly one backend form is required; the normalized backend reaches VMM policy without opening its path. |
...
| `PUT /snapshot/load` | `resume_vm` | optional; retained as intent | The private native-v1 load executor returns this bit after committing an initially paused session and never silently resumes. The public endpoint still returns unsupported. |
...
| `PUT /pmem/{id}` | `rate_limiter` | optional when absent, `null`, empty, or all-null; rejected when configured | Empty, all-null, or omitted limiter shapes do not create stored limiter state. Configured Firecracker-shaped rate limiter objects are validated and rejected without mutating stored pmem configuration. Real pmem flush rate limiting remains deferred. |
...
| `PATCH /pmem/{id}` | `rate_limiter` | optional when absent, `null`, empty, or all-null; rejected when configured | Missing, `null`, empty, or all-null objects are accepted as runtime no-ops for existing pmem devices after startup. Configured Firecracker-shaped rate limiter objects are validated before VMM dispatch and rejected without retaining or echoing their values. Real pmem runtime rate-limiter updates remain deferred. |
...
| `PUT /network-interfaces/{iface_id}` | `rx_rate_limiter`, `tx_rate_limiter` | optional when absent, `null`, empty, or all-null; rejected when configured | Empty or all-null Firecracker `bandwidth`/`ops` bucket objects are treated as unconfigured. The internal model rejects configured network rate limiters until virtio-net rate limiting behavior exists. |
...
| `PATCH /network-interfaces/{iface_id}` | `rx_rate_limiter`, `tx_rate_limiter` | optional when absent, `null`, empty, or all-null; rejected when configured | Omitted, `null`, empty, or all-null rate limiters are accepted as runtime no-ops for existing interfaces after startup. Configured Firecracker-shaped rate limiter objects are parsed and then rejected before any runtime network mutation. Real rate limiting belongs with future network update work. |
...
| `PUT /logger` | `show_level` | optional | When true, minimal API request, action, and boot-timer log lines include a `level=Info` prefix. |
| `PUT /logger` | `show_log_origin` | optional | When true, minimal API request, action, and boot-timer log lines include an `origin=<file>:<line>` field for the callsite. |
| `PUT /logger` | `module` | optional | Filters minimal logger events with Firecracker-style module-path prefix matching. API request method/path lines use `bangbang_runtime::api_server`, action logs use `bangbang_runtime::vmm_action`, and boot-timer logs use `bangbang_runtime::boot_timer`; non-matching filters suppress those lines without failing the action. |
...
unsupported state before startup and while running, and reaches the snapshot
unsupported fault only while the instance is paused. A process-owned paused
create first crosses the internal supervisor scoped-admission barrier: earlier
FIFO commands complete, later ordinary supervisor commands reject during the
scope, the worker waits for acknowledged quiescence from the block and entropy
limiter retry schedulers, and those guards release before ordinary admission is
restored and the same fault is returned. Pending wakeup tokens are drained into
deferred work and absolute retry deadlines age while the guards are held, so
due work is preserved for asynchronous publication after release. This covers
only the supervisor command boundary plus those two schedulers, not full
snapshot-ready quiescence. `PUT /snapshot/load` reaches the snapshot unsupported
fault before startup and fails as an unsupported state after startup. A private
process seam now uses the same pristine preflight to load committed native-v1
artifacts into a fresh initially paused destination, but public dispatch does
not call it. A separate private create seam uses paused/profile preflight,
streams the complete supervisor capture into destination-anchored publisher
staging, and commits an exact kind-2 pair without mutating controller state.
Malformed or schema-invalid snapshot bodies are still rejected by the parser
first.

Public snapshot persistence and restore still need endpoint routing, response
and metric semantics, post-load resume handling, and fresh-process API evidence
before either API request can move beyond recognized unsupported behavior.
...
network/vsock restore overrides before optionally resuming the VM. bangbang now
defines those equivalents for its private one-vCPU/read-only-root native-v1
baseline, including paused handoff and recoverable-versus-terminal cleanup
evidence. Public create/load routing, optional resources and overrides, dirty
tracking, and broader portability remain deferred; unknown HVF feasibility
should not be reported as a platform limit by default. bangbang has a native
...
anonymous memory only after identity, length, CRC, and EOF checks. The private
process create transaction publishes a close-proven composite capture, and the
private loader decodes that HVF payload and commits an initially paused restored
session; no public API path calls either transaction, and public create/load
emit no artifacts.
...
metrics use the Firecracker-shaped top-level `entropy` object for implemented
virtio-rng request, byte, host-randomness failure, event-failure, throttling,
and limiter-event activity. RTC device runtime metrics use the
Firecracker-shaped top-level `rtc` object for implemented PL031 invalid
read/write and error counters. Current virtio-net runtime
metrics are limited to aggregate `net` and per-interface `net_{iface_id}`
counters for implemented RX/TX queue events, packet counts, byte counts,
malformed TX frames, dispatch failures, packet I/O provider failures, handler
lookup failures, and signal failures; rate-limiter, vmnet/TAP-specific,
activation, config-space, spoofed-MAC, and hotplug counters remain deferred
until those producers exist.
Process-local MMDS packet handling reports a top-level `mmds` object when
guest MMDS data-plane metrics are non-empty. The implemented producers cover
accepted guest MMDS ARP/TCP packets, accepted errors, unusual unsupported
empty-control packets, V2 missing or invalid guest GET tokens, TCP receive
count, response TX count/frame/byte/error activity, and connection
create/destroy events. Full Firecracker MMDS TCP stack parity and malformed
Ethernet producer coverage remain deferred.
Parsed deprecated HTTP API usage is counted under
...
Successful runtime `PATCH /vm` pause and resume requests also record the most
recent API-layer action duration as `latencies_us.pause_vm` and
...
logger API request, action, and boot-timer write failures increment `logger.missed_log_count`; broader logger
...
`bangbang_runtime::boot_timer`. It is not wired into the full process logging
backend yet.
...
5, `VIRTIO_F_VERSION_1`, optional deflate/statistics/hinting feature bits,
compacted inflate/deflate/statistics/hinting queue metadata with 256
descriptors per queue, and Firecracker's 12-byte config space:
`num_pages`, `actual_pages`, and `free_page_hint_cmd_id`.
Reporting feature-bit and queue metadata code remains internal future-work
scaffolding and is not reachable through validated API or config-file
configuration.
...
running. Reporting and host memory reclaim remain deferred.
...
systemd startup.
Block hotplug, remaining cache-mode semantics, full Firecracker timerfd/eventfd
limiter parity, and remaining non-block/non-entropy limiter wakeups remain
deferred. It does not provide a public
runner control, support vhost-user-block sockets, or use an async I/O engine.
...
| `GET /hotplug/memory`, `PUT /hotplug/memory`, `PATCH /hotplug/memory` | `PUT` implemented for config storage; `GET` returns status after config; startup-visible virtio-mem shell, request-queue completion, plugged-block tracking, failure-aware HVF dynamic memory mutations, active status, and runtime requested-size update modeled; `PATCH` unsupported-state before start | `GET` returns pre-start stored status or active runtime status if configured; `PUT` unsupported after start; `PATCH` validates and commits requested-size updates after start | `PUT` stores a validated Firecracker-shaped pre-boot config and replaces the prior config without opening host resources. `GET` returns `total_size_mib`, `block_size_mib`, `slot_size_mib`, `plugged_size_mib: 0`, and the current `requested_size_mib` before startup; after startup it reports the active virtio-mem handler's current `plugged_size_mib` with the stored requested size. Without a stored config it returns the memory-hotplug-specific unsupported fault. `InstanceStart` can attach a guest-visible virtio-mem MMIO/FDT shell backed by the runtime handler foundation; the config space exposes the configured block and total region sizes with zero usable, plugged, and requested bytes. Runtime `PATCH` updates the active requested-size config field, grows usable config-space size to a slot boundary when needed, increments config generation, and raises a config interrupt before committing stored status. The runtime handler can parse and complete virtio-mem request descriptors, publish used-ring entries, raise queue interrupts through HVF, track plugged block ranges, return plugged/unplugged/mixed `STATE` responses, apply accepted `PLUG`/`UNPLUG`/`UNPLUG_ALL` requests through the active HVF dynamic guest-memory executor before ACK publication, roll back applied backend mutations on late completion failures when possible, and update config-space `plugged_size` only after mutation execution and guest-visible completion publication succeed. A signed executable e2e scenario covers a direct-rootfs guest binding `virtio_mem` and observing a runtime requested-size update. Broader public guest-memory accounting and public hot-unplug semantics remain deferred. |
...
| `PUT /logger` | implemented; `204` empty response on successful pre-boot configuration | unsupported after start; `400` `fault_message` | Logger output is process observability state, not guest configuration. Repeated pre-boot requests update provided fields; malformed parser failures increment the selected logger request count/fail counters without mutating logger state; minimal API request method/path logging omits bodies and supports configured level, origin, and module-prefix fields alongside minimal successful action logging, but full log routing remains deferred. |
| `PUT /serial` | implemented; `204` empty response on successful pre-boot output configuration, rate-limiter configuration, or clear request | unsupported after start; `400` `fault_message` | Serial output is process observability state, not guest configuration. Valid `serial_out_path` values and token-bucket `rate_limiter` values are stored without opening host resources during the request; startup opens the path, wraps the configured or default output in the limiter when enabled, and routes guest TX serial bytes to it. Malformed parser failures increment the selected serial request count/fail counters without mutating previous serial output configuration; empty/control-character paths are rejected without mutating it. |
...
| `PUT /actions` with `FlushMetrics` | VMM-routed; `400` unsupported-state `fault_message` | implemented; `204` empty response or `400` logger/metrics output fault | Firecracker treats this as runtime-only. bangbang writes one minimal JSON line when metrics output was configured, including selected API request counters, `logger.missed_metrics_count` after a previous metrics write failure, `logger.missed_log_count` after a previous logger API request, action, or boot-timer write failure, `boot_run_loop_status` as `running`, `paused`, `exited`, or `failed` when a process-owned boot worker exists, optional aggregate and per-drive block device metrics including implemented backing update counters, optional aggregate and per-interface network metrics for implemented RX/TX packet movement and failures, optional top-level MMDS metrics for implemented guest packet detour and response queue activity, optional entropy device metrics for implemented virtio-rng request/byte/failure activity, optional RTC device metrics for implemented PL031 MMIO errors, and optional balloon device metrics, writes one minimal API request method/path line before dispatch plus one minimal action line when logger output is configured and allowed by level/module filters, and otherwise succeeds without writing. Periodic metrics flushes reuse the same metrics payload every 60 seconds while running, but are not `/actions` requests and do not write logger action lines. |
...
- full Firecracker balloon device counters, free-page reporting, and host reclaim
- pmem root-device semantics, real runtime rate-limiter mutation, dirty-range
