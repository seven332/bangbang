...
| `PUT` | `/mmds/config` | parser/action boundary implemented; application deferred | Parses Firecracker-shaped MMDS config with required non-empty `network_interfaces`, optional `version`, optional link-local `ipv4_address`, and optional `imds_compat`. The runtime action currently returns `400` `fault_message`, with post-start requests using the normal unsupported-state policy; validating referenced interface existence, storing config, and exposing MMDS to the guest remain tied to #16. |
