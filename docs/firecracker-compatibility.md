...
`InstanceStart` preflight, transactional startup executor, and successful-start state transition helpers, an internal MMDS guest TCP packet classifier, process-local packet-payload HTTP exchange, and process vmnet TX detour, backend-neutral guest
...
and retains generated HTTP response bytes in a bounded process-local queue for
future RX delivery. TCP stream tracking, split request buffering, response
packet synthesis, and virtio-net RX delivery remain deferred.
...
- snapshots
- full MMDS TCP routing, response packet synthesis, and guest-visible RX delivery
