...
four reviewed arm64 identification registers are applied to the complete owner
topology before boot overrides, with requested-set read-before-write preflight
and immediate readback. Other register classes remain #1394 work. Firecracker
...
Complete requested-set baseline collection across all vCPUs precedes mutation;
each common target is written and immediately reread on its owner thread before
boot resource assembly. A failure destroys the unpublished topology and VM.
...
evidence, and #1394/Wave handoffs are in the checked
...
selection, and custom templates containing only the four reviewed 64-bit arm
ID-register modifiers become the effective startup selection. KVM capability,
KVM vCPU-init feature, mixed-category, unsupported-width, and outside-profile
requests fail with the same value-redacted platform faults as the socket path.
...
| `PUT` | `/cpu-config` | supported four-register custom subset; KVM-only categories platform-excluded | Parses bounded ordered Firecracker aarch64 custom templates with a 256-entry limit per array, exact 32/64/128-bit ARM identities/bitmaps, fixed seven-word vCPU-feature indexes, stronger duplicate-identity checks, and value-redacted diagnostics. A successful custom PUT replaces static/custom state; empty input clears it. Exact `(baseline & !filter) | value` execution is implemented only for 64-bit `ID_AA64PFR0_EL1`, `ID_AA64ISAR0_EL1`, `ID_AA64ISAR1_EL1`, and `ID_AA64MMFR2_EL1`. Every requested baseline is read on every owner before any write, targets are common, and each write is immediately reread; any failure destroys the unpublished VM. KVM capability numbers, KVM vCPU-init feature words, mixed input, other registers, and other widths receive distinct fixed faults. Custom contents are omitted from GET and excluded from native-v1 snapshots. See the checked CPU-template contract. |
