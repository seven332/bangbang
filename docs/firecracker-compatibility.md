...
| `PATCH /network-interfaces/{iface_id}` | `rx_rate_limiter`, `tx_rate_limiter` | optional when absent, `null`, empty, or all-null; rejected when configured | Omitted, `null`, empty, or all-null rate limiters are accepted as runtime no-ops for existing interfaces after startup. Configured Firecracker-shaped rate limiter objects are parsed and then rejected before any runtime mutation. Updating the initial live limiter state belongs to the runtime network update work. |
...
forwarded frames and failures remain charged. Runtime PATCH mutation, exposed
retry timing, HVF retry wakeups, limiter-specific metrics, snapshot state, and
