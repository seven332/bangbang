...
| `PUT` | `/cpu-config` | recognized; rejected | Returns a CPU-config-specific unsupported fault. Real custom CPU template parsing, storage, and HVF CPU feature wiring need a dedicated design covering VM creation, vCPU setup, and boot compatibility. |
...
| `PUT /drives/{drive_id}` | `rate_limiter` | optional when absent or `null`; deferred when configured | The internal model rejects configured rate limiters; non-null rate limiting needs future block I/O performance work. |
