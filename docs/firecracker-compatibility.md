...
non-zero regular host file and retains the handles, but it does not mmap or
attach virtio-pmem devices yet. Known unsupported sections such as
...
| `PUT`, `PATCH` | `/pmem/{id}` | partial; pre-boot config storage and startup backing validation implemented | `PUT /pmem/{id}` stores Firecracker-shaped pmem configuration before boot, replaces prior config for the same ID, and appears in `GET /vm/config` under `pmem`. It rejects empty backing paths without opening host files during the request. `InstanceStart` opens each configured backing, validates it as a non-zero regular file, and retains the handle without attaching a guest-visible virtio-pmem device yet. Configured rate limiters are rejected without mutating stored pmem configuration. `PATCH /pmem/{id}` remains post-boot-only unsupported, and real runtime updates, rate limiting, root-device semantics, host-file mapping, guest attachment, and hot-unplug need dedicated device designs. |
...
| `PUT /pmem/{id}` | implemented; `204` empty response on successful config storage | unsupported after start; `400` `fault_message` | Records Firecracker-shaped pre-boot config and replaces prior config for the same ID without opening the host backing file during the request. Stored configs appear in `GET /vm/config`; startup opens and validates each backing as a non-zero regular file and retains the handles. Root-device semantics, backing-file mapping, guest-visible virtio-pmem attachment, rate limiting, runtime updates, and hot-unplug remain deferred. |
...
- pmem mmap/FDT attachment, root-device semantics, runtime updates, and rate
  limiting beyond startup backing-file validation
