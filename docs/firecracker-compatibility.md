...
bounded no-op scope, and ordinary admission is restored before the same fault is
returned. This is only the supervisor command boundary, not full snapshot-ready
quiescence. `PUT /snapshot/load` reaches the snapshot unsupported fault before
startup and fails as an unsupported state after startup. Malformed or
schema-invalid snapshot bodies are still rejected by the parser first.
