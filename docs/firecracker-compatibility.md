...
`InstanceStart` preflight, transactional startup executor, and successful-start state transition helpers, an internal MMDS guest TCP packet classifier, backend-neutral guest
...
metadata. TCP session tracking, HTTP extraction from TCP payloads, and RX
response delivery remain deferred.
