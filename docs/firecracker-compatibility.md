...
`logger.metrics_fails` and `logger.missed_metrics_count` counters, and logger action write failures
...
`logger.metrics_fails` and `logger.missed_metrics_count`; logger action write failures increment
...
- full logger integration, full Firecracker metrics counters beyond the minimal
  `logger.metrics_fails`, `logger.missed_metrics_count`, and `logger.missed_log_count`
- memory hotplug
