...
accounting. Free-page hinting command descriptors and active-run range
descriptors are validated and recorded by runtime queue dispatch, but host
memory reclaim, guest-reported optional statistics, statistics polling, and
reporting remain deferred. The `pmem` section is
parsed through
`PUT /pmem/{id}`;
...
accounting.
Guest-reported optional statistics, statistics polling, reporting, and host
memory reclaim remain deferred.
