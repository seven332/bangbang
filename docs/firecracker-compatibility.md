...
implemented block limiter throttling;
...
implemented virtio-balloon queue activity and failures. Block limiter timer
wakeup parity, hotplug/removal counters, vhost-user-block metrics, entropy
...
| `PATCH /drives/{drive_id}` | `rate_limiter` | optional when absent, `null`, empty, or all-null; rejected when configured | The parser accepts the Firecracker-shaped field name and validates configured rate-limiter shape; empty or all-null `bandwidth`/`ops` bucket objects are treated as unconfigured. The runtime update model rejects configured rate-limiter mutations with `drive rate_limiter is not supported` before opening a replacement backing or mutating drive state. Runtime limiter mutation belongs with future block update work. |
...
systemd startup.
Block hotplug, remaining cache-mode semantics, runtime limiter mutation, and
active limiter wakeup parity remain deferred. It does not provide a public
...
| `PUT /drives/{drive_id}` | implemented; `204` empty response on successful config storage | unsupported after start; `400` `fault_message` | Records validated pre-boot config, including optional bandwidth/ops rate limiters. Startup preparation opens backing files, registers initial block MMIO devices, and applies configured limiters in queue dispatch without sleeping. Runtime hotplug and active limiter wakeup parity remain deferred. |
...
- full Firecracker active timerfd/eventfd rate-limiter wakeup parity for block
  and entropy
