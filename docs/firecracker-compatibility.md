...
with zero plugged, requested, and usable bytes until resize semantics are
implemented.
...
| `GET`, `PUT`, `PATCH` | `/hotplug/memory` | partial; pre-boot config storage, initial status, startup-visible virtio-mem shell, and request-queue completion implemented | Parses Firecracker-shaped `PUT` and `PATCH` memory hotplug request bodies, rejects malformed or schema-invalid bodies first, and then routes valid requests through the VMM state/action policy. `PUT` is pre-boot-only, applies Firecracker-shaped block/slot/total size semantic validation, stores valid configs, replaces prior configs, and exposes the stored config through `GET /vm/config`. `GET /hotplug/memory` returns the stored config plus initial `plugged_size_mib: 0` and `requested_size_mib: 0`; without a stored config it returns the memory-hotplug-specific unsupported fault. `PATCH` remains post-boot-only and reaches the memory-hotplug-specific unsupported fault after startup. Startup can attach a guest-visible virtio-mem MMIO/FDT shell whose config space exposes the configured block size, a deterministic hotplug aperture, the configured total region size, and zero usable, plugged, and requested bytes. The backend-neutral queue path parses request and response descriptors, completes valid-usable `STATE` requests as unplugged, completes zero-usable startup requests plus mutation and unsupported request types with explicit error responses, publishes used-ring entries, and wires queue-interrupt intent through boot runtime/HVF. Hotpluggable guest memory accounting, runtime resize, plugged-block state tracking, guest memory-map changes, and memory hot-unplug need dedicated designs. |
...
| `GET /hotplug/memory`, `PUT /hotplug/memory`, `PATCH /hotplug/memory` | `PUT` implemented for config storage; `GET` returns initial status after config; startup-visible virtio-mem shell and request-queue completion modeled; `PATCH` unsupported-state before start | `GET` returns the stored initial status if configured; `PUT` unsupported after start; `PATCH` memory-hotplug unsupported after start | `PUT` stores a validated Firecracker-shaped pre-boot config and replaces the prior config without opening host resources. `GET` returns `total_size_mib`, `block_size_mib`, `slot_size_mib`, `plugged_size_mib: 0`, and `requested_size_mib: 0` when configured; without a stored config it returns the memory-hotplug-specific unsupported fault. `InstanceStart` can attach a guest-visible virtio-mem MMIO/FDT shell backed by the runtime handler foundation; the config space exposes the configured block and total region sizes with zero usable, plugged, and requested bytes. The runtime handler can parse and complete virtio-mem request descriptors, publish used-ring entries, and raise queue interrupts through HVF. Runtime resize, plugged-size tracking, hot-unplug, guest memory accounting, and guest memory-map changes remain deferred. |
