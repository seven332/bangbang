...
and failures. HVF block limiter retry wakeups are wired for active block queues.
Hotplug/removal counters, vhost-user-block metrics, entropy rate-limiter metrics
beyond the implemented subset, and full Firecracker block metric coverage remain
...
- full Firecracker active timerfd/eventfd rate-limiter wakeup parity beyond the
  current HVF block retry scheduler and entropy retry-delay reporting, including
  HVF entropy wakeups and shared event-source behavior
