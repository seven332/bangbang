...
for active queues and share acknowledged native-v1 quiescence. Direct
vhost-user block uses the same per-drive block metric identity: guest kicks and
...
| `PUT` | `/drives/{drive_id}` | supported target; pre-boot plus PCI-only file and eligible direct-vhost runtime attach implemented | Before boot, stores initial virtio-block configuration including optional file-backed bandwidth/ops rate limiters or the strict direct socket matrix. After startup with public `--enable-pci`, Running or Paused requests may transactionally attach one new non-root file endpoint or a direct vhost endpoint when the immutable live guest-memory profile is already shared. File paths open before owner submission; contained file requests may consume only an exact still-unused initial-manifest grant. Direct vhost requests run side-effect-free owner profile/capacity preflight before one bounded socket discovery, materialize against exact live shared regions on the owner, and publish `/vm/config` only after every endpoint lease succeeds. Default MMIO, root, duplicate, anonymous RAM, invalid backing/grant/negotiation, unavailable session, and exhausted capacity are nonmutating; duplicate and owner-preflight failures do not contact the candidate socket. Same-ID runtime PUT remains duplicate rejection. |
...
| `PUT`, `PATCH` | `/pmem/{id}` | implemented direct MMIO-or-PCI startup, pmem root, and non-root PCI runtime subset; snapshots excluded | `PUT /pmem/{id}` stores strict Firecracker-shaped pre-boot configuration and exposes accepted state through `GET /vm/config`. One root is permitted across ordinary block and pmem; pmem order selects `/dev/pmem<i>` and `ro`/`rw`, while conflicts fail before grant or configuration mutation. Direct paths remain unopened until startup; contained grant tags claim and retain an exact-ID, exact-access regular-file descriptor during successful pre-boot PUT, then move it into startup without reopening the tag. `InstanceStart` validates each nonzero regular backing, assigns a deterministic aligned guest range, and registers a cloned lease on the exact file/private-tail mapping before attaching the selected transport. Running/Paused public PCI PUT remains non-root-only and commits configuration and grant consumption only after direct mapping and endpoint publication succeed. Flush selects only the notified device and synchronizes exactly the file prefix with `MS_SYNC`; empty or malformed-only events do not sync, peer devices are not traversed, and the aligned tail remains volatile. Optional bandwidth/ops buckets and post-boot PATCH retain their atomic limiting/retry behavior. Signed coverage proves pre-flush backing coherence, read-only enforcement, MMIO read-only and PCI writable root boot, contained descriptor identity after pathname replacement, runtime attach/delete, and exact same-ID/PCI-slot/guest-range reuse. Dirty tracking, optional-device snapshot state, and broad pmem certification remain excluded. |
...
| `PUT /drives/{drive_id}` | `rate_limiter` | optional bandwidth/ops token buckets for file-backed drives; omitted for vhost-user | File-backed missing/null/empty/all-null values are unconfigured; valid buckets are stored, reported, and applied without sleeping. A socket-backed request must omit this field because direct vhost limiting is not implemented. |
...
| `PUT /drives/{drive_id}` | `socket` | optional; direct startup and eligible PCI runtime vhost-user block implemented | A nonempty socket selects the vhost backend only when `path_on_host`, explicit `is_read_only`, `io_engine`, and `rate_limiter` are absent; drive ID, root selection, `partuuid`, and cache mode remain valid. Successful pre-boot requests store and exactly report the socket without inventing a path. `InstanceStart` connects with a bounded redacted deadline, performs strict feature/protocol/CONFIG discovery before VM construction, selects shared RAM, and activates one queue over MMIO or PCI. In Running or Paused all-PCI state, a new non-root ID may connect only after the owner proves the live profile is already shared and all deterministic publication capacity is available; publication commits last and caller-coordinated DELETE releases the complete endpoint. ID-only PATCH refreshes active config; same-ID PUT remains duplicate rejection. Failure drops candidates and preserves public/live configuration. Contained mode rejects ambient socket access pending authorized connected-stream delivery, and dynamic memory plus native-v1 remain incompatible. |
...
through the selected MMIO or PCI startup handle. A direct vhost drive admits an
...
When any direct startup drive uses vhost-user, startup selects shared guest RAM
...
native-v1 capture rejects vhost before artifact staging. Runtime direct
insertion reuses this constructor only after an already-shared live owner
...
Contained socket-backed requests fail before grant claim or ambient connection;
an authorized connected-stream role is not yet part of the launcher protocol.
...
The same concrete device supports file-backed PCI hotplug and direct pre-boot
vhost-user execution. Signed vhost cases boot an MMIO read-only root and a PCI
...
same-ID/slot reuse. Linux io_uring `Async`, contained socket authority, live
same-ID reconnect, and automatic PCI notification remain unavailable. Native-v1
...
| `PUT /drives/{drive_id}` | implemented; `204` empty response for valid file-backed or direct socket-backed pre-boot config | PCI-only non-root file-backed and eligible direct-vhost runtime attach implemented; `204` on success | File-backed requests retain optional limiters, deferred direct opening, exact contained grants, selected-transport startup, and transactional PCI runtime attachment. A direct socket-backed request uses the strict vhost field matrix; startup performs bounded discovery before VM construction, selects shared RAM, exposes exact backend-derived config/features, and activates one queue over MMIO or PCI. In Running or Paused all-PCI state, a new non-root vhost ID may attach only when the live memory profile is already shared: owner-side profile/capacity preflight runs before socket access, API-thread discovery stays bounded, owner-thread materialization uses exact live regions, and public config commits after endpoint publication. Default MMIO, root, duplicate, anonymous RAM, contained ambient authority, dynamic-memory coexistence, invalid backing/grant/socket/negotiation, unavailable session, and capacity exhaustion preserve state; duplicate/preflight failures accept zero candidate connections. Same-ID live PUT remains duplicate rejection. Manual guest PCI rescan is required. |
| `PUT /pmem/{id}` | implemented; `204` empty response on successful config storage | implemented in Running/Paused public PCI; `204` after live commit | Pre-boot requests record Firecracker-shaped config and replace prior config for the same ID failure-atomically; empty or all-null limiter objects are unconfigured and valid bandwidth/ops buckets are stored and reported. One pmem or ordinary block root is accepted; pmem order supplies `/dev/pmem<i>` and `ro`/`rw`, and cross-family or second-root failure precedes grant/config mutation. Direct pre-boot paths remain unopened until startup; contained grant tags claim and retain exact-ID, exact-access nonzero regular-file descriptors. Startup moves provided descriptors or opens missing direct paths, maps each file/private-tail range once, registers a cloned lease directly with HVF, attaches the selected transport, and retains every owner. After startup, public PCI accepts only a new non-root ID: direct mode opens before owner submission, contained mode reserves an exact unused grant with rollback, and the owner maps the file, allocates a non-overlapping reusable guest range, registers the exact direct mapping, and publishes the endpoint before configuration and grant-consumption commit. Exact-prefix `MS_SYNC`, read-only guest protection, recoverable rollback, and failed-unmap lease retention preserve the prior projection and authority; incomplete cleanup is terminal. Default MMIO rejects runtime insertion before path use. Dirty-range tracking and optional-device snapshot state remain deferred. |
...
- pmem dirty-range tracking, broad sharing/performance certification, and
  optional-device snapshots beyond direct file-backed HVF mapping, deterministic
