...
| `GET` | `/balloon` | recognized; VMM-routed unsupported | Routes through the VMM state/action policy and currently returns the balloon-specific unsupported fault before or after startup. Real virtio-balloon device support, memory accounting, and runtime update behavior need a dedicated design. |
| `PUT`, `PATCH` | `/balloon` | recognized; VMM-routed unsupported | Parses Firecracker-shaped balloon config and target-size update request bodies, rejects malformed or invalid bodies first, and then routes valid requests through the VMM state/action policy. `PUT` is treated as pre-boot-only and reaches the balloon-specific unsupported fault before startup. `PATCH` is treated as post-boot-only based on Firecracker's current VMM source classification and reaches the balloon-specific unsupported fault after startup. Requests made in the wrong lifecycle state return the normal unsupported-state fault. Real virtio-balloon device support, memory accounting, and runtime update behavior need a dedicated design. |
| `GET` | `/balloon/statistics` | recognized; VMM-routed unsupported | Routes through the VMM state/action policy. Statistics queries are treated as post-boot-only and currently return the balloon-specific unsupported fault after startup. Real statistics polling and guest-reported memory accounting need a dedicated design. |
| `PATCH` | `/balloon/statistics` | recognized; VMM-routed unsupported | Parses Firecracker-shaped statistics interval update request bodies, rejects malformed or invalid bodies first, and then routes valid requests through the VMM state/action policy. Statistics updates are treated as post-boot-only based on Firecracker's current VMM source classification and currently return the balloon-specific unsupported fault after startup. Real statistics polling and guest-reported memory accounting need a dedicated design. |
...
| `PUT /balloon` | `amount_mib` | required; unsupported after parser validation | Parsed as an unsigned 32-bit Firecracker-shaped target balloon size before VMM dispatch. The value is not retained or echoed by the current unsupported action path. Real memory accounting and guest-visible target changes remain deferred. |
| `PUT /balloon` | `deflate_on_oom` | required; unsupported after parser validation | Parsed as a boolean before VMM dispatch. The value is not retained or echoed by the current unsupported action path. Real guest OOM deflation behavior remains deferred. |
| `PUT /balloon` | `stats_polling_interval_s` | optional | Missing values follow Firecracker's parser default shape before VMM dispatch. Real statistics polling remains deferred. |
| `PUT /balloon` | `free_page_hinting` | optional | Missing values follow Firecracker's parser default shape before VMM dispatch. Real free-page hinting remains deferred. |
| `PUT /balloon` | `free_page_reporting` | optional | Missing values follow Firecracker's parser default shape before VMM dispatch. Real free-page reporting remains deferred. |
| `PUT /balloon` | unknown fields | rejected | Matches Firecracker's strict request model behavior. |
| `PATCH /balloon` | `amount_mib` | required; unsupported after parser validation | Parsed as an unsigned 32-bit Firecracker-shaped target balloon size before VMM dispatch. The value is not retained or echoed by the current unsupported action path. Real runtime target-size updates remain deferred. |
...
| `PATCH /balloon/hinting/start` | body | optional when absent or empty | Missing or empty bodies use Firecracker's default hinting start command before VMM dispatch. An empty JSON array is also accepted as a default command, matching the current Firecracker Serde parser behavior. |
