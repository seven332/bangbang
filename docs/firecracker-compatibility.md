...
`logger.missed_metrics_count` counter, and logger action or boot-timer write
failures increment `logger.missed_log_count`, for later successful metrics output. Public drive configuration is
...
logger action and boot-timer write failures increment `logger.missed_log_count`; broader logger
