...
response bytes, retains those frames in bounded per-interface queues, and
exposes queued frames through the matching virtio-net RX source before vmnet
reads. TCP stream tracking, split request buffering, retransmission policy, and
stricter per-interface TCP session ownership remain deferred.
