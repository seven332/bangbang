...
read/write/flush counts and bytes, and observable request/event failures;
`balloon` reports implemented virtio-balloon queue activity and failures. Block
rate-limiter/update metrics and latency aggregates remain deferred. The startup timing fields from Firecracker's
