...
allocated balloon interrupt line when queue completion requires it. Balloon
...
acknowledges descriptor heads with zero-length used-ring entries. Boot runtime
resources and the HVF boot loop can drain pending balloon inflate/deflate
notifications and signal the allocated balloon interrupt line when the runtime
dispatch summary reports queue-interrupt intent. Host memory accounting,
