...
implemented virtio-balloon queue activity and failures. HVF timer wakeup wiring
for block limiter retries, hotplug/removal counters, vhost-user-block metrics,
entropy rate-limiter metrics beyond the implemented subset, and full
Firecracker block metric coverage remain deferred. The startup timing fields
from Firecracker's
...
systemd startup.
Block hotplug, remaining cache-mode semantics, and HVF wakeup scheduling for
limiter retries remain deferred. It does not provide a public
...
| `PUT /drives/{drive_id}` | implemented; `204` empty response on successful config storage | unsupported after start; `400` `fault_message` | Records validated pre-boot config, including optional bandwidth/ops rate limiters. Startup preparation opens backing files, registers initial block MMIO devices, and applies configured limiters in queue dispatch without sleeping. Throttled block dispatches expose a backend-neutral retry delay, but runtime hotplug and HVF wakeup scheduling remain deferred. |
...
- full Firecracker active timerfd/eventfd rate-limiter wakeup scheduling for
  block and entropy
