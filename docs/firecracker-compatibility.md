...
value rather than a successful response. Process-local guest response-byte
serialization currently emits fixed HTTP/1.1 response bytes; preserving a
guest request's original HTTP version remains deferred with guest HTTP request
parsing.
...
- snapshots
- MMDS guest packet access, token handling, guest HTTP request parsing, TCP routing, and response delivery
