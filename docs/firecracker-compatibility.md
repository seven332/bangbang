...
memory reclaim, periodic statistics polling, runtime statistics interval
updates, and reporting remain deferred. The `pmem` section is
parsed through
`PUT /pmem/{id}`;
...
Periodic statistics polling, runtime statistics interval updates, reporting,
and host memory reclaim remain deferred.
