...
| `--snapshot-version` | implemented native-envelope inspection | Prints the current public writer version `v2.3.0` and exits successfully before fd-table setup, API socket publication, signal setup, or HVF startup. This names bangbang's native data format and does not claim Firecracker v10 artifact compatibility. |
...
| `PUT` | `/snapshot/create`, `/snapshot/load` | supported narrow native-v2 Full/File lifecycle plus frozen native-v1 File/Uffd reader | Parses Firecracker-shaped bodies, rejects malformed input first, normalizes deprecated load fields for native-v1 compatibility, and keeps paths/overrides redacted through typed API/runtime values. Create is paused-only and admits `Full` for a 1–32-vCPU source with configured boot metadata, no drives or optional devices/MMDS/PCI/boot timer, default serial configuration, and a live UART equal to the canonical empty reset-compatible model. Direct and exact contained outputs share the anchored no-clobber memory-first/state-last transaction, cancellation seal, cleanup, durability, metrics, and source-Paused behavior; `Diff` rejects before namespace or capture work. Load is pre-boot-only and requires a pristine process except logger/metrics. It opens and decodes state once, routes pinned Firecracker or unknown bytes to explicit incompatibility, routes frozen native-v1 to the unchanged eager File or macOS pager path with its root/grant/deprecation semantics, and routes native-v2 only to retained read-only File/COW memory. Direct and contained v2 loads validate the complete state/memory/FDT/UART/HVF/topology/time/identity graph before construction, keep guest writes private, and can repeatedly restore isolated destinations without mutating the pair. Both families commit a real session `Paused` before optional ordinary resume; requested-resume failure is terminal. Retryable preparation failures preserve pristine eligibility, while committed identity/time, publication, incomplete cleanup, or resume failure latches the process terminal. Signed direct two-vCPU and production App Sandbox cases prove Paused then explicit/automatic resume, all-vCPU continuation, private COW writes, repeated loads, retained descriptor identity, cleanup, and a real closed-stdout terminal boundary. No Firecracker state-file interoperability, Linux UFFD wire compatibility, native-v2 `Uffd`, `Diff`, realtime adjustment, overrides, optional-device/general-serial profile, editing, or broad portability is claimed. |
...
frozen native-v1 additionally adopts its persisted-root input. Direct load
keeps no-follow pathname adapters. Malformed bodies are rejected by the parser
first, while execution faults remain typed and path/reference-redacted.
...
supports a minimal native-v2 2.3 Full/File profile with 1–32 vCPUs, no
drives/optional devices, and a canonical default UART, plus a frozen
one-vCPU/read-only-root native-v1 File/Uffd reader. Both include public Paused
...
- snapshot behavior beyond the implemented narrow native-v2 Full/File profile.
  The native-v2 reader retains readable `2.0.0`/`2.1.x`/`2.2.x` state. The
  current public `2.3.0` writer carries an exact typed 1–32-vCPU
  ...
  reviewed debug/SME state. The focused boundary captures a
  ...
  fallback. The CLI reports `v2.3.0` and describes exact v1/v2 versions. There
  is still no general device aggregate. Remaining work includes
