...
| `GET /mmds` | implemented; `200` JSON | implemented; `200` JSON when initialized, `400` `fault_message` when uninitialized | Returns the current process-local MMDS JSON object. Before startup, an uninitialized store returns JSON `null` to match Firecracker's preboot controller. After startup, an uninitialized store returns the Firecracker-shaped MMDS not-initialized fault. Initialized data is also used by the implemented guest-visible MMDS path when MMDS config selects startup network interfaces; guest-visible queries still fail if the store is uninitialized. Packet handling remains limited to the documented internal vmnet detour boundary. |
