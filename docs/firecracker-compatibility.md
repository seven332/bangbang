...
deterministic SYN-ACK frames, identify pure empty-payload ACK-only and FIN close
packets, synthesize ACK plus FIN-ACK frames for empty FIN close packets, and for
non-empty candidate TCP payloads, it can
...
close packets, and non-empty MMDS candidate TX payloads only on configured MMDS
interfaces, keeps shared MMDS state visible to both API and packet paths,
synthesizes deterministic Ethernet/ARP replies, Ethernet/IPv4/TCP SYN-ACK
frames, minimal Ethernet/IPv4/TCP FIN close frames, and Ethernet/IPv4/TCP
response frames carrying the generated HTTP response bytes, retains those frames in
...
out-of-order reassembly, retransmission policy, RST policy, session timeout
policy, and stricter
