...
`VSOCK_OP_RST` headers; connect, retention, RW forwarding, host-stream EOF, or
host-stream read failures deliver guest-visible `VSOCK_OP_RST` headers.
Guest-visible socket lifecycle beyond connection setup and forceful guest
reset/full-shutdown cleanup, host-to-guest data movement beyond one bounded
pending packet per established stream, full graceful half-close state tracking,
buffered RW retry, full credit accounting, CID routing beyond current
host/guest checks, and full event payload dispatch remain deferred. Event
queue notifications are accepted as no-op dispatch metadata.
...
- virtio-vsock socket lifecycle beyond connection setup and forceful guest
  reset/full-shutdown cleanup, host-to-guest data movement beyond one bounded
  pending packet per established stream, full graceful half-close state
  tracking, buffered RW retry, full credit accounting, CID routing beyond
  current host/guest checks, and full event payload dispatch.
