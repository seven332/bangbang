...
can accept one pending host connection per call into an owned nonblocking
stream, and can read and parse bounded accepted-stream `CONNECT` handshakes.
RX queue delivery of modeled host request packets, guest-visible socket
...
then read and parse bounded accepted-stream `CONNECT` handshakes. RX queue
delivery of modeled host request packets, guest-visible socket lifecycle,
