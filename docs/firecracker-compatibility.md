...
payload bytes to the retained host stream; connect, retention, or RW forwarding
failures deliver guest-visible `VSOCK_OP_RST` headers. Guest-visible socket
lifecycle beyond connection setup, host-to-guest data movement, buffered RW
retry, full credit accounting, CID routing beyond current host/guest checks,
and event queue dispatch remain deferred.
...
- virtio-vsock socket lifecycle beyond connection setup, host-to-guest data
  movement, buffered RW retry, full credit accounting, CID routing beyond
  current host/guest checks, and event queue dispatch
