...
unsupported fault only while the instance is paused. `PUT /snapshot/load`
reaches the snapshot unsupported fault before startup and fails as an
unsupported state after startup. Malformed or schema-invalid snapshot bodies are
still rejected by the parser first.
