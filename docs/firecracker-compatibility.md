...
| `GET`, `PUT`, `PATCH` | `/mmds` | supported target; control-plane storage, runtime guest-query formatting, and internal guest GET response modeling implemented | Stores bounded in-memory JSON object contents in the process runtime, returns stored JSON for control-plane `GET`, applies RFC 7396 merge-patch semantics for `PATCH`, rejects uninitialized `GET`/`PATCH`, and keeps previous data on oversized update failure. The runtime can also resolve initialized metadata by JSON-pointer path, format JSON or Firecracker-shaped IMDS text, and map internal guest GET requests to process-local status/content-type/body response values as prerequisites for later guest-visible delivery. Guest packet handling, token handling, raw guest HTTP parsing/serialization, and response delivery remain tied to later #16 work. |
...
value rather than a successful response.
...
- snapshots
- MMDS guest packet access, token handling, raw guest HTTP parsing/serialization, and response delivery
