...
`VSOCK_OP_RW` payload at a time into guest RX buffers; connect, retention, RW
forwarding, host-stream EOF, or host-stream read failures deliver guest-visible
`VSOCK_OP_RST` headers. Guest-visible socket lifecycle beyond connection
setup, host-to-guest data movement beyond one bounded pending packet per
established stream, buffered RW retry, full credit accounting, CID routing
beyond current host/guest checks, and full event payload dispatch remain
deferred. Event queue notifications are accepted as no-op dispatch metadata.
...
- virtio-vsock socket lifecycle beyond connection setup, host-to-guest data
  movement beyond one bounded pending packet per established stream, buffered
  RW retry, full credit accounting, CID routing beyond current host/guest
  checks, and full event payload dispatch. Implemented vsock scope is limited
  ...
  local port allocation, retained host connection table modeling with pending host request packet headers, guest `RESPONSE` acknowledgement to retained host streams, bounded guest-visible `RST` queueing for unsupported or orphan host-destined guest packets, guest `REQUEST` connection to Firecracker-shaped `${uds_path}_${PORT}` sockets with guest-visible `RESPONSE` or `RST` header delivery, bounded guest `RW` payload forwarding from established guest-initiated connections to retained host streams, bounded host `RW` payload delivery from established host-initiated or guest-initiated streams into guest RX buffers, and the internal prepared resource/MMIO
