...
lifecycle, start owner, and packet descriptor boundaries with vmnet mode,
status, operation error, XPC descriptor configuration, backend start/stop
ownership, packet `iovec` layout, and owned cleanup models for a later host
backend. These boundaries are not connected to the default process provider.
...
include a concrete system vmnet backend, call `vmnet_start_interface`,
`vmnet_stop_interface`, `vmnet_read`, or `vmnet_write`, choose a live host packet
