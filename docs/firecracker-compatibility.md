...
supplies balloon encoding, exact 2.10 supplies virtio-mem encoding, and current
...
after matching direct and contained signed certification. The remaining-device
aggregate selector is 84 terminal and one `audit-required`; the network/MMDS
selector is 33 terminal and two `audit-required`, and the current global counts
are 246/152/3/17.
...
| `--enable-pci` | selects all-virtio PCI startup on supported macOS arm64/HVF hosts | Exact flag syntax is immutable for the process. Required target and GIC/MSI symbols are checked before API/no-api readiness; one shared 31-endpoint slot/BAR/dispatcher budget plus exact fixed and worst-case runtime vector demand is checked before `Running`. Balloon, block, network, pmem, vsock, entropy, and virtio-mem use deterministic modern PCI functions while serial, RTC, boot timer, GIC, VMGenID, and VMClock remain platform MMIO devices. PCI mode omits the VMM-supplied `pci=off` and publishes only the PCI/GICv2m transport FDT; default startup remains all-virtio-MMIO. Current native-v2 2.11 create/load supports required platform-MMIO serial plus independently optional rooted/rootless profile-3 regular-file block/pmem storage, entropy, balloon, virtio-mem, and network/MMDS in all 32 products up to the same endpoint/vector budget; exact 2.3–2.10 retain their original profiles. Kind 12 reconstructs network in canonical PCI slot/route order from one exact clone-local override per interface with fresh backend, packet-I/O, callback, metric, scheduler, endpoint, route, and MMDS owners. Frozen native-v1 and snapshot profiles for vsock still reject PCI restore. Running/Paused non-root block, pmem, and network PUT/DELETE share type-scoped identity and one owner-thread inventory with manual guest coordination. |
...
| `--snapshot-version` | implemented native-envelope inspection | Prints the current public writer version `v2.11.0` and exits successfully before fd-table setup, API socket publication, signal setup, or HVF startup. This names bangbang's native data format and does not claim Firecracker v10 artifact compatibility. |
...
| `PUT` | `/snapshot/create`, `/snapshot/load` | supported current native-v2 2.11 complete-serial Full/File lifecycle with independently optional entropy, balloon, virtio-mem, network/MMDS, and regular-file block-and-pmem profile-3 storage; exact native-v2 2.3–2.10 loaders plus frozen native-v1 File/Uffd reader | Parses Firecracker-shaped bodies, rejects malformed input first, and keeps paths/overrides redacted. Paused-only `Full` create admits a 1–32-vCPU source with required serial and independently optional kinds 7 and 9–12 over coherent MMIO or PCI, yielding all 32 products; vsock, boot timer, and vhost-user reject before artifacts. Optional network/MMDS kind 12 is capped at 512 KiB with an 83,552-byte 16-interface worst case. It retains portable interface, queue, limiter/retry, placement, and MMDS protocol configuration while excluding host backend owners, handles, packets, connections, data, tokens, metrics, and clocks. Load opens state once, selects frozen native-v1 or exact 2.3–2.11 File/COW, validates the complete graph and external authority transaction, and requires one exact unique clone-local `network_overrides` entry per saved interface. It maps memory and constructs fresh serial/entropy/balloon/virtio-mem/network/MMDS owners before Paused publication. State/memory and guest File/COW RAM stay immutable/private; writable external block/pmem prefixes remain deliberately shared. Focused public matrices prove all 32 products. Signed direct and normal-production/App-Sandbox MMIO/PCI V1/V2 matrices prove explicit-Paused recapture and automatic restore, empty then reseeded MMDS, source TCP/token loss, fresh-session success, fresh metrics, immutable clones, malformed state/memory, missing/duplicate/unknown override redaction, cancellation/death cleanup, retry, exact grants, pathname replacement resistance, and all-MMDS containment without vmnet authority. No Firecracker artifact interoperability, native-v2 `Uffd`, `Diff`, vsock restore, source dirty/owner/packet/session identity, synchronous RSS, or broad portability is claimed. |
...
| `PUT`, `PATCH` | `/pmem/{id}` | implemented direct MMIO-or-PCI startup, pmem root, non-root PCI runtime, capture-ready state, aggregate live certification, and exact native-v2 2.6 through current 2.11 serialization/restore | `PUT /pmem/{id}` stores strict Firecracker-shaped configuration with one cross-storage root, exact direct or contained regular-file authority, one file/private-tail mapping, and exact-prefix `MS_SYNC`. Runtime PCI insertion remains non-root-only and preflights every owner capacity before authority or mapping work. Profile 3 serializes exact file/mapped geometry plus configuration/runtime/limiter/queue/interrupt/mapping/transport state and restores it in the same typed complete-set transaction as block; exact 2.7 composes required serial, exact 2.8 may add entropy, exact 2.9 may add balloon, exact 2.10 may add virtio-mem, and current 2.11 may add network/MMDS. Signed focused and aggregate coverage proves live protection/coherence/reuse; signed rooted pmem-only and rootless mixed MMIO/PCI direct/contained matrices prove shared writable prefix epochs, unchanged read-only peers, zero fresh private tails, immutable state/memory, recapture, exact grants after pathname replacement, and cleanup. |
...
current 2.11 derives the serial endpoint intent, every inert optional profile-3
block and pmem request, and optional entropy/balloon/virtio-mem/network state
...
MMDS owners. MMDS-only destinations require no vmnet authority.
...
supports current native-v2 2.11 Full/File for 1–32 vCPUs with required complete
serial state, independently optional entropy, balloon, virtio-mem, and
network/MMDS, and
...
destination-local mapping, dirty, notifier, interrupt, dispatcher, route,
metrics, and cleanup owners. Vsock restore remains excluded.
...
| `PUT /pmem/{id}` | implemented; `204` empty response on successful config storage | implemented in Running/Paused public PCI; `204` after live commit | Pre-boot requests record Firecracker-shaped configuration with one cross-storage root and exact direct or contained regular-file authority. Startup maps each file/private-tail range once and retains the selected MMIO/PCI owner; runtime PCI insertion preflights all capacity before authority or mapping work. Exact-prefix `MS_SYNC`, read-only guest protection, rollback, and cleanup preserve the prior projection. Exact native-v2 2.6 through current 2.11 profile 3 serialize and restore exact pmem configuration/runtime/limiter/queue/interrupt/mapping/transport state through the same complete-set transaction as block; exact 2.7 composes required serial, exact 2.8 may add entropy, exact 2.9 may add balloon, exact 2.10 may add virtio-mem, and current 2.11 may add network/MMDS. Focused and signed direct/contained rooted pmem-only and rootless mixed matrices certify live and restored behavior, immutable-pair reuse, shared writable prefixes, zero private tails, exact grants, and cleanup. |
...
contained certification; current 2.11 preserves that profile-3 graph alongside
required serial state and optional entropy/balloon/virtio-mem/network. Parser recognition, config echo, or a family-level test
...
- snapshot behavior beyond the implemented narrow native-v2 Full/File profile.
  ...
  process-death cleanup, retry, redaction, and no-vmnet containment. The CLI
  reports `v2.11.0` and describes exact v1/v2 versions. Remaining work includes
  vsock serialization and restore, native-v2 Uffd, Diff artifacts, per-drive
  and vsock overrides, editing tools, Firecracker artifact compatibility,
