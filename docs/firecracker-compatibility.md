...
dynamic CPU add/remove remains deferred. Exact expert-controlled masks for the
four reviewed arm64 identification registers and reviewed U32/U64/U128
core/SIMD profile are applied to the complete owner topology before boot
overrides, with requested-set read-before-write preflight and immediate
readback. Remaining system-register policy and final inventory closure remain
#1403 work. Firecracker
...
implements expert masks for four reviewed 64-bit identification registers,
U64 X0 and X4-X30 plus SP_EL0/PC/PSTATE/SP_EL1/ELR_EL1/SPSR_EL1, U128 Q0-Q31,
and U32 FPCR/FPSR. Q transport is explicitly little-endian; FP scalar reads
must fit U32 before any write. X1-X3 are boot-reserved and AArch32 banked SPSRs
are unavailable. Complete mixed-width baseline collection across all vCPUs
precedes mutation; each common target is written and immediately reread on its
owner thread before boot resource assembly. Primary and secondary boot setup
then own X0-X3/PC/PSTATE as appropriate. A failure destroys the unpublished
topology and VM.
...
evidence, and #1403/Wave handoffs are in the checked
...
| `PUT` | `/cpu-config` | supported reviewed arm64 custom subset; KVM-only categories platform-excluded | Parses bounded ordered Firecracker aarch64 custom templates with a 256-entry limit per array, exact 32/64/128-bit ARM identities/bitmaps, fixed seven-word vCPU-feature indexes, stronger duplicate-identity checks, and value-redacted diagnostics. A successful custom PUT replaces static/custom state; empty input clears it. Exact `(baseline & !filter) | value` execution covers the four existing U64 ID registers, U64 X0/X4-X30 and reviewed SP/PC/PSTATE fields, U128 Q0-Q31 with explicit little-endian transport, and U32 FPCR/FPSR with fail-closed scalar conversion. X1-X3 and AArch32 banked SPSRs have explicit policy faults. Every requested typed baseline is read on every owner before any write, targets are common, and each write is immediately reread; boot setup then overrides X0/PC/PSTATE. Any failure destroys the unpublished VM. KVM capability numbers, KVM vCPU-init feature words, mixed KVM-only input, and remaining system registers receive distinct fixed faults pending #1403. Custom contents are omitted from GET and excluded from native-v1 snapshots. See the checked CPU-template contract. |
