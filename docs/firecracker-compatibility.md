...
| `PUT /snapshot/load` | `snapshot_path` | required; retained/redacted, unopened | Retained through API/runtime values without file access or diagnostic exposure. Host reads and restore remain deferred. |
...
| `PUT /snapshot/load` | `resume_vm` | optional; retained without effect | Retained for the later load executor, but this endpoint still returns unsupported and never resumes a VM. |
...
Snapshot persistence and restore still need the dedicated macOS/HVF design
...
not be reported as a platform limit by default. bangbang now has a native fixed
outer state envelope with exact `1.0.0`, arm64, 4096-byte page-size and
...
anonymous memory only after identity, length, CRC, and EOF checks. The process
still treats the state payload as opaque, no API path calls those primitives,
and public create/load emit no artifacts. Firecracker on-disk compatibility was
...
diagnostics, but it performs no restore.
...
restore commands provide no cross-step no-run lease. Native-v1 composite
capture now persists both values under one aggregate runner admission window;
the public process snapshot barrier still invokes none of these commands.
`ICC_SRE_EL2`, ICH/ICV, destination validation, host-update preflight, restore
