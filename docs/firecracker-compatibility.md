...
| --- | --- |
| [#5786 PCI hotplug/hot-unplug](https://github.com/firecracker-microvm/firecracker/pull/5786) for block, pmem, and net | Implemented for non-root block and pmem devices in the public PCI profile, including transactional Running/Paused PUT/DELETE, manual guest rescan/removal, exact backing ownership, and capacity reuse. Pmem additionally owns dynamic HVF shadow map/flush/unmap and same guest-range reuse. Default virtio-MMIO and root mutation retain nonmutating rejection; runtime network mutation remains deferred. |
...
semantic identities. After the #1420 block and #1421 pmem hotplug promotions,
the current 418 records contain 76 `implemented-and-verified`, 322
...
| `PUT` | `/drives/{drive_id}` | supported target; pre-boot plus PCI-only runtime attach implemented | Before boot, stores initial virtio-block configuration including optional bandwidth/ops rate limiters. After startup with public `--enable-pci`, Running or Paused requests may transactionally attach one non-root file-backed endpoint through the retained owner-thread inventory. Direct backing paths open before owner submission; contained requests may consume only an exact still-unused initial-manifest drive grant with matching access and retain that authority on aborted insertion. Live PCI publication and `/vm/config` commit only after all endpoint leases succeed. Default MMIO, root, duplicate, invalid backing/grant, and capacity failures are nonmutating. |
...
| `PUT`, `PATCH` | `/pmem/{id}` | implemented MMIO-or-PCI startup plus non-root PCI runtime subset; root/direct mapping and snapshots excluded | `PUT /pmem/{id}` stores strict Firecracker-shaped pre-boot configuration, rejects `root_device: true`, and exposes accepted state through `GET /vm/config`. Direct paths remain unopened until startup; contained grant tags claim and retain an exact-ID, exact-access regular-file descriptor during successful pre-boot `PUT`, then move it into startup without reopening the tag. `InstanceStart` validates each nonzero regular backing, assigns a deterministic aligned guest range, creates and registers an HVF-compatible anonymous shadow, attaches one endpoint over the selected transport, and preserves read-only versus writable cleanup policy. In a Running/Paused public PCI session, PUT instead reserves configuration and exact direct or unused contained backing authority, maps it on the owner thread, assigns the first aligned range outside DRAM, full virtio-mem reservation, and live pmem, registers one exact dynamic shadow, publishes the endpoint, and only then commits configuration and grant consumption. Flush is selected lazily for only the notified device after a valid request; empty or malformed-only events do not sync, peer devices are not traversed, and one event result is cached for later valid descriptors on that same device. Optional bandwidth/ops buckets charge one operation plus exact backing length before a non-empty coalesced event, retain a throttled cursor for a session-owned retry, and expose throttling/wakeup metrics. Post-boot `PATCH /pmem/{id}` atomically replaces or clears present live buckets before stored commit and preserves both states on failure. Signed coverage proves initial limiter, live partial replacement, guest read, selected-backing flush, contained descriptor identity, runtime attach/delete, and exact same-ID/PCI-slot/guest-range reuse. Pmem root boot, dirty tracking, direct file-backed HVF mapping, and optional-device snapshots remain excluded. |
...
| `DELETE` | `/drives/{drive_id}` | PCI-only non-root runtime removal implemented | Firecracker routes this bodyless operation in `parsed_request.rs` outside the v1.16 Swagger surface. In Running or Paused public all-virtio PCI sessions, bangbang removes MMIO/ECAM visibility, drains admitted work/messages, then releases endpoint, MSI-X, BAR, function, dispatcher, backing, metrics, and config ownership before capacity reuse. Recoverable preparation failure restores the same usable endpoint; post-commit corruption is terminal. Missing/root/default-MMIO failures are nonmutating. Linux must remove the PCI function through guest sysfs before DELETE. A body still fails first with `Empty Delete request.`. |
| `DELETE` | `/pmem/{id}`, `/network-interfaces/{iface_id}` | pmem implemented for non-root public PCI; network recognized and unsupported | Both pinned bodyless hot-unplug routes retain normal unsupported-state behavior before startup, and body-bearing requests fail first with `Empty Delete request.`. In a Running/Paused public PCI session, pmem DELETE requires prior guest sysfs removal, prepares endpoint teardown, flushes and unregisters only the exact dynamic shadow, commits endpoint teardown, then releases backing, metrics generation, guest range, grant state, and configuration. Pre-commit failures restore the endpoint or mapping; incomplete restoration and post-boundary corruption are terminal. Default MMIO, root, and missing pmem requests are nonmutating. Network runtime removal remains deferred with its stable device-specific fault. |
...
| `PUT /pmem/{id}` | `path_on_host` | required; mapped, range-assigned, guest-attached, HVF-registered through an exact shadow, and writeback-capable | Required Firecracker-shaped host backing path. The value is retained and reported in `GET /vm/config` after rejecting empty paths. Pre-boot ordinary paths open at startup; runtime PCI ordinary paths open before the owner command so failure cannot commit configuration. In contained mode an exact grant tag claims one `PmemBacking` descriptor with access matching `read_only`; pre-boot PUT retains it by pmem ID for startup, while runtime PUT uses a rollback claim and consumes it only after live publication. Both paths avoid reopening the tag and are failure-atomic. The owner mmaps the non-zero regular file to a 2 MiB-aligned host range, assigns matching guest physical range/config-space metadata, attaches an endpoint over the selected transport, copies the mapping into an HVF-compatible anonymous shadow, registers that shadow with HVF, and writes writable shadows back for guest queue-driven flush requests and after clean unmap. Runtime first-fit allocation excludes DRAM, the full virtio-mem reservation, and every live pmem range; DELETE releases that exact dynamic mapping and range for reuse only after teardown. Errors redact path and tag details. Path normalization remains deferred. |
...
| `PUT /pmem/{id}` | `read_only` | optional; backing access, host mapping, HVF permission access, guest attachment, and writeback policy | Missing values default to `false` and are reported in `GET /vm/config`. Direct paths open at startup or before a runtime owner command; contained tags claim during successful pre-boot PUT or reserve through a rollback claim during runtime PUT. Every path requires read-only access when this is `true` and read/write access when it is `false`. HVF shadow registration mirrors that intent with read-only or read/write non-executable memory permissions. Read-only shadows never write back, while writable shadows write back for guest queue-driven flush requests and during clean runtime unmap. The current guest-visible virtio-pmem attachment exposes config-space, range discovery, and flush completion behavior. |
...
feature has no automatic guest notification. bangbang now supports immutable
all-virtio PCI startup with `--enable-pci`, but it does not yet add functions to
an active bus: runtime PUT still fails before the proposed backing is opened or
retained, and bodyless DELETE returns a redacted, nonmutating device-specific
unsupported fault. Existing drive backing/rate-limiter PATCH operates through
the selected MMIO or PCI startup handle.
...
GICv3 MBI/ITS, runtime network attach/delete, PCI snapshot persistence, or
delivery rollback guarantee. Runtime block and pmem PUT/DELETE instead use the
retained bounded PCI/GICv2m resource manager and require manual guest
coordination; pmem additionally owns an exact dynamic HVF shadow mapping and
reusable guest range.
...
| `PUT /drives/{drive_id}` | implemented; `204` empty response on successful config storage; recognized unsupported for vhost-user-block `socket` configs | PCI-only non-root runtime attach implemented; `204` on success | Records validated pre-boot virtio-block config, including optional bandwidth/ops rate limiters. Direct paths open during startup; contained grant tags claim and retain exact-ID, exact-access regular-file descriptors during successful requests. Startup moves provided descriptors or opens missing direct paths, attaches initial block devices over the selected startup transport, applies configured limiters in queue dispatch without sleeping, and wires session-owned HVF retry wakeups when a throttled descriptor remains pending. Firecracker-shaped socket-backed vhost-user-block configs are parsed and rejected with `drive socket is not supported` without mutating stored config or consuming a grant. After public PCI startup, Running/Paused requests transactionally attach non-root file-backed drives through the retained owner inventory; direct mode opens before submission and contained mode consumes only an exact unused initial grant with rollback on abort. Publication and stored configuration commit only together. Default MMIO, root, duplicate, invalid backing/grant, and exhausted-capacity requests remain nonmutating. Guest PCI rescan is manual. External vhost-user-block execution remains unsupported. |
| `PUT /pmem/{id}` | implemented; `204` empty response on successful config storage | implemented in Running/Paused public PCI; `204` after live commit | Pre-boot requests record Firecracker-shaped config and replace prior config for the same ID failure-atomically; empty or all-null limiter objects are unconfigured, valid bandwidth/ops buckets are stored and reported through `GET /vm/config`, and `root_device: true` is rejected without mutation. Direct pre-boot paths remain unopened until startup; contained grant tags claim and retain exact-ID, exact-access nonzero regular-file descriptors. Startup moves provided descriptors or opens missing direct paths, maps and registers each guest range, attaches one endpoint over the selected startup transport, applies the limiter before coalesced flush dispatch, writes writable shadows back for guest queue-driven flush requests and after clean unmap, and retains every owner. After startup, public PCI accepts only a new non-root ID: direct mode opens before owner submission, contained mode reserves an exact unused grant with rollback, and the owner maps the file, allocates a non-overlapping reusable guest range, registers one exact dynamic HVF shadow, and publishes the endpoint before configuration and grant consumption commit. Recoverable failures leave the projection and authority unchanged; incomplete cleanup is terminal. Default MMIO rejects before path use. Root-device semantics, dirty-range tracking, and direct file-backed HVF mapping remain deferred. |
...
- pmem root-device semantics, dirty-range tracking, direct file-backed HVF
  mapping, and optional-device snapshots beyond targeted notified-device flush,
  per-event bandwidth/ops limiting, retained retry, runtime limiter replacement,
  guest-visible MMIO/FDT or PCI attachment, transactional non-root PCI
  PUT/DELETE, dynamic shadow teardown, and signed guest read/flush/reuse proof
