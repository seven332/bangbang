...
acknowledge that deterministic SYN-ACK, FIN close, guest RST, and unsupported
control packets, synthesize ACK plus FIN-ACK frames for empty FIN close packets,
synthesize minimal RST frames for unsupported empty controls, consume guest RST
packets without response, and for non-empty
candidate TCP payloads that acknowledge that deterministic SYN-ACK, it can
also produce the same process-local HTTP response bytes as the existing guest
HTTP helper, including token PUT and MMDS v2 GET token enforcement. The process
vmnet packet I/O path detours MMDS ARP requests, pure empty-payload MMDS SYN
packets, pure empty-payload MMDS ACK-only packets that acknowledge bangbang's
deterministic SYN-ACK, pure empty-payload MMDS FIN close packets, guest RST
packets, unsupported empty control packets, and non-empty MMDS candidate TX
payloads that acknowledge bangbang's deterministic SYN-ACK only on configured
MMDS interfaces, keeps
shared MMDS state visible to both API and packet paths,
...
RST frames, and Ethernet/IPv4/TCP response frames carrying the generated HTTP response bytes, retains those frames in
bounded per-interface queues, exposes queued frames through the matching
...
out-of-order reassembly, retransmission policy, stateful RST policy, session timeout policy, and stricter
per-interface TCP session ownership remain deferred.
