...
and failures, including separate inflate/hint discard attempt,
zero-and-free-advised byte, skipped-edge byte, and failed-attempt fields;
...
descriptor consumption. Throttled work retains its queue cursor and is retried
by a dedicated session-owned HVF wakeup. Runtime `PATCH /pmem/{id}` accepts
missing, `null`, empty, and all-null rate limiter objects as no-op updates and
atomically replaces or clears present buckets on the exact active device before
committing stored configuration. `PUT /pmem/{id}` rejects `root_device: true`
...
| `PUT` | `/vsock` | supported target; implemented supported live virtio-MMIO/Unix-socket subset | Repeated valid pre-boot requests atomically replace stored configuration without opening host resources; post-start PUT is stably rejected without mutation. Startup binds and inode-tracks the process-owned `uds_path`, attaches one guest-visible MMIO/FDT device with three 256-entry queues, and cleans up only its own socket. The live handler supports guest- and host-initiated streams, bounded handshakes and four-packet directional backlogs, 256 connections per direction, dynamic 64-KiB credit windows with wrapping counters, partial/full shutdown, two-second request/shutdown cleanup, reset/error handling, `EVENT_IDX`, no-op event notifications, and path/payload-redacted diagnostics. Signed Apple Silicon cases incrementally verify ≥1 MiB in each direction, both peers' write-half-close/EOF, terminal cleanup, and two-stream isolation. Indirect descriptors are a supported bangbang extension. PATCH, DELETE, runtime hotplug, broader CID routing, full event payloads, PCI/vhost/KVM, and general performance/artifact parity remain excluded. Native-v1 snapshot UDS override, event-queue `TRANSPORT_RESET`, and post-restore RX gating remain the stable #543 exclusions. |
...
| `PUT /snapshot/create` | `snapshot_path` | required; redacted, opened after preflight | Retained with redacted `Debug`; an admitted create opens its parent/final namespace only after paused/profile preflight. It is never logged or echoed. |
| `PUT /snapshot/create` | `mem_file_path` | required; redacted, opened after preflight | Uses the same redaction and gate ordering as `snapshot_path`; guest memory streams directly into the destination-anchored staging inode. |
| `PUT /snapshot/create` | unknown fields | rejected | Matches Firecracker's strict request model behavior. |
| `PUT /snapshot/load` | `snapshot_path` | required; redacted, opened after preflight | The native-v1 loader opens it only after pristine/profile preflight and never exposes it in diagnostics. |
| `PUT /snapshot/load` | `mem_backend` | required unless deprecated `mem_file_path` is present; redacted | Parsed as a strict `backend_path`/`backend_type` object. Exactly one backend form is required; an admitted normalized backend is opened only by the no-follow committed-pair loader. |
...
last; success returns `204` with the source still paused. `PUT /snapshot/load`
is pre-boot-only, validates process freshness and the committed pair before VM
construction, commits a real restored session as `Paused`, and optionally uses
ordinary resume. Malformed bodies are rejected by the parser first, while
execution faults remain typed and path-redacted.
...
running. Metrics report separate inflate/hint discard attempts, actual advised
bytes, skipped-edge bytes, and failures. Free-page reporting remains deferred.
...
| `PUT /vsock` | implemented; repeated valid requests replace stored config and return `204` | unsupported after start; `400` `fault_message` without mutation | Implements the supported live virtio-MMIO/Unix-socket subset without opening a host socket during the API request. Startup binds and inode-tracks the process-owned listener, attaches three 256-entry queues, and activates guest-/host-initiated connection handling with bounded handshakes/backlogs, dynamic 64-KiB wrapping-counter credit windows, partial/full shutdown, two-second request/shutdown cleanup, reset/error handling, `EVENT_IDX`, and no-op event notifications. Signed cases verify ≥1-MiB deterministic bidirectional streams, write-half-close/EOF, cleanup, redaction, and two-stream isolation for both initiation paths. Indirect descriptors are a supported bangbang extension. There is no PATCH, DELETE, runtime hotplug, broader CID routing, full event payload, or general performance/artifact contract; #543 owns native-v1 snapshot UDS override, `TRANSPORT_RESET`, and post-restore RX gating exclusions. |
...
- full Firecracker balloon device counters and free-page reporting
- pmem root-device semantics, dirty-range
