...
| `PUT` | `/snapshot/create`, `/snapshot/load` | recognized; typed VMM-routed unsupported | Parses Firecracker-shaped snapshot create and load bodies, rejects malformed input first, normalizes deprecated load `mem_file_path` to a `File` backend and dirty tracking to the OR of old/new flags, and carries every field through debug-redacted API/runtime values. Paths plus network/vsock override contents are retained but never logged, opened, canonicalized, or statted. A centralized native-v1 gate currently admits only Full create, File load, no dirty tracking/clock adjustment/overrides, and a create profile with one vCPU, exactly one read-only root drive, default serial, and no optional devices or MMDS. State-invalid requests keep their existing faults. Rejected paused create profiles return the same snapshot-specific unsupported fault before the supervisor barrier; an admitted profile crosses the scoped-admission/block/entropy-quiescence barrier and still returns unsupported. Load is pre-boot-only and requires both successful-action history and current non-logger/metrics configuration to be pristine; logger/metrics remain allowed, explicit-default/no-op VM configuration is detected by history, and residual state such as failed-patch MMDS presence is detected live. This is deliberately stricter and more state-based than Firecracker `v1.16.0`'s uneven pre-validation `boot_path` flag. Public faults and snapshot latency/deprecation metrics remain compatible. Internally, kind-1 `BANGCMT\0` remains byte-compatible and kind 2 binds memory to an exact five-component `BANGHVF\0` payload containing machine/profile, CPU/cache/GIC/RTC compatibility metadata, mutable one-vCPU state, timer/interrupt/GIC state, and the nested `BANGDEV\0` baseline-device value. A private supervisor command can hold paused admission and limiter quiescence through aggregate HVF capture and cancellable 1 MiB memory streaming, returning a detached bundle while leaving the source paused and reusable. The existing macOS `RENAME_EXCL` memory-first/state-last publisher and state-first loader remain separate. The endpoint invokes only the original barrier and does not call composite capture, open request paths, or publish files; no Firecracker state-file compatibility is claimed. The private process seam now loads the committed pair, validates and installs baseline devices without boot writes, constructs and restores a fresh HVF VM, replaces and signals VMGenID, and hands an unrun session to an initially paused supervisor while returning resume intent. Public artifact I/O and endpoint success, create publication, dirty tracking, optional devices, and fresh-process API continuity remain deferred. |
...
| `PUT /snapshot/create` | `snapshot_path` | required; retained/redacted, unopened | Retained through API/runtime values with redacted `Debug`; it is not opened, canonicalized, statted, logged, or echoed. |
| `PUT /snapshot/create` | `mem_file_path` | required; retained/redacted, unopened | Retained through API/runtime values with the same redaction/no-file policy as `snapshot_path`. Guest memory persistence remains deferred. |
...
not call it. Malformed or schema-invalid snapshot bodies are still rejected by
the parser first.

Public snapshot persistence and restore still need create/load orchestration
...
process loader decodes the HVF payload and commits an initially paused restored
session; no public API path calls it, and public create/load emit no artifacts.
