...
| `--snapshot-version` | implemented native-envelope inspection | Prints the current public writer version `v2.4.0` and exits successfully before fd-table setup, API socket publication, signal setup, or HVF startup. This names bangbang's native data format and does not claim Firecracker v10 artifact compatibility. |
...
| `PUT` | `/snapshot/create`, `/snapshot/load` | supported narrow native-v2 2.4 root Full/File lifecycle, legacy device-free native-v2 2.3 loader, plus frozen native-v1 File/Uffd reader | Parses Firecracker-shaped bodies, rejects malformed input first, normalizes deprecated load fields for native-v1 compatibility, and keeps paths/overrides redacted through typed API/runtime values. Create is paused-only and admits `Full` for a 1–32-vCPU source with configured boot metadata, exactly one read-only File/Sync root over the selected MMIO or PCI transport, no optional devices/MMDS/boot timer, default serial configuration, and a live UART equal to the canonical empty reset-compatible model. Direct and exact contained outputs share the anchored no-clobber memory-first/state-last transaction, cancellation seal, cleanup, durability, metrics, and source-Paused behavior; `Diff` rejects before namespace or capture work. Load is pre-boot-only and requires a pristine process except logger/metrics. It opens and decodes state once, routes pinned Firecracker or unknown bytes to explicit incompatibility, routes frozen native-v1 to the unchanged eager File or macOS pager path with its root/grant/deprecation semantics, retains legacy device-free 2.3 loading, and routes current 2.4 to retained read-only File/COW memory plus an exact newly authorized root backing. Direct and contained 2.4 loads validate the complete state/memory/live-FDT identity/UART/HVF/topology/time/identity/device/transport graph before construction. Because Linux may reclaim FDT bytes after boot, exact 2.4 requires a versioned source-product marker and derives the root shell from typed state rather than reparsing those bytes; their address, length, and CRC still bind the memory artifact. Loads keep the root lease provisional until Paused session/controller publication, keep guest memory writes private, and can repeatedly restore isolated destinations without mutating the pair. All families commit a real session `Paused` before optional ordinary resume; requested-resume failure is terminal. Retryable preparation failures preserve pristine eligibility, while committed identity/time, publication, incomplete cleanup, or resume failure latches the process terminal. Signed direct two-vCPU and production App Sandbox cases prove exact root reconstruction, Paused then explicit/automatic resume, all-vCPU continuation, private COW writes, repeated loads, retained descriptor identity, cleanup, and a real closed-stdout terminal boundary. No Firecracker state-file interoperability, Linux UFFD wire compatibility, native-v2 `Uffd`, `Diff`, realtime adjustment, overrides, optional-device/general-serial profile, editing, or broad portability is claimed. |
...
current 2.4 additionally resolves the inert device-graph selector through an
exact destination read-only drive-backing grant and commits that lease only
with the restored session/controller. Frozen native-v1 additionally adopts its
persisted-root input. Direct load keeps no-follow pathname adapters. Malformed
bodies are rejected by the parser first, while execution faults remain typed
and path/reference-redacted.
...
supports a narrow native-v2 2.4 Full/File profile with 1–32 vCPUs, exactly one
read-only File/Sync root over MMIO or PCI, no optional devices, and a canonical
default UART. The process-standard-stream endpoint is destination-local and is
rebound rather than serialized. It also retains the device-free native-v2 2.3 loader and a frozen
one-vCPU/read-only-root native-v1 File/Uffd reader. All include public Paused
handoff, optional resume, and recoverable-versus-terminal cleanup evidence.
...
- snapshot behavior beyond the implemented narrow native-v2 Full/File profile.
  ...
  carries an exact typed 1–32-vCPU
  ...
  reviewed debug/SME state, and one mandatory root-device graph containing
  configuration, block runtime, common virtio, and MMIO-or-PCI transport state.
  ...
  validates the live-FDT identity plus versioned product/UART/RTC/time/root
  graph before HVF, authorizes and retains the root backing without reopening a
  contained selector, installs
  ...
  fallback. The CLI reports `v2.4.0` and describes exact v1/v2 versions. There
