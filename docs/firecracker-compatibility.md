...
deterministic SYN-ACK frames, identify pure empty-payload ACK-only packets, and
for non-empty candidate TCP payloads, it can
...
packets, pure empty-payload MMDS ACK-only packets, and non-empty MMDS candidate
TX payloads only on configured MMDS
...
frames, and Ethernet/IPv4/TCP response frames carrying the generated HTTP
response bytes, retains those frames in
...
out-of-order reassembly, retransmission policy, FIN/RST/session timeout policy,
and stricter
