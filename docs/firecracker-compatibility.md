...
| `GET`, `PUT`, `PATCH` | `/mmds` | supported target; control-plane storage implemented | Stores bounded in-memory JSON object contents in the process runtime, returns stored JSON for `GET`, applies RFC 7396 merge-patch semantics for `PATCH`, rejects uninitialized `GET`/`PATCH`, and keeps previous data on oversized update failure. Guest-visible delivery remains tied to later #16 work. |
...
- snapshots
- MMDS guest access, token handling, and IMDS response formatting
