...
supplies balloon encoding, and current 2.10 supplies virtio-mem encoding,
fresh owner reconstruction, immutable clone, and signed direct/contained
continuation. The time/identity capture-ready values remain private validated
...
#1697 activates current exact native-v2 2.10 optional virtio-mem, and #1698
promotes exactly the two memory-hotplug aggregates after matching signed
certification. The current aggregate selector is 84 terminal and one
`audit-required`, and the current global counts are 242/156/3/17.
...
| `--snapshot-version` | implemented native-envelope inspection | Prints the current public writer version `v2.10.0` and exits successfully before fd-table setup, API socket publication, signal setup, or HVF startup. This names bangbang's native data format and does not claim Firecracker v10 artifact compatibility. |
...
accounting. Current public native-v2 2.10 serializes that state as optional
kind 11 and binds its compact plugged bitmap to exact kind-1 memory extents.
...
| `PUT` | `/snapshot/create`, `/snapshot/load` | supported current native-v2 2.10 complete-serial Full/File lifecycle with independently optional entropy, balloon, virtio-mem, and regular-file block-and-pmem profile-3 storage; exact native-v2 2.3–2.9 loaders plus frozen native-v1 File/Uffd reader | Parses Firecracker-shaped bodies, rejects malformed input first, and keeps paths/overrides redacted. Paused-only `Full` create admits a 1–32-vCPU source with required serial and independently optional profile-3 storage, entropy, balloon, and virtio-mem over coherent MMIO or PCI, yielding all sixteen products; network, vsock, MMDS, boot timer, and vhost-user reject before artifacts. Required serial kind 8 and optional storage kind 7, entropy kind 9, and balloon kind 10 retain their exact profiles. Optional virtio-mem kind 11 retains configuration, features, config space, one inactive or active queue, common virtio and MMIO/PCI placement, and a canonical plugged-block bitmap bound to exact kind-1 memory extents; it is capped at 128 KiB inside the 16-MiB state cap. Load opens state once, selects frozen native-v1 or exact 2.3–2.10 File/COW, validates every balloon range and virtio-mem extent against adopted RAM, performs one exact external authority transaction, maps base RAM privately, creates one fresh unlinked shared virtio-mem aperture with block-granular plugged views and a clean dirty epoch, and constructs fresh serial/entropy/balloon/virtio-mem owners before Paused publication without fallback or override. State/memory and guest File/COW RAM stay immutable/private; writable external block/pmem prefixes remain deliberately shared. Focused public complete-product and signed HVF owner matrices prove all sixteen products over MMIO/PCI. Signed direct and normal-production/App-Sandbox matrices prove existing storage/serial/entropy/balloon behavior plus virtio-mem byte-first continuation, partial UNPLUG, driver-reprobe UNPLUG_ALL/replug, later PLUG/final UNPLUG, destination-only metrics, recapture, immutable clone isolation, malformed state/truncated-memory/cancellation/death cleanup, containment, exact grants, pathname replacement resistance, explicit/automatic resume, and worker/launcher cleanup. No Firecracker artifact interoperability, native-v2 `Uffd`, `Diff`, network/vsock/MMDS restore, source dirty/owner identity, synchronous RSS, or broad portability is claimed. |
...
| `PUT`, `PATCH` | `/pmem/{id}` | implemented direct MMIO-or-PCI startup, pmem root, non-root PCI runtime, capture-ready state, aggregate live certification, and exact native-v2 2.6 through current 2.10 serialization/restore | `PUT /pmem/{id}` stores strict Firecracker-shaped configuration with one cross-storage root, exact direct or contained regular-file authority, one file/private-tail mapping, and exact-prefix `MS_SYNC`. Runtime PCI insertion remains non-root-only and preflights every owner capacity before authority or mapping work. Profile 3 serializes exact file/mapped geometry plus configuration/runtime/limiter/queue/interrupt/mapping/transport state and restores it in the same typed complete-set transaction as block; exact 2.7 composes required serial, exact 2.8 may add entropy, exact 2.9 may add balloon, and current 2.10 may add virtio-mem. Signed focused and aggregate coverage proves live protection/coherence/reuse; signed rooted pmem-only and rootless mixed MMIO/PCI direct/contained matrices prove shared writable prefix epochs, unchanged read-only peers, zero fresh private tails, immutable state/memory, recapture, exact grants after pathname replacement, and cleanup. |
...
current 2.10 derives the serial endpoint intent, every inert optional profile-3
block and pmem request, and optional entropy/balloon/virtio-mem state from the decoded graph. It
...
supports current native-v2 2.10 Full/File for 1–32 vCPUs with required complete
serial state, independently optional entropy, balloon, and virtio-mem, and
...
| `PUT /pmem/{id}` | implemented; `204` empty response on successful config storage | implemented in Running/Paused public PCI; `204` after live commit | Pre-boot requests record Firecracker-shaped configuration with one cross-storage root and exact direct or contained regular-file authority. Startup maps each file/private-tail range once and retains the selected MMIO/PCI owner; runtime PCI insertion preflights all capacity before authority or mapping work. Exact-prefix `MS_SYNC`, read-only guest protection, rollback, and cleanup preserve the prior projection. Exact native-v2 2.6 through current 2.10 profile 3 serialize and restore exact pmem configuration/runtime/limiter/queue/interrupt/mapping/transport state through the same complete-set transaction as block; exact 2.7 composes required serial, exact 2.8 may add entropy, exact 2.9 may add balloon, and current 2.10 may add virtio-mem. Focused and signed direct/contained rooted pmem-only and rootless mixed matrices certify live and restored behavior, immutable-pair reuse, shared writable prefixes, zero private tails, exact grants, and cleanup. |
...
contained certification; current 2.10 preserves that profile-3 graph alongside
required serial state and optional entropy/balloon/virtio-mem. Parser recognition, config echo, or a family-level test
...
- snapshot behavior beyond the implemented narrow native-v2 Full/File profile.
  ...
  malformed/cancellation/death cleanup over MMIO and PCI. The CLI reports
  `v2.10.0` and describes exact v1/v2 versions. There is still no general
  optional-device aggregate. Remaining work includes network/vsock/MMDS
  serialization and restore, native-v2 Uffd, Diff artifacts,
