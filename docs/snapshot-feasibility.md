...
paths. No setter, mode transition, maximum-SVL query, persistence, schema, or
restore ordering is defined. Signed coverage calls the getter twice on an idle
vCPU without assuming or logging values, changing PSTATE, reading SME data, or
executing the guest.
