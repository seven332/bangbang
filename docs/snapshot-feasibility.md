...
SME ZT0-register, translation, pointer-authentication,
...
the ZA matrix and fixed ZT0 register use separate conditional commands. None of
...
- HVF vCPU state capture: X0-X30, PC, and CPSR; raw SP_EL0, SP_EL1, ELR_EL1,
  ...
  restore path on the owning thread. The general-register primitive already
  supplies only its isolated, nontransactional owner-thread write sequence; it
  is not snapshot validation, ordering, rollback, or load orchestration.
