...
`SMCR_EL1.LEN`; P predicates and ZA are captured separately. ZT0, setters and
...
`SMCR_EL1.LEN`; Z registers and ZA are captured separately. ZT0, setters and
...
row/tile contract. ZT0, setters and transitions, layout interpretation,
feature/destination policy, protected persistence, schema, orchestration,
restore ordering, and multi-vCPU association remain deferred. Signed coverage
accepts only documented unavailability/inactivity or two complete equal idle-
...
translation, pointer-authentication,
and thread-context, SME system-register, and system-context captures.
...
the wider Z contents that the conditional SME command captures separately and
the P predicates that the other conditional SME command captures separately;
the ZA matrix uses another conditional command. ZT0 remains omitted. None of
these values defines a restore or snapshot-schema
contract.
...
captures and is exposed through both boot-session
...
- HVF vCPU state capture: X0-X30, PC, and CPSR; raw SP_EL0, SP_EL1, ELR_EL1,
  ...
  Remaining system registers, streaming SVE/SME ZT0 data, and other
