...
execution-control/cache-selection/breakpoint/debug-control/identification/
translation/pointer-authentication/thread-context command-owned admission domain and is exposed through both
...
breakpoint, debug-control, identification, translation, pointer-authentication,
and SIMD/FP captures and is exposed
...
- HVF vCPU state capture: X0-X30, PC, and CPSR; raw SP_EL0, SP_EL1, ELR_EL1,
  ...
  DFR0-reported raw DBGBVR/DBGBCR hardware-breakpoint pair;
  ...
  Hardware-breakpoint capture still needs control-bit and destination-count
  validation, protected persistence, host trap coordination, and ordered
  restore. Debug-control capture remains incomplete without watchpoint
  comparators, HVF debug trap configuration, feature and writable/status-bit
  validation, security policy, and ordered restore; raw breakpoint and
