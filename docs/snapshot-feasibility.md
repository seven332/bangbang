...
identification/translation/pointer-authentication/thread-context command-owned
admission domain and is exposed through both boot-session forms without
involving the supervisor lease or public snapshot paths.
...
breakpoint, watchpoint, debug-control, identification, translation, pointer-
authentication, and SIMD/FP captures and is exposed
...
- HVF vCPU state capture: X0-X30, PC, and CPSR; raw SP_EL0, SP_EL1, ELR_EL1,
  ...
  TPIDR_EL0, TPIDRRO_EL0, and TPIDR_EL1; baseline Q0-Q31, FPCR, and FPSR; raw
  ...
  coordination, and ordered restore. Debug-control capture remains incomplete
  without HVF debug trap configuration, feature and writable/status-bit
  validation, security policy, and ordered restore; raw comparator and
  MDCCINT/MDSCR values must not be treated as safe restore input.
