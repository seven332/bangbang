...
supports public bangbang-native Full/File and Diff/File lifecycles with a
rooted or rootless ordered regular-file block-and-pmem profile-3 graph. `Full`
writes exact native-v2 2.12 state plus a complete memory image; `Diff` writes
exact native-v2 2.13 state plus a detached differential layer. Both retain
required complete serial state and independently permit optional storage,
entropy, balloon, virtio-mem, network/MMDS, and vsock components, so all 64
optional-component products share one coherent MMIO-or-PCI lifecycle. Exact network/MMDS
native-v2 2.11, virtio-mem native-v2 2.10,
balloon-profile native-v2 2.9, entropy-profile native-v2
...
Exact `2.10.0` adds optional virtio-mem component kind 11. Exact `2.11.0`
adds optional network/MMDS component kind 12. Exact `2.12.0` adds optional
vsock component kind 13. Current `2.13.0` requires final semantic kind 14,
which commits the exact Diff base, selected GPA ranges, and complete result
binding without changing the nested 2.12 device product.
...
registers, bounded RX bytes, status, and pending input/interrupt work. Entropy
retains features/activation, one-queue geometry and cursors, both limiter
buckets, one pending descriptor, retry intent, common virtio state, and
transport placement, but not random output, source identity, metrics, or
absolute host time. Balloon retains exact configuration/features, active queue
cursors, latest optional statistics, one pending statistics descriptor,
DONE-normalized hint history, common virtio/transport state, and canonical
host-side inflated-PFN accounting. Kind 10 admits at most 262,144 ranges and
...
Network/MMDS retains interface order and IDs, requested configuration,
captured inert selector identity, requested and realized MAC/MTU, backend
class, queue/common-virtio state, limiter/retry state, exact transport
placement, and MMDS selection/version/IP/IMDS compatibility. Kind 12 is capped
at 512 KiB, with an 83,552-byte worst case for 16 interfaces. It excludes
packet-I/O and provider owners, handles, callbacks, peer/cached packets,
ARP/TCP sessions, MMDS data, token keys/tokens, metrics, and host clocks.
Vsock retains CID, an inert logical selector, the host-local port cursor,
common virtio features and queues, interrupt/reset state, and coherent MMIO/PCI
...
notifier/interrupt/dispatcher/route/metrics/cleanup owners plus fresh
network/backend/packet-I/O/metric/scheduler/route/endpoint/MMDS owners through
...
publisher/loader. The public writer captures an accepted paused source either
as exact-2.12 Full state plus a complete `BANGM2A` image or as exact-2.13 Diff
state plus a `BANGD2A` layer. A fresh process consumes a Full pair, a proven
zero-root Diff layer privately promoted into an unlinked complete image, or a
matching already-rebased complete Diff result through retained read-only
private File/COW mappings and one exact
...
virtio-mem aperture with block-granular plugged views. Network-bearing loads
also require one clone-local selector override per saved interface and build
fresh network/MMDS owners before construction. A vsock-bearing load validates
the saved device plus at most one `vsock_override` before authority access and
constructs fresh clone-local socket, device, metrics, dispatcher, interrupt,
connection, and cleanup ownership. The complete destination is committed
initially paused and optionally resumes through the ordinary lifecycle path.
The public loader also recognizes network/MMDS native-v2 2.11,
balloon-profile native-v2
...
Exact 2.10 adds optional virtio-mem kind 11. Signed direct and
...
Exact 2.11 adds optional network/MMDS kind 12. Signed direct and
normal-production/App Sandbox MMIO/PCI destinations cover V1 and V2 MMDS
sources, exact explicit-Paused recapture and automatic loads, complete
clone-local overrides, empty destination MMDS data followed by reseeding,
source TCP-session loss, old V2-token rejection, fresh V1/V2 success,
immutable artifacts, malformed state/memory, redaction, cancellation, both
death orders, retry, containment, and cleanup. MMDS-only contained loads
consume no vmnet authority.
Exact 2.12 adds optional vsock kind 13. Signed direct MMIO/PCI certification
creates one immutable pair while both connection directions are live, proves
their reset/loss, restores independent original-selector and override clones,
and exercises preserved guest listeners plus fresh guest-to-host and
host-to-guest multistream 4-KiB traffic with half-close and clone-local cursor
continuation. The automatic destination completes while a second destination
remains Paused with host work queued; that work makes no guest progress until
resume and reset acknowledgement. Signed normal-production/App Sandbox
MMIO/PCI coverage repeats the restored-guest protocol through exact launcher
grants and additionally covers malformed state/memory, a no-device override,
missing authority, pathname replacement, cancellation, independent
worker/launcher death, retry, redaction, and cleanup.
Current 2.13 adds mandatory Diff kind 14. A tracked source selects its unified
guest-dirty pages; an untracked source selects every current guest-memory
range. Successful publication advances the exact memory lineage and resets a
tracked dirty epoch only after the layer-first/state-last pair is visible.
Cancellation or any pre-visible failure restores the prior lineage. A restored
destination starts from the proven result binding, so later Diff recapture
names that image as its predecessor.

Public rebase materializes one complete native-v2 result by applying a verified
successor layer to its exact base. Repeated layers must be rebased in lineage
order, each time using the preceding complete result. Rebase never merges
state: every Diff already carries complete final state, and restore uses the
state paired with the last applied memory layer/result. The macOS tools replace
the base through a private staged no-clobber transaction and leave the Diff
immutable. Signed real-HVF evidence covers both command surfaces; the ordinary
production bundle additionally creates, describes, loads Paused, and resumes a
tracked zero-root exact-2.13 Diff through the nested App Sandbox worker over
MMIO and PCI.

The Firecracker-shaped snapshot editor now inspects every admitted frozen
native-v1 and exact native-v2 2.3–2.13 state document. `version` returns its
exact native token; `vcpu-states` and `vm-state` emit deterministic schema
`bangbang.snapshot-editor.info.v1` JSON with explicit portable values, literal
host-authority redaction, and domain-separated confidential-state equality
fingerprints. Reviewed aarch64 register editing admits only a closed 67-ID
Firecracker v1.16.0 scalar registry and publishes a distinct canonical
owner-only state through the descriptor-anchored no-clobber transaction. It
does not edit memory or claim raw KVM-vector compatibility.

Signed production certification applies one DBGBVR0 value edit to real Full
2.12 and zero-root Diff 2.13 state over MMIO and PCI, preserves every other
inspection field including memory and complete Diff binding, and restores that
exact edited state with the unchanged memory/layer through the fixed launcher
and nested App Sandbox worker. Each destination is Paused first, explicitly
resumes, reaches guest `SYSTEM_OFF`, and preserves original and edited bytes
and inode facts after adopted pathname replacement.
...
- Create is paused-state-only and supports `Full` and `Diff` for a 1–32-vCPU source
  with a configured boot source, complete live serial state, optional entropy,
  balloon, virtio-mem, network/MMDS, and one vsock, and zero or 1–64 ordered
  regular-file block/pmem devices. Boot timer and vhost-user are not admitted.
...
- The profile-3 device graph persists each ordered block or pmem
  ...
  persistence to vhost-user or host-adapter state. Entropy's
  ...
  Network/MMDS kind 12 carries its selected MMIO or PCI placement in exact
  2.11 without persisting host-adapter authority. Vsock kind 13 carries its
  selected placement in exact 2.12 without persisting socket authority or live
  work.
- Before applying those native-v2 profile exclusions, paused create asks
  ...
  2.6, 2.7, 2.8, 2.9, 2.10, 2.11, 2.12, and current 2.13.
...
- An admitted create holds one scoped supervisor transaction from FIFO
  ...
  captures aggregate state including normalized vsock reset/cursor state.
  Full streams complete mixed base/aperture memory; Diff selects unified dirty
  pages when tracking is active and every current page otherwise, then streams
  a layer against the exact live lineage base,
...
- Load is pre-boot-only and requires a pristine process except logger/metrics.
  ...
  only `File`. Full retains the read-only complete-memory descriptor. Diff
  either retains a matching rebased complete result or verifies a zero-root
  layer and promotes it through owner-only, pre-unlinked private staging. An
  image-based layer presented without its exact base is rejected before guest
  memory, resource owners, HVF, or VM construction. Destination mappings remain
  private so guest writes cannot modify the pair. Frozen native-v1
...
- A valid current native-v2 load performs full bounded state, memory, FDT,
  ...
  virtio-mem, network, MMDS, and vsock
...
- Native-v2 `Uffd`, dirty-tracked or bypassing-consumer native-v1
  ...
  certification.
  Full/File or Diff/File load can enable a clean destination dirty epoch,
  independently of the source. A tracked source resets only after visible Full
  or Diff publication, and uncertain post-commit reset failures require source
  teardown rather than risking stale dirty lineage.
...
- The checked
  ...
  certifies all 14 directly named API, live, and snapshot records as terminal.
  Exact 2.12 evidence covers source reset/capture, direct and contained
  authority, empty restored work, reset acknowledgement/RX gating, live TX,
  reconnect through preserved listeners, override/no-device behavior,
  independent immutable clones, cursor continuation, and bounded portability.
- `--snapshot-version` prints the current supported ceiling `v2.13.0`.
...
- Contained load accepts exact `bangbang-grant:<GrantId>` state and memory
  ...
  and is created only from destination descriptors; entropy requires no
  external grant and constructs fresh destination-local owners after complete
  state/resource preflight. Exact 2.11 network loads require a complete,
  unique override set naming every saved interface; MMDS-only selectors require
  no contained grant or vmnet authority. Missing, duplicate, and unknown
  interface IDs reject with selector values redacted before construction.
  Exact 2.12 Full and exact 2.13 Diff vsock loads then resolve the captured selector or one
  `vsock_override` through the same exact no-fallback transaction. A no-device
  override, wrong resource class, substitution, missing/extra grant, or changed
  identity rejects before publication with selector and authority values
  redacted.
...
- The runtime can encode a bounded state-embeddable GPA manifest, stream a full
  ...
  publisher-owned staging writer to complete capture and requires a closed
  Full image or Diff layer commitment; the public load transaction consumes
  only a matching state/result pair.
...
### Native V2 2.8 Entropy Activation

#1665 activates exact `2.8.0` and #1666 certifies its public continuation and
containment boundary. Kind `8` serial remains mandatory, kind `7` remains the
optional unchanged profile-3 storage graph, and new optional kind `9` contains
one bounded `BANGEN2\0` profile-1 entropy value. Entropy may appear without
storage or alongside it; its transport must match the process-selected MMIO or
PCI product transport.

The entropy value has a fixed header and exactly three ordered sections:
entropy-local state, common virtio state, and exact MMIO or PCI transport
state. It retains external rate-limiter configuration, negotiated and
available features, activation, one queue's mapped geometry and cursors, both
token-bucket budgets/bursts/refill ages, at most one pending descriptor,
host-time-free none/immediate/delayed retry intent, pending notifications and
interrupt intents, and canonical placement. Decode cross-validates queue
ranges against the immutable File/COW memory image, external configuration
against bucket state, pending work against retry intent, and every transport
field before owner construction.

Random bytes, host-source handles or identity, metrics, scheduler/notifier
objects, interrupt authority, routes, endpoints, `Instant`, and absolute
deadlines do not enter the artifact. Each destination instead creates one fresh
OS entropy source, empty metrics set, scheduler, notifier, interrupt/route
authority, and MMIO or PCI endpoint. A retained delay is reconstructed
relative to the destination clock. The scheduler completes an already
outstanding Linux `/dev/hwrng` request without another guest queue kick.

Focused fixed fixtures and hostile mutations cover exact encoding, all
queue/limiter/pending/retry relations, transport placement, construction,
publication, cancellation, and rollback. Deterministic factory counts prove a
new source per MMIO/PCI destination without pointer identity or random-byte
comparison. Signed direct and normal-production/App Sandbox matrices cover
entropy-only-relative-to-serial and storage-plus-entropy products, explicit
and automatic resume, nonempty restored reads, fresh metrics, recapture,
immutable same-process and fresh-process clones, pathname replacement,
malformed state, graceful cancellation, worker-first and launcher-first death,
redaction, and exact cleanup. Exact 2.3 through 2.7 readers retain their
unchanged formats.

### Native V2 2.9 Balloon Activation

#1680 activates exact `2.9.0` and #1681 certifies its public continuation and
containment boundary. Kind `8` serial remains mandatory; optional kind `7`
storage and kind `9` entropy retain their exact codecs; new optional kind `10`
contains one bounded `BANGBL2\0` profile-1 balloon value. Independent
storage/entropy/balloon presence yields all eight products, and every present
virtio component uses the process-selected MMIO or PCI transport.

Kind 10 has one fixed header and ordered balloon-local, common-virtio, and
transport sections. It retains external configuration; available/negotiated
features and activation; guest-visible target and actual values; exact two
through five active queue mappings/cursors/notification and interrupt intent;
the polling interval, all 16 latest optional statistics, and at most one legal
pending statistics descriptor; checked hint host/guest/last-command history and
acknowledgement policy; canonical host-side inflated-PFN accounting; and exact
MMIO or PCI placement. Accounting admits at most 262,144 canonical
`(start_pfn, page_count)` ranges and the complete component is capped at 4 MiB
under the 16-MiB state-file ceiling. Every range is checked, nonoverlapping,
and wholly mapped in the adopted destination `GuestMemory`.

Guest-memory borrows and host virtual addresses, timers and absolute deadlines,
metrics, reclaim advisers, notification/interrupt/dispatcher handles, PCI
registry authority, threads, endpoints, and cleanup ownership do not enter the
artifact. Every destination constructs those owners fresh. Guest balloon PFNs
always denote 4-KiB pages; Darwin discard separately aligns and coalesces to
destination host pages, normally 16 KiB, and remains best effort without a
synchronous RSS promise.

Capture validates the complete hint tuple and normalizes an active host/config
command to `VIRTIO_BALLOON_FREE_PAGE_HINT_DONE`; no source acknowledgement is
awaited. Restore preserves checked history but starts no old hint run, so a new
destination-local run may begin. Polling persists no timer or remaining
duration. A retained descriptor is eligible only after one full
destination-local interval after resume, and no interval advances while the
destination remains Paused. Destination metrics start empty.

Focused fixed fixtures and hostile mutations cover version, directory, flags,
reserved fields, lengths, allocation bounds, feature/layout/queue relationships,
statistics, pending work, hint state, accounting, adopted-memory mapping,
transport/order/capacity, owner construction, publication, rollback, and exact
2.3–2.8 compatibility. Public complete-product and signed HVF owner matrices
cover all four balloon-bearing products over MMIO and PCI. Signed direct and
normal-production/App Sandbox MMIO/PCI storage-plus-balloon matrices cover
explicit and automatic resume, latest and pending statistics, DONE then fresh
hinting, reporting, inflate/deflate/API accounting continuity, semantic
recapture, immutable same- and fresh-process clones, destination-only metrics,
...
independent death orders, and session cleanup. Diff, native-v2 Uffd, vsock
Firecracker artifact bytes, source owner/dirty identity, synchronous host RSS,
and unconstrained cross-host portability remain explicit non-claims.

### Native V2 2.11 Network and MMDS Activation

#1715 activates exact `2.11.0` and #1716 certifies its public continuation and
containment boundary. Required kind `8` serial and optional kinds `7`, `9`,
`10`, and `11` retain their exact codecs. New optional kind `12` contains one
bounded `BANGNW2\0` profile-1 value, up to 16 ordered `BANGNI2\0` interface
records, and one optional `BANGMD2\0` MMDS configuration. Independent
presence of network/MMDS doubles the previous sixteen optional-component
products to 32, while every present virtio device keeps the process-selected
MMIO or PCI transport.

Kind 12 retains each interface ID and order, requested configuration, captured
inert selector identity, requested and realized MAC/MTU, backend class,
available/negotiated features, queue mappings and cursors, interrupt intent,
limiter buckets and destination-relative retry intent, exact MMIO/PCI
placement, and MMDS interface selection, version, IPv4 address, and IMDS
compatibility. The public component cap is 512 KiB; the exact 16-interface
worst case is 83,552 bytes. Source provider handles, packet-I/O owners,
callbacks, cached or peer packets, active ARP/TCP state, MMDS data, token keys
and tokens, metrics, generation owners, and absolute clocks or deadlines never
enter the artifact.

Every 2.11 network load requires one clone-local backend selector override for
each saved interface. The interface-ID set must be exact and unique; missing,
duplicate, and unknown records reject before owner construction, and selector
contents stay redacted. A destination reconstructs fresh providers, packet-I/O
owners, metrics, schedulers, routes, endpoints, queues, callbacks, and MMDS
stacks. The MMDS store starts as JSON `null` until the operator reseeds it,
source TCP connections are lost, and source V2 tokens fail against the fresh
instance-bound key. A new V1 session or newly minted V2 token can read the
reseeded destination data.

Focused codec, graph, override-set, capacity, placement, limiter, artifact,
materialization, cancellation, rollback, and process tests cover all 32
products and both transports. Signed direct and normal-production/App Sandbox
MMIO/PCI matrices cover V1 and V2 sources, explicit-Paused recapture and
automatic destinations, immutable state/memory, empty and reseeded MMDS,
source-connection loss, old-token rejection, fresh-session success, fresh
...
#1735 activates exact `2.12.0` and #1736 certifies its public continuation,
clone, failure, and containment boundary. Required kind `8` serial and optional
kinds `7`, `9`, `10`, `11`, and `12` retain their exact codecs. New optional
kind `13` contains one bounded `BANGVS2\0` profile-1 value with the guest CID,
logical selector, next host-local port cursor, common virtio features and
state, three queues, interrupt/reset state, and coherent MMIO/PCI placement.
Independent vsock presence doubles the previous 32 optional-component products
to 64. The calculated worst case is 640 bytes and the defensive component cap
is 64 KiB.

Connections, accepts, packets, wakeups, deadlines, metrics, descriptors,
grants, sessions, host handles, and source socket ownership never enter kind
13. Capture closes every source connection, queues `TRANSPORT_RESET`, retains
the cursor and portable device state, and normalizes all live work away.
Restore constructs fresh clone-local socket/device/metrics/dispatcher/
interrupt/connection/cleanup owners. RX remains gated until the guest consumes
the reset event while TX stays live; guest listener configuration persists and
can accept fresh traffic.

Load uses the saved logical selector unless one `vsock_override` replaces it.
No-device overrides and invalid selectors fail before authority access. Direct
mode owns one exact inode; contained mode resolves one exact
directory/listener/connector transaction without ambient fallback. Each
immutable clone starts from the same saved cursor, advances only its own cursor,
and owns distinct socket, connection, metric, session, and cleanup state.

Focused format, graph, placement, override, authority, stage-failure,
cancellation, retry/terminal, replacement, adoption, death, redaction, and
cleanup tests cover both transports and all 64 products. Signed direct MMIO/PCI
matrices prove old bidirectional stream loss, reset acknowledgement, Paused RX
gating with TX progress, preserved listeners, 4 guest-to-host plus 16
host-to-guest deterministic 4-KiB streams, symmetric half-close, exact cursor
continuation, simultaneous independent clones, immutable artifacts, fresh
metrics, and recapture. Signed normal-production/App Sandbox MMIO/PCI coverage
crosses the launcher/worker grant boundary and adds malformed state/memory,
no-device override, missing authority, pathname replacement, graceful
cancellation, both independent death orders, later valid retry, containment,
redaction, and cleanup. Firecracker bytes, live-peer migration, automatic
socket/grant migration, Diff/native-v2 Uffd, editing tools, and unconstrained
cross-host portability remain explicit non-claims.
...
earlier components and adds optional virtio-mem kind 11. Minor 11 retains all
earlier components and adds optional network/MMDS kind 12. Minor 12 retains all
earlier components and adds optional vsock kind 13. Every directory entry is
semantic, singleton kinds appear exactly once, and per-vCPU instances are
contiguous:
...
| `(7, 0)` | `BANGD2A\0` exact profile-1 singleton root, profile-2 ordered block graph, or profile-3 ordered block-and-pmem graph | exactly one in 2.4/2.5/2.6; optional profile 3 in 2.7/2.8/2.9/2.10/2.11/2.12, after time state |
| `(8, 0)` | `BANGSR2\0` exact endpoint intent, limiter configuration, complete UART state, RX FIFO, and pending work | exactly one in 2.7/2.8/2.9/2.10/2.11/2.12, after optional storage |
| `(9, 0)` | `BANGEN2\0` exact entropy configuration, queue, dual buckets, pending/retry, common virtio, and MMIO/PCI transport state | optional in 2.8/2.9/2.10/2.11/2.12, after serial |
| `(10, 0)` | `BANGBL2\0` exact balloon configuration, variable queues, statistics/pending work, hint history, accounting, common virtio, and MMIO/PCI transport state | optional in 2.9/2.10/2.11/2.12, after entropy |
| `(11, 0)` | `BANGME2\0` exact virtio-mem configuration, config space, queue, plugged bitmap, common virtio, and MMIO/PCI transport state | optional in 2.10/2.11/2.12, after balloon |
| `(12, 0)` | `BANGNW2\0` exact ordered network interfaces plus optional `BANGMD2\0` MMDS configuration, with source backend/session state excluded | optional in 2.11/2.12, after virtio-mem |
| `(13, 0)` | `BANGVS2\0` exact CID, logical selector, host-local cursor, queues/common virtio, reset/interrupt state, and MMIO/PCI placement, with live socket/connection state excluded | optional in 2.12, after network/MMDS |
...
virtio-mem. Exact 2.11 requires 7–43 depending on optional storage, entropy,
balloon, virtio-mem, and network/MMDS. Exact 2.12 requires 7–44 depending on
optional storage, entropy, balloon, virtio-mem, network/MMDS, and vsock. All
...
exact 2.6/2.7/2.8/2.9/2.10/2.11/2.12; kind 8 requires profile 1 at exact
2.7/2.8/2.9/2.10/2.11/2.12; kind 9 requires profile 1 at exact
2.8/2.9/2.10/2.11/2.12; kind 10 requires profile 1 at exact
2.9/2.10/2.11/2.12; kind 11 requires profile 1 at exact 2.10/2.11/2.12; kind 12
requires profile 1 at exact 2.11/2.12; and kind 13 requires profile 1 at exact
2.12. Every payload requires its exact fixed header size, zero flags/reserved
...
profile word. Exact 2.3 requires the legacy zero value; exact 2.4 through 2.12
...
exact default process shell. Exact 2.4 through 2.12 and the current 2.13 result
product instead require the
...
optional 2.9 balloon, 2.10 virtio-mem, 2.11 network/MMDS, and 2.12 vsock
components after checking every cross-binding. Exact 2.13 additionally closes
the kind-14 Diff lineage, selection, and result binding before reconstructing
that unchanged result product. The
...
profile-2 block owners, or profile-3 block/pmem owners, and from exact 2.7
onward reconstructs the complete UART with fresh default or configured
destination endpoints. Exact 2.8 onward additionally reconstructs optional
...
interrupt, dispatcher, route, metrics, endpoint, and cleanup ownership. Exact
2.11 additionally reconstructs optional network/MMDS from a complete
clone-local override set with fresh provider, packet-I/O, metrics, scheduler,
route, endpoint, callback, and MMDS owners. Exact 2.12 additionally
reconstructs optional vsock from the saved or overridden logical selector with
fresh socket, metrics, dispatcher, interrupt, connection, endpoint, and cleanup
owners, the saved clone-local cursor, empty live work, and a reset-event RX
gate. It
commits that owner into the normal lifecycle initially `Paused`. Public
native-v2 activation uses this exact boundary; vhost-user
serialization/restoration remains outside the admitted profile.
...
| Auxiliary and host | Limiter retry threads retain deadlines and can request vCPU cancellation during ordinary running or paused operation. Native snapshot publication temporarily quiesces every applicable auxiliary publisher through artifact commit and the post-publication hook. The synchronous process owner cannot dispatch another API/MMDS/controller mutation or periodic callback until publication returns. Vmnet and vsock host handles, retained streams, peers, and host/kernel buffers remain outside snapshot state. Network capture preserves only portable kind-12 metadata. Vsock capture joins its transient poller, closes source connections, queues reset, and preserves only portable kind-13 device/cursor/selector state. |
...
The current native-v2 profile composes it with the complete regular-file
storage, serial, entropy, balloon, virtio-mem, network/MMDS, and vsock drain,
graph, resource, and destination transactions described above. It still is
not a generic snapshot-ready contract for vhost-user, boot-timer state, or
other unmodeled host resources.
...
Native-v2 exact 2.13 serializes and restores Diff layers and can materialize a
rebased complete result. Those artifacts, dirty-tracked or unsupported-profile
external paging, and broader snapshot compatibility remain outside native-v1.
...
follow the implemented metadata/discard/reconnect policy: kind 12 retains
portable network/MMDS configuration and requires exact destination overrides,
while kind 13 closes source connections, retains portable device/cursor/
selector state, and reconstructs fresh socket ownership. Live host descriptors
and opaque kernel buffers never enter guest snapshot state.
...
The supported Full/Diff, memory-rebase, inspection, and reviewed-edit baseline
is complete. Broader
snapshot support still requires:

- explicit per-drive restore overrides and external-resource policy beyond the
  current exact complete-set network/vsock and storage authority contracts;
- native-v2 external paging/Uffd policy and evidence;
- Firecracker-byte interoperability, authentication, live-peer migration, and
  broader cross-host portability if those are admitted.
...
Shared dirty epochs, exact-2.13 Diff artifacts, and the complete supported
optional-device product are active. Public Full remains exact 2.12; public Diff
...
publisher/loader; native envelope version reporting and read-only description
