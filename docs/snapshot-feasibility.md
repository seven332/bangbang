...
- HVF vCPU state capture: X0-X30, PC, and CPSR; raw SP_EL0, SP_EL1, ELR_EL1,
  ...
  restore path on the owning thread. The six general-, core-system-, exception-
  register, execution-control, thread-context, and translation primitives
  already supply only their isolated, nontransactional owner-thread write
  sequences; none is snapshot validation, wider ordering, rollback,
  feature/MMU transition, dependent-memory or maintenance coordination, or load
  orchestration.
