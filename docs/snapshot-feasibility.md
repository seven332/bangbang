...
- HVF vCPU state capture: X0-X30, PC, and CPSR; raw SP_EL0, SP_EL1, ELR_EL1,
  ...
  VBAR_EL1; raw ACTLR_EL1 and CPACR_EL1; raw CSSELR_EL1 cache selection;
  ...
  Debug-control capture remains incomplete without breakpoint/watchpoint
  comparators, DFR0 implemented-count policy, HVF debug trap configuration,
  feature and writable/status-bit validation, security policy, and ordered
  restore; raw MDCCINT/MDSCR values must not be treated as safe restore input.
