...
- Interrupt-controller state: #1178 captures Apple's stable, versioned opaque
  ...
  pre-first-run owner-thread apply, and #1180 captures all ten EL1 ICC registers
  exposed by the current SDK. ICC setters, `ICC_SRE_EL2`, ICH/ICV inventory,
  compatible composite ordering, host-update preflight, multi-vCPU association,
  a cross-step no-run lease, and a bangbang schema remain required before
