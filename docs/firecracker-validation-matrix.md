...
| Guest-memory backing profiles | anonymous-default and descriptor-backed-shared RAM foundation plus direct file-backed pmem implemented; public vhost activation deferred | unit, signed HVF, signed executable, signed production bundle, docs | #1439, #1441, #1444 | Shared-RAM selection remains an internal startup resource: exact-sized owner-only files are unlinked before publication, descriptors are reserved before mmap, exports clone only descriptor/offset/length, dynamic RAM inherits the profile, native snapshot streaming supports either profile internally, and Darwin shared discard uses zero-safe hole punching. Pmem remains outside ordinary RAM and instead shares one reference-counted file/private-tail mapping directly with HVF. Signed direct evidence covers RAM protection/dirty/readback/unmap plus pmem pre-flush coherence, exact guest protection, a one-mapping virtual-size bound with generous resident slack, root boot, and teardown; the normal production bundle proves a granted pmem root still uses the launcher-opened descriptor after pathname replacement with unchanged App Sandbox plus Hypervisor entitlements. Public `socket` drives remain rejected, no external backend receives RAM, public native-v1 restore remains anonymous, and broad vhost/pmem aggregates stay under audit. |
