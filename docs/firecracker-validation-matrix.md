...
| Observability and serial | **Implemented subset.** The documented logger and metrics transactions, serial output/input policy, exact native-v2 serial state, redaction, and cleanup are supported. Log rotation, remote telemetry, and a public serial streaming API are not claimed. | [Observability contract](firecracker-compatibility.md#firecracker-v1160-observability-contract), [serial contract](../compat/firecracker/v1.16.0/serial-contract.md) |
| Current snapshots | **Implemented subset.** Public Full/File emits bangbang-native `2.12.0` state plus a complete image; Diff/File emits `2.13.0` state plus a tracked-dirty or all-current-page layer. Zero-root layers and matching rebased results load; standalone image-based layers fail closed. Exact native-v2 2.3–2.12 readers remain accepted. Artifacts are not Firecracker snapshot bytes. | [Snapshot current status](snapshot-feasibility.md#current-status), [native-v2 structure](snapshot-feasibility.md#native-v2-structural-state-foundation) |
| Snapshot rebase tools | **Implemented tool subset.** Deprecated `rebase-snap` and replacement `snapshot-editor edit-memory rebase` share one macOS native-v2 staged no-clobber transaction, deterministic redacted outcomes, ordered repeated-layer semantics, and signed real-chain evidence. State merging and the editor's inspection/register operations are excluded. | [Snapshot Rebase Tools](firecracker-compatibility.md#snapshot-rebase-tools), [Diff and rebase contract](../compat/firecracker/v1.16.0/snapshot-diff-rebase-contract.md) |
