...
| --- | --- | --- | --- | --- |
| Direct pmem mapping and root boot | implemented for startup MMIO/PCI and non-root runtime PCI; complete capture-ready live state implemented; serialization/restore deferred | unit, process e2e, signed HVF capture equality, signed executable preflight, signed production bundle, docs, pinned-source compare | #1439, #1444, #1448 | One reference-counted file/private-tail mapping is registered directly with HVF; there is no anonymous data copy or writeback path. Writable files use a shared host mapping. Read-only descriptors use a private write-capable host mapping required by HVF while guest permissions stay read-only, so accidental host writes are COW and signed guest writes fault without changing the backing. Flush synchronizes exactly the nonzero file prefix with `MS_SYNC`; the aligned tail is volatile. Exactly one ordinary block or pmem root is accepted before startup, pmem order selects `/dev/pmem<i>` plus `ro`/`rw`, and runtime root mutation remains rejected. Signed evidence covers pre-flush writable coherence, read-only protection, MMIO `ro` root, PCI `rw` root, exact teardown/reuse, and a normal contained root using the launcher-opened descriptor after pathname replacement with unchanged entitlements. Pmem remains outside ordinary RAM dirty epochs; #1448 delivers the exact live mapping/config/queue/limiter/retry/transport handoff, while #1450 owns serialized restore and the broad pmem aggregate. |
| Guest-memory backing profiles | anonymous default plus descriptor-backed boot RAM and stable virtio-mem aperture implemented; direct and contained vhost-user block support dynamic memory | unit, real Unix stream/SCM_RIGHTS/pipe/kqueue, signed HVF, signed executable, signed production bundle, docs | #1439, #1441, #1443, #1444, #1445, #1449, #1462 | Ordinary-only VMs retain anonymous RAM. Vhost or virtio-mem startup selects unlinked sparse shared backing; virtio-mem reserves one complete deterministic aperture before block preparation, while only plugged views enter CPU/HVF mappings, FDT/current accounting, byte access, and dirty metadata. Every initial or runtime backend receives one immutable guest-ordered table containing boot RAM plus that aperture (at most three arm64 regions), can read/write currently unplugged bytes, and receives no unrelated mapping. This is an explicit trusted confidentiality/integrity/availability capability without Darwin memfd seals, a bundled backend, helper, or backend-policy claim. Exact offset discard follows committed unplug, backend death/DELETE drops only frontend clones, and shutdown drops the reservation after active views. Signed direct and contained MMIO/PCI guests prove both configuration orders, storage I/O across `0 -> 128 MiB -> 0`, exact stable table geometry, CONFIG refresh, Running/Paused runtime attach/delete/reuse, unchanged entitlements, no helper, and vhost snapshot rejection before artifacts. Broader storage aggregation remains #1450 work. |
| Capability inventory enforcement | structural inventory implemented; capability audit in progress (86 implemented, 312 audit, 3 feasible-missing, 17 proven impossible) | unit, workspace CI, docs, pinned-source compare | #1348, #1349, #1420, #1421, #1422, #1423, #1444, #1445, #1446, #1447, #1448, #1449 | The machine-owned v1.16.0 source manifest records exact 26/38/44/152 Swagger identities, 23 configured executable arguments, three non-Swagger DELETE routes, public-tool operations/arguments, and an explicit source corpus. A separate human overlay owns every disposition and adds cross-leaf semantic records. Delivery validation permits honest `audit-required`/missing work; final mode rejects it. The 17 terminal exclusions cover strict runtime seccomp/jailer, exact Linux hugetlbfs `2M`, and narrow KVM/static CPU categories. #1420-#1423 promote runtime device hotplug, #1444 promotes exact pmem leaves, #1445/#1447/#1449 record direct and contained vhost behavior, and #1446 promotes the exact `Drive.io_engine` and applicable block-I/O-engine corpus after portable Async execution, multi-drive generation routing, live replacement, hotplug/delete/reuse, and signed direct/contained proof. #1448 records complete capture-ready regular/Async/pmem traversal plus typed vhost rejection without promoting any disposition. Broad pmem/storage serialization and restore, snapshot, vmnet, and observability owners remain nonterminal. |
...
| Product PCI and modern virtio-pci | all-virtio startup plus aggregate block, pmem, and network runtime hotplug implemented; complete storage capture state implemented; persistence/restore deferred | unit, process e2e, signed HVF capture equality, signed executable, signed production bundle, docs, pinned-source compare | #1416, #1417, #1418, #1419, #1420, #1421, #1422, #1423, #1444, #1448 | The supported macOS arm64/HVF product path accepts exact `--enable-pci` after target/GIC-MSI preflight and selects one immutable all-virtio transport. It owns Firecracker-shaped segment-0 ECAM/BAR apertures, deterministic generation-bound slots/functions, one 512-KiB BAR per endpoint, exact fixed MSI-X demand plus worst-case runtime headroom, and independently revocable full-pool registries. Balloon, block, network, pmem, vsock, entropy, and virtio-mem publish in Firecracker order; legacy virtio MMIO nodes are suppressed while platform devices remain MMIO. Default startup remains all-virtio-MMIO with `pci=off`. The retained manager supports failure-atomic Running/Paused non-root block, pmem, and network PUT/DELETE under one 31-endpoint budget with manual guest coordination, serialized mutation, type-scoped IDs, generation-bound metrics, exact teardown, and capacity reuse. Pmem registers a cloned lease on its one direct file/private-tail mapping, synchronizes only `file_len` with `MS_SYNC`, and retains that lease across failed unmap/rollback. Signed product and focused gates cover every startup class, read-only MMIO and writable PCI pmem root boot, direct pre-flush coherence, targeted flush, runtime remove/reuse, and contained descriptor identity; network also proves real MMDS exchange without vmnet authority. Native-v1 create runs the complete storage preflight before rejecting PCI and before artifact mutation; load keeps its pre-mutation PCI rejection. Automatic guest notification, PCI serialization/restore, external vmnet certification, and KVM ITS identity remain deferred. |
| Drives and virtio-block | MMIO-default Sync/Async regular and macOS block-special startup plus all-PCI runtime block-special insertion implemented; Firecracker-compatible backing-metadata GET_ID implemented; live limiter and regular/block replacement implemented; PCI file/vhost PUT/DELETE and active vhost config refresh implemented; complete live capture handoff implemented with block-special native-v1 rejection | unit, real regular/block file/pipe/kqueue and Unix stream/SCM_RIGHTS/shared mapping, fixed grant/control codecs, api socket, process e2e, signed HVF capture equality, signed executable preflight, signed production bundle | #539, #916, #962, #992, #994, #996, #998, #1020, #1068, #1268, #1304, #1362, #1418, #1419, #1420, #1443, #1445, #1446, #1447, #1448, #1449, #1460, #1461, #1464, #1465, #1466 | File-backed drives accept an existing regular file or exact macOS block-special descriptor with default Sync or explicit Async over MMIO/PCI and direct/contained ownership. Direct block geometry and persistence use public `DKIOCGETBLOCKSIZE`, `DKIOCGETBLOCKCOUNT`, and `DKIOCSYNCHRONIZECACHE`. App Sandbox permits worker positional I/O but denies those ioctls, so contained BBG2 grants bind exact kind/identity/access/status/geometry and a fixed descriptor-7 `BBC1` facet permits only fresh inspect/cache-sync on the launcher's retained descriptor; it adds no entitlement, path lookup, enumeration, generic ioctl, or ambient reopen. GET_ID derives the exact 20-byte Firecracker v1.16.0 decimal `st_dev`/`st_rdev`/`st_ino` identity from the opened descriptor and updates atomically with regular-to-block, block-to-regular, or block-to-block replacement. One lazy bounded executor and pending rate-limited queue state preserve Async and empty-notification retry behavior. Four new signed direct/contained MMIO/PCI block-special scenarios prove read-only/read-write, complementary Sync/Async and Unsafe/Writeback, 4/6/8-MiB capacity/config refresh, GET_ID, limiter retry, guest read/write/flush/readback persistence, rollback, capture twice without artifacts, DELETE, same-ID/slot reuse, unchanged entitlements, complete owner release, and strict virtual-media cleanup. Native-v1 stays regular-file/Sync-only and rejects block-special after fresh capture inspection but before artifacts. Vhost behavior remains separately implemented. Automatic guest PCI notification, storage serialization/restore beyond native-v1, vhost snapshot support, and the broad #1450 storage aggregate remain deferred. |
...
| Remaining Firecracker devices | implemented supported subsets; transport and profile limits explicit | unit, api socket, process e2e, signed HVF, signed executable, signed production bundle, docs | #544, #797, #800, #802, #804, #806, #808, #810, #812, #814, #815, #818, #869, #873, #875, #877, #888, #890, #892, #894, #896, #898, #900, #902, #904, #905, #908, #910, #912, #914, #920, #922, #926, #928, #930, #932, #934, #936, #938, #940, #960, #962, #964, #968, #970, #972, #988, #990, #1000, #1002, #1016, #1018, #1024, #1329, #1328, #1330, #1331, #1335, #1336, #1337, #1338, #1362, #1418, #1419, #1420, #1421, #1422, #1444 | Balloon implements validated inflate, hinting, statistics, and free-page reporting queues with best-effort Darwin discard and signed reporting evidence. Pmem implements selected-transport direct mapping, exact-prefix targeted rate-limited flush, deterministic root boot, exact contained backing ownership, and PCI-only transactional Running/Paused non-root PUT/DELETE with generation-safe metrics, failure rollback, and exact ID/slot/guest-range reuse. Signed HVF proves writable backing visibility before flush and read-only guest protection; signed root guests cover MMIO `ro`, PCI `rw`, and normal contained descriptor identity after pathname replacement. Network implements selected-transport startup and PCI-only runtime PUT/DELETE with independent limiter/retry/metrics/MMDS or vmnet ownership, coordinated teardown, and exact ID/MAC/slot reuse; networkless guests prove real MMDS exchange. Hidden and signed product PCI gates reuse the canonical device implementations. Entropy implements the 64-KiB request cap, 64-byte aarch64 seed, optional limiter/retry, metrics, and signed `/dev/hwrng` proof. Paired reusable-page accounting, optional-device snapshots and pmem dirty-state capture, ARM PVTime, mutable VMClock restore, automatic PCI notification, and externally certified vmnet connectivity remain explicit limits. |
