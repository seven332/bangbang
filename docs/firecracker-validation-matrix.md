...
| Machine and boot configuration | partial | unit, api socket, process e2e, signed HVF, signed executable | #538, #1284, #1285 | Pre-boot machine config, empty/no-op CPU config, value-redacted classification of non-empty KVM capability, KVM vCPU-init feature, arm register modifier, mixed, and deprecated static CPU-template requests, boot source, kernel/initrd loading, FDT generation including arm64 `/chosen/linux,pci-probe-only` and 64-byte `/chosen/rng-seed`, direct-rootfs boot paths, ordered owner-thread vCPU topology, owning concurrent boot-session coordination with active-only batch cancellation, all-MPIDR FDT input, indexed PSCI CPU_ON/level-0 affinity transitions, per-vCPU timer PPIs, internal signed Linux CPU1 execution, and public host-limited SMP startup are covered. Public `InstanceStart` admits `1..=min(32, host_max)` and keeps capacity/construction failures before session retention or `Running`. Signed executable proof configures two vCPUs through the API and observes independent pinned CPU0/CPU1 progress without fixed sleeps. Custom/static CPU templates remain intentionally unapplied on arm64 HVF; any writable subset needs a separate Apple feature-view and snapshot contract. Multi-vCPU native-v1 snapshots, CPU_OFF/broader hotplug, reboot-in-place, and cross-host portability remain deferred. |
