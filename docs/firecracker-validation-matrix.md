...
| Observability and serial | **Implemented subset; logger aggregate certified.** The exact pinned logger corpus and delivery/filtering/loss/redaction semantic are terminal across focused, process, signed direct, and ordinary-production/App Sandbox evidence. The documented metrics transactions, serial output/input policy, exact native-v2 serial state, redaction, and cleanup are supported subsets. Log rotation, durable or remote telemetry, developer tracing, and a public serial streaming API are not claimed. | [Observability contract](firecracker-compatibility.md#firecracker-v1160-observability-contract), [logger contract](../compat/firecracker/v1.16.0/logger-contract.md), [serial contract](../compat/firecracker/v1.16.0/serial-contract.md) |
