...
| --- | --- | --- | --- | --- |
| Native-v2 state, File/COW memory, artifact transaction, public process lifecycle, and focused reconstruction | current 2.3 multi-vCPU Full/File lifecycle implemented and verified; frozen native-v1 remains readable | unit, fixed binary fixtures, hostile component/registry/FDT mutations, artifact fault/authority checks, API/process family routing, descriptor/mapping and restore-stage fault injection, signed three-vCPU HVF, signed public two-vCPU executable, signed App Sandbox and production bundle, docs | #1490, #1525, #1526, #1528, #1529, #1566, #1567, #1568, #1569, #1575, #1576, #1577, #1578 | The immutable `2.0.0`, memory-only `2.1.x`, and base-platform `2.2.x` states remain structurally readable; the current writer emits `2.3.0` with singleton memory/machine/global/topology/time kinds and contiguous per-vCPU state. Exact state-bound memory metadata validates before retained read-only private File/COW mappings. The public producer admits only Paused Full sources with a configured boot source, 1–32 vCPUs, no drives/devices/MMDS/PCI/boot timer, default serial configuration, and canonical live UART state. Public load opens and decodes state once, routes native-v2 to File/COW, frozen native-v1 to File/Uffd, and pinned Firecracker or unknown bytes to explicit incompatibility. Both families publish Paused before optional resume. Signed direct evidence proves two-vCPU continuation, private guest writes, immutable repeated loads, Uffd rejection, and real closed-stdout terminal cleanup; signed production evidence proves exact state/memory grants, retained descriptor identity after path replacement, App Sandbox worker lifecycle, collision/staging recovery, and cleanup. CRCs cover state/metadata only; external authentication/encryption is required. General devices/serial state, native-v2 Uffd, Diff/merge, overrides, editing, Firecracker bytes, and broad portability remain Wave 6 work. |
...
| Capability inventory enforcement | structural inventory implemented; capability audit in progress (233 implemented, 165 audit, 3 feasible-missing, 17 proven impossible) | unit, workspace CI, docs, pinned-source compare | #1348, #1349, #1527, #1546, #1547, #1548, #1549, #1550, #1551, #1552, #1553, #1554, #1555, #1578 | The machine-owned v1.16.0 source manifest and human overlay keep every disposition mechanically checked. #1578 promotes the snapshot-version argument, complete instance/version-output semantic, aggregate run operation, and minimal Full create/load public-lifecycle semantic after current-head direct/App Sandbox/production certification. Snapshot corpora and API/schema aggregates remain under audit for Diff, devices, tools, Firecracker interoperability, and portability. |
...
| Process CLI and API socket | 25 of 29 inventory records implemented and verified; two proven platform impossible; only two broad source corpora remain under audit | unit, API socket, process e2e, signed process, signed App Sandbox process, signed production bundle, pinned-source compare | #536, #545, #1008, #1010, #1048, #1058, #1060, #1070, #1092, #1260, #1302, #1352, #1365, #1368, #1384, #1419, #1578 | The checked 23-argument contract has 21 implemented leaves and two terminal seccomp platform exclusions. It covers argument precedence, Unicode identity, limits, readiness, fd-table behavior, signals including nonfatal SIGPIPE, socket ownership/cleanup, configuration startup, PCI, and current native snapshot output. `--snapshot-version` prints `v2.3.0`; bounded direct or exact-granted description reports the actual validated native-v1 or native-v2 version and explicitly rejects Firecracker bytes. The complete instance/version-output semantic and aggregate run operation are terminal. Only broad `corpus:design` and `corpus:getting-started` remain under audit for unrelated repository-wide claims. |
...
| Snapshots and restore | minimal public native-v2 2.3 Full/File multi-vCPU lifecycle implemented and verified; frozen native-v1 File/Uffd reader retained | unit, API socket, process e2e, signed HVF capture/reconstruction, signed direct executable, signed App Sandbox and production-bundle restore, docs | #543, #1263, #1264, #1390, #1395, #1396, #1441, #1525, #1526, #1528, #1529, #1554, #1575, #1576, #1577, #1578 | Public create admits only Paused Full sources with 1–32 vCPUs, configured boot metadata, no drives/optional devices/MMDS/PCI/boot timer, and canonical default UART state; it publishes current `2.3.0` memory first and state last. Public load requires a pristine destination except logger/metrics, opens/decodes state once, retains native-v2 read-only File/COW memory, and preserves frozen native-v1 eager File or macOS pager behavior. Both families validate before construction, commit Paused, and optionally resume normally; only proven-pristine preconstruction failure retries, while committed identity/time, publication, incomplete cleanup, or resume failure is terminal. Signed direct evidence proves repeated isolated two-vCPU COW destinations and a real terminal boundary; signed production evidence proves exact grants and retained descriptors. The format remains bangbang-native and unauthenticated. Diff, native-v2 Uffd, Linux UFFD compatibility, overrides, drives/devices/general serial, editing, Firecracker artifacts, and broad portability remain unsupported or under audit. |
...
| Public native-v2 Full/File endpoint activation | implemented narrow subset with frozen native-v1 reader | runtime, API socket, process lifecycle, signed direct executable, signed App Sandbox, signed production bundle, docs | #543, #1276, #1396, #1575, #1576, #1577, #1578 | Public Full create emits native-v2 2.3. Public load classifies one opened state and selects frozen native-v1 File/Uffd or native-v2 File/COW without fallback. Paused-before-resume, dirty tracking, metrics/latency, redaction, collision/cancellation, immutable repeated destinations, retained descriptors, explicit/automatic resume, terminal failure, cleanup, and exact version/description output are covered. |
