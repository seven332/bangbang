...
| Drives and virtio-block | MMIO-default and all-PCI startup implemented; PCI-only file-backed runtime PUT/DELETE implemented; direct pre-boot vhost-user block implemented | unit, real Unix stream/SCM_RIGHTS/shared mapping/pipe, api socket, process e2e, signed HVF, signed executable, signed production bundle | #539, #916, #962, #992, #994, #996, #998, #1020, #1068, #1268, #1304, #1362, #1418, #1419, #1420, #1443, #1445 | File-backed drives retain existing startup, limiter, runtime PCI, contained grant, metrics, and accepted native-v1 behavior. A direct socket drive uses an exhaustive file/vhost config matrix, bounded pre-VM discovery, exact backend-derived 60-byte config and read-only/flush features, shared RAM, one translated queue, kick/call descriptors, and the existing GIC/MSI interrupt path. Exact socket-only `GET /vm/config`, root ordering, partuuid, `Unsafe`/`Writeback`, invalid negotiation rollback, redacted terminal backend death, block metrics, and pre-artifact snapshot rejection are covered. A strict external test backend validates transferred memory and descriptors and serves signed MMIO read-only-root and PCI writable-PARTUUID-root guests with scratch read/write/flush; it is not shipped. Runtime vhost replacement/hotplug, contained socket brokerage, memory hotplug coexistence, config-change messages, and vhost snapshot state remain deferred. |
