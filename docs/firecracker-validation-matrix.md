...
| CPU templates | **Implemented subset.** A finite reviewed arm64 custom register profile, transactional selection, exact startup/readback behavior, native-v2 evidence, and signed `cpu-template-helper template dump/verify` over a real topology-common HVF application checkpoint are supported. Dump is canonical, private, and absent-only; verification is silent and value-redacted. KVM/static-template execution, strip/fingerprint operations, corpus-wide template parity, and broader cross-host portability remain separate or excluded as documented. | [Arm64 CPU-template subset](firecracker-compatibility.md#arm64-cpu-template-subset), [CPU-template dump and verify helper](firecracker-compatibility.md#cpu-template-dump-and-verify-helper), [CPU-template contract](../compat/firecracker/v1.16.0/cpu-template-contract.md), [helper contract](../compat/firecracker/v1.16.0/cpu-template-helper-contract.md) |
