...
| UFFD-equivalent snapshot paging | public macOS equivalent feasible; native-v1 runtime still rejected | checked upstream/platform ledger, unit/API rejection, signed public prototype | #1527, #1546 | The pinned Firecracker contract delegates anonymous page population and removal to an external UFFD handler. Signed arm64 macOS probes establish that task-local public Mach exceptions can mediate host faults and HVF stage-two protection can mediate guest faults while a bounded socket peer owns contents and removal state; the combined ordinary and production-entitlement cases passed population, refault-to-zero, peer-death detection, and cleanup. File/COW remains distinct, direct external Mach exceptions are over-authoritative, and `bangbang-pager-v1` will not be Linux UFFD wire-compatible. The checked ledger is nonterminal: current `Uffd` rejection remains until #1527's restore gate, and inventory promotion waits for final signed certification. |
