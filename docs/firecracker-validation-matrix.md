...
| Current snapshots | **Implemented subset.** Public Full/File create emits bangbang-native `2.12.0` for one coherent MMIO or PCI graph with required serial and independently optional supported devices. Exact native-v2 2.3–2.11 readers remain accepted. Artifacts are not Firecracker snapshot bytes. | [Snapshot current status](snapshot-feasibility.md#current-status), [native-v2 structure](snapshot-feasibility.md#native-v2-structural-state-foundation) |
