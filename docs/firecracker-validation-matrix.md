...
| macOS isolation and platform limits | production App Sandbox worker, lifecycle v2, and typed startup-grant foundation implemented; direct jailer parity platform-limited; consumer/dynamic brokerage incomplete | unit, docs, process e2e, signed App Sandbox HVF and process, signed production bundle | #545, #924, #1102, #1302, #1351, #1354, #1356, #1358 | The ordinary CLI remains uncontained. Production has a fixed unsandboxed launcher without App Sandbox/HVF authority and one separately signed nested worker with exactly both entitlements; both use Hardened Runtime. Assembly remains private, inspected, no-clobber, exclusive, and explicitly excludes the integration-only grant probe. Suspended default-close spawn retains standard streams plus fixed lifecycle-stream and grant-datagram endpoints. Static/live code validation, effective credentials/direct-parent PID, random SessionId/BatchId values, exact sequences, closed states, mandatory empty or populated atomic grant acknowledgment, and an independently validated empty namespace gate public work. Strict bounded manifests prepare no-follow existing resources before spawn; regular files use SCM_RIGHTS with exact access/type/device/inode checks, while three mutable-directory roles use fragmented one-session implicit bookmarks plus exact anchors and balanced scope. The worker exposes only a bounded redacted one-time typed registry after exact Commit; sender close is cleanup, not revocation. Signed Apple Silicon proof covers malformed bootstrap, unexpected-fd closure, outside-path denial, read-only/write-only/directory grants, mismatch rollback, cancellation and one absolute deadline, worker-first/launcher-first/both-killed cleanup, grant-bearing concurrent noninterchangeability, normal-build probe absence, API/no-API behavior, and a real sandboxed HVF guest through `SYSTEM_OFF`. Authentication remains asymmetric, and same-identifier workers share cooperative container cleanup authority. Existing Firecracker path consumers, dynamic post-Ready brokerage, vmnet/network policy, jailer limits/credentials, seccomp outcomes, launch constraints, Developer ID possession/notarization, and automatic restart remain #1351 work. |
