...
| --- | --- | --- | --- | --- |
| Process CLI and API socket | partial | unit, api socket, process e2e, signed process | #536, #545 | Firecracker-shaped startup args, API socket binding, config-file startup, no-api mode, payload limits, cleanup, and process-local resource isolation are covered for the current subset. Linux jailer/seccomp behavior is platform-limited. |
