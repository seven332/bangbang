...
| Remaining Firecracker devices | partial | unit, api socket, process e2e, docs | #544, #797, #800, #802, #804, #806, #808, #810, #812, #814, #815 | Balloon, pmem, and memory hotplug request shapes are recognized or parsed before returning device-specific unsupported faults. Valid pre-boot entropy requests without configured rate limiters now store public configuration, appear in `GET /vm/config`, and feed `InstanceStart` so the existing HVF virtio-rng MMIO/FDT device can use the session-owned host OS randomness source. Entropy remains outside current API request metrics output because Firecracker does not define entropy request counters. Config-file entropy (#815), entropy rate limiting, signed executable guest entropy e2e, and other remaining device implementations still need separate designs. |
