...
| macOS isolation and platform limits | production App Sandbox worker and per-VM session boundary implemented; direct jailer parity platform-limited; resource brokerage incomplete | unit, docs, process e2e, signed App Sandbox HVF and process, signed production bundle | #545, #924, #1102, #1302, #1351, #1354, #1356 | The ordinary CLI remains uncontained. Production has a fixed unsandboxed launcher without App Sandbox/HVF authority and one separately signed nested worker with exactly both entitlements; both use Hardened Runtime. Assembly remains private, inspected, no-clobber, and exclusive. Direct suspended spawn retains only standard streams plus one unnamed session endpoint; static and live PID code validation, effective peer credentials/direct-parent PID, a random identity, 4-KiB frames, exact sequences, and closed monotonic states gate public work. One empty locked mode-0700 worker-container namespace is independently validated before `Proceed`. Signed Apple Silicon proof covers malformed bootstrap, unexpected-fd closure, API/no-API readiness, cancellation, worker-first/launcher-first/both-killed cleanup, concurrent-session isolation, outside-path denial/redaction, and a real sandboxed HVF guest through `SYSTEM_OFF`. Authentication is asymmetric because App Sandbox denies worker-side parent-code lookup; same-identifier workers share cooperative container cleanup authority. Security-scoped grants, typed descriptor/resource brokerage, vmnet/network policy, jailer limits/credentials, seccomp outcomes, launch constraints, Developer ID possession/notarization, and automatic restart remain #1351 work. |
