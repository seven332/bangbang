...
| Observability and serial | **Implemented subset; logger aggregate, metrics API/schema, all 69 process producers, and the complete 231-field device gate are certified.** The exact pinned logger corpus and delivery/filtering/loss/redaction semantic is terminal. The dedicated process-final gate requires two implemented process profiles and an exact audit at 64 implemented, one source-neutral, four platform-zero, and no planned records. This includes generation-consistent request/startup/latency/logger values, persistent caught-panic and classified Darwin fatal-signal Stores, incremental SIGPIPE, immediate ambiguous hardware-fault exits, and macOS seccomp zero. The device-final gate requires ten implemented shared profiles and the exact #1838–#1846 audit at 212 implemented, two source-neutral, and 17 terminal platform-zero fields. A 231-leaf portable oracle covers exact active zeros, static/configured agreement, ambiguous retry, and idle shape; signed direct, App Sandbox, real-periodic production, snapshot, and actual contained-worker/supervisor tests cover decoded HVF MMIO exits and latency, terminal vCPU execution failures, MMIO/PCI interrupt attempts, active MSI-X configuration updates, literal-zero i8042/PIO/KVM-clock retention, and the applicable product boundaries. Corpus/cross-producer aggregates remain assigned to #1790. Serial output/input policy and exact native-v2 serial state remain supported subsets. Log rotation, crash durability, remote telemetry, developer tracing, and a public serial streaming API are not claimed. | [Observability contract](firecracker-compatibility.md#firecracker-v1160-observability-contract), [logger contract](../compat/firecracker/v1.16.0/logger-contract.md), [metrics contract](../compat/firecracker/v1.16.0/metrics-contract.md), [serial contract](../compat/firecracker/v1.16.0/serial-contract.md) |
...
| CPU templates | **Implemented subset.** A finite reviewed arm64 custom register profile, transactional selection, exact startup/readback behavior, and native-v2 evidence are supported. KVM/static-template execution and broader cross-host portability remain excluded as documented. | [Arm64 CPU-template subset](firecracker-compatibility.md#arm64-cpu-template-subset), [CPU-template contract](../compat/firecracker/v1.16.0/cpu-template-contract.md) |
