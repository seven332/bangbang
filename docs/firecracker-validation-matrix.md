...
| --- | --- | --- | --- | --- |
| Native-v2 state, File/COW memory, artifact transaction, public process lifecycle, and focused reconstruction | current 2.8 complete-serial multi-vCPU Full/File lifecycle with optional regular-file block-and-pmem profile-3 storage and optional entropy implemented and verified; exact entropy-free 2.7, storage-only 2.6, block-only 2.5, single-root 2.4, device-free 2.3, and frozen native-v1 remain readable | unit, fixed binary fixtures, hostile component/registry/device/FDT mutations, artifact fault/authority checks, API/process family routing, typed complete-set storage-plus-serial-plus-entropy authority and restore-stage fault injection, signed three-vCPU HVF, signed public serial and entropy-only/storage-bearing MMIO/PCI executable, signed App Sandbox and production bundle, docs | #1490, #1525, #1526, #1528, #1529, #1566, #1567, #1568, #1569, #1575, #1576, #1577, #1578, #1583, #1584, #1585, #1586, #1587, #1588, #1589, #1616, #1617, #1634, #1651, #1652, #1665, #1666 | The current writer emits `2.8.0` with required serial kind 8, optional unchanged device-graph kind 7 profile 3, and optional entropy kind 9 after the memory/machine/global/topology/vCPU/time graph. It admits Paused Full sources with 1–32 vCPUs, complete bounded UART state, and either no storage or 1–64 ordered regular-file block/pmem records over matching MMIO or the product PCI budget, with no root or one first cross-storage root, plus an optional entropy owner on the matching transport. Serial state retains endpoint intent, limiter configuration, guest-visible registers, bounded RX, status, and pending work; restore prepares fresh default stdio or a fresh configured file/FIFO or contained write-only sink without inheriting source descriptors, terminal/FIFO buffers, TX bytes, counters, limiter budget, or metrics. Entropy state retains exact configuration, queue cursors, dual limiter buckets, pending notifications/interrupts, and retry timing; restore constructs a fresh OS source, scheduler, notifier, route/endpoint, limiter, and metrics owner. Profile 3 retains exact block and pmem configuration/runtime/limiter/queue/interrupt/mapping/transport state and loads with serial through one direct or contained typed authority transaction without fallback. Signed direct and normal-production evidence proves serial-only, entropy-only, and storage-bearing continuation, UART register/RX/interrupt preservation, fresh serial/entropy metrics, endpoint, scheduler, notifier, route, and OS-source ownership, rooted pmem-only/rootless mixed MMIO/PCI reconstruction, active persistence, limiter and interrupt progress, recapture, immutable state/memory reuse, deliberate shared writable external prefixes, zero fresh pmem private tails, retained descriptor identity, worker/launcher cleanup, and explicit/automatic resume. CRCs cover state/metadata only; external backing authentication/encryption and operator serialization are required. Balloon, virtio-mem, network, and vsock serialization, native-v2 Uffd, Diff/merge, overrides/editing, Firecracker bytes, and broad portability remain Wave 6 work. |
| Direct pmem mapping, root boot, and native-v2 restore | implemented and verified for startup MMIO/PCI, non-root runtime PCI, complete capture-ready live state, exact native-v2 2.6 and current 2.8 profile-3 serialization/restore | unit, process e2e, signed HVF capture equality, focused profile-3 fault/authority tests, signed executable storage matrix, signed normal-production bundle storage matrix, docs, pinned-source compare | #1439, #1444, #1448, #1471, #1634, #1651, #1652, #1665, #1666 | One reference-counted file/private-tail mapping is registered directly with HVF. Writable file prefixes use a shared host mapping; read-only guest mappings use a private write-capable host view required by HVF while guest permissions stay read-only. Flush synchronizes exactly the nonzero file prefix with `MS_SYNC`; the aligned tail is volatile. Profile 3 binds exact file/mapped geometry and restores direct paths or exact `PmemBacking` grants with the complete block/pmem batch; current 2.8 composes that unchanged storage graph with the required serial component and optional entropy. Signed rooted pmem-only and rootless mixed MMIO/PCI cells prove shared writable prefix epochs, unchanged read-only peers, a zero DAX private tail on every fresh mapping, limiter/retry and interrupt continuation, recapture, immutable state/memory, pathname-replacement resistance, and cleanup. Pmem remains outside ordinary RAM dirty epochs, and both checked pmem composite records are terminal. |
| Guest-memory backing profiles | anonymous default plus descriptor-backed boot RAM and stable virtio-mem aperture implemented; direct and contained vhost-user block support dynamic memory and aggregate storage certification | unit, real Unix stream/SCM_RIGHTS/pipe/kqueue, signed HVF, signed executable, signed production bundle, docs | #1439, #1441, #1443, #1444, #1445, #1449, #1462, #1471, #1634 | Ordinary-only VMs retain anonymous RAM. Vhost or virtio-mem startup selects unlinked sparse shared backing; virtio-mem reserves one complete deterministic aperture before block preparation, while only plugged views enter CPU/HVF mappings, FDT/current accounting, byte access, and dirty metadata. Every initial or runtime backend receives one immutable guest-ordered table containing boot RAM plus that aperture (at most three arm64 regions), can read/write currently unplugged bytes, and receives no unrelated mapping. This is an explicit trusted confidentiality/integrity/availability capability without Darwin memfd seals, a bundled backend, helper, or backend-policy claim. Exact offset discard follows committed unplug, backend death/DELETE drops only frontend clones, and shutdown drops the reservation after active views. Signed direct and contained MMIO/PCI guests prove both configuration orders, storage I/O across `0 -> 128 MiB -> 0`, exact stable table geometry, CONFIG refresh, Running/Paused runtime attach/delete/reuse, unchanged entitlements, no helper, and vhost snapshot rejection before artifacts. #1471 adds the single-session aggregate Sync/Async/vhost/pmem lifecycle; #1634 separately closes both pmem snapshot composites without changing vhost-user or virtio-mem snapshot limits. |
| Capability inventory enforcement | structural inventory implemented; capability audit in progress | unit, workspace CI, docs, pinned-source compare | #1348, #1349, #1527, #1546, #1547, #1548, #1549, #1550, #1551, #1552, #1553, #1554, #1555, #1578, #1589, #1616, #1617, #1634, #1651, #1652, #1665, #1666 | The machine-owned v1.16.0 source manifest and human overlay keep every disposition mechanically checked. #1578 promoted the snapshot-version argument, complete instance/version-output semantic, aggregate run operation, and minimal Full create/load public-lifecycle semantic. #1589 advanced that lifecycle to exact 2.4 singleton-root artifacts; #1616/#1617 added exact 2.5 profile-2 block artifacts; #1634 added exact 2.6 profile 3 and promoted the final two pmem composites; #1651 activated required exact 2.7 serial state with optional unchanged profile-3 storage; #1652 promoted the serial snapshot semantic after signed direct/contained certification; #1665 activated current 2.8 with optional entropy; and #1666 promotes the two entropy artifact/restore aggregates after signed direct/contained certification. The resulting disposition counts are 238/160/3/17. Other snapshot corpora and API/schema aggregates remain under audit for Diff, optional devices other than pmem/serial/entropy/MMDS, tools, Firecracker interoperability, and portability. |
...
| Process CLI and API socket | 25 of 29 inventory records implemented and verified; two proven platform impossible; only two broad source corpora remain under audit | unit, API socket, process e2e, signed process, signed App Sandbox process, signed production bundle, pinned-source compare | #536, #545, #1008, #1010, #1048, #1058, #1060, #1070, #1092, #1260, #1302, #1352, #1365, #1368, #1384, #1419, #1578, #1589, #1616, #1617, #1634, #1651, #1652, #1665, #1666 | The checked 23-argument contract has 21 implemented leaves and two terminal seccomp platform exclusions. It covers argument precedence, Unicode identity, limits, readiness, fd-table behavior, signals including nonfatal SIGPIPE, socket ownership/cleanup, configuration startup, PCI, and current native snapshot output. `--snapshot-version` prints `v2.8.0`; bounded direct or exact-granted description reports the actual validated native-v1 or native-v2 version and explicitly rejects Firecracker bytes. The complete instance/version-output semantic and aggregate run operation are terminal. Only broad `corpus:design` and `corpus:getting-started` remain under audit for unrelated repository-wide claims. |
...
| Product PCI and modern virtio-pci | all-virtio startup, aggregate runtime hotplug/live storage certification, exact native-v2 2.6, exact 2.7 serial, and current 2.8 block/pmem/serial plus optional entropy persistence, with vsock capture-ready ownership implemented; other optional-device persistence/restore deferred | unit, process e2e, signed HVF capture/equality/reconstruction, signed executable aggregate plus storage/serial/entropy restore, signed production bundle aggregate plus storage/serial/entropy restore, docs, pinned-source compare | #1416, #1417, #1418, #1419, #1420, #1421, #1422, #1423, #1444, #1448, #1471, #1475, #1516, #1583, #1584, #1585, #1586, #1587, #1588, #1589, #1616, #1617, #1634, #1651, #1652, #1665, #1666 | The supported macOS arm64/HVF product path selects one immutable all-virtio transport after target/GIC-MSI and exact 31-endpoint capacity preflight. The retained manager supports failure-atomic Running/Paused non-root block, pmem, and network transactions with exact teardown/reuse. Exact native-v2 2.6 profile 3 and retained 2.7 persist and reconstruct every admitted block and pmem PCI endpoint, including configuration, queue, limiter/retry, mapping, common virtio, BAR/capability/MSI-X state, and registry placement under the same budget. Current 2.8 retains that graph, composes the required platform-MMIO serial component without consuming a PCI endpoint, and optionally persists and reconstructs the entropy PCI endpoint with exact queue, dual-limiter/retry, common-virtio, BAR/capability/MSI-X, interrupt-route, and registry state. Signed direct and normal-production matrices prove active storage I/O, serial and entropy continuation, fresh entropy source/metrics/scheduler/notifier/route ownership, external prefix epochs, fresh pmem private tails, recapture, immutable-pair reuse, and cleanup. Automatic guest notification beyond restored queue/interrupt continuation, persistence for optional devices other than serial, pmem, and entropy, external vmnet certification, and KVM ITS identity remain deferred. |
...
| Snapshots and restore | current public native-v2 2.8 complete-serial Full/File multi-vCPU lifecycle with optional regular-file block-and-pmem profile-3 storage and optional entropy implemented and verified; exact entropy-free 2.7, storage-only 2.6, block-only 2.5, singleton-root 2.4, device-free 2.3, and frozen native-v1 File/Uffd readers retained | unit, API socket, process e2e, signed HVF capture/reconstruction, typed complete-set resource/failure injection, signed direct serial/entropy-only and rooted/rootless MMIO/PCI executable, signed App Sandbox and normal production-bundle restore, docs | #543, #1263, #1264, #1390, #1395, #1396, #1441, #1525, #1526, #1528, #1529, #1554, #1575, #1576, #1577, #1578, #1583, #1584, #1585, #1586, #1587, #1588, #1589, #1609, #1610, #1611, #1612, #1613, #1614, #1615, #1616, #1617, #1634, #1651, #1652, #1665, #1666 | Public create admits Paused Full sources with 1–32 vCPUs, complete bounded serial state, and either no storage or 1–64 ordered regular-file block/pmem records over matching MMIO or the product PCI budget, with no root or one first cross-storage root, plus optional entropy on the matching transport and no balloon/virtio-mem/network/vsock/MMDS/boot timer; it publishes current `2.8.0` memory first and state last. Public load derives and atomically authorizes fresh default stdio or a fresh configured output together with the exact optional typed block/pmem vector, then constructs a fresh entropy source, scheduler, notifier, route/endpoint, limiter, and metrics owner when kind 9 is present, without fallback/override, and commits the graph with the Paused owner. Signed direct and normal-production evidence proves restored UART registers, buffered RX and pending interrupt progress, retained entropy queue/dual-limiter/retry completion without another guest kick, fresh serial/entropy endpoint/source/scheduler/notifier/route/metrics containment, configured file/FIFO replacement resistance, serial-only and rooted pmem-only/rootless mixed MMIO/PCI persistence, read-only protection, storage limiter/interrupt progress, recapture, exact grants, immutable state/memory, deliberate shared writable external prefixes, zero fresh private tails, explicit/automatic resume, and worker/launcher cleanup. The format remains bangbang-native and unauthenticated. Diff, native-v2 Uffd, optional devices other than serial, pmem, and entropy, Firecracker artifacts, and broad portability remain unsupported or under audit. |
| Memory hotplug | implemented supported MMIO-or-PCI startup, live metrics, and capture-ready state; optional serialization excluded | unit, api socket, process e2e, signed HVF, signed executable, signed production bundle, docs | #544, #942, #952, #1022, #1026, #1028, #1030, #1032, #1034, #1040, #1042, #1044, #1046, #1050, #1333, #1334, #1419, #1462, #1474 | Pre-boot `PUT /hotplug/memory`, public requested/plugged status, runtime requested-size PATCH, config-generation signaling, and the one-queue virtio-mem device over the selected startup transport are implemented. Valid `STATE`, `PLUG`, `UNPLUG`, and `UNPLUG_ALL` requests operate in configured block units over complete guest ranges. Exact active guest/HVF mappings can be split or combined; with vhost, they are offset views of one retained full-aperture shared reservation. Backend mutation precedes ACK publication, device state commits only after guest-visible completion, and partial or late failures remap the same retained views in reverse order. Committed unplug performs exact-offset best-effort discard and removes current dirty/accounting state without changing the vhost table. One producer exposes Firecracker's exact 18 singleton `memory_hotplug` metrics plus separate interrupt, rollback, owner-cleanup, and teardown counters. Paused capture validates available/negotiated features, exact queue geometry and cursors, pending notification/interrupt state, compact plugged ranges, the reservation's opaque mapping identity, guest owners, actual HVF maps, dirty tracking/epoch, and active/offline/current byte accounting. Current public native-v2 create performs this preflight before optional-profile rejection but writes no optional-device encoding. Focused tests cover adjacent sequential plugs, partial multi-block unplug, one request crossing the conceptual slot boundary, metrics/failure ordering, exact mapping capture, limits, dirty state, same-address rollback/reuse, backend death, and pause/run without claiming Firecracker's KVM slot identity. Signed direct and contained MMIO/PCI coverage proves Linux binds `virtio_mem`, storage remains live, and public requested/plugged size completes `0 -> 128 MiB -> 0`. No DELETE route is exposed. Runtime device deletion, broader public guest-memory accounting, and optional-device serialization/restore remain deferred. |
...
| Remaining Firecracker devices | implemented supported subsets; transport and profile limits explicit | unit, api socket, process e2e, signed HVF, signed executable, signed production bundle, docs | #544, #797, #800, #802, #804, #806, #808, #810, #812, #814, #815, #818, #869, #873, #875, #877, #888, #890, #892, #894, #896, #898, #900, #902, #904, #905, #908, #910, #912, #914, #920, #922, #926, #928, #930, #932, #934, #936, #938, #940, #960, #962, #964, #968, #970, #972, #988, #990, #1000, #1002, #1016, #1018, #1024, #1329, #1328, #1330, #1331, #1335, #1336, #1337, #1338, #1362, #1418, #1419, #1420, #1421, #1422, #1444, #1473, #1474, #1475, #1477, #1478, #1479, #1480, #1529, #1634, #1651, #1652, #1665, #1666 | Balloon, virtio-mem, entropy, serial, network, vsock, RTC, VMGenID, VMClock, and PVTime retain their documented live/capture-ready subsets and explicit artifact handoffs. Serial implements exact native-v2 2.7 serialization and restoration through fresh default stdio or configured direct/contained output authority, with exact UART/RX/pending-work continuation and signed serial-only plus storage-bearing certification. Pmem implements exact native-v2 2.6 and current 2.8 profile-3 serialization and restore over MMIO/PCI with exact direct or contained backing authority, shared writable prefixes, private volatile tails, limiter/queue/interrupt continuation, and signed rooted/rootless certification. Entropy implements optional exact native-v2 2.8 serialization and restoration over MMIO/PCI with exact queue, dual-limiter, pending/retry continuation and fresh source, scheduler, notifier, route/endpoint, and metrics ownership at direct and normal-production boundaries. Serialization/restore for optional devices other than serial, pmem, and entropy, automatic PCI notification, and externally certified vmnet connectivity remain explicit limits. |
...
| Public native-v2 Full/File endpoint activation | implemented current 2.8 complete-serial subset with optional regular-file block-and-pmem profile-3 storage and optional entropy, plus exact native-v2 2.7/2.6/2.5/2.4/2.3 and frozen native-v1 readers | runtime, API socket, process lifecycle, signed direct executable entropy matrix, signed App Sandbox, signed production bundle entropy matrix, docs | #543, #1276, #1396, #1575, #1576, #1577, #1578, #1589, #1616, #1617, #1634, #1651, #1652, #1665, #1666 | Public Full create emits graph-required native-v2 2.8 with required complete serial state, either no storage or a rooted/rootless ordered vector of 1–64 regular-file block and pmem devices over MMIO or the product PCI budget, and optional entropy on the matching transport. Public load classifies one opened state and selects frozen native-v1 File/Uffd, legacy device-free 2.3, singleton-root 2.4, block-only profile-2 2.5, storage profile-3 2.6, retained serial-required 2.7, or current serial-required optional-entropy 2.8 File/COW plus one exact storage-and-serial authority transaction and fresh entropy construction when present. There is no ambient fallback or load-time override. Each destination receives fresh serial endpoints, limiter budget, and metrics plus a fresh entropy source, scheduler, notifier, route/endpoint, limiter, and metrics owner when present; repeated destinations share only externally managed writable block and pmem prefixes and require operator serialization. State/memory and File/COW RAM remain immutable/private, and each fresh pmem private tail starts zero. Signed direct and normal-production matrices cover serial-only, entropy-only, and rooted pmem-only/rootless mixed storage-plus-entropy MMIO/PCI continuation, exact grants, endpoint replacement resistance, recapture, explicit/automatic resume, finite retryable/terminal failures, and death cleanup. |
...
#1665 activates current native-v2 2.8 with optional entropy. #1666 certifies
...
seven entropy, six serial, and one time/identity): 80 are terminal and five
...
except where the exact 2.7 serial contract now supplies that public encoding
and restored-guest evidence and the exact 2.8 entropy contract supplies the
corresponding entropy evidence. Wave 7 #1491 owns no record in this selector, and
...
after exact 2.7 activation and certification. #1665 activates current exact
2.8, and #1666 promotes the two entropy artifact/restore records after direct
and normal-production certification, so the current inventory is
238/160/3/17.
