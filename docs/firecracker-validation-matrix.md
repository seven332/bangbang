...
| Drives and virtio-block | MMIO-default and all-PCI startup implemented; PCI-only file-backed and eligible direct-vhost runtime PUT/DELETE implemented; active direct-vhost config refresh implemented | unit, real Unix stream/SCM_RIGHTS/shared mapping/pipe, api socket, process e2e, signed HVF, signed executable, signed production bundle | #539, #916, #962, #992, #994, #996, #998, #1020, #1068, #1268, #1304, #1362, #1418, #1419, #1420, #1443, #1445, #1447 | File-backed drives retain existing startup, limiter, runtime PCI, contained grant, metrics, and accepted native-v1 behavior. A direct socket drive uses an exhaustive file/vhost config matrix, bounded discovery, exact backend-derived 60-byte config and read-only/flush features, shared RAM, one translated queue, kick/call descriptors, and the existing GIC/MSI interrupt path. ID-only PATCH repeats exact CONFIG acquisition, advances one transport generation, signals one guest configuration interrupt, and records optional config-change latency only on success. An already-shared all-PCI VM may attach a new non-root direct backend after zero-effect owner preflight and remove it after manual guest PCI removal; duplicate/root/anonymous/capacity/negotiation failures preserve live config, and teardown releases frontend, RAM descriptor clones, metrics, BAR, MSI-X, and slot ownership for Paused reuse. Signed Linux evidence covers capacity refresh, runtime I/O, invalid negotiation rollback, duplicate/anonymous zero-connect rejection, remove/reuse, and clean closure. A strict external test backend validates transferred memory and descriptors; it is not shipped. Contained socket brokerage, dynamic-memory coexistence, same-ID live reconnect, automatic guest PCI notification, Async, and vhost snapshot state remain deferred. |
