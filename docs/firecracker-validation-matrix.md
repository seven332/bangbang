...
| Drives and virtio-block | MMIO-default and all-PCI Sync/Async file startup implemented; Firecracker-compatible backing-metadata GET_ID implemented; live path/limiter and same-ID engine/backing replacement implemented; PCI file/vhost PUT/DELETE and active vhost config refresh implemented; complete live capture handoff implemented | unit, real file/pipe/kqueue and Unix stream/SCM_RIGHTS/shared mapping, api socket, process e2e, signed HVF capture equality, signed executable preflight, signed production bundle | #539, #916, #962, #992, #994, #996, #998, #1020, #1068, #1268, #1304, #1362, #1418, #1419, #1420, #1443, #1445, #1446, #1447, #1448, #1449, #1460 | File-backed drives accept default Sync or explicit Async over MMIO/PCI and direct/contained backings. GET_ID derives the exact 20-byte Firecracker v1.16.0 decimal `st_dev`/`st_rdev`/`st_ino` identity from the opened descriptor and updates atomically with backing replacement; native-v1 preserves exact current or legacy IDs. One lazy bounded portable executor per VM session uses owner-thread completion publication for status, used entries, dirty ranges, interrupts, limiter accounting, and metrics; generation routing isolates multiple drives and safely quiesces path PATCH, same-ID backing/engine replacement, hotplug, DELETE, reset, and shutdown. Four signed executable and two signed production scenarios cover MMIO/config-file/concurrent-root-data startup, contained authority, first-use PCI hotplug, live replacement, limiter PATCH, guest I/O, guest-visible metadata identity, delete/reuse, and clean exit. Native-v1 remains Sync-only; paused create first drains, publishes, captures, and safely reopens every live Async generation, then rejects before artifact creation. A direct or contained socket drive separately implements bounded discovery, shared-memory vring transfer, exact config refresh, new-ID PCI attach, caller-coordinated removal, and exact resource reuse; vhost same-ID PUT remains duplicate. Dynamic-memory coexistence for vhost, automatic guest PCI notification, storage serialization/restore, vhost snapshot support, and broad storage aggregates remain deferred. |
