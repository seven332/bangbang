...
| Capability inventory enforcement | structural inventory implemented; capability audit in progress (84 implemented, 314 audit, 3 feasible-missing, 17 proven impossible) | unit, workspace CI, docs, pinned-source compare | #1348, #1349, #1420, #1421, #1422, #1423, #1444, #1445, #1447, #1449 | The machine-owned v1.16.0 source manifest records exact 26/38/44/152 Swagger identities, 23 configured executable arguments, three non-Swagger DELETE routes, public-tool operations/arguments, and an explicit source corpus. A separate human overlay owns every disposition and adds cross-leaf semantic records. Delivery validation permits honest `audit-required`/missing work; final mode rejects it. The 17 terminal exclusions cover strict runtime seccomp/jailer, exact Linux hugetlbfs `2M`, and narrow KVM/static CPU categories. #1420-#1423 promote runtime device hotplug, #1444 promotes exact pmem leaves, #1445/#1447 record direct vhost startup/runtime behavior, and #1449 records exact contained grant/child brokerage, startup/runtime connect, retry, PATCH reuse, DELETE lease release, and signed multi-child execution without promoting the still-broad socket/corpus/storage records. Broad pmem/storage, snapshot, Async, vmnet, and observability owners remain nonterminal. |
...
| Product PCI and modern virtio-pci | all-virtio startup plus aggregate block, pmem, and network runtime hotplug implemented; snapshots deferred | unit, process e2e, signed HVF, signed executable, signed production bundle, docs, pinned-source compare | #1416, #1417, #1418, #1419, #1420, #1421, #1422, #1423, #1444 | The supported macOS arm64/HVF product path accepts exact `--enable-pci` after target/GIC-MSI preflight and selects one immutable all-virtio transport. It owns Firecracker-shaped segment-0 ECAM/BAR apertures, deterministic generation-bound slots/functions, one 512-KiB BAR per endpoint, exact fixed MSI-X demand plus worst-case runtime headroom, and independently revocable full-pool registries. Balloon, block, network, pmem, vsock, entropy, and virtio-mem publish in Firecracker order; legacy virtio MMIO nodes are suppressed while platform devices remain MMIO. Default startup remains all-virtio-MMIO with `pci=off`. The retained manager supports failure-atomic Running/Paused non-root block, pmem, and network PUT/DELETE under one 31-endpoint budget with manual guest coordination, serialized mutation, type-scoped IDs, generation-bound metrics, exact teardown, and capacity reuse. Pmem registers a cloned lease on its one direct file/private-tail mapping, synchronizes only `file_len` with `MS_SYNC`, and retains that lease across failed unmap/rollback. Signed product and focused gates cover every startup class, read-only MMIO and writable PCI pmem root boot, direct pre-flush coherence, targeted flush, runtime remove/reuse, and contained descriptor identity; network also proves real MMDS exchange without vmnet authority. Native-v1 rejects PCI create/load before mutation. Automatic guest notification, PCI persistence, external vmnet certification, and KVM ITS identity remain deferred. |
| Drives and virtio-block | MMIO-default and all-PCI startup implemented; PCI-only file-backed and eligible direct/contained-vhost runtime PUT/DELETE implemented; active direct/contained-vhost config refresh implemented | unit, real Unix stream/SCM_RIGHTS/shared mapping/pipe, api socket, process e2e, signed HVF, signed executable, signed production bundle | #539, #916, #962, #992, #994, #996, #998, #1020, #1068, #1268, #1304, #1362, #1418, #1419, #1420, #1443, #1445, #1447, #1449 | File-backed drives retain existing startup, limiter, runtime PCI, contained grant, metrics, and accepted native-v1 behavior. A direct or contained socket drive uses an exhaustive file/vhost config matrix, bounded discovery, exact backend-derived 60-byte config and read-only/flush features, shared RAM, one translated queue, kick/call descriptors, and the existing GIC/MSI interrupt path. ID-only PATCH repeats exact CONFIG acquisition, advances one transport generation, signals one guest configuration interrupt, and records optional config-change latency only on success. An already-shared all-PCI VM may attach a new non-root direct backend or exact contained grant/child stream after zero-effect owner preflight and remove it after manual guest PCI removal; duplicate/root/anonymous/capacity/negotiation failures preserve live config, and teardown releases frontend, RAM descriptor clones, metrics, BAR, MSI-X, and slot ownership for Paused reuse. Signed Linux evidence covers capacity refresh, runtime I/O, invalid negotiation rollback, duplicate/anonymous zero-connect rejection, remove/reuse, and clean closure. Signed production boots an exact contained root and scratch child alongside vsock, proves scratch read/write/flush and guest-observed refresh on the existing stream, then proves invalid-target and negotiation rollback, all-PCI runtime attach, duplicate zero-connect rejection, manual removal, DELETE, Paused same-ID reuse through another child, resumed I/O, and complete closure with no steady-state helper. A strict external test backend validates transferred memory and descriptors; it is not shipped. Contained brokerage retains exact directory authority across retry, multiple children, DELETE, and reinsertion while each device owns only its child lease. Dynamic-memory coexistence, same-ID replacement without DELETE, automatic guest PCI notification, Async, and vhost snapshot state remain deferred. |
