...
| Machine and boot configuration | partial | unit, api socket, process e2e, signed HVF | #538 | Pre-boot machine config, boot source, kernel/initrd loading, FDT generation, direct-rootfs boot paths, and current multi-vCPU startup rejection are covered. Multi-vCPU execution and broader CPU feature behavior remain deferred. |
...
| Observability: logger, metrics, serial | partial | unit, api socket, process e2e, signed process, signed HVF | #542 | Minimal logger output with level, origin, and module-prefix filtering, metrics, startup timing, explicit and 60-second periodic metrics flushes, selected GET, PUT, PATCH, and `/actions` API request counters, and serial output paths are covered. Full Firecracker counters, immediate startup metrics flushing, missed periodic-metrics accounting, and full log routing remain deferred. |
...
| Snapshots and restore | recognized unsupported | unit, api socket, process e2e | #543 | Snapshot create/load request shapes are parsed and rejected with the snapshot fault. Real memory, vCPU, and device-state persistence remains deferred. |
| Remaining Firecracker devices | recognized unsupported | unit, api socket, process e2e | #544 | Balloon, pmem, entropy, and memory hotplug request shapes are recognized or parsed before returning device-specific unsupported faults. Process e2e covers representative valid requests; device implementation needs separate designs. |
