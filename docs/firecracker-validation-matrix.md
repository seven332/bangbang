...
| Guest-memory backing profiles | anonymous default and descriptor-backed shared RAM implemented; direct and contained vhost-user block consume shared RAM | unit, real Unix stream/SCM_RIGHTS/pipe/kqueue, signed HVF, signed executable, signed production bundle, docs | #1439, #1441, #1443, #1444, #1445, #1449 | Ordinary-only VMs retain anonymous RAM. A pre-boot socket drive selects exact-sized unlinked owner-only shared files before allocation, then transfers cloned descriptors for every current region to the selected backend during activation. Direct mode opens an operator path; contained mode receives only an exact connected child stream through the launcher. This grants that backend read/write access to complete guest RAM and is an explicit trusted capability without Linux memfd sealing parity. The same mappings retain HVF, dirty, discard, and byte-access behavior; dynamic memory hotplug is rejected while a one-time vhost memory table is configured. Signed MMIO/PCI guests prove cross-process root and scratch I/O; signed production proves contained root/scratch I/O plus all-PCI runtime attach/delete/reuse across exact children from one connect-only grant without a helper; and vhost snapshots reject before artifacts. Broad memory/vhost aggregates remain under audit. |
