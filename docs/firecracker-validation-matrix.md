...
| VM lifecycle and run-loop control | partial | unit, api socket, process e2e, signed HVF | #537 | `InstanceStart`, Running transition, retained boot worker status, unsupported pause/resume request routing, and guest PSCI `SYSTEM_OFF`/`SYSTEM_RESET` process exits are covered. Public pause/resume, reboot-in-place, and guest error state handling remain deferred. |
