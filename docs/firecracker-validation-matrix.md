...
| Remaining Firecracker devices | recognized unsupported | unit, api socket, process e2e, docs | #544, #797, #800, #802, #804, #806, #808, #810, #812 | Balloon, pmem, entropy, and memory hotplug request shapes are recognized or parsed before returning device-specific unsupported faults. Balloon, entropy, pmem, and memory hotplug valid requests now route through the VMM state/action policy; process e2e covers representative valid requests. Entropy remains unsupported API/device work and is not part of the current API request metrics output because Firecracker does not define entropy request counters; #797 defines the virtio-rng implementation split, #800 covers the explicit entropy API/runtime configuration boundary, #802 adds the backend-neutral virtio-rng queue handler under unit tests, #804 adds runtime MMIO activation/notification dispatch under unit tests, #806 adds internal MMIO registration, #808 adds internal startup/FDT wiring under unit tests, #810 adds internal startup-runtime/HVF queue notification dispatch and interrupt signaling under unit tests, and #812 adds a session-owned host OS randomness source for internal HVF entropy dispatch. Other device implementations still need separate designs. |
