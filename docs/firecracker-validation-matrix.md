...
| --- | --- | --- | --- | --- |
| Native-v2 state and lazy memory | typed 2.2 multi-vCPU platform graph and signed lazy memory implemented; destination construction and public lifecycle deferred | unit, fixed binary fixtures, hostile component/registry mutations, descriptor/mapping fault injection, signed HVF, docs | #1490, #1525, #1526, #1528, #1566, #1567, #1568 | The immutable empty `2.0.0` and memory-only `2.1.x` states remain readable; exact decoded memory versions continue to pair with their unchanged image headers. Current `2.2.0` registers singleton memory/machine/global/topology kinds 1–4 and contiguous per-vCPU kind 5. Its allocation-free exact-profile scan, bounded typed payloads, retained CPU logical/common/effective evidence, singular GIC, stable PSCI topology, explicit mandatory vCPU fields, closed debug/SME registry, and whole-graph validation expose data only and stay below 16 MiB. The state-bound 64-KiB-aligned memory table still validates topology, exact file identity/length/header/padding, host compatibility, and all mappings before the first `mmap`; retained read-only files map as private COW without eager guest-byte reads or path reopen. Signed evidence covers demand faults, dirty/COW/discard behavior, continuation, rollback, and cleanup. CRCs cover state and metadata only; external authentication/encryption is required. Public v2 create/load/describe, transactional pair publication, general device state, destination HVF/owner/time/identity orchestration, Diff/merge, clone, and portability remain Wave 6 work. |
