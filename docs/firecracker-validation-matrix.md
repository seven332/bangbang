...
| Observability: logger, metrics, serial | partial | unit, api socket, process e2e, signed process, signed HVF | #542 | Minimal logger output with level, origin, and module-prefix filtering, metrics, startup timing, immediate startup metrics flushing, explicit and 60-second periodic metrics flushes, selected GET, PUT, PATCH, and `/actions` API request counters, and serial output paths are covered. Full Firecracker counters, missed periodic-metrics accounting, and full log routing remain deferred. |
