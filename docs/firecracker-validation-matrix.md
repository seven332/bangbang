...
| Observability and serial | **Implemented subset; logger aggregate and metrics API/schema slice certified.** The exact pinned logger corpus and delivery/filtering/loss/redaction semantic is terminal. The exact twelve-row metrics API/schema slice is also terminal, including strict request shapes, full `u64` token-bucket parsing, `/vm/config` privacy, canonical numeric lines, and real initial/Paused-periodic/Running-periodic/explicit/terminal production evidence. Metrics producer populations and their corpus/cross-producer aggregates remain assigned to #1788, #1789, and #1790. Serial output/input policy and exact native-v2 serial state remain supported subsets. Log rotation, durable or remote telemetry, developer tracing, and a public serial streaming API are not claimed. | [Observability contract](firecracker-compatibility.md#firecracker-v1160-observability-contract), [logger contract](../compat/firecracker/v1.16.0/logger-contract.md), [metrics contract](../compat/firecracker/v1.16.0/metrics-contract.md), [serial contract](../compat/firecracker/v1.16.0/serial-contract.md) |
