...
| Time and identity devices | PL031, VMGenID, VMClock, and public ARM PVTime live/capture-ready accounting implemented | unit, signed HVF, signed executable, docs | #543, #544, #946, #1076, #1078, #1080, #1082, #1084, #1261, #1272, #1276, #1477, #1478, #1480 | Startup emits Firecracker-shaped DeviceTree VMGenID and VMClock nodes, initializes a nonzero generation and complete typed 112-byte VMClock ABI, and allocates deterministic SPI lines. Native-v1 capture records the live ABI as `BANGDEV\0` 1.1.0 under paused ownership while retaining 1.0.0 memory-derived loads. Restore reconstructs no-alarm PL031 from destination wall clock, completes aggregate runner/GIC state, writes/notifies a fresh VMGenID, then publishes/notifies the fenced VMClock sequence and counter update before resume. Clean precommit failures may retry; every committed write/notification failure is terminal. Signed two-destination guest polling proves both VMGenID halves, stable even VMClock sequence and counters, non-regressing RTC, and continuation. #1478 adds the aligned per-vCPU ABI and owner-thread HVF measurement primitive. #1480 adds bounded runnable wall-minus-execution accounting, dirty-aware atomic publication, fail-closed 64-bit SMCCC discovery, topology-ordered pause-gated capture values, and signed Linux proof of `stolen time PV`, nonzero monotonic contention steal, unchanged idle accounting, and unchanged paused captures. PVTime artifact restore and final aggregate reconciliation remain under #1481. Firecracker v1.16.0's ACPI HID correction and x86 KVM clock adjustment are not applicable to this aarch64 DeviceTree/HVF device. |
