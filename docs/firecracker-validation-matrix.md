...
| Machine and boot configuration | partial | unit, api socket, process e2e, signed HVF | #538 | Pre-boot machine config, empty/no-op CPU config, boot source, kernel/initrd loading, FDT generation including arm64 `/chosen/linux,pci-probe-only` and 64-byte `/chosen/rng-seed`, direct-rootfs boot paths, current early public multi-vCPU startup rejection, ordered owner-thread vCPU topology, owning concurrent boot-session coordination with active-only batch cancellation, all-MPIDR FDT input, indexed PSCI CPU_ON/level-0 affinity transitions, per-vCPU timer PPIs, and signed Linux CPU1 execution are covered. The signed proof pins a deterministic init to CPU1 and verifies it with `getcpu` without fixed sleeps. Public SMP activation, non-empty custom CPU templates, multi-vCPU native-v1 snapshots, CPU_OFF/broader hotplug, and reboot-in-place remain deferred. |
