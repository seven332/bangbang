...
| --- | --- | --- | --- | --- |
| Native-v2 state, lazy memory, artifact transaction, private process publication/restoration, and focused reconstruction | typed 2.3 multi-vCPU graph with time/clone identity, shared native-family transaction, signed lazy memory, focused paused destination, and private minimal production-source publication plus normal-process restoration implemented; public lifecycle deferred | unit, fixed binary fixtures, hostile component/registry/FDT mutations, artifact fault/authority checks, descriptor/mapping and restore-stage fault injection, signed three-vCPU HVF, signed two-vCPU process, docs | #1490, #1525, #1526, #1528, #1529, #1575, #1576, #1577, #1566, #1567, #1568, #1569 | The immutable empty `2.0.0`, memory-only `2.1.x`, and base-platform `2.2.x` states remain structurally readable; exact decoded memory versions continue to pair with unchanged image headers. Current `2.3.0` registers singleton memory/machine/global/topology kinds 1–4, contiguous per-vCPU kind 5, then singleton time kind 6. Its allocation-free exact-profile scan, bounded typed payloads, retained CPU evidence, singular GIC, stable PSCI topology, explicit mandatory vCPU fields, closed debug/SME registry, fixed portable time policies, and whole-graph validation stay below 16 MiB. The state-bound 64-KiB-aligned memory table validates topology, exact file identity/length/header/padding, host compatibility, and all mappings before the first `mmap`; retained read-only files map as private COW without eager guest-byte reads or path reopen. #1575 adds one closed state-derived native commitment and routes current-v2 publication plus compatible native-v1/v2 direct/opened preparation and loading through the existing anchored no-clobber transaction. #1576 privately composes that transaction with a fail-closed paused production profile, complete canonical-reset UART comparison, bounded inert boot metadata, cancellable topology/memory capture, exact encoding, inner-coordinator recovery before seal, and terminal post-publication handling. #1577 adds pristine File/COW destination admission, state-first direct/contained prepared loading, inert boot commit, exact minimal FDT/UART/RTC/time-shell validation, fresh buffered or stdout-only output without stdin, a closed focused supervisor, initially-Paused controller/session publication, and one terminal construction latch shared with boot and native-v1 restore. Public v1 entry points and bytes remain unchanged. The focused restore guard accepts only decoded state plus authorized memory, preflights ranges/FDT/cache/time bytes and destinations before HVF, builds the never-run topology, restores CPU/global/per-vCPU state, creates destination-current PL031 and downtime-excluding PVTime, signals a fresh VMGenID then saved-counter VMClock, imports lifecycle state, and first publishes Paused with typed reverse cleanup and a terminal post-identity commit boundary. Signed three-vCPU evidence proves cancellation recovery, reusable fresh-image capture, repeat immutable loads, distinct clone IDs, guest-observed notification order/time values, recapture-to-restore, no early progress, timer-suspended PPI/PSCI continuation, offline CPU_ON, final recapture, and cleanup. The separately signed private process publishes and recaptures a real two-vCPU source, then restores the same immutable pair into two fresh normal processes for paused and resume-requested lifecycle/shutdown proof without a production activation trigger. CRCs cover state and metadata only; external authentication/encryption is required. Public v2 process create/load/describe, general devices, Diff/merge, and broader portability remain Wave 6 work. |
...
| Capability inventory enforcement | structural inventory implemented; capability audit in progress (229 implemented, 169 audit, 3 feasible-missing, 17 proven impossible) | unit, workspace CI, docs, pinned-source compare | #1348, #1349, #1420, #1421, #1422, #1423, #1444, #1445, #1446, #1447, #1448, #1449, #1461, #1462, #1471, #1473, #1474, #1475, #1479, #1493, #1496, #1494, #1518, #1527, #1546, #1547, #1548, #1549, #1550, #1551, #1552, #1553, #1554, #1555 | The machine-owned v1.16.0 source manifest and human overlay keep every disposition mechanically checked. #1546 moves only `corpus:snapshot-page-faults` to #1527-owned feasible-but-undelivered status; #1547-#1553 deliver the protocol, coordinator, host/guest fault planes, contained grant, removal/failure behavior, and complete consumer gates. #1554 adds direct and contained native-v1 pager restore with state-bound sessions and transactional owner assembly. #1555 adds exact paused-host/restored-guest demand, signed removal-generation and exact entitlement evidence, runs the full matrix, and promotes only that corpus row to produce 229/169/3/17. The family ledgers retain only their named snapshot/restore, external vmnet, performance/observability, tool, and final-export owners. |
| UFFD-equivalent snapshot paging | narrow native-v1 direct/contained macOS restore implemented and verified | checked upstream/platform/consumer ledger, pager unit/process/reference-peer tests, session/launcher tests, lazy-memory concurrency tests, signed host/guest/removal/App Sandbox tests, exact direct and contained restore demand, exact entitlement dictionaries, full validation matrix | #1527, #1546, #1547, #1548, #1549, #1550, #1551, #1552, #1553, #1554, #1555 | `bangbang-pager-v1` provides the bounded offset-only peer protocol, and `LazyGuestMemory` plus the task-local Mach and HVF stage-two bridges retain external demand ownership without exposing task ports or host addresses. Removal is revoke-hide-zero-acknowledge-commit, and peer failure releases pending work once. #1554 accepts `backend_type = "Uffd"` only on macOS Apple Silicon for the narrow fixed-memory native-v1 profile with dirty tracking disabled. Platform/profile/consumer/grant preflight precedes path, socket, artifact, and backend access. State-bound image ID/checksum/length derive the pager session; exact GPA topology, header-relative source offsets, limits, root, and peer negotiation validate before the rollback-owned memory/bridge/mapping/vCPU/device/runtime graph is published. Direct mode performs a bounded Unix connection. Contained mode one-time claims the launcher-connected stream and gives the worker no snapshot-memory file grant. #1555 proves host demand while Paused, exact restored-guest instruction/read/write-first pages after resume, before/during/after removal and zero refault, multi-vCPU/failure/death/repeat/cleanup composition, and an empty launcher plus exactly App Sandbox/Hypervisor worker entitlement dictionary. Signed direct and production-bundle guests reach `SYSTEM_OFF` and orderly pager shutdown. File/COW remains distinct, no lazy failure falls back eagerly, and the protocol is not Linux UFFD wire-compatible. |
...
| Snapshots and restore | partial; public native-v1 Full create plus eager `File` and narrow macOS external-pager restore, with complete baseline time/identity state | unit, API socket, process e2e, signed HVF capture equality, signed direct executable restore, signed production-bundle contained restore, docs | #543, #1048, #1072, #1086, #1158, #1160, #1162, #1164, #1166, #1168, #1170, #1172, #1174, #1176, #1178, #1180, #1182, #1184, #1186, #1188, #1190, #1192, #1194, #1196, #1198, #1200, #1202, #1204, #1206, #1208, #1210, #1212, #1214, #1216, #1218, #1220, #1222, #1224, #1226, #1228, #1230, #1232, #1234, #1236, #1238, #1240, #1242, #1244, #1246, #1248, #1250, #1252, #1254, #1255, #1258, #1260, #1261, #1263, #1264, #1268, #1270, #1272, #1274, #1276, #1390, #1395, #1396, #1448, #1473, #1474, #1475, #1477, #1479, #1497, #1496, #1516, #1517, #1554 | `PUT /snapshot/create` supports native-v1 `Full` only from the paused one-vCPU, one regular read-only root, default-serial, no-optional-device/MMDS profile and publishes memory first, state last. `PUT /snapshot/load` is pristine-process-only, validates complete machine/vCPU/GIC/time/device/root identity before construction, commits `Paused`, and optionally resumes normally. `File` eagerly loads the committed image and may start a clean destination dirty epoch. On macOS Apple Silicon, `Uffd` instead requires dirty tracking off and one `bangbang-pager-v1` peer; it validates the state-bound session/layout/source offsets and transactionally retains private-anonymous memory plus both fault planes without reading or mapping the memory file in the VMM. Direct and launcher-granted contained signed guests restore, resume, reach `SYSTEM_OFF`, and drain the peer. Snapshot execution errors remain typed and redacted; only fully cleaned precommit failures retry, while uncertain cleanup or time/identity mutation is terminal. The format remains bangbang-native and unauthenticated. `Diff`, Linux UFFD wire compatibility, x86 clock adjustment, overrides, writable/additional drives, optional-device encoding/restore, active optional architecture/debug state, EL2 GIC state, cross-host portability, and multi-vCPU artifacts remain unsupported. |
...
Wave 7 #1491 owns no record in this selector, and at that checkpoint the global
inventory remained 191/207/3/17.
...
snapshot-paging corpus, so the current inventory is 229/169/3/17.
