...
| Machine and boot configuration | partial | unit, api socket, process e2e, signed HVF | #538 | Pre-boot machine config, empty/no-op CPU config, boot source, kernel/initrd loading, FDT generation including arm64 `/chosen/linux,pci-probe-only` and 64-byte `/chosen/rng-seed`, direct-rootfs boot paths, current multi-vCPU startup rejection, ordered owner-thread vCPU topology, and internal PSCI CPU_ON/level-0 affinity power transitions with deferred owner-thread completion are covered. The PSCI machinery is not connected to public startup; non-empty custom CPU templates, concurrent multi-vCPU execution, SMP FDT/session activation, and broader CPU feature behavior remain deferred. |
