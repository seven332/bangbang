...
| --- | --- | --- | --- | --- |
| Native-v2 state, lazy memory, and unpublished platform reconstruction | typed 2.2 multi-vCPU graph, signed lazy memory, and focused paused destination implemented; public lifecycle deferred | unit, fixed binary fixtures, hostile component/registry mutations, descriptor/mapping and restore-stage fault injection, signed three-vCPU HVF, docs | #1490, #1525, #1526, #1528, #1566, #1567, #1568, #1569 | The immutable empty `2.0.0` and memory-only `2.1.x` states remain readable; exact decoded memory versions continue to pair with their unchanged image headers. Current `2.2.0` registers singleton memory/machine/global/topology kinds 1–4 and contiguous per-vCPU kind 5. Its allocation-free exact-profile scan, bounded typed payloads, retained CPU logical/common/effective evidence, singular GIC, stable PSCI topology, explicit mandatory vCPU fields, closed debug/SME registry, and whole-graph validation stay below 16 MiB. The state-bound 64-KiB-aligned memory table validates topology, exact file identity/length/header/padding, host compatibility, and all mappings before the first `mmap`; retained read-only files map as private COW without eager guest-byte reads or path reopen. The unpublished reconstruction guard accepts only decoded state plus authorized memory, preflights ranges/FDT/cache before HVF, builds the complete never-run topology, restores CPU/global/per-vCPU/lifecycle state, and first publishes Paused with typed reverse cleanup. Signed three-vCPU evidence proves reusable capture, structural decode before construction, no early progress, equivalent recapture, timer-suspended PPI/PSCI continuation, offline CPU_ON, final recapture, and cleanup. CRCs cover state and metadata only; external authentication/encryption is required. Public v2 create/load/describe, transactional pair publication, general devices, production owner/time/identity composition, Diff/merge, clone, and portability remain Wave 6 work. |
