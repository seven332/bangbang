...
| macOS isolation and platform limits | production App Sandbox worker boundary implemented; direct jailer parity platform-limited; resource brokerage incomplete | unit, docs, process e2e, signed App Sandbox HVF and process, signed production bundle | #545, #924, #1102, #1302, #1351, #1354 | The ordinary CLI remains uncontained. The production `Bangbang.app` has a fixed unsandboxed launcher with no App Sandbox/HVF authority and one separately signed nested worker with exactly both entitlements; both use Hardened Runtime. Assembly is private, inspected, no-clobber, and exclusively published. Runtime validation fixes identities and placement and rejects modified/missing nested code before execution. The launcher forwards unchanged arguments, inherited streams, graceful signals, and worker status. Signed Apple Silicon proof covers signature/entitlement separation, tamper rejection, container-socket service/cleanup, outside-path denial/redaction, both graceful signals, and a real sandboxed HVF guest through `SYSTEM_OFF`. Static validation is an at-rest gate, not an atomic same-user replacement defense. Security-scoped grants, authenticated resource brokerage, vmnet/network policy, complete crash coupling, launch constraints, Developer ID possession/notarization, and exact Linux jailer/seccomp outcome classification remain #1351 work. |
