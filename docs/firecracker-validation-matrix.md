...
| Observability and serial | **Implemented subset; logger aggregate, metrics API/schema, all 69 process producers, and all 231 device fields certified at field level.** The exact pinned logger corpus and delivery/filtering/loss/redaction semantic is terminal. The dedicated process-final gate requires two implemented process profiles and an exact audit at 64 implemented, one source-neutral, four platform-zero, and no planned records. This includes generation-consistent request/startup/latency/logger values, persistent caught-panic and classified Darwin fatal-signal Stores, incremental SIGPIPE, immediate ambiguous hardware-fault exits, and macOS seccomp zero. The incremental device audit now certifies #1838–#1846 at 212 implemented, two source-neutral, and 17 terminal platform-zero fields. The latest slices bind decoded HVF MMIO exits and latency, terminal vCPU execution failures, MMIO/PCI interrupt attempts, active MSI-X configuration updates, and exact literal-zero i8042/PIO/KVM-clock retention to fresh VM-local roots and public output. Signed direct, App Sandbox, real-periodic production, snapshot, and actual contained-worker/supervisor tests cover the applicable terminal boundaries. The dedicated whole-device gate/profile promotion and corpus/cross-producer aggregates remain assigned to #1789/#1847 and #1790. Serial output/input policy and exact native-v2 serial state remain supported subsets. Log rotation, crash durability, remote telemetry, developer tracing, and a public serial streaming API are not claimed. | [Observability contract](firecracker-compatibility.md#firecracker-v1160-observability-contract), [logger contract](../compat/firecracker/v1.16.0/logger-contract.md), [metrics contract](../compat/firecracker/v1.16.0/metrics-contract.md), [serial contract](../compat/firecracker/v1.16.0/serial-contract.md) |
