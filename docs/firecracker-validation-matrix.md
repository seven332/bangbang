...
| --- | --- | --- | --- | --- |
| Process CLI and API socket | partial | unit, api socket, process e2e, signed process | #536, #545, #1008, #1010, #1048, #1058, #1060, #1070, #1092, #1260 | Firecracker-shaped startup args including `--boot-timer`, native snapshot envelope version and inspection commands, exit codes for argument parsing, bad startup configuration, best-effort non-clobbering fd-table preallocation, the implemented fatal host signal subset, and non-terminating `SIGPIPE` handling, API socket binding, config-file startup, no-api mode, body-based HTTP payload limits including `413 Payload Too Large` responses plus separate request-head parser bounds, empty mutating request fault messages including unknown bodyless PUT/PATCH parity, cleanup, and selected process-local MMDS/startup metrics isolation are covered for the current subset. Linux jailer/seccomp behavior is platform-limited. |
...
| Drives and virtio-block | partial | unit, api socket, process e2e, signed HVF | #539, #916, #962, #992, #994, #996, #998, #1020, #1068, #1268 | Initial drives, cache policy handling, optional bandwidth/ops rate limiters, Firecracker-shaped in-place replacement ordering for existing pre-boot drive IDs, runtime per-bucket limiter updates for existing active drives, backend-neutral block limiter retry timing, HVF block limiter retry wakeups, block read/write, root-drive boot args, basic guest-visible block behavior, signed executable writeback block flush coverage, runtime backing refresh for existing drives, recognized unsupported Firecracker-shaped vhost-user-block `socket` configs without storing or leaking socket paths, aggregate plus per-drive block queue/read/write/flush/update/failure/read/write latency/throttling metrics, and internal native-v1 persistence/preflight for one read-only root transport, queue cursors, limiter age/retry, and exact no-follow backing identity are covered. Hotplug, removal, real vhost-user-block device support, full Firecracker timerfd/eventfd parity, optional-device snapshot state, and vhost-user-block metrics remain deferred. |
...
| Native-v1 baseline device state | internal partial | unit, signed HVF ownership, docs | #543, #1268 | A standalone exact `BANGDEV\0` value capped at 16 KiB persists one read-only root virtio-block transport, queue geometry/cursors, interrupt state, frozen bandwidth/ops buckets and retry timing, UART registers with fresh default output, and canonical VMGenID/VMClock topology without reusable generation bytes. Decode is bounded and exact; staged preflight validates guest-memory rings, reopens the external root file read-only/no-follow with descriptor stat identity, and builds drop-safe resources without mutating or installing a VM. #1270 nests this exact value as the fifth composite component. #1272 consumes and installs this prepared baseline without boot writes in a private restored session. Public create/load success and signed fresh-process API active-queue continuity remain deferred. |
| Native-v1 composite capture | internal implemented | unit, supervisor ownership, signed HVF, docs | #543, #1270 | Kind-2 `BANGCMT\0` binds an exact memory image to a bounded five-component `BANGHVF\0` payload for the accepted one-vCPU/read-only-root profile. It fixes machine, atomic same-configuration CPU/cache compatibility, GIC and fresh-RTC platform policy, restorable mutable vCPU state, normalized timer/IRQ/FIQ/opaque-GIC/ICC state, and nested baseline-device state. One aggregate runner command holds metadata/core/timer/interrupt admission, while one private supervisor operation holds paused admission and block/entropy quiescence through non-memory preflight and cancellable 1 MiB memory streaming. Recoverable failures publish no bundle and leave the source paused/retryable; signed coverage decodes two complete captures and reuses the source owner. Public endpoints still invoke only the unsupported barrier and perform no final artifact I/O. #1272 consumes this bundle through the private exact-native paused restore path. Public endpoint activation, optional devices, dirty tracking, portability beyond exact local compatibility, and fresh-process API continuity remain deferred. |
| Native-v1 paused restore | internal implemented | unit, process lifecycle, signed HVF, docs | #543, #1272 | A private pristine-process seam loads a committed kind-2 pair, validates platform/cache/root compatibility before VM creation, installs the baseline runtime without kernel/FDT/boot-register writes, constructs a fresh VM/GIC/vCPU, and applies one exact never-run architecture/GIC/ICC/timer/pending restore before VMGenID replacement. A recoverable initially paused worker handoff precedes the infallible controller commit to `Paused`; `resume_vm` is returned only as intent. Explicit teardown evidence distinguishes retryable preparation/construction failures from uncertain terminal cleanup. Public create/load dispatch, optional profiles, dirty tracking, overrides, and Firecracker artifact compatibility remain unsupported. |
| Native-v1 composite publication | internal implemented | unit, process lifecycle, signed HVF, docs | #543, #1274 | A generic publisher producer receives a pathless, move-only memory staging writer only after both destination namespaces and staging files pass preflight. Publisher-observed writer closure and fixed-size output/binding matching precede the existing file barriers and exclusive memory-first/state-last renames; kind 1 remains byte-compatible, producer failures clean private staging, late memory finals remain typed orphans, and state-directory sync uncertainty remains committed success. The private ProcessVmm/supervisor seam requires the accepted paused profile and kind 2, streams capture directly into that staging inode, preserves recoverable pause/retry, and treats worker panic as terminal. Signed Apple Silicon coverage publishes through this production path, loads the pair, restores a distinct VM, replaces VMGenID, and proves continuation. Public create/load still invoke no artifact transaction and remain unsupported. |
...
landing. The #1270 composite row supersedes their older statements that cache
queries were necessarily non-atomic, captured subsets lacked schema or
orchestration, and composite capture remained deferred. Their restore,
destination-policy, optional-state, and public-endpoint limitations still
apply.
