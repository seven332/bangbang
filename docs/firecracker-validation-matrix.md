...
| --- | --- | --- | --- | --- |
| Native-v2 state and lazy memory | library foundation implemented and signed; public lifecycle and complete VM state deferred | unit, fixed binary fixtures, descriptor/mapping fault injection, signed HVF, docs | #1490, #1525, #1526 | The immutable empty `2.0.0` state remains readable; current `2.1.0` registers exactly semantic memory kind 1/instance 0. Its state-bound 64-KiB-aligned extent table validates topology, exact file identity/length/header/padding, host compatibility, and all mappings before the first `mmap`. A direct no-final-symlink adapter or exact contained `File` is retained read-only and mapped per extent as writable private COW memory without eager guest-byte reads or path reopen. Runtime and signed Apple Silicon evidence covers demand faults, clean dirty baseline, exact COW dirty pages, continuation, immutable source bytes, mixed owners, discard-to-anonymous-zero, partial rollback, ordinary cleanup, and post-VM-destroy cleanup. CRCs cover state and metadata only; external authentication/encryption is required for lazy guest bytes. Public v2 create/load/describe, transactional pair publication, complete machine/vCPU/device state, Diff/merge, clone, and portability remain Wave 6 work. |
...
| Capability inventory enforcement | structural inventory implemented; capability audit in progress (228 implemented, 169 audit, 4 feasible-missing, 17 proven impossible) | unit, workspace CI, docs, pinned-source compare | #1348, #1349, #1420, #1421, #1422, #1423, #1444, #1445, #1446, #1447, #1448, #1449, #1461, #1462, #1471, #1473, #1474, #1475, #1479, #1493, #1496, #1494, #1518, #1527, #1546, #1547, #1548, #1549, #1550, #1551, #1552, #1553 | The machine-owned v1.16.0 source manifest records exact 26/38/44/152 Swagger identities, 23 configured executable arguments, three non-Swagger DELETE routes, public-tool operations/arguments, and an explicit source corpus. A separate human overlay owns every disposition and adds cross-leaf semantic records. Delivery validation permits honest `audit-required`/missing work; final mode rejects it. The 17 terminal exclusions cover strict runtime seccomp/jailer, exact Linux hugetlbfs `2M`, and narrow KVM/static CPU categories. #1420-#1423 promote runtime device hotplug; #1444-#1449 cover direct pmem, vhost, portable Async, and capture-ready storage; #1461/#1462 close block-special and vhost/virtio-mem delivery; #1471 promotes 28 directly owned storage records; #1473 promotes 50 directly owned balloon records; #1474 promotes 17 directly owned memory-hotplug API records; #1475 promotes five directly owned entropy API records; and #1479 promotes five directly owned serial API records. #1496's checked network/MMDS ledger promotes 29 more rows, terminalizes 31 of its exact 35-record selector, and leaves only four explicit #1378/#1490/#1491 outcomes. #1518's checked vsock ledger promotes eight API/live rows from its exact 14-record selector and retains six precise #1490 artifact/restore/clone outcomes. #1546 moves only `corpus:snapshot-page-faults` to #1527-owned feasible-but-undelivered status after a public API and signed prototype audit; #1547 adds its standalone protocol, #1548 its internal lazy-anonymous-memory coordinator, #1549 its task-local public-Mach host-fault bridge, #1550 its HVF guest read/write/execute fault bridge, #1551 its contained connected-peer grant, #1552 its removal/peer-failure mechanics, and #1553 its checked complete consumer inventory plus protected-view/preflight/teardown gates without changing the disposition or runtime rejection. The family ledgers retain only their named snapshot/restore, external vmnet, performance/observability, tool, and final-export owners. |
| UFFD-equivalent snapshot paging | public macOS equivalent feasible; standalone pager protocol/client, contained connected-peer grant, backend-neutral lazy-memory coordinator, internal host/guest fault bridges, ordered removal, peer-failure propagation, and complete consumer gates implemented; native-v1 runtime still rejected | checked upstream/platform/consumer ledger, pager unit/process/reference-peer tests, session/launcher unit tests, signed production-bundle broker/consumer tests, lazy-memory concurrency tests, host/guest fault unit and signed/App Sandbox tests, unit/API rejection | #1527, #1546, #1547, #1548, #1549, #1550, #1551, #1552, #1553 | The pinned Firecracker contract delegates anonymous page population and removal to an external UFFD handler. `bangbang-pager-v1` supplies a closed offset-only codec, exact role state machines, deadline-bounded already-connected transport, a bounded concurrent VMM client, and a support-only reference peer. The production launcher securely anchors and connects one current-user local socket outside App Sandbox, then transfers only a typed connected stream and minimum redacted identity metadata through the atomic startup grant. The worker independently revalidates descriptor status/type/identity, connection, peer credentials, role, and one-time GrantId. Signed cases cover client-driven data/zero/removal/refault, cancellation/terminal behavior, malformed traffic, EOF/timeout, both process-death orders, repeat launch, cleanup, redaction, signatures, and unchanged entitlements. `LazyGuestMemory` transactionally owns distinct private-anonymous mappings with compact page state, bounded duplicate-content coalescing, exact scoped publication, counted retired requests, acknowledged removal, nonblocking first-terminal signaling, fail-closed termination, and a one-shot protected ordinary-access view. The tagged view permits in-process slice/atomic/raw/device/snapshot access through the Mach bridge while rejecting dirty, discard, shared/export, and dynamic-topology bypasses. An HVF composite retains that view and the Mach owner across partial mapping or failed cleanup, public session memory borrows stay closed, and signed App Sandbox coverage exercises a connected-peer slice/raw/atomic/virtqueue/full-snapshot/removal chain. The macOS Apple-Silicon HVF adapter generates public `mach_exc` stubs, retains task-local ports and private writable aliases, resolves owned host accesses through the coordinator, forwards unrelated exceptions, conditionally restores the prior handler, and uses fixed terminal exit status 70. Its guest plane maps the same regions with zero initial stage-two access, classifies owned data/instruction aborts, resolves all touched pages, and serializes least read/write/execute permission unions before retry without PC advance. `HvfLazyPager` wires the peer to both planes; a writer-preferring transition gate makes removal revoke guest access, hide host access, zero privately, await exact acknowledgement, commit `Absent`, and admit only a newer zero refault. Peer failures release pending work once with stable `PeerFailure`. File/COW remains distinct, direct external Mach exceptions are over-authoritative, and the protocol is not Linux UFFD wire-compatible. The checked ledger is nonterminal: consumer coverage is closed, while restore and final certification remain open; current `Uffd` rejection remains until #1527's restore gate. |
...
so the current inventory is 228/169/4/17.
