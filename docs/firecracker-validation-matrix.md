...
| Network and MMDS | partial | unit, api socket, process e2e, signed HVF | #540 | Initial network config, vmnet mode selection, process-local MMDS, internal guest-visible MMDS packet handling, and one signed executable guest MMDS fetch are covered. Broader public packet movement, rate limiting, and runtime updates remain deferred. |
