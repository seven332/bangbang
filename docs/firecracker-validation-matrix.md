...
| Virtio-vsock | partial | unit, api socket, process e2e, signed HVF | #541 | Initial config, startup listener ownership, connection setup, bounded buffering, reset/shutdown cleanup, executable startup paths, and signed executable guest-initiated and host-initiated multi-payload connection exchanges are covered. Full throughput-oriented streaming semantics, multiple simultaneous streams, half-close behavior, and credit accounting remain deferred. |
