...
| --- | --- | --- | --- | --- |
| Native-v2 state, File/COW memory, artifact transaction, public process lifecycle, and focused reconstruction | current 2.4 single-root multi-vCPU Full/File lifecycle implemented and verified; legacy device-free 2.3 and frozen native-v1 remain readable | unit, fixed binary fixtures, hostile component/registry/device/FDT mutations, artifact fault/authority checks, API/process family routing, descriptor/mapping/root-lease and restore-stage fault injection, signed three-vCPU HVF, signed public two-vCPU executable, signed App Sandbox and production bundle, docs | #1490, #1525, #1526, #1528, #1529, #1566, #1567, #1568, #1569, #1575, #1576, #1577, #1578, #1583, #1584, #1585, #1586, #1587, #1588, #1589 | The immutable `2.0.0`, memory-only `2.1.x`, and base-platform `2.2.x` states remain structurally readable; exact `2.3.0` remains the runnable legacy device-free platform. The current writer emits `2.4.0` with mandatory singleton root-device kind 7 after the memory/machine/global/topology/vCPU/time graph. Exact state-bound memory metadata validates before retained read-only private File/COW mappings. The public producer admits only Paused Full sources with configured boot metadata, 1–32 vCPUs, exactly one read-only File/Sync root over matching MMIO or PCI, no optional devices/MMDS/boot timer, default serial configuration, and canonical live UART state. Public load opens and decodes state once, routes current 2.4 to File/COW plus a separately authorized root backing, legacy 2.3 to the platform loader, frozen native-v1 to File/Uffd, and pinned Firecracker or unknown bytes to explicit incompatibility. Every process-restorable family publishes Paused before optional resume. Signed direct evidence proves root reconstruction, two-vCPU continuation, private guest writes, immutable repeated loads, Uffd rejection, and real closed-stdout terminal cleanup; signed production evidence proves exact state/memory/root grants, retained descriptor identity after path replacement, App Sandbox worker lifecycle, collision/staging recovery, and cleanup. CRCs cover state/metadata only; external authentication/encryption is required. Optional devices/general serial state, native-v2 Uffd, Diff/merge, overrides, editing, Firecracker bytes, and broad portability remain Wave 6 work. |
...
| Capability inventory enforcement | structural inventory implemented; capability audit in progress (233 implemented, 165 audit, 3 feasible-missing, 17 proven impossible) | unit, workspace CI, docs, pinned-source compare | #1348, #1349, #1527, #1546, #1547, #1548, #1549, #1550, #1551, #1552, #1553, #1554, #1555, #1578, #1589 | The machine-owned v1.16.0 source manifest and human overlay keep every disposition mechanically checked. #1578 promoted the snapshot-version argument, complete instance/version-output semantic, aggregate run operation, and minimal Full create/load public-lifecycle semantic. #1589 advances that already-terminal narrow lifecycle to exact 2.4 single-root MMIO/PCI artifacts without promoting broad snapshot records. Snapshot corpora and API/schema aggregates remain under audit for Diff, optional devices/MMDS, tools, Firecracker interoperability, and portability. |
...
| Process CLI and API socket | 25 of 29 inventory records implemented and verified; two proven platform impossible; only two broad source corpora remain under audit | unit, API socket, process e2e, signed process, signed App Sandbox process, signed production bundle, pinned-source compare | #536, #545, #1008, #1010, #1048, #1058, #1060, #1070, #1092, #1260, #1302, #1352, #1365, #1368, #1384, #1419, #1578, #1589 | The checked 23-argument contract has 21 implemented leaves and two terminal seccomp platform exclusions. It covers argument precedence, Unicode identity, limits, readiness, fd-table behavior, signals including nonfatal SIGPIPE, socket ownership/cleanup, configuration startup, PCI, and current native snapshot output. `--snapshot-version` prints `v2.4.0`; bounded direct or exact-granted description reports the actual validated native-v1 or native-v2 version and explicitly rejects Firecracker bytes. The complete instance/version-output semantic and aggregate run operation are terminal. Only broad `corpus:design` and `corpus:getting-started` remain under audit for unrelated repository-wide claims. |
...
| Snapshots and restore | narrow public native-v2 2.4 single-root Full/File multi-vCPU lifecycle implemented and verified; legacy device-free 2.3 and frozen native-v1 File/Uffd readers retained | unit, API socket, process e2e, signed HVF capture/reconstruction, signed direct executable, signed App Sandbox and production-bundle restore, docs | #543, #1263, #1264, #1390, #1395, #1396, #1441, #1525, #1526, #1528, #1529, #1554, #1575, #1576, #1577, #1578, #1583, #1584, #1585, #1586, #1587, #1588, #1589 | Public create admits only Paused Full sources with 1–32 vCPUs, configured boot metadata, exactly one read-only File/Sync root over matching MMIO or PCI, no optional devices/MMDS/boot timer, and canonical default UART state; it publishes current `2.4.0` memory first and graph-bearing state last. Public load requires a pristine destination except logger/metrics, opens/decodes state once, retains native-v2 read-only File/COW memory, authorizes the current root selector separately and commits its lease with the Paused owner, preserves exact device-free 2.3 loading, and preserves frozen native-v1 eager File or macOS pager behavior. Every family validates before construction, commits Paused, and optionally resumes normally; only proven-pristine preconstruction failure retries, while committed identity/time, publication, incomplete cleanup, or resume failure is terminal. Signed direct evidence proves exact MMIO/PCI root reconstruction, repeated isolated two-vCPU COW destinations, and a real terminal boundary; signed production evidence proves exact state/memory/root grants and retained descriptors. The format remains bangbang-native and unauthenticated. Diff, native-v2 Uffd, Linux UFFD compatibility, overrides, optional devices/general serial, editing, Firecracker artifacts, and broad portability remain unsupported or under audit. |
...
| Public native-v2 Full/File endpoint activation | implemented narrow 2.4 single-root subset with legacy native-v2 2.3 and frozen native-v1 readers | runtime, API socket, process lifecycle, signed direct executable, signed App Sandbox, signed production bundle, docs | #543, #1276, #1396, #1575, #1576, #1577, #1578, #1589 | Public Full create emits graph-required native-v2 2.4 for exactly one read-only File/Sync root over MMIO or PCI. Public load classifies one opened state and selects frozen native-v1 File/Uffd, legacy device-free native-v2 2.3 File/COW, or current root-bearing native-v2 2.4 File/COW plus an authorized root backing without fallback. Paused-before-resume, dirty tracking, exact root/transport reconstruction, metrics/latency, redaction, collision/cancellation, immutable repeated destinations, retained descriptors and grants, explicit/automatic resume, terminal failure, cleanup, and exact version/description output are covered. |
...
#1589 extends the macOS containment row's snapshot claim without widening the
grant protocol: current native-v2 2.4 atomically adopts state/memory and resolves
the inert root selector through an exact destination `DriveBacking`/`ReadOnly`
grant. The root lease commits only with Paused session/controller publication;
signed production coverage replaces state, memory, and root pathnames after
launcher adoption and proves retained identity, redaction, and cleanup.
