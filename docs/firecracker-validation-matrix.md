...
| HVF guest-CPU dirty-write primitive | implemented internal foundation; public epoch and Diff remain deferred | unit, signed HVF, docs | #1395 | Before vCPU ownership, complete writable guest-RAM mappings can be transactionally write-protected. The runner accepts only the signed-observed exact lower-EL write DFSC `0x07` at a tracker-owned IPA, restores that page before publishing one bitmap bit, bounds peer stale exits, and requires every owner to join before stop. Activation, page-unprotect, and stop failures fail closed. This does not merge boot-loader, VMM, device, pmem, balloon, or other userspace writes, and does not enable public dirty flags, epoch reset/commit, or Diff artifacts. |
