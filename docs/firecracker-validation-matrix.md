...
| UFFD-equivalent snapshot paging | public macOS equivalent feasible; standalone pager protocol, backend-neutral lazy-memory coordinator, and internal host/guest fault bridges implemented; native-v1 runtime still rejected | checked upstream/platform ledger, pager unit/process tests, lazy-memory concurrency tests, host/guest fault unit and signed/App Sandbox tests, unit/API rejection | #1527, #1546, #1547, #1548, #1549, #1550 | The pinned Firecracker contract delegates anonymous page population and removal to an external UFFD handler. `bangbang-pager-v1` supplies a closed offset-only codec, exact role state machines, and deadline-bounded already-connected transport. `LazyGuestMemory` transactionally owns distinct private-anonymous mappings with compact page state, bounded duplicate-content coalescing, exact scoped publication, counted retired requests, acknowledged removal, and fail-closed termination. The macOS Apple-Silicon HVF adapter generates public `mach_exc` stubs, retains task-local ports and private writable aliases, resolves owned host accesses through the coordinator, forwards unrelated exceptions, conditionally restores the prior handler, and uses fixed terminal exit status 70. Its guest plane maps the same regions with zero initial stage-two access, classifies owned data/instruction aborts, resolves all touched pages, and serializes least read/write/execute permission unions before retry without PC advance. Signed evidence covers host population/forwarding/lifecycle plus guest execute/read/write population, lazy entry boot, two-vCPU coalescing, unowned-exit preservation, source failure, cancellation, and cleanup. Both planes still accept only a trusted in-process source. File/COW remains distinct, direct external Mach exceptions are over-authoritative, and the protocol is not Linux UFFD wire-compatible. The checked ledger is nonterminal: broker, peer removal/failure, consumer, restore, and final certification remain open; current `Uffd` rejection remains until #1527's restore gate. |
