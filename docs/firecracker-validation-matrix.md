...
| VM lifecycle and run-loop control | partial | unit, api socket, process e2e, signed HVF | #537 | `InstanceStart`, Running transition, retained boot worker status, unsupported pause/resume request routing, and guest PSCI `SYSTEM_OFF` process exit are covered. Public pause/resume and guest reset/error state handling remain deferred. |
