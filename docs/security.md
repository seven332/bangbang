...
tests must run through `scripts/run-hvf-tests.sh`, which builds the HVF test
binary, creates a temporary entitlement plist, ad-hoc signs a copy, and runs the
signed copy with one test thread. CI may use `--allow-unsupported` only to
compile and sign on runners that cannot execute HVF; local HVF verification
should fail when HVF is unavailable.
