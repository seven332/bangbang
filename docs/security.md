...
notification dispatcher follows the same bounded PFN parsing path before
acknowledging descriptor heads. Completed descriptors update only internal
inflated-page accounting on the owning balloon device; they do not release,
remap, or otherwise alter host memory. The HVF boot loop can drain these
balloon notifications and signal the allocated balloon interrupt line, but
parsed PFNs, statistics descriptors, free-page hinting commands, and reporting
queue data remain untrusted guest input and must not change host memory
accounting or reclaim behavior until those host-side paths are implemented and
reviewed.
