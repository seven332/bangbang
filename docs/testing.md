...
Focused #1839 coverage pins balloon queue-invocation counts, unrecognized guest
statistics, success-only freed-byte semantics, transport-owner attachment, and
coherent compound snapshots. It also pins memory-hotplug request/failure/byte
...
output path must be absent; the signed sibling stage is published atomically
without replacement, and every occupied final object remains unchanged. The
...
`production_bundle` is selected. The network-independent artifact policy suite
and standalone artifact commands require Python 3.9 or newer. The suite is:

```sh
python3 -m unittest discover -s scripts/tests -p 'test_*.py'
```

It covers strict manifest input, cache reuse and announced repair, nonblocking
locks, all final object kinds, absent-only publication, deterministic generated
cache behavior, ext4 sidecar drift/interruption, signing collisions, failure
cleanup, and Bash 3.2 syntax. To prepare only the kernel cache, run:
...
root. The script verifies the pinned byte size and SHA-256 before reusing or
installing the cached kernel. Download caches use a fail-fast advisory lock;
an invalid regular cache is repaired only after a diagnostic, while final
symlinks and nonregular objects fail.
...
The default cache may be refreshed when its generated bytes differ. An explicit
`--output` is caller-owned: identical bytes are reused, but an occupied path
with different bytes is left unchanged and fails.
...
installing the cached squashfs, including its exact expected byte size. The
upstream Firecracker artifact is a
...
should use a scratch copy of that image. A sibling `.bangbang.json` sidecar is
the cache validity marker and binds source, requested size, variant, tracked
recipe digest, bounded tool versions, output size/hash, and `e2fsck -fn`.
Image publication precedes sidecar publication, so the pair is deliberately
not claimed as crash-atomic; an incomplete or mismatched pair is diagnosed and
rebuilt on the next locked invocation.
