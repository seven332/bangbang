...
`production_bundle` is selected. Exact artifact identities, standalone
preparation commands, cache repair/output policy and optional ext4 operator
instructions live only in the [macOS Guest Workflow](macos-guest-workflow.md)
and its checked manifest. This section describes how signed tests consume those
fixtures.
...
The signed `guest_boot` rootfs-access case consumes the cached read-only
squashfs directly and never mutates it. Tests needing a writable filesystem use
a scratch copy of the separately prepared, sidecar-verified ext4 fixture; its
operator preparation and reproducibility boundary remain in the owner guide.
